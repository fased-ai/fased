...
This document explains how Fased manages sessions end-to-end:
...
Fased is designed around a single **Gateway process** that owns session state.
...
- Transcripts: `~/.fased/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

Fased resolves these via `src/config/sessions.ts`.
...
In `mode: "warn"`, Fased reports potential evictions but does not mutate the store/files.
...
Fased intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
- `contextTokens` in the store is a runtime estimate/reporting value; don’t treat it as an exact limit.
...
These are Pi runtime semantics (Fased consumes the events, but Pi decides when to compact).
...
Fased also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, Fased bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, Fased leaves it alone.
...
- Chat `/status` (current session)
- Agent > Sessions (session list, last activity, transcript metadata, and protected delete actions)
- Usage page (token usage history; session counters are only a fallback when no better usage record exists)
- `fased status` (CLI)
- `fased sessions` / `sessions --json`
...
Pi also exposes a `session_before_compact` hook in the extension API, but Fased’s
