...
service via the external `fased` CLI (no embedded runtime). This gives you
...
- Writes `~/.fased/disable-launchagent`.
...
`--attach-only` (or `--no-launchd`). This sets `~/.fased/disable-launchagent`,
