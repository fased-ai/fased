...
- **Working (agent running):** `AppState.isWorking=true` drives a “tail/leg scurry” micro-motion: faster leg wiggle and slight offset while work is in-flight. Currently toggled around Chat agent runs; add the same toggle around other long tasks when you wire them.
...
- Agent activity: set `AppStateStore.shared.setWorking(true/false)` around work spans (already done in the Chat agent call). Keep spans short and reset in `defer` blocks to avoid stuck animations.
