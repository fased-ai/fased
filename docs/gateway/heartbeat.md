...
> **Heartbeat vs Tasks?** See [Tasks vs Heartbeat](/automation/cron-vs-heartbeat) for guidance on when to use each.
...
- During heartbeat runs, Fased treats `HEARTBEAT_OK` as an ack when it appears
...
If **all three** are false, Fased skips the heartbeat run entirely (no model call).
...
headers like `# Heading`), Fased skips the heartbeat run to save API calls.
