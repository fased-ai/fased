...
description: "Complete field-by-field reference for ~/.fased/fased.json"
summary: "Complete reference for every Fased config key, default, and channel setting."
read_when:
...
Use **Agent > Channels** for normal channel setup. Raw config is documented here
for advanced review and automation.

Core channels start automatically when their config section exists (unless
`enabled: false`). Bundled extension channels may also require the extension to
be enabled and the gateway restarted so the runtime code loads.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["fased-dm"] },
      guilds: {
        "123456789012345678": {
...
Fased spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@fased"] },
      },
    ],
  },
}
...
- `none`: per-scope sandbox workspace under `~/.fased/sandboxes`
    ...
    list: [
      {
        ...
        agentDir: "~/.fased/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@fased"] },
        sandbox: { mode: "off" },
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:fased`      | All built-in tools (excludes provider plugins)                                           |
...
- Override agent config root with `FASED_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
- `models.providers.*.baseUrl`: provider API base URL.
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `fased onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `fased onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `fased onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `fased onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `fased onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `fased onboard --auth-choice minimax-api`.
...
- Loaded from `~/.fased/extensions`, `<workspace>/.fased/extensions`, plus `plugins.load.paths`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or FASED_GATEWAY_PASSWORD
    ...
    controlUi: {
      ...
      // allowedOrigins: ["https://control.example.com"], // use when browser Origin differs from Gateway host
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.fased/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-fased-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `FASED_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    liveReload: true,
    // enabled: false, // or FASED_SKIP_CANVAS_HOST=1
  },
}
```
...
- Hostname defaults to `fased`. Override with `FASED_MDNS_HOSTNAME`.
...
- `.env` files: CWD `.env` + `~/.fased/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${FASED_GATEWAY_TOKEN}" },
  },
}
```
...
```json5
{
  secrets: {
    providers: {
      ...
      filemain: {
        source: "file",
        path: "~/.fased/secrets.json",
        mode: "json",
        timeoutMs: 5000,
      },
      vault: {
        source: "exec",
        command: "/usr/local/bin/fased-vault-resolver",
...
- Default log file: `/tmp/fased/fased-YYYY-MM-DD.log`.
