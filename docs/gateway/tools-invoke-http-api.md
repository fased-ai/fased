...
You can customize this HTTP deny list via `gateway.tools`:
...
`gateway.tools.allow` only removes a tool from the HTTP deny list. It does not
bypass the session, agent, provider, group, or subagent tool policy chain.

To help group policies resolve context, you can optionally set:

- `x-fased-message-channel: <channel>` (example: `slack`, `telegram`)
- `x-fased-account-id: <accountId>` (when multiple accounts exist)
- `x-fased-message-to: <recipient>` (channel-specific target)
- `x-fased-thread-id: <threadId>` (thread or topic context)
