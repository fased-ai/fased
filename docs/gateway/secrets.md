...
Fased supports additive secret references so credentials do not need to be stored as plaintext in config files.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/Fased QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
  ...
  secrets: {
    providers: {
      vault_openai: {
        ...
        allowSymlinkCommand: true, // required for Homebrew symlinked binaries
        trustedDirs: ["/opt/homebrew"],
        args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/fased"],
...
When reload-time activation fails after a healthy state, Fased enters degraded secrets state.
...
- plaintext values at rest (`fased.json`, `auth-profiles.json`, `.env`)
- unresolved refs
...
- lets you select secret-bearing fields in `fased.json`
...
- `fased secrets configure --providers-only`
- `fased secrets configure --skip-provider-setup`
...
Fased intentionally does **not** write rollback backups that contain pre-migration plaintext secret values.
...
For static credentials, Fased runtime no longer depends on plaintext `auth.json`.
...
- Field matrix: [SecretRef Matrix](/reference/secretref-matrix)
