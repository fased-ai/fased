...
read_when:
  - You want to place an outbound voice call from Fased
...
- Install plugin
- Restart Gateway
- Configure the provider account under `plugins.entries.voice-call.config`
- Grant or block the `voice_call` tool for an Agent in **Agent > Tools**
- Use `fased voicecall ...` or the `voice_call` tool

In the Control UI, Voice Call is a global runtime extension. Manage install,
source trust, restart, and diagnostics in **Extensions**. Agent access to the
tool is per-Agent.
...
```bash
fased voicecall call --to "+15555550123" --message "Hello from Fased"
fased voicecall continue --call-id <id> --message "Any questions?"
fased voicecall speak --call-id <id> --message "One moment"
fased voicecall end --call-id <id>
fased voicecall status --call-id <id>
fased voicecall tail
