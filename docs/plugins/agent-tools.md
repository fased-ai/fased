...
Fased plugins can register **agent tools** (JSON-schema functions) that are exposed
to the LLM during agent runs. Tools can be **required** (always available) or
**optional** (opt-in).

Normal operators manage tool access in **Agent > Tools** for the selected Agent.
Plugin install, update, restart, source trust, scanner warnings, and hook/runtime
lifecycle stay global in **Extensions**. Raw config remains the advanced escape
hatch: global tool defaults live under `tools`, and per-Agent overrides live under
`agents.list[].tools`.
