---
summary: "Signal setup through signal-cli, account linking or SMS registration, and Fased access policy."
read_when:
...
- Fased installed on your server (Linux flow below tested on Ubuntu 24).
...
3. Choose one setup path:
   - **Path A (QR link):** `signal-cli link -n "Fased"` and scan with Signal.
   - **Path B (SMS register):** register a dedicated number with captcha + SMS verification.
4. Open **Agents**, select the Agent, then use **Agent > Channels > Signal** to
   configure the account, `signal-cli` path, and access policy. Restart the
   gateway if the UI asks for it.
5. Send a first DM and approve pairing (`fased pairing approve signal <CODE>`).
...
2. Link a bot account:
   - `signal-cli link -n "Fased"` then scan the QR in Signal.
...
4. Configure Fased, restart gateway, verify channel:
...
- Config validation errors after edits: run `fased doctor --fix`.
...
fased pairing list signal
pgrep -af signal-cli
grep -i "signal" "/tmp/fased/fased-$(date +%Y-%m-%d).log" | tail -20
```
