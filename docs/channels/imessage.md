---
summary: "Legacy iMessage path through imsg. Keep it for older installs only; new setups should use BlueBubbles."
read_when:
...
```
      ...
      <Step title="Configure Fased">
        Open **Agents**, select the Agent, then use **Agent > Channels >
        iMessage**. Set the `imsg` CLI path and Messages database path. Scripted
...
- Full Disk Access is required for the process context running Fased/`imsg` (Messages DB access).
...
<AccordionGroup>
  <Accordion title="Dedicated bot macOS user (separate iMessage identity)">
    ...
    4. Create SSH wrapper so Fased can run `imsg` in that user context.
...
```
  ...
  <Accordion title="DMs are ignored">
    ...
    - pairing approvals (`fased pairing list imessage`)
...
```
    ...
    Confirm Full Disk Access + Automation are granted for the process context that runs Fased/`imsg`.
