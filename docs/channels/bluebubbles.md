...
Normal browser setup is **Agent > Channels > BlueBubbles**. CLI onboarding can
also prompt for the same fields when you prefer terminal setup:
...
```
fased channels add --channel bluebubbles --http-url http://192.168.1.100:1234 --password <password>
```
...
Fased may surface _short_ message IDs (e.g., `1`, `2`) to save tokens.
