...
```bash
fased plugins install ./extensions/twitch
```
...
4. 配置令牌：
   - 环境变量：`FASED_TWITCH_ACCESS_TOKEN=...`（仅限默认账户）
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "fased", // 机器人的 Twitch 账户
      accessToken: "oauth:abc123...", // OAuth Access Token（或使用 FASED_TWITCH_ACCESS_TOKEN 环境变量）
