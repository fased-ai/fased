...
这是部分实现的目标设计。Fased 已有 schema 验证、Doctor 修复流程、插件允许列表警告和更严格的
UI 表面，但当前运行时代码仍保留部分兼容别名和 Doctor 迁移。本文是实现方向，不是当前操作员手册。
...
- 所有插件都需要插件清单（`fased.plugin.json`）。

## 插件 schema 强制执行

- 每个插件为其配置提供严格的 JSON Schema（内联在清单中）。
- 插件加载流程：
  1. 解析插件清单 + schema（`fased.plugin.json`）。
  2. 根据 schema 验证配置。
...
- 目标行为：加载配置时运行 Doctor dry-run。
- 当前行为：Doctor 和定向修复流程可用，部分启动/配置路径会显示警告，而不是静默加载不安全状态。
- 如果配置无效：
  - 打印摘要 + 可操作的错误。
  - 指示：`fased doctor --fix`。
- `fased doctor --fix`：
...
其他所有命令必须硬失败并显示："Config invalid. Run `fased doctor --fix`."
