...
- 任务持久化存储在 `~/.fased/cron/` 下，因此重启不会丢失计划。
- 两种执行方式：
...
fased task list
fased task run <job-id>
fased task runs --id <job-id>
```
...
定时任务默认持久化存储在 Gateway网关主机的 `~/.fased/cron/jobs.json` 中。Gateway网关将文件加载到内存中，并在更改时写回，因此仅在 Gateway网关停止时手动编辑才是安全的。请优先使用 `fased task add/edit` 或定时任务工具调用 API 进行更改。
...
- 任务存储：`~/.fased/cron/jobs.json`（Gateway网关管理的 JSON）。
- 任务活动日志：`~/.fased/cron/runs/<jobId>.jsonl`（JSONL，自动清理）。
...
fased task add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# 切换或清除现有任务的智能体
fased task edit <jobId> --agent ops
fased task edit <jobId> --clear-agent
```

手动运行（调试）：

```bash
fased task run <jobId>
```

编辑现有任务（补丁字段）：

```bash
fased task edit <jobId> \
...
```bash
fased task runs --id <jobId> --limit 50
```

不创建任务直接发送系统事件：

```bash
fased system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run`（强制或到期）、`cron.runs`
  如需不创建任务直接发送系统事件，请使用 [`fased system event`](/cli/system)。

## 故障排除

### "没有任何任务运行"

- 检查定时任务是否已启用：`cron.enabled` 和 `FASED_SKIP_CRON`。
