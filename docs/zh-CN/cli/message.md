...
read_when:
  - 添加或修改消息 CLI 操作
  - 更改出站渠道行为
summary: "`fased message`（发送 + 渠道操作）的 CLI 参考"
title: message
x-i18n:
...
- 缓存未命中时，如果提供商支持，Fased 将尝试实时目录查找。
...
```
fased message send --channel discord \
...
```
fased message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```
fased message react --channel slack \
...
```
fased message react --channel signal \
...
```
fased message send --channel telegram --target @mychat --message "Choose:" \
