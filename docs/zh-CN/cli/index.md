...
read_when:
  - 添加或修改 CLI 命令或选项
  - 为新命令界面编写文档
summary: Fased `fased` 命令、子命令和选项的 CLI 参考
title: CLI 参考
x-i18n:
...
- `--dev`：将状态隔离到 `~/.fased-dev` 下并调整默认端口。
- `--profile <name>`：将状态隔离到 `~/.fased-<name>` 下。
- `--no-color`：禁用 ANSI 颜色。
- `--update`：`fased update` 的简写（仅限源码安装）。
- `-V`、`--version`、`-v`：打印版本并退出。
...
- 默认/粗体文本：向导标题。
- `warn`（#FACC15）：分区标签、阻塞项、回退、警告。
- `success`（#4ADE80）：命令、链接、令牌、需要复制的值、成功状态。
- `accent`（#22D3EE）：主要高亮。
- `accentDim`（#0891B2）：次要高亮文本。
- `info`（#93C5FD）：信息性值。
- `accentBright`（#67E8F9）：强调值。
- `error`（#F87171）：错误、失败。
- `muted`（#A3A3A3）：弱化、元数据。
...
```
fased [--dev] [--profile <name>] <command>
  setup
  onboard
  configure
  config
...
注意：插件可以添加额外的顶级命令（例如 `fased voicecall`）。

## 安全

- `fased security audit` — 审计配置 + 本地状态中常见的安全隐患。
- `fased security audit --deep` — 尽力进行实时 Gateway 网关探测。
- `fased security audit --fix` — 收紧安全默认值并 chmod 状态/配置。

## 插件

管理扩展及其配置：

- `fased plugins list` — 发现插件（使用 `--json` 获取机器可读输出）。
- `fased plugins info <id>` — 显示插件详情。
- `fased plugins install <path|.tgz|npm-spec>` — 安装插件（或将插件路径添加到 `plugins.load.paths`）。
- `fased plugins enable <id>` / `disable <id>` — 切换 `plugins.entries.<id>.enabled`。
- `fased plugins doctor` — 报告插件加载错误。
...
- `fased memory status` — 显示索引统计。
- `fased memory index` — 重新索引记忆文件。
- `fased memory search "<query>"` — 对记忆进行语义搜索。
...
- `--workspace <dir>`：智能体工作区路径（默认 `~/.fased/workspace`）。
- `--wizard`：运行新手引导向导。
...
非交互式配置辅助工具（get/set/unset）。不带子命令运行 `fased config` 会启动向导。
...
- `channels status`：检查 Gateway 网关可达性和渠道健康状况（`--probe` 运行额外检查；使用 `fased health` 或 `fased status --deep` 进行 Gateway 网关健康探测）。
- 提示：`channels status` 在检测到常见配置错误时会打印带有建议修复的警告（然后指向 `fased doctor`）。
...
```bash
fased channels add --channel telegram --account alerts --name "Alerts Bot" --token $TELEGRAM_BOT_TOKEN
fased channels add --channel discord --account work --name "Work Bot" --token $DISCORD_BOT_TOKEN
fased channels remove --channel discord --account work --delete
...
提示：使用 `fased skills marketplace install <slug>` 安装 Skills，使用 `fased skills marketplace update --dry-run` 预览更新。
...
- `fased message send --target +15555550123 --message "Hi"`
- `fased message poll --channel discord --target channel:123 --poll-question "Snack?" --poll-option Pizza --poll-option Sushi`
...
当 OAuth/API 凭证可用时，Fased 可以显示提供商用量/配额。

显示位置：

- `/status`（可用时添加简短的提供商用量行）
- `fased status --usage`（打印完整的提供商明细）
- macOS 菜单栏（上下文下的用量部分）

说明：

- 数据直接来自提供商用量端点（非估算）。
- 提供商：Anthropic、GitHub Copilot、OpenAI Codex OAuth，以及启用对应提供商插件时的 Gemini CLI。
- 如果没有匹配的凭证，用量会被隐藏。
...
- `gateway status` 在检测到旧版或额外的 Gateway 网关服务时也会显示（`--deep` 添加系统级扫描）。配置文件命名的 Fased 服务被视为一等公民，不会被标记为"额外"。
...
`fased models` 是 `models status` 的别名。
...
`node` 运行**无头节点主机**或将其作为后台服务管理。参见 [`fased node`](/cli/node)。
...
浏览器控制 CLI（专用 Chrome/Brave/Edge/Chromium）。参见 [`fased browser`](/cli/browser) 和[浏览器工具](/tools/browser)。
