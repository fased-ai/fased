...
read_when:
  - 启动新的 Fased 智能体会话
  - 启用或审计默认 Skills
summary: 个人助手设置的默认 Fased 智能体指令和 Skills 列表
x-i18n:
...
# AGENTS.md — Fased 个人助手（默认）

## 首次运行（推荐）

Fased 为智能体使用专用的工作区目录。默认：`~/.fased/workspace`（可通过 `agents.defaults.workspace` 配置）。
...
```bash
cp docs/reference/AGENTS.default.md ~/.fased/workspace/AGENTS.md
```
...
如果你将此工作区视为运行时记忆，请将其设为 git 仓库（最好是私有的），这样 `AGENTS.md` 和你的记忆文件就会被备份。
...
- 运行一个自托管个人智能体运行时，带有浏览器仪表盘、可选消息渠道和工具访问。
- 在基础运行时稳定后，可以进一步叠加钱包、联邦和 SAT 挖矿能力。
- macOS 应用管理权限（屏幕录制、通知、麦克风）并通过其内置二进制文件暴露 `fased` CLI。
...
- 对于浏览器驱动的验证，使用带有 Fased 管理的 Chrome 配置文件的 `fased browser`（tabs/status/screenshot）。
- 对于 DOM 检查，使用 `fased browser eval|query|dom|snapshot`（需要机器输出时使用 `--json`/`--out`）。
- 对于交互，使用 `fased browser click|type|hover|drag|select|upload|press|wait|navigate|back|evaluate|run`（click/type 需要 snapshot 引用；CSS 选择器使用 `evaluate`）。
