...
Fased 围绕一个拥有会话状态的单一 **Gateway 网关进程**设计。
...
- 存储：`~/.fased/agents/<agentId>/sessions/sessions.json`
- 记录：`~/.fased/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram 话题会话：`.../<sessionId>-topic-<threadId>.jsonl`

Fased 通过 `src/config/sessions.ts` 解析这些位置。
...
Fased 有意**不**"修复"记录；Gateway 网关使用 `SessionManager` 来读/写它们。
...
在嵌入式 Fased Agent 运行时中，自动压缩在两种情况下触发：
...
这些是 Agent 运行时语义：Fased 根据模型上下文和运行状态决定何时压缩。
...
Fased 还为嵌入式运行强制执行安全下限：

- 如果 `compaction.reserveTokens < reserveTokensFloor`，Fased 会提升它。
- 默认下限是 `20000` 个 token。
...
Fased 支持用于后台任务的"静默"回合，用户不应该看到中间输出。
...
从 `2026.1.10` 开始，当部分块以 `NO_REPLY` 开头时，Fased 还会抑制**草稿/打字流式输出**，因此静默操作不会在回合中途泄漏部分输出。
...
1. 监控会话上下文使用情况。
2. 当它越过"软阈值"（低于 Agent 运行时压缩阈值）时，向 Agent 运行一个静默的"现在写入记忆"指令。
...
- 刷新仅对嵌入式 Fased Agent 会话运行（CLI 后端跳过它）。
- 当会话工作空间是只读时（`workspaceAccess: "ro"` 或 `"none"`），刷新会被跳过。
- 参见[记忆](/concepts/memory)了解工作空间文件布局和写入模式。

Fased 还在扩展 API 中公开了 `session_before_compact` 钩子，但刷新逻辑目前位于 Gateway 网关端。
...
- 存储 vs 记录不匹配？从 `fased status` 确认 Gateway 网关主机和存储路径。
- 压缩过于频繁？检查：
  - 模型上下文窗口（太小）
