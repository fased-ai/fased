...
Windows 上的 Fased 推荐**通过 WSL2**（推荐 Ubuntu）。CLI + Gateway 网关在 Linux 内运行，这保持了运行时的一致性并使工具兼容性大大提高（Node/Bun/pnpm、Linux 二进制文件、Skills）。原生 Windows 可能更棘手。WSL2 给你完整的 Linux 体验——一条命令安装：`wsl --install`。
...
`./install.sh` 默认会运行新手引导。只有在你想先安装 CLI/运行时、稍后再运行新手引导时，才使用
`./install.sh --no-onboard`，之后再运行 `fased onboard --install-daemon`。
