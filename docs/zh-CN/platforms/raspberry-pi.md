---
read_when:
  - 在 Raspberry Pi 上设置 Fased 时
  - 在 ARM 设备上运行 Fased 时
  - 构建低成本常驻个人 AI 时
summary: 在 Raspberry Pi 上运行 Fased（低成本自托管设置）
title: Raspberry Pi
x-i18n:
...
# 在 Raspberry Pi 上运行 Fased

## 目标

在 Raspberry Pi 上运行持久、常驻的 Fased Gateway 网关，**一次性成本约 $35-80**（无月费）。
...
# 验证
node --version  # 应显示 v24.x，或至少 v22.14.x
...
可修改安装让你可以直接访问日志和代码，同时安装正常的 `fased` 命令 — 对调试 ARM 特定问题很有用。
...
1. **Gateway 网关模式：** Hosting
2. **Gateway 认证：** 按提示使用 token/password
3. **钱包/挖矿：** 可选；除非正在设置钱包策略或 SAT 挖矿，否则先跳过
4. **托管安全：** 通过 Tailscale 保持私有访问
5. **守护进程：** 是（systemd）

Gateway 在线后，从浏览器 Control UI 的所选 Agent 完成设置：
**Agent > Models** 配置 API key/OAuth 和模型角色，**Agent > Channels** 配置
Telegram/WhatsApp/Discord 路由，**Agent > Services** 配置 web/search 和 API
连接器，**Agent > Skills** 配置技能。
...
大多数 Fased 功能在 ARM64 上可用，但某些外部二进制文件可能需要 ARM 构建：

| 工具               | ARM64 状态 | 说明                                |
| ------------------ | ---------- | ----------------------------------- |
| Node.js            | ✅         | 运行良好                            |
| WhatsApp 频道      | ✅         | 纯 JS，无问题                       |
| Telegram           | ✅         | 纯 JS，无问题                       |
...
由于 Pi 只是 Gateway 网关（模型在云端运行），使用 API-based 模型。在
**Agent > Models** 中配置。如果需要 raw config 示例：
...
不要尝试在 Pi 上运行本地 LLM，即使是小模型也太慢。把模型推理交给托管 provider。
...
```bash
# 检查服务是否已启用
sudo systemctl is-enabled fased

# 如果没有则启用
sudo systemctl enable fased

# 开机启动
sudo systemctl start fased
```
...
```bash
# 检查日志
journalctl -u fased --no-pager -n 100
...
实际成本取决于硬件、存储、电费以及你是否已有设备。该表只是粗略规划参考。
