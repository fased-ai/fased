---
read_when:
  - 在 Oracle Cloud 上设置 Fased
  - 寻找 Fased 的低成本 VPS 托管
  - 想要在小型服务器上 24/7 运行 Fased
summary: 在 Oracle Cloud 上运行 Fased（OCI ARM 免费层路径）
title: Oracle Cloud
x-i18n:
...
在 Oracle Cloud 的 ARM 免费层路径上运行持久化的 Fased Gateway 网关。

Oracle 的免费层可以用于 Fased（特别是如果你已经有 OCI 账户），但有一些权衡：
...
| Hetzner      | CX22            | 2 vCPU，4GB RAM       | ~ $4    | 低成本 VPS           |
...
这会启用 Tailscale SSH，所以你可以从 tailnet 上的任何设备通过 `ssh fased` 连接——不需要公网 IP。
...
**从现在开始，通过 Tailscale 连接：** `ssh ubuntu@fased`（或使用 Tailscale IP）。
...
fased config set gateway.bind loopback
...
此设置通常消除了纯粹为了阻止互联网范围的 SSH 暴力破解而需要额外的基于主机的防火墙规则的*需求*——但你仍应保持操作系统更新，运行 `fased security audit`，并验证你没有意外地在公共接口上监听。
...
- **凭证权限：** `chmod 700 ~/.fased`
- **安全审计：** `fased security audit`
...
- `~/.fased/` — 配置、凭证、会话数据
- `~/.fased/workspace/` — 工作区（SOUL.md、记忆、产物）
...
- [DigitalOcean 指南](/platforms/digitalocean) — 更简单的 VPS 设置路径
