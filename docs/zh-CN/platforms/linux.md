...
本地 Linux 笔记本/开发机可以使用普通的 `./install.sh` 或
`./install.sh --local`。除非传入 `--no-onboard`，安装器会运行新手引导。

分步 VPS 指南：[Hetzner](/install/hetzner) 或通用的 [VPS 托管](/install/vps) 中心。
...
Fased 默认安装 systemd **用户**服务。对于共享或常驻服务器使用**系统**
服务。完整的单元示例和指南
