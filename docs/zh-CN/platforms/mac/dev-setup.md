...
read_when:
  - 设置 macOS 开发环境
summary: 为在 Fased macOS 应用上工作的开发者提供的设置指南
title: macOS 开发设置
x-i18n:
...
本指南涵盖从源代码构建和运行 Fased macOS 应用程序的必要步骤。
...
2.  **Node.js 24 推荐，或带 `node:sqlite` 的 Node 22.14+，以及 pnpm**：
    Gateway 网关、CLI 和打包脚本所需。
...
要构建 macOS 应用并将其打包到 `dist/FasedAgent.app`，运行：

```bash
./scripts/package-mac-app.sh
```

如果你没有 Apple 签名身份，脚本默认会失败。只有在快速本地构建、且不需要 macOS 权限持久化时，才设置
...
macOS 应用期望已安装 `fased` CLI 来管理后台任务。
