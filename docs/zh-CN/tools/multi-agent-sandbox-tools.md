...
有关调试"为什么这被阻止了？"，请参见[沙箱 vs 工具策略 vs 提权](/gateway/sandbox-vs-tool-policy-vs-elevated) 和 `fased sandbox explain`。
...
提供商工具键接受 `provider`（例如 `anthropic`）或 `provider/model`（例如 `openai/gpt-5.5`）。
...
旧版 `agent.*` 配置由 `fased doctor` 迁移；今后请优先使用 `agents.defaults` + `agents.list`。
...
2. **验证沙箱容器：**
   ...
   ```exec
   docker ps --filter "name=fased-sbx-"
   ```
...
4. **监控日志：**
   ```exec
   tail -f "${FASED_STATE_DIR:-$HOME/.fased}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
