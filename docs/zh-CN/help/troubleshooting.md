...
```bash
cd ~/fased
FASED_VERBOSE=1 ./install.sh --beta
```

你也可以设置 `FASED_VERBOSE=1` 代替标志。
...
### `docs.fased.ai` 显示 SSL 错误（Comcast/Xfinity）

一些 Comcast/Xfinity 连接通过 Xfinity Advanced Security 阻止 `docs.fased.ai`。
禁用 Advanced Security 或将 `docs.fased.ai` 添加到允许列表，然后重试。
...
- 检查允许列表：`fased config get agents.defaults.models`
...
如果可以的话，包含来自 `fased logs --follow` 的相关日志尾部。
