...
面向用户的概览（CLI + Control UI + 配置），请参阅 [/logging](/gateway/logging)。
...
- 默认滚动日志文件位于 `/tmp/fased/` 下（每天一个文件）：`fased-YYYY-MM-DD.log`
  - 日期使用 Gateway 网关主机的本地时区。
- 日志文件路径和级别可以通过 `~/.fased/fased.json` 配置：
  - `logging.file`
  - `logging.level`
...
`fased gateway` 支持每个 Gateway 网关的样式切换：
