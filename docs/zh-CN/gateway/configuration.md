...
Fased 从 `~/.fased/fased.json` 读取可选的 **JSON5** 配置（支持注释和尾逗号）。

如果文件不存在，Fased 会使用安全默认值。大多数设置中，`fased onboard` 会为你写入第一版配置。

常见需要添加配置的原因：

- 连接渠道并控制谁可以给 bot 发消息
- 设置模型、工具、sandbox 或自动化（cron、hooks）
- 调整 sessions、media、networking 或 UI

完整字段列表见[完整参考](/gateway/configuration-reference)。

<Tip>
**初次接触配置？** 先运行 `fased onboard`，然后在 Control UI 里从选定 Agent 完成普通设置。
**Agent > Models**、**Agent > Channels**、**Agent > Services**、**Agent > Skills**、
**Agent > Memory** 和 **Agent > Tasks** 拥有常见用户控制项。此页用于原始配置 contract 和高级字段。
</Tip>

## Local profile vs hosting profile

onboarding profile 会设置运行时基线：

- **Local profile**
  - 个人机器
  - Gateway 默认 loopback-first
  - 最适合私有浏览器访问和本地直接操作
  - 只有需要从其他设备访问时再叠加 Tailscale Serve

- **Hosting profile**
  - VPS 或 always-on 机器
  - 通常期望安装 supervised service
  - 默认仍然 loopback-first
  - 远程访问通常应通过 Tailscale Serve、直接 tailnet bind、可信反向代理或 SSH

无论哪种 profile，除非你明确设计了更宽的暴露模型，否则不要把原始 Gateway 端口直接开放到公网。
...
<Tabs>
  ...
  <Tab title="Control UI">
    打开 [http://127.0.0.1:18789](http://127.0.0.1:18789)。常见设置在选定 Agent 的 tabs 中。
    原始配置在 **Advanced > Config**，旁边的 **Debug** 和 **Nodes** 用于操作员诊断。
  </Tab>
  <Tab title="Direct edit">
    直接编辑 `~/.fased/fased.json`。Gateway 会监视文件并自动应用变更（见 hot reload）。
  </Tab>
</Tabs>

## 严格配置验证

Fased 只接受完全匹配 schema 的配置。
未知键、类型错误或无效值会导致 Gateway 网关 **拒绝启动**以确保安全。

验证失败时：

- Gateway 网关不会启动。
- 只允许诊断命令（例如：`fased doctor`、`fased logs`、`fased health`、`fased status`、`fased service`、`fased help`）。
- 运行 `fased doctor` 查看具体问题。
- 运行 `fased doctor --fix`（或 `--yes`）应用迁移/修复。
...
请使用 `config.patch` 或 `fased config set`。请备份 `~/.fased/fased.json`。
...
{
  agents: {
    defaults: { workspace: "~/.fased/workspace" },
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@fased", "reisponde"] },
      },
    ],
  },
  channels: {
...
// ~/.fased/agents.json5
{
  ...
  list: [{ id: "main", workspace: "~/.fased/workspace" }],
}
```
...
{ "$include": "/etc/fased/base.json5" } // 绝对路径
...
```json5
// ~/.fased/clients/mueller/agents.json5
[
...
```json5
// ~/.fased/clients/mueller/broadcast.json5
{
...
Fased 从父进程（shell、launchd/systemd、CI 等）读取环境变量。

此外，它还会加载：

- 当前工作目录中的 `.env`（如果存在）
- `~/.fased/.env`（即 `$FASED_STATE_DIR/.env`）作为全局回退 `.env`
...
可选便利功能：如果启用且预期键均未设置，Fased 会运行你的登录 shell 并仅导入缺失的预期键（不会覆盖）。
...
Fased 在以下位置存储**每个智能体的**认证配置文件（OAuth + API 密钥）：

- `<agentDir>/auth-profiles.json`（默认：`~/.fased/agents/<agentId>/agent/auth-profiles.json`）

另请参阅：[/concepts/oauth](/concepts/oauth)

旧版 OAuth 导入：

- `~/.fased/credentials/oauth.json`（或 `$FASED_STATE_DIR/credentials/oauth.json`）

Fased Agent 运行时在以下位置维护运行时缓存：

- `<agentDir>/auth.json`（自动管理；请勿手动编辑）

旧版智能体目录（多智能体之前）：

- `~/.fased/agent/*`（由 `fased doctor` 迁移到 `~/.fased/agents/<defaultAgentId>/agent/*`）

覆盖：

- OAuth 目录（仅旧版导入）：`FASED_OAUTH_DIR`
- 智能体目录（默认智能体根目录覆盖）：`FASED_AGENT_DIR`（推荐）、`PI_CODING_AGENT_DIR`（旧版）

首次使用时，Fased 会将 `oauth.json` 条目导入到 `auth-profiles.json` 中。
...
- 默认日志文件：`/tmp/fased/fased-YYYY-MM-DD.log`
- 如需稳定路径，将 `logging.file` 设为 `/tmp/fased/fased.log`。
- 控制台输出可通过以下方式单独调整：
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // 可选覆盖。默认：~/.fased/credentials/whatsapp/biz
          // authDir: "~/.fased/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- 旧版单账号 Baileys 认证目录由 `fased doctor` 迁移到 `whatsapp/default`。
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@fased", "fased"] } }],
  },
}
```
    ...
    list: [
      {
        ...
        groupChat: {
          // 仅这些文本模式会触发响应
          mentionPatterns: ["reisponde", "@fased"],
        },
      },
    ],
  },
...
- `agents.list[]`：每智能体覆盖。
  ...
  - `name`：智能体的显示名称。
  - `workspace`：默认 `~/.fased/workspace-<agentId>`（对于 `main`，回退到 `agents.defaults.workspace`）。
  - `agentDir`：默认 `~/.fased/agents/<agentId>/agent`。
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.fased/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.fased/workspace-public",
        sandbox: {
          mode: "all",
          scope: "agent",
...
- `commands.config: true` 启用 `/config`（读写 `fased.json`）。
...
Fased 仅在存在 `channels.telegram` 配置段时启动 Telegram。机器人 token 从 `channels.telegram.botToken`（或 `channels.telegram.tokenFile`）解析，`TELEGRAM_BOT_TOKEN` 作为默认账号的回退。
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["fased-dm"], // 可选群组私聊白名单
      },
      guilds: {
        "123456789012345678": {
          // 服务器 id（推荐）或 slug
          slug: "friends-of-fased",
...
Fased 仅在存在 `channels.discord` 配置段时启动 Discord。token 从 `channels.discord.token` 解析，`DISCORD_BOT_TOKEN` 作为默认账号的回退（除非 `channels.discord.enabled` 为 `false`）。在为 cron/CLI 命令指定投递目标时，使用 `user:<id>`（私聊）或 `channel:<id>`（服务器频道）；裸数字 ID 有歧义会被拒绝。
...
Fased 在提供商启用且两个 token 都已设置时启动 Slack（通过配置或 `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`）。在为 cron/CLI 命令指定投递目标时使用 `user:<id>`（私聊）或 `channel:<id>`。
...
Fased 在账号已配置（机器人 token + 基础 URL）且已启用时启动 Mattermost。token + 基础 URL 从 `channels.mattermost.botToken` + `channels.mattermost.baseUrl` 或默认账号的 `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` 解析（除非 `channels.mattermost.enabled` 为 `false`）。
...
Fased 会生成 `imsg rpc`（通过 stdio 的 JSON-RPC）。无需守护进程或端口。
...
在系统提示的 Runtime 行中显示的可选仓库根目录。如果未设置，Fased 会从工作区（和当前工作目录）向上查找 `.git` 目录进行检测。路径必须存在才能使用。

```json5
{
  agents: { defaults: { repoRoot: "~/Projects/fased" } },
}
```
...
当文件超过此限制时，Fased 会记录警告并注入带标记的头尾截断内容。
...
设置用户时区用于**系统提示上下文**（不用于消息信封中的时间戳）。如果未设置，Fased 在运行时使用主机时区。
...
如果未设置 `messages.responsePrefix`，默认不应用前缀。WhatsApp 自聊天回复是例外：它们在设置时默认为 `[{identity.name}]`，否则为 `[fased]`，以保持同一手机上的对话可读性。
...
WhatsApp 入站前缀通过 `channels.whatsapp.messagePrefix` 配置（已弃用：`messages.messagePrefix`）。默认保持**不变**：当 `channels.whatsapp.allowFrom` 为空时为 `"[fased]"`，否则为 `""`（无前缀）。使用 `"[fased]"` 时，如果路由的智能体设置了 `identity.name`，Fased 会改用 `[{identity.name}]`。
...
为出站回复启用文字转语音。开启后，Fased 使用 ElevenLabs 或 OpenAI 生成音频并附加到回复中。Telegram 使用 Opus 语音消息；其他渠道发送 MP3 音频。
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.fased/settings/tts.json",
      elevenlabs: {
...
`voiceAliases` 允许 Talk 指令使用友好名称（例如 `"voice":"Fased"`）。
...
Fased 还内置了一些别名快捷方式。默认值仅在模型已存在于 `agents.defaults.models` 中时才应用：
...
示例：Opus 4.5 主模型，MiniMax M2.7 回退（托管 MiniMax）：
...
如果省略提供商，Fased 目前假定 `anthropic` 作为临时弃用回退。
...
- `applyPatch.allowModels`：可选的模型 id 白名单（例如 `gpt-5.5` 或 `openai/gpt-5.5`）
  注意：`applyPatch` 仅在 `tools.exec` 下。

`tools.web` 配置 Web 搜索 + 获取工具：

- `tools.web.search.enabled`（默认：有密钥时为 true）
- `tools.web.search.apiKey`（推荐：通过 `fased configure --section web` 设置，或使用 `BRAVE_API_KEY` 环境变量）
...
提供商键接受 `provider`（例如 `anthropic`）或 `provider/model`
（例如 `openai/gpt-5.5`）。

示例（保持全局编码配置文件，但为 Anthropic 使用最小工具）：
...
- `group:nodes`：`nodes`
- `group:fased`：所有内置 Fased 工具（不包含提供商插件）
...
- 智能体工作区访问：`workspaceAccess: "none"`（默认）
  - `"none"`：在 `~/.fased/sandboxes` 下使用每范围的沙箱工作区
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        browser: {
          enabled: false,
          image: "fased-sandbox-browser:bookworm-slim",
          containerPrefix: "fased-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
Fased 使用 **pi-coding-agent** 模型目录。你可以通过编写
`~/.fased/agents/<agentId>/agent/models.json` 或在 Fased 配置中的 `models.providers` 下定义相同的 schema 来添加自定义提供商（LiteLLM、本地 OpenAI 兼容服务器、Anthropic 代理等）。
按提供商的概述 + 示例：[/concepts/model-providers](/concepts/model-providers)。

当存在 `models.providers` 时，Fased 在启动时将 `models.json` 写入/合并到
`~/.fased/agents/<agentId>/agent/`：
...
OpenCode Zen 是一个具有每模型端点的多模型网关。Fased 使用
pi-ai 内置的 `opencode` 提供商；从 https://opencode.ai/auth 设置 `OPENCODE_API_KEY`（或
`OPENCODE_ZEN_API_KEY`）。

说明：

- 模型引用使用 `opencode/<modelId>`（示例：`opencode/gpt-5.5`）。
- 如果你通过 `agents.defaults.models` 启用白名单，请添加你计划使用的每个模型。
- 快捷方式：`fased onboard --auth-choice opencode-zen`。

```json5
{
  agents: {
    defaults: {
      model: { primary: "opencode/gpt-5.5" },
      models: { "opencode/gpt-5.5": { alias: "GPT-5.5" } },
    },
  },
}
```
...
快捷方式：`fased onboard --auth-choice zai-api-key`。
...
- 在环境中设置 `MOONSHOT_API_KEY` 或使用 `fased onboard --auth-choice moonshot-api-key`。
...
- 在环境中设置 `KIMI_API_KEY` 或使用 `fased onboard --auth-choice kimi-code-api-key`。
...
- 设置 `SYNTHETIC_API_KEY` 或使用 `fased onboard --auth-choice synthetic-api-key`。
- 模型引用：`synthetic/hf:MiniMaxAI/MiniMax-M2.5`。
- 基础 URL 应省略 `/v1`，因为 Anthropic 客户端会自动附加。

### 本地模型（LM Studio）— 推荐设置

参见 [/gateway/local-models](/gateway/local-models) 了解当前本地指南。简而言之：在高性能硬件上通过 LM Studio Responses API 运行 MiniMax M2.7；保留托管模型合并作为回退。

### MiniMax M2.7

不通过 LM Studio 直接使用 MiniMax M2.7：
...
- 设置 `MINIMAX_API_KEY` 环境变量或使用 `fased onboard --auth-choice minimax-api`。
...
- 如果你希望 `models.json` 存储在其他位置，请使用 `FASED_AGENT_DIR`（或 `PI_CODING_AGENT_DIR`）覆盖智能体配置根目录（默认：`~/.fased/agents/main/agent`）。
...
```json5
{
  session: {
    ...
    // 默认已按智能体存储在 ~/.fased/agents/<agentId>/sessions/sessions.json
...
- `resetByType`：`dm`、`group` 和 `thread` 的每会话覆盖。
  - 如果你只设置了旧版 `session.idleMinutes` 而没有任何 `reset`/`resetByType`，Fased 保持仅空闲模式以向后兼容。
...
控制内置白名单、安装偏好、额外 Skills 文件夹和每 Skills 覆盖。适用于**内置**Skills 和 `~/.fased/skills`（工作区 Skills 在名称冲突时仍然优先）。
...
控制插件发现、允许/拒绝和每插件配置。插件从 `~/.fased/extensions`、`<workspace>/.fased/extensions` 以及任何 `plugins.load.paths` 条目加载。**配置更改需要重启 Gateway 网关。**
...
### `browser`（Fased 管理的浏览器）

Fased 可以为 Fased 启动一个**专用、隔离的** Chrome/Brave/Edge/Chromium 实例并暴露一个小型 local loopback 控制服务。
...
- 配置文件颜色：`#FF4500`（龙虾橙）
- 注意：控制服务器由运行中的 Gateway 网关（FasedAgent.app 菜单栏或 `fased gateway`）启动。
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/fased" }
...
- 当连接来自这些 IP 之一时，Fased 使用 `x-forwarded-for`（或 `x-real-ip`）来确定客户端 IP，用于本地配对检查和 HTTP 认证/本地检查。
- 仅列出你完全控制的代理，并确保它们**覆盖**传入的 `x-forwarded-for`。

说明：

- `fased gateway` 拒绝启动，除非 `gateway.mode` 设为 `local`（或你传递了覆盖标志）。
...
- 优先级：`--port` > `FASED_GATEWAY_PORT` > `gateway.port` > 默认 `18789`。
...
- FasedAgent.app 监视 `~/.fased/fased.json`，当 `gateway.mode` 或 `gateway.remote.url` 变更时实时切换模式。
...
Gateway 网关监视 `~/.fased/fased.json`（或 `FASED_CONFIG_PATH`）并自动应用更改。
...
- `~/.fased/fased.json`（或 `FASED_CONFIG_PATH`）
...
- `FASED_CONFIG_PATH`（每实例配置）
- `FASED_STATE_DIR`（会话/凭据）
- `agents.defaults.workspace`（记忆）
- `gateway.port`（每实例唯一）

便利标志（CLI）：

- `fased --dev …` → 使用 `~/.fased-dev` + 端口从基础 `19001` 偏移
- `fased --profile <name> …` → 使用 `~/.fased-<name>`（端口通过配置/环境变量/标志）
...
Gmail 辅助配置（由 `fased webhooks gmail setup` / `run` 使用）：
...
默认根目录：`~/.fased/workspace/canvas`
默认端口：`18793`（选择此端口以避免 Fased 浏览器 CDP 端口 `18792`）
...
- 监视目录并通过 `/__fased__/ws` 的 WebSocket 端点广播重载
- 目录为空时自动创建起始 `index.html`（以便你立即看到内容）
- 同时在 `/__fased__/a2ui/` 提供 A2UI，并作为 `canvasHostUrl` 通告给节点
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // 省略时使用 ~/.fased/bridge/tls/bridge-{cert,key}.pem。
      // certPath: "~/.fased/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.fased/bridge/tls/bridge-key.pem"
    },
  },
}
```
...
控制 LAN mDNS 发现广播（`_fased-gw._tcp`）。

- `minimal`（默认）：从 TXT 记录中省略 `cliPath` + `sshPort`
- `full`：在 TXT 记录中包含 `cliPath` + `sshPort`
- `off`：完全禁用 mDNS 广播
- 主机名：默认为 `fased`（通告 `fased.local`）。通过 `FASED_MDNS_HOSTNAME` 覆盖。
...
启用后，Gateway 网关在 `~/.fased/dns/` 下使用配置的发现域（示例：`fased.internal.`）为 `_fased-gw._tcp` 写入单播 DNS-SD 区域。
