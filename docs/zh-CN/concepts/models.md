...
- `agents.defaults.models` 是 Fased 可使用的模型白名单/目录（加上别名）。
...
正常用户应在 Control UI 的 **Agent > Models** 中连接 provider 认证，并为该 Agent 选择 Primary、Fallback 和任务角色模型。
...
如果设置了 `agents.defaults.models`，它将成为 `/model` 和会话覆盖的**白名单**。当用户选择不在该白名单中的模型时，Fased 返回：
...
`fased models`（无子命令）是 `models status` 的快捷方式。
...
`fased models scan` 检查 OpenRouter 的**免费模型目录**，并可选择性地探测模型的工具和图像支持。
...
`models.providers` 中的自定义提供商会写入智能体目录下的 `models.json`（默认 `~/.fased/agents/<agentId>/models.json`）。除非 `models.mode` 设置为 `replace`，否则此文件默认会被合并。
