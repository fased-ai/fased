...
- **会话存储**（聊天历史 + 路由状态）位于 `~/.fased/agents/<agentId>/sessions` 下。
...
Skills 通过每个工作区的 `skills/` 文件夹实现每智能体独立，共享的 Skills 可从 `~/.fased/skills` 获取。参见 [Skills：每智能体 vs 共享](/tools/skills#per-agent-vs-shared-skills)。
...
- 状态目录：`~/.fased`（或 `FASED_STATE_DIR`）
- 工作区：`~/.fased/workspace`（或 `~/.fased/workspace-<agentId>`）
- 智能体目录：`~/.fased/agents/<agentId>/agent`（或 `agents.list[].agentDir`）
- 会话：`~/.fased/agents/<agentId>/sessions`

### 单智能体模式（默认）

如果你什么都不做，Fased 运行单个智能体：

- `agentId` 默认为 **`main`**。
- 会话键为 `agent:main:<mainKey>`。
- 工作区默认为 `~/.fased/workspace`（或当设置了 `FASED_PROFILE` 时为 `~/.fased/workspace-<profile>`）。
- 状态默认为 `~/.fased/agents/main/agent`。
...
```json5
{
  agents: {
    list: [
      { id: "alex", workspace: "~/.fased/workspace-alex" },
      { id: "mia", workspace: "~/.fased/workspace-mia" },
    ],
  },
  bindings: [
...
```js
{
    ...
    list: [
      {
        ...
        default: true,
        name: "Home",
        workspace: "~/.fased/workspace-home",
        agentDir: "~/.fased/agents/home/agent",
      },
      {
        id: "work",
        name: "Work",
        workspace: "~/.fased/workspace-work",
        agentDir: "~/.fased/agents/work/agent",
      },
    ],
  },
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // 可选覆盖。默认：~/.fased/credentials/whatsapp/personal
          // authDir: "~/.fased/credentials/whatsapp/personal",
        },
        biz: {
    ...
    list: [
      {
        id: "chat",
        name: "Everyday",
        workspace: "~/.fased/workspace-chat",
        model: "anthropic/claude-sonnet-4-5",
      },
      {
        id: "opus",
        name: "Deep Work",
        workspace: "~/.fased/workspace-opus",
