...
x-i18n:
  ...
  source_path: concepts/date-time.md
  workflow: 14
---

# 日期与时间

Fased 默认使用**主机本地时间作为传输时间戳**，并且**仅在系统提示词中使用用户时区**。
...
当 `timeFormat: "auto"` 时，Fased 会检查操作系统偏好设置（macOS/Windows），并回退到区域格式。检测到的值会**按进程缓存**，以避免重复的系统调用。
