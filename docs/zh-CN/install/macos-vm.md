---
read_when:
  - 你想让 Fased 与你的主 macOS 环境隔离
  - 你想在沙箱中集成 iMessage（BlueBubbles）
  - 你想要一个可重置、可克隆的 macOS 环境
  - 你想比较本地与托管 macOS VM 选项
summary: 在沙箱隔离的 macOS VM（本地或托管）中运行 Fased，当你需要隔离或 iMessage 时
title: macOS 虚拟机
x-i18n:
...
# 在 macOS 虚拟机上运行 Fased（沙箱隔离）

## 推荐默认方案（大多数用户）

- **小型 Linux VPS** 用于永久在线的 Gateway 网关，成本低。参见 [VPS 托管](/install/vps)。
...
使用 [Lume](https://cua.ai/docs/lume) 在你现有的 Apple Silicon Mac 上的沙箱 macOS VM 中运行 Fased。
...
`./install.sh` 默认会运行新手引导。按照提示设置你的模型提供商
（Anthropic、OpenAI 等）。如果你使用了 `./install.sh --no-onboard`，
之后再运行 `fased onboard --install-daemon`。
...
VM 在后台运行。Fased 的守护进程保持 Gateway 网关运行。
...
这是在 macOS 上运行的杀手级功能。使用 [BlueBubbles](https://bluebubbles.app) 将 iMessage 添加到 Fased。
...
对于真正的永久在线，考虑专用 Mac mini 或小型 VPS。参见 [VPS 托管](/install/vps)。
...
| VM IP 未显示            | 等待 VM 完全启动，再次运行 `lume get fased`                   |
| 找不到 Lume 命令        | 将 `~/.local/bin` 添加到你的 PATH                             |
| WhatsApp 二维码扫描失败 | 确保运行 `fased channels login` 时你是登录到 VM（而不是主机） |
