---
read_when:
  - 你想在 GCP 上 24/7 运行 Fased
  - 你想要在自己的 VM 上运行常驻 Gateway 网关
  - 你想完全控制持久化、二进制文件和重启行为
summary: 在 GCP Compute Engine VM（Docker）上运行 Fased Gateway 网关并持久化状态
title: GCP
x-i18n:
...
# 在 GCP Compute Engine 上运行 Fased（Docker VPS 指南）

## 目标

使用 Docker 在 GCP Compute Engine VM 上运行持久化的 Fased Gateway 网关，具有持久状态、内置二进制文件和可预测的重启行为。

价格因机器类型和区域而异；选择适合你工作负载的 VM，如果遇到 OOM 则扩容。
...
- 在主机上持久化 `~/.fased` + `~/.fased/workspace`（重启/重建后仍保留）
- 在 onboarding 前把 VM 加入 Tailscale
...
```bash
gcloud compute ssh fased-gateway --zone=us-central1-a -- -L 18789:127.0.0.1:18789
```
...
Fased 在 Docker 中运行，但 Docker 不是真实来源。
...
| WhatsApp 会话    | `/home/node/.fased/`           | 主机卷挂载    | 保留 QR 登录                |
