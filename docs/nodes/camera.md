...
- **Advanced > Nodes** shows whether a node is paired, connected, and
  advertising camera capabilities.
- Device settings own camera and microphone permission prompts.
- **Agent > Tools** decides whether a selected Agent may call node-backed camera
  tools.
...
```bash
fased nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
fased nodes camera snap --node <id> --facing front
fased nodes camera clip --node <id> --duration 3000
...
- **Settings → General → Allow Camera** (`fased.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return “Camera disabled by user”.

### CLI helper (node invoke)

Use the main `fased` CLI to invoke camera commands on the macOS node.

Examples:

```bash
fased nodes camera list --node <id>            # list camera ids
fased nodes camera snap --node <id>            # prints MEDIA:<path>
...
```bash
fased nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```
