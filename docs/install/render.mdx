...
Deploy Fased on Render using Infrastructure as Code. The included `render.yaml`
...
<Note>
  Render is listed because this repository includes `render.yaml`. Fased does not document hosted
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/fased-ai/agent)
...
- Stale Blueprint prompts — if Render asks for variables that are not shown in
  the current `render.yaml`, sync your fork before deploying
