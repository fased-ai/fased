---
summary: "Manage paired devices, pending requests, and device-scoped tokens."
read_when:
...
Approve a pending device pairing request. If `requestId` is omitted, Fased
...
The command prints the new token result as JSON. Treat the returned token as a
secret.
...
Note: when you set `--url`, the CLI requires explicit credentials for that
target. Pass `--token` or `--password` explicitly. Missing explicit credentials
is an error.
