...
```bash
fased tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Messages are sent to the Gateway; external channel delivery is off by default.
- Turn delivery on:
  - `/deliver on`
  - or the Settings panel
  - or start with `fased tui --deliver`
...
- `--deliver`: deliver assistant replies through the session route (default off)
...
- Check the Gateway logs: `fased logs --follow`.
- Confirm the agent can run: `fased status` and `fased models status`.
...
- No agents in picker: check `fased agents list` and your routing config.
