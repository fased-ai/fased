---
summary: "Scripted onboarding and agent setup for the Fased CLI"
read_when:
...
Use `--non-interactive` to automate `fased onboard`.
...
Use `fased agents add <name>` to create a separate agent with its own workspace,
...
- Add `bindings` to route inbound messages. Normal users should do this from
  `Agent > Channels`; CLI flags are for automation.
