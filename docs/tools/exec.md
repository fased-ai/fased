...
`fased security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `fased doctor --fix` can scaffold missing custom `safeBinProfiles` entries.
