...
- **Availability gates**: `tools.elevated` is the global baseline.
  `agents.list[].tools.elevated` can further restrict elevated per agent. Both
  must allow.
...
- **Groups**: in group chats, elevated directives are only honored when the
  agent is mentioned. Command-only messages that bypass mention requirements are
...
- If elevated access is disabled or the sender is not on the approved allowlist,
  the directive replies with an actionable error and does not change session
  state.
...
- Unprefixed allowlist entries match sender-scoped identity values only:
  `SenderId`, `SenderE164`, `From`. Recipient routing fields are never used for
...
- Per-agent allowlist: `agents.list[].tools.elevated.allowFrom`. When set, the
  sender must match **both** global + per-agent allowlists.
- Discord fallback: if `tools.elevated.allowFrom.discord` is omitted, Fased uses
  `channels.discord.allowFrom` as a fallback. Legacy fallback:
  `channels.discord.dm.allowFrom`.
- Set `tools.elevated.allowFrom.discord`, even to `[]`, to override the
  fallback. Per-agent allowlists do **not** use the fallback.
