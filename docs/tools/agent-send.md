---
summary: "Direct `fased agent` CLI runs (with optional delivery)"
read_when:
  - Adding or modifying the agent CLI entrypoint
title: "Agent Send"
---

# `fased agent` (direct agent runs)

`fased agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `fased message --target`).
...
fased agent --to +15555550123 --message "Summon reply" --deliver
