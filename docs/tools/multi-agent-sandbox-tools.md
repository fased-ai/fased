...
Provider tool keys accept either `provider` (e.g. `anthropic`) or
...
- `group:fased`: all built-in Fased tools (excludes provider plugins)
...
Legacy `agent.*` configs are migrated by `fased doctor`; prefer `agents.defaults` + `agents.list` going forward.
