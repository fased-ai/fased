---
summary: "Chrome extension: let Fased drive your existing Chrome tab"
read_when:
  - You want the agent to drive an existing Chrome tab (toolbar button)
  - You need remote Gateway + local browser automation via Tailscale
  - You want to understand the security implications of attached tab control
...
The extension ships inside the Fased install as static files. There is no separate “build” step.

After upgrading Fased:

- Re-run `fased browser extension install` to refresh the installed files under your Fased state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

Fased ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `FASED_GATEWAY_TOKEN`)
...
- Open the tab you want Fased to control.
...
- `ON`: attached; Fased can drive that tab.
...
- Chrome extension relay control requires controlling the **host** browser control server.
...
Debugging: `fased sandbox explain`
...
This is powerful. Treat it like giving the model “hands on your browser”.
...
- **This is not isolated** like the dedicated fased-managed profile.
