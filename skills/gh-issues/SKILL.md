...
user-invocable: true
metadata: { "fased": { "requires": { "bins": ["curl", "git", "gh"] }, "primaryEnv": "GH_TOKEN" } }
---
...
IMPORTANT — No `gh` CLI dependency. This skill uses curl + the GitHub REST API exclusively. The GH_TOKEN env var is already injected by FasedAgent. Pass it as a Bearer token in all API calls:
...
**Token Resolution:**
`GH_TOKEN` must already be injected by FasedAgent or configured in the skill settings. Check only whether it exists; never print, echo, or read token values into the transcript:

```
test -n "${GH_TOKEN:-}" && printf 'GH_TOKEN is set\n'
```

If it is empty, stop and ask the user to configure the `gh-issues` skill API key in FasedAgent. Do not ask the user to paste the token into chat.
...
- If curl returns an HTTP 401 or 403 → stop and tell the user:
  > "GitHub authentication failed. Please check your apiKey in the FasedAgent dashboard or in ~/.fased/fased.json under skills.entries.gh-issues."
...
3. **Verify remote access:**
   ...
   - Verify the fork remote exists. Check if a git remote named `fork` exists:
     ...
     ```
     If it doesn't exist, add it without embedding a token in the URL:
     ```
     git remote add fork https://github.com/{PUSH_REPO}.git
     ```
...
7. **Check claim-based in-progress tracking:**
   ...
   ```
   CLAIMS_FILE="/data/.fased/gh-issues-claims.json"
...
- **Sequential cursor tracking:** Use a cursor file to track which issue to process next:
  ...
  ```
  CURSOR_FILE="/data/.fased/gh-issues-cursor-{SOURCE_REPO_SLUG}.json"
  # SOURCE_REPO_SLUG = owner-repo with slashes replaced by hyphens (e.g., fased-fased)
  ```
...
First, ensure GH_TOKEN is set without printing it:
test -n "${GH_TOKEN:-}" || { echo "GH_TOKEN is not configured for gh-issues"; exit 1; }
...
0. SETUP — Ensure GH_TOKEN is available without printing it:
```
...
Use the configured remote and the injected GitHub token environment. Do not write the token into git remote URLs.
...
Do not write the token into git remote URLs.
