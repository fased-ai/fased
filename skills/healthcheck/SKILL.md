---
name: healthcheck
description: Audit and harden hosts running Fased Agent for SSH, firewall, updates, exposure, cron checks, and risk posture.
---

# FasedAgent Host Hardening

## Overview

Assess and harden the host running FasedAgent, then align it to a user-defined risk tolerance without breaking access. Use FasedAgent security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim FasedAgent changes the host firewall, SSH, or OS updates; it does not.
...
5. FasedAgent gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if FasedAgent can access sensitive data.
...
### 2) Run FasedAgent security audits (read-only)

As part of the default read-only checks, run `fased security audit --deep`. Only offer alternatives if the user requests them:

1. `fased security audit` (faster, non-probing)
2. `fased security audit --json` (structured output)

Offer to apply FasedAgent safe defaults (numbered):

1. `fased security audit --fix`

Be explicit that `--fix` only tightens FasedAgent defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check FasedAgent version/update status (read-only)

As part of the default read-only checks, run `fased update status`.
...
- Credential hygiene notes (location of FasedAgent creds, prefer disk encryption)
...
- FasedAgent security audit (re-run)
...
After FasedAgent install or first hardening pass, run at least one baseline audit and version check:

- `fased security audit`
- `fased security audit --deep`
- `fased update status`

Ongoing monitoring is recommended. Use the FasedAgent cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `fased cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `fased update status`
...
Before creating, `fased cron list` and match on exact `name`. If found, `fased cron edit <id> ...`.
If not found, `fased cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `fased update status` (preferred for source checkouts and channels)
2. `npm view fased version` (published npm version)

## FasedAgent command accuracy

Use only supported commands and flags:

- `fased security audit [--deep] [--fix] [--json]`
- `fased status` / `fased status --deep`
- `fased health --json`
- `fased update status`
- `fased cron add|list|runs|run`

Do not invent CLI flags or imply FasedAgent enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by FasedAgent compaction:
