...
metadata: { "fased": { "requires": { "bins": ["node"] } } }
---

# Node Inspect Debugger

Use for local Node.js debugging that needs inspector access: hidden locals, async hangs, flaky tests, child processes, startup races, memory growth, or CPU hot paths.
...
- Fased CLI path: `node --inspect-brk fased.mjs ...`
- Fased test path: `node --inspect-brk ./node_modules/vitest/vitest.mjs run <file> --maxWorkers=1`
...
- Prefer `127.0.0.1` inspector binds.
- Do not expose `--inspect=0.0.0.0` unless the network is isolated and the operator explicitly asked for it.
- For Vitest, debug one file with one worker. Avoid worker pools while stepping.
- For TypeScript source breakpoints, use `--enable-source-maps` when useful.
