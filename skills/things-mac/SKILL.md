...
- If DB reads fail: grant **Full Disk Access** to the calling app (Terminal for manual runs; `FasedAgent.app` for gateway runs).
...
- Auth: set `THINGS_AUTH_TOKEN` in your local shell or keychain-backed environment. Do not paste it into chat.
...
- Complete/cancel (soft-delete-ish): `things update --id <UUID> --completed` / `--canceled`
