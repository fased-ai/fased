...
- **Wake word**: Default `fased` (alias `hey fased`), optional `--no-wake` bypass.
...
## Use as a library
Use swabble as a local SwiftPM package and import the `Swabble` or `SwabbleKit` product:
...
```json
{
  ...
  "wake": {"enabled": true, "word": "fased", "aliases": ["hey fased"]},
  "hook": {
    "command": "",
    "args": [],
    "prefix": "Voice request from ${hostname}: ",
    "cooldownSeconds": 1,
    "minCharacters": 24,
    "timeoutSeconds": 5,
    "env": {}
