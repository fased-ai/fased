...
Goal: always-on local voice hooks for macOS 26 using Apple Speech.framework (SpeechAnalyzer + SpeechTranscriber). Local-only, wake word gated, dispatches a shell hook with the transcript. Shared wake-gate utilities live in `SwabbleKit` for reuse by other apps (iOS/macOS).

## Requirements
- macOS 26+, Swift 6.2, Speech.framework with on-device assets.
- Local only; no network calls during transcription.
- Wake word gating (default "fased" plus aliases) with bypass flag `--no-wake`.
...
- Hook redaction placeholder; no implementation yet.
