...
final class MotionService: MotionServicing {
    func activities(params: FasedAgentMotionActivityParams) async throws -> FasedAgentMotionActivityPayload {
        ...
        let mapped: [FasedAgentMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return FasedAgentMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: FasedAgentPedometerParams) async throws -> FasedAgentPedometerPayload {
        ...
        let payload: FasedAgentPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = FasedAgentPedometerPayload(
