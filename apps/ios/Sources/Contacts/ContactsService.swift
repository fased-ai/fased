...
final class ContactsService: ContactsServicing {
    ...
    func search(params: FasedAgentContactsSearchParams) async throws -> FasedAgentContactsSearchPayload {
        ...
        return FasedAgentContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: FasedAgentContactsAddParams) async throws -> FasedAgentContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return FasedAgentContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return FasedAgentContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> FasedAgentContactPayload {
        FasedAgentContactPayload(
