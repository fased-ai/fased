...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__FASED_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<FasedAgentTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.fased", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = FasedAgentTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> FasedAgentTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return FasedAgentTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> FasedAgentTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> FasedAgentTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return FasedAgentTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> FasedAgentTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = FasedAgentTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: FasedAgentTalkPTTStopPayload) {
