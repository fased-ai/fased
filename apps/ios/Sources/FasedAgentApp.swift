...
@MainActor
final class FasedAgentAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.fased.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.fased.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.fased.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    static let typeKey = "fased.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "fased.watch.promptId"
    static let sessionKeyKey = "fased.watch.sessionKey"
    static let actionPrimaryIDKey = "fased.watch.action.primary.id"
    static let actionPrimaryLabelKey = "fased.watch.action.primary.label"
    static let actionSecondaryIDKey = "fased.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "fased.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "fased.watch.action.primary"
    static let actionSecondaryIdentifier = "fased.watch.action.secondary"
    static let actionIdentifierPrefix = "fased.watch.action."
    static let actionIDKeyPrefix = "fased.watch.action.id."
    static let actionLabelKeyPrefix = "fased.watch.action.label."
    static let categoryPrefix = "fased.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: FasedAgentWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> FasedAgentWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return FasedAgentWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "FasedAgent" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [FasedAgentWatchAction]) -> [UNNotificationAction] {
...
@main
struct FasedAgentApp: App {
    @State private var appModel: NodeAppModel
    @State private var gatewayController: GatewayConnectionController
    @UIApplicationDelegateAdaptor(FasedAgentAppDelegate.self) private var appDelegate
...
extension FasedAgentApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("FasedAgent: installing uncaught exception handler")
