...
final class NodeAppModel {
    ...
    private let deepLinkLogger = Logger(subsystem: "ai.fased.ios", category: "DeepLink")
    private let pushWakeLogger = Logger(subsystem: "ai.fased.ios", category: "PushWake")
    private let locationWakeLogger = Logger(subsystem: "ai.fased.ios", category: "LocationWake")
    private let watchReplyLogger = Logger(subsystem: "ai.fased.ios", category: "WatchReply")
    enum CameraHUDKind {
        case photo
        case recording
        case success
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = FasedAgentCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = FasedAgentCanvasA2UIAction.compactJSON(userAction["context"])
        let sessionKey = self.mainSessionKey

        let messageContext = FasedAgentCanvasA2UIAction.AgentMessageContext(
            actionName: name,
            session: .init(key: sessionKey, surfaceId: surfaceId),
            component: .init(id: sourceComponentId, host: host, instanceId: instanceId),
            contextJSON: contextJSON)
        let message = FasedAgentCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = FasedAgentCanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    func requestLocationPermissions(mode: FasedAgentLocationMode) async -> Bool {
        ...
        self.gatewayHealthMonitor.start(
            check: { [weak self] in
                ...
                do {
                    ...
                    guard let decoded = try? JSONDecoder().decode(FasedAgentGatewayHealthOK.self, from: data) else {
                        return false
                    }
        ...
        do {
            ...
            case .unknownCommand:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: FasedAgentNodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            case .handlerUnavailable:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: FasedAgentNodeError(code: .unavailable, message: "node handler unavailable"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: FasedAgentNodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(FasedAgentLocationGetParams.self, from: req.paramsJSON)) ??
            FasedAgentLocationGetParams()
        ...
        let payload = FasedAgentLocationPayload(
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case FasedAgentCanvasCommand.present.rawValue:
            // iOS ignores placement hints; canvas always fills the screen.
            let params = (try? Self.decodeParams(FasedAgentCanvasPresentParams.self, from: req.paramsJSON)) ??
                FasedAgentCanvasPresentParams()
        ...
        case FasedAgentCanvasCommand.hide.rawValue:
            self.screen.showDefaultCanvas()
            return BridgeInvokeResponse(id: req.id, ok: true)
        case FasedAgentCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(FasedAgentCanvasNavigateParams.self, from: req.paramsJSON)
            self.screen.navigate(to: params.url)
            return BridgeInvokeResponse(id: req.id, ok: true)
        case FasedAgentCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(FasedAgentCanvasEvalParams.self, from: req.paramsJSON)
            let result = try await self.screen.eval(javaScript: params.javaScript)
            let payload = try Self.encodePayload(["result": result])
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case FasedAgentCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(FasedAgentCanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let command = req.command
        switch command {
        case FasedAgentCanvasA2UICommand.reset.rawValue:
            ...
            (() => {
              const host = globalThis.fasedA2UI;
              if (!host) return JSON.stringify({ ok: false, error: "missing fasedA2UI" });
              return JSON.stringify(host.reset());
            })()
            """)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case FasedAgentCanvasA2UICommand.push.rawValue, FasedAgentCanvasA2UICommand.pushJSONL.rawValue:
            let messages: [FasedAgentKit.AnyCodable]
            if command == FasedAgentCanvasA2UICommand.pushJSONL.rawValue {
                let params = try Self.decodeParams(FasedAgentCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
                messages = try FasedAgentCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
            } else {
                do {
                    let params = try Self.decodeParams(FasedAgentCanvasA2UIPushParams.self, from: req.paramsJSON)
            ...
            let messagesJSON = try FasedAgentCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
            let js = """
            (() => {
              try {
                const host = globalThis.fasedA2UI;
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case FasedAgentCameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case FasedAgentCameraCommand.snap.rawValue:
            self.showCameraHUD(text: "Taking photo…", kind: .photo)
            self.triggerCameraFlash()
            let params = (try? Self.decodeParams(FasedAgentCameraSnapParams.self, from: req.paramsJSON)) ??
                FasedAgentCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case FasedAgentCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(FasedAgentCameraClipParams.self, from: req.paramsJSON)) ??
                FasedAgentCameraClipParams()
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(FasedAgentScreenRecordParams.self, from: req.paramsJSON)) ??
            FasedAgentScreenRecordParams()
    ...
    private func handleSystemNotify(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(FasedAgentSystemNotifyParams.self, from: req.paramsJSON)
        ...
        if title.isEmpty, body.isEmpty {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: FasedAgentNodeError(code: .invalidRequest, message: "INVALID_REQUEST: empty notification"))
        }
        ...
        guard finalStatus == .authorized || finalStatus == .provisional || finalStatus == .ephemeral else {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: FasedAgentNodeError(code: .unavailable, message: "NOT_AUTHORIZED: notifications"))
        }
        ...
        if case let .failure(error) = addResult {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: FasedAgentNodeError(code: .unavailable, message: "NOTIFICATION_FAILED: \(error.message)"))
        }
        return BridgeInvokeResponse(id: req.id, ok: true)
    }

    private func handleChatPushInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(FasedAgentChatPushParams.self, from: req.paramsJSON)
        let text = params.text.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !text.isEmpty else {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: FasedAgentNodeError(code: .invalidRequest, message: "INVALID_REQUEST: empty chat.push text"))
        }
        ...
        let payload = FasedAgentChatPushPayload(messageId: messageId)
        let json = try Self.encodePayload(payload)
        return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
    }
    ...
    private func handleDeviceInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case FasedAgentDeviceCommand.status.rawValue:
            let payload = try await self.deviceStatusService.status()
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case FasedAgentDeviceCommand.info.rawValue:
    ...
    private func handlePhotosInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(FasedAgentPhotosLatestParams.self, from: req.paramsJSON)) ??
            FasedAgentPhotosLatestParams()
    ...
    private func handleContactsInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case FasedAgentContactsCommand.search.rawValue:
            let params = (try? Self.decodeParams(FasedAgentContactsSearchParams.self, from: req.paramsJSON)) ??
                FasedAgentContactsSearchParams()
            let payload = try await self.contactsService.search(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case FasedAgentContactsCommand.add.rawValue:
            let params = try Self.decodeParams(FasedAgentContactsAddParams.self, from: req.paramsJSON)
    ...
    private func handleCalendarInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case FasedAgentCalendarCommand.events.rawValue:
            let params = (try? Self.decodeParams(FasedAgentCalendarEventsParams.self, from: req.paramsJSON)) ??
                FasedAgentCalendarEventsParams()
            let payload = try await self.calendarService.events(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case FasedAgentCalendarCommand.add.rawValue:
            let params = try Self.decodeParams(FasedAgentCalendarAddParams.self, from: req.paramsJSON)
    ...
    private func handleRemindersInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case FasedAgentRemindersCommand.list.rawValue:
            let params = (try? Self.decodeParams(FasedAgentRemindersListParams.self, from: req.paramsJSON)) ??
                FasedAgentRemindersListParams()
            let payload = try await self.remindersService.list(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case FasedAgentRemindersCommand.add.rawValue:
            let params = try Self.decodeParams(FasedAgentRemindersAddParams.self, from: req.paramsJSON)
    ...
    private func handleMotionInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case FasedAgentMotionCommand.activity.rawValue:
            let params = (try? Self.decodeParams(FasedAgentMotionActivityParams.self, from: req.paramsJSON)) ??
                FasedAgentMotionActivityParams()
            let payload = try await self.motionService.activities(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case FasedAgentMotionCommand.pedometer.rawValue:
            let params = (try? Self.decodeParams(FasedAgentPedometerParams.self, from: req.paramsJSON)) ??
                FasedAgentPedometerParams()
    ...
    private func handleTalkInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case FasedAgentTalkCommand.pttStart.rawValue:
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case FasedAgentTalkCommand.pttStop.rawValue:
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case FasedAgentTalkCommand.pttCancel.rawValue:
            let payload = await self.talkMode.cancelPushToTalk()
            self.voiceWake.resumeAfterExternalAudioCapture(wasSuspended: self.pttVoiceWakeSuspended)
            self.pttVoiceWakeSuspended = false
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case FasedAgentTalkCommand.pttOnce.rawValue:
            self.pttVoiceWakeSuspended = self.voiceWake.suspendForExternalAudioCapture()
            defer {
                self.voiceWake.resumeAfterExternalAudioCapture(wasSuspended: self.pttVoiceWakeSuspended)
                self.pttVoiceWakeSuspended = false
...
private extension NodeAppModel {
    ...
    func buildCapabilityRouter() -> NodeCapabilityRouter {
        ...
        register([FasedAgentLocationCommand.get.rawValue]) { [weak self] req in
        ...
        register([
            ...
            FasedAgentCanvasCommand.hide.rawValue,
            FasedAgentCanvasCommand.navigate.rawValue,
            FasedAgentCanvasCommand.evalJS.rawValue,
        ...
        register([
            ...
            FasedAgentCanvasA2UICommand.push.rawValue,
            FasedAgentCanvasA2UICommand.pushJSONL.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            FasedAgentCameraCommand.list.rawValue,
            FasedAgentCameraCommand.snap.rawValue,
            FasedAgentCameraCommand.clip.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleCameraInvoke(req)
        }

        register([FasedAgentScreenCommand.record.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleScreenRecordInvoke(req)
        }

        register([FasedAgentSystemCommand.notify.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleSystemNotify(req)
        }

        register([FasedAgentChatCommand.push.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleChatPushInvoke(req)
        }

        register([
            FasedAgentDeviceCommand.status.rawValue,
            FasedAgentDeviceCommand.info.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            FasedAgentWatchCommand.status.rawValue,
            FasedAgentWatchCommand.notify.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleWatchInvoke(req)
        }

        register([FasedAgentPhotosCommand.latest.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handlePhotosInvoke(req)
        }

        register([
            FasedAgentContactsCommand.search.rawValue,
            FasedAgentContactsCommand.add.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            FasedAgentCalendarCommand.events.rawValue,
            FasedAgentCalendarCommand.add.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            FasedAgentRemindersCommand.list.rawValue,
            FasedAgentRemindersCommand.add.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            FasedAgentMotionCommand.activity.rawValue,
            FasedAgentMotionCommand.pedometer.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            FasedAgentTalkCommand.pttStart.rawValue,
            FasedAgentTalkCommand.pttStop.rawValue,
            FasedAgentTalkCommand.pttCancel.rawValue,
            FasedAgentTalkCommand.pttOnce.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
    ...
    func handleWatchInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case FasedAgentWatchCommand.status.rawValue:
            let status = await self.watchMessagingService.status()
            let payload = FasedAgentWatchStatusPayload(
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case FasedAgentWatchCommand.notify.rawValue:
            let params = try Self.decodeParams(FasedAgentWatchNotifyParams.self, from: req.paramsJSON)
            ...
            do {
                ...
                let payload = FasedAgentWatchNotifyPayload(
    ...
    func locationMode() -> FasedAgentLocationMode {
        let raw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        return FasedAgentLocationMode(rawValue: raw) ?? .off
    }
                            ...
                            if let requestId, !requestId.isEmpty {
                                self.gatewayStatusText =
                                    "Pairing required (requestId: \(requestId)). "
                                        + "Approve on gateway and return to FasedAgent."
                            } else {
                                self.gatewayStatusText =
                                    "Pairing required. Approve on gateway and return to FasedAgent."
                            }
                        }
...
extension NodeAppModel {
    ...
    func runSharePipelineSelfTest() async {
        ...
        let payload = SharedContentPayload(
            title: "FasedAgent Share Self-Test",
            url: URL(string: "https://fased.ai/share-self-test"),
    ...
    func handleSilentPushWake(_ userInfo: [AnyHashable: Any]) async -> Bool {
        ...
        let pushKind = Self.fasedPushKind(userInfo)
        let receivedMessage =
    ...
    private static func fasedPushKind(_ userInfo: [AnyHashable: Any]) -> String {
        if let payload = userInfo["fased"] as? [String: Any],
           let kind = payload["kind"] as? String
        {
            let trimmed = kind.trimmingCharacters(in: .whitespacesAndNewlines)
            if !trimmed.isEmpty { return trimmed }
        }
        if let payload = userInfo["fased"] as? [AnyHashable: Any],
           let kind = payload["kind"] as? String
        {
            let trimmed = kind.trimmingCharacters(in: .whitespacesAndNewlines)
            if !trimmed.isEmpty { return trimmed }
