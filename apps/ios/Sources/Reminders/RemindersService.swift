...
final class RemindersService: RemindersServicing {
    func list(params: FasedAgentRemindersListParams) async throws -> FasedAgentRemindersListPayload {
        ...
        let payload: [FasedAgentReminderPayload] = try await withCheckedThrowingContinuation { cont in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return FasedAgentReminderPayload(
        ...
        return FasedAgentRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: FasedAgentRemindersAddParams) async throws -> FasedAgentRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = FasedAgentReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return FasedAgentRemindersAddPayload(reminder: payload)
    }
