...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.fased.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to FasedAgent", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to FasedAgent gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to FasedAgent.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "FasedAgentShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "FasedAgent is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "FasedAgent Share",
        ...
        try await gateway.connect(
            ...
            connectOptions: makeOptions("fased-ios"),
            sessionBox: nil,
            onConnected: {},
            onDisconnected: { _ in },
            onInvoke: { req in
