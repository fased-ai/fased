...
@Suite struct MacGatewayChatTransportMappingTests {
    @Test func snapshotMapsToHealth() {
        let snapshot = Snapshot(
            presence: [],
            health: FasedAgentProtocol.AnyCodable(["ok": FasedAgentProtocol.AnyCodable(false)]),
    ...
    @Test func healthEventMapsToHealth() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: FasedAgentProtocol.AnyCodable(["ok": FasedAgentProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func chatEventMapsToChat() {
        let payload = FasedAgentProtocol.AnyCodable([
            "runId": FasedAgentProtocol.AnyCodable("run-1"),
            "sessionKey": FasedAgentProtocol.AnyCodable("main"),
            "state": FasedAgentProtocol.AnyCodable("final"),
        ])
    ...
    @Test func unknownEventMapsToNil() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: FasedAgentProtocol.AnyCodable(["a": FasedAgentProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
