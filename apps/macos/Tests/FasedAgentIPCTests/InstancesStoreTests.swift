...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: FasedAgentProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: FasedAgentProtocol.AnyCodable] = [
            "presence": .init([FasedAgentProtocol.AnyCodable(entry)]),
        ]
        let payload = FasedAgentProtocol.AnyCodable(payloadMap)
