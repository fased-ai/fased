...
enum CommandResolver {
    private static let projectRootDefaultsKey = "fased.gatewayProjectRootPath"
    private static let helperName = "fased"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let fasedEntry = root.appendingPathComponent("fased.mjs").path
        if FileManager().isReadableFile(atPath: fasedEntry) { return fasedEntry }
        let binEntry = root.appendingPathComponent("bin/fased.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/fased")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let fasedPaths = self.fasedManagedPaths(home: home)
        if !fasedPaths.isEmpty {
            extras.insert(contentsOf: fasedPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + fasedPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func fasedManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".fased"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func fasedExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectFasedAgentExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("fased.mjs").path,
            root.appendingPathComponent("bin/fased.js").path,
        ]
    ...
    static func hasAnyFasedAgentInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.fasedExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        let root = self.projectRoot()
        if let fasedPath = self.projectFasedAgentExecutable(projectRoot: root) {
            return [fasedPath, subcommand] + extraArgs
        }
        if let fasedPath = self.fasedExecutable(searchPaths: searchPaths) {
            return [fasedPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "fased", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            fased entrypoint missing (looked for dist/index.js or fased.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real fased CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/fased"
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/fased.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/fased.mjs"
            node "$PRJ/fased.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/fased.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/fased.js"
            node "$PRJ/bin/fased.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent fased"
          pnpm --silent fased \(quotedArgs);
        else
          echo "fased CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? FasedAgentConfigFile.loadDict()
