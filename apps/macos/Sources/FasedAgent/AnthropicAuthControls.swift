...
struct AnthropicAuthControls: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                Spacer()
                Button("Reveal") {
                    NSWorkspace.shared.activateFileViewerSelecting([FasedAgentOAuthStore.oauthURL()])
                }
                .buttonStyle(.bordered)
                .disabled(!FileManager().fileExists(atPath: FasedAgentOAuthStore.oauthURL().path))
            ...
            Text(FasedAgentOAuthStore.oauthURL().path)
    ...
    private func refresh() {
        let imported = FasedAgentOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = FasedAgentOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try FasedAgentOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. FasedAgent can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: FasedAgentOAuthStore.AnthropicOAuthStatus,
