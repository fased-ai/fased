...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: FasedAgentProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.fased", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: FasedAgentKit.AnyCodable]()) {
                $0[$1.key] = FasedAgentKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: FasedAgentProtocol.AnyCodable?) -> [String: FasedAgentProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: FasedAgentProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: FasedAgentKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: FasedAgentProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("fased.control.heartbeat")
    static let controlAgentEvent = Notification.Name("fased.control.agent")
}
