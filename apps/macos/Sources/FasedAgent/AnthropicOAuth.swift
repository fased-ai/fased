...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (FasedAgent token file)"
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "ai.fased", category: "anthropic-oauth")
...
enum FasedAgentOAuthStore {
    static let oauthFilename = "oauth.json"
    private static let providerKey = "anthropic"
    private static let fasedOAuthDirEnv = "FASED_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "FasedAgent OAuth token file not found"
            case .unreadableFile: "FasedAgent OAuth token file not readable"
            case .invalidJSON: "FasedAgent OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in FasedAgent OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "FasedAgent OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.fasedOAuthDirEnv]?
        ...
        return home.appendingPathComponent(".fased", isDirectory: true)
