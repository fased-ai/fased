...
enum DebugActions {
    private static let verboseDefaultsKey = "fased.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "fased.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = FasedAgentPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "FasedAgent", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = FasedAgentPaths.configURL
        guard
