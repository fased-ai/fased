...
enum FasedAgentLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = FasedAgentOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = FasedAgentFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        FasedAgentLogging.bootstrapIfNeeded()
        let label = FasedAgentLogging.makeLabel(subsystem: subsystem, category: category)
...
struct FasedAgentOSLogHandler: LogHandler {
...
struct FasedAgentFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = FasedAgentLogging.parseLabel(self.label)
