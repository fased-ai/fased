...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.fased/fased.json. Prefer FASED_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.fased/fased.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.fased/fased.json. Restart the gateway to apply."
        }
