...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.fased.mac`) even if the shipped
    ...
    {
        if environment["FASED_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "fased.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "fased.nixMode") { return true }
