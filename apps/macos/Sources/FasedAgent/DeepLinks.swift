...
private let deepLinkLogger = Logger(subsystem: "ai.fased", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "FasedAgent is paused", message: "Unpause FasedAgent to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run FasedAgent agent?", message: body) else { return }
        }
