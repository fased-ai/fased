...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.fased", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> FasedAgentSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: FasedAgentSessionPreviewEntry,
    ...
    private static func cache(payload: FasedAgentSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: FasedAgentChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [FasedAgentKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [FasedAgentKit.AnyCodable]) -> [FasedAgentChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(FasedAgentChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: FasedAgentChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: FasedAgentChatMessage) -> Bool {
    ...
    private static func toolNames(for message: FasedAgentChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: FasedAgentChatMessage) -> String? {
