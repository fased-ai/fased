...
private let webChatSwiftLogger = Logger(subsystem: "ai.fased", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: FasedAgentChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> FasedAgentChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(FasedAgentChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> FasedAgentChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                FasedAgentGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "chat":
                ...
                guard let chat = try? JSONDecoder().decode(
                    FasedAgentChatEventPayload.self,
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    FasedAgentAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<FasedAgentChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any FasedAgentChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = FasedAgentChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: FasedAgentChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<FasedAgentChatView>) -> NSViewController
    {
