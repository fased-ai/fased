...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.fased", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: FasedAgentPeekabooBridgeServices?
    private static var fasedSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("FasedAgent", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = FasedAgentPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.fasedSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.fasedSocketPath, privacy: .public)")
    }
...
@MainActor
private final class FasedAgentPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.fased.peekaboo")
