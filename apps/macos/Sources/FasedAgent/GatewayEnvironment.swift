...
enum GatewayEnvironment {
    private static let logger = Logger(subsystem: "ai.fased", category: "gateway.env")
    ...
    static func check() -> GatewayEnvironmentStatus {
        ...
        case let .success(runtime):
            let gatewayBin = CommandResolver.fasedExecutable()
            ...
            if gatewayBin == nil, projectEntrypoint == nil {
                return GatewayEnvironmentStatus(
                    ...
                    gatewayVersion: nil,
                    requiredGateway: expectedString,
                    message: "fased CLI not found in PATH; install the CLI.")
            }
    ...
    private static func preferredGatewayBind() -> String? {
        ...
        if let env = ProcessInfo.processInfo.environment["FASED_GATEWAY_BIND"] {
    ...
    static func installGlobal(versionString: String?, statusHandler: @escaping @Sendable (String) -> Void) async {
        ...
        let (label, cmd): (String, [String]) =
            if let npm {
                ("npm", [npm, "install", "-g", "fased@\(target)"])
            } else if let pnpm {
                ("pnpm", [pnpm, "add", "-g", "fased@\(target)"])
            } else if let bun {
                ("bun", [bun, "add", "-g", "fased@\(target)"])
            } else {
                ("npm", ["npm", "install", "-g", "fased@\(target)"])
            }

        statusHandler("Installing fased@\(target) via \(label)…")
        ...
        if response.success {
            statusHandler("Installed fased@\(target)")
        } else {
