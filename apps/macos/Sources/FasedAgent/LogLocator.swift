...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["FASED_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/fased")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("fased-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("fased-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/fased/ (rolling or stdout), or nil if none exist.
