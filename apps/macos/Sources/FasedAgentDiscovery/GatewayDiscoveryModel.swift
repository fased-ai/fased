...
public final class GatewayDiscoveryModel {
    ...
    private let logger = Logger(subsystem: "ai.fased", category: "gateway-discovery")
    ...
    public func start() {
        ...
        for domain in FasedAgentBonjour.gatewayServiceDomains {
            ...
            let browser = NWBrowser(
                for: .bonjour(type: FasedAgentBonjour.gatewayServiceType, domain: domain),
                using: params)
            ...
            browser.start(queue: DispatchQueue(label: "ai.fased.macos.gateway-discovery.\(domain)"))
        }
    ...
    private func updateGateways(for domain: String) {
        ...
        if let wideAreaDomain = FasedAgentBonjour.wideAreaGatewayServiceDomain,
    ...
    private nonisolated static func prettifyInstanceName(_ decodedName: String) -> String {
        ...
        let stripped = normalized.replacingOccurrences(of: " (FasedAgent)", with: "")
