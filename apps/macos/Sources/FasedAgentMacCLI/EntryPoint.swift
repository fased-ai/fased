...
struct FasedAgentMacCLI {
    static func main() async {
        ...
        default:
            fputs("fased-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      fased-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      fased-mac discover [--timeout <ms>] [--json] [--include-local]
      fased-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      fased-mac connect
      fased-mac connect --url ws://127.0.0.1:18789 --json
      fased-mac discover --timeout 3000 --json
      fased-mac wizard --mode local
    """)
}
