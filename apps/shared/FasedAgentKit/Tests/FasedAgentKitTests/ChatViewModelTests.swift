...
import Foundation
import Testing
@testable import FasedAgentChatUI
...
private final class TestChatTransport: @unchecked Sendable, FasedAgentChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [FasedAgentChatHistoryPayload]
    private let sessionsResponses: [FasedAgentChatSessionsListResponse]

    private let stream: AsyncStream<FasedAgentChatTransportEvent>
    private let continuation: AsyncStream<FasedAgentChatTransportEvent>.Continuation

    init(
        historyResponses: [FasedAgentChatHistoryPayload],
        sessionsResponses: [FasedAgentChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<FasedAgentChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> FasedAgentChatHistoryPayload {
        ...
        return self.historyResponses.last ?? FasedAgentChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [FasedAgentChatAttachmentPayload]) async throws -> FasedAgentChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return FasedAgentChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> FasedAgentChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? FasedAgentChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: FasedAgentChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = FasedAgentChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = FasedAgentChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { FasedAgentChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = FasedAgentChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = FasedAgentChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = FasedAgentChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { FasedAgentChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
