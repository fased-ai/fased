...
@MainActor
public struct FasedAgentChatView: View {
    public enum Style {
        case standard
        case onboarding
    }

    @State private var viewModel: FasedAgentChatViewModel
    ...
    public init(
        viewModel: FasedAgentChatViewModel,
    ...
    public var body: some View {
        ZStack {
            if self.style == .standard {
                FasedAgentChatTheme.background
                    .ignoresSafeArea()
            }
    ...
    private var visibleMessages: [FasedAgentChatMessage] {
        let base: [FasedAgentChatMessage]
    ...
    private func mergeToolResults(in messages: [FasedAgentChatMessage]) -> [FasedAgentChatMessage] {
        ...
        for message in messages {
            ...
            let merged = FasedAgentChatMessage(
                id: last.id,
                role: last.role,
                content: content,
    ...
    private func isToolResultMessage(_ message: FasedAgentChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func toolCallIds(in message: FasedAgentChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: FasedAgentChatMessage) -> String {
