...
enum FasedAgentChatTheme {
    ...
    static let assistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("FasedAgentChatTheme.assistantBubble"),
        dynamicProvider: resolvedAssistantBubbleColor(for:))

    static let onboardingAssistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("FasedAgentChatTheme.onboardingAssistantBubble"),
...
enum FasedAgentPlatformImageFactory {
    static func image(_ image: FasedAgentPlatformImage) -> Image {
        #if os(macOS)
        Image(nsImage: image)
        #else
        Image(uiImage: image)
