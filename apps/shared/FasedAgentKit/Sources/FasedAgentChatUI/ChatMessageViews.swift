...
struct ChatMessageBubble: View {
    let message: FasedAgentChatMessage
    let style: FasedAgentChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? FasedAgentChatTheme.userText : FasedAgentChatTheme.assistantText
    ...
    private var inlineAttachments: [FasedAgentChatMessageContent] {
    ...
    private var toolCalls: [FasedAgentChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [FasedAgentChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? FasedAgentChatTheme.userBubble
        }
        if self.style == .onboarding {
            return FasedAgentChatTheme.onboardingAssistantBubble
        }
        return FasedAgentChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return FasedAgentChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: FasedAgentChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? FasedAgentChatTheme.userText : FasedAgentChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: FasedAgentChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(FasedAgentChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(FasedAgentChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [FasedAgentChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: FasedAgentChatTheme.assistantText)
            }
        }
    }
}
