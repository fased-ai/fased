...
private let chatUILogger = Logger(subsystem: "ai.fased", category: "FasedAgentChatUI")

@MainActor
@Observable
public final class FasedAgentChatViewModel {
    public private(set) var messages: [FasedAgentChatMessage] = []
    ...
    public var attachments: [FasedAgentPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [FasedAgentChatPendingToolCall] = []
    public private(set) var sessions: [FasedAgentChatSessionEntry] = []
    private let transport: any FasedAgentChatTransport
    ...
    private var pendingToolCallsById: [String: FasedAgentChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any FasedAgentChatTransport) {
    ...
    public var sessionChoices: [FasedAgentChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }

        var result: [FasedAgentChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: FasedAgentPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [FasedAgentChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: FasedAgentChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: FasedAgentChatMessage) -> FasedAgentChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> FasedAgentChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return FasedAgentChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageIdentityKey(for message: FasedAgentChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [FasedAgentChatMessage],
        incoming: [FasedAgentChatMessage]) -> [FasedAgentChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [FasedAgentChatMessage]) -> [FasedAgentChatMessage] {
        var result: [FasedAgentChatMessage] = []
    ...
    private static func dedupeKey(for message: FasedAgentChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [FasedAgentChatMessageContent] = [
            FasedAgentChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> FasedAgentChatAttachmentPayload in
            FasedAgentChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> FasedAgentChatSessionEntry {
        FasedAgentChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: FasedAgentChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: FasedAgentChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: FasedAgentAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = FasedAgentChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> FasedAgentPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
