...
@MainActor
struct FasedAgentChatComposer: View {
    @Bindable var viewModel: FasedAgentChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(FasedAgentChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(FasedAgentChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \FasedAgentPendingAttachment.id)
                { (att: FasedAgentPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            FasedAgentPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if !self.isComposerCompacted {
                Rectangle()
                    .fill(FasedAgentChatTheme.divider)
                    .frame(height: 1)
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(FasedAgentChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(FasedAgentChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(FasedAgentChatTheme.subtleCard)
        .clipShape(Capsule())
    }
