...
// Set HOME/state isolation before importing any runtime FasedAgent modules.
...
const createStubPlugin = (params: {
  ...
  config: {
    listAccountIds: (cfg: FasedAgentConfig) => {
    ...
    resolveAccount: (cfg: FasedAgentConfig, accountId?: string | null) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
      const entry = channels?.[params.id];
      if (!entry || typeof entry !== "object") {
        return {};
    ...
    isConfigured: async (_account, cfg: FasedAgentConfig) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
