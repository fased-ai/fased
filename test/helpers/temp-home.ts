...
function snapshotEnv(): EnvSnapshot {
  return {
    ...
    fasedHome: process.env.FASED_HOME,
...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("FASED_HOME", snapshot.fasedHome);
  restoreKey("FASED_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked FASED_HOME.
  delete process.env.FASED_HOME;
  process.env.FASED_STATE_DIR = path.join(base, ".fased");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "fased-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".fased", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
