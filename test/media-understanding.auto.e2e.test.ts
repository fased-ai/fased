...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("fased-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("fased-bin-sherpa-");
      const modelDir = await makeTempDir("fased-sherpa-model-");
      ...
      const cfg: FasedAgentConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("fased-bin-whispercpp-");
      const modelDir = await makeTempDir("fased-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("fased-bin-gemini-");
      ...
      const cfg: FasedAgentConfig = { tools: { media: { image: {} } } };
