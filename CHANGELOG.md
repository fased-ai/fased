...
- Restore the non-Linux Hosting adapter boundary so portable lifecycle host
  binaries compile while continuing to reject Hosting outside Linux.
...
- Bind both predecessor-compatible schema-v1 and platform-qualified schema-v2
  release indexes into their independently attested lifecycle root heads.

## 0.1.76-rc.135

- Restore and verify executable modes for native platform supplements after
  cross-job artifact transport before assembling signed lifecycle generations.
...
- Close the pristine Hosting onboarding ordering deadlock by deferring the
  dashboard probe until the lifecycle supervisor activates the exact Gateway.
- Prove literal 2 GB Hosting installation, independent Tailscale access,
  hardening, managed swap and identical-command convergence before allocation.

## 0.1.76-rc.121

- Preserve the complete optimized Go-managed core, modular Wallet, signer,
  Mining, task-ledger and digest-bound optional component architecture.
- Recover an exact active managed swap after a canceled Hosting rollback and
  verify root-managed Gateway units by canonical account name or exact UID.
- Reprove literal 2 GB Hosting staging, onboarding recovery, state preservation
  and identical-command convergence before immutable candidate allocation.
...
- Preserve the complete optimized Go-managed core, modular Wallet, signer,
  Mining, task-ledger and digest-bound optional component architecture.
- Make the lifecycle trust root reproducible across builds and bind fixture
  release sequences explicitly so real cross-build Hosting updates remain
  monotonic without weakening immutable candidate authority.
- Prove the exact unpublished bytes on a 2 GB Hosting VPS, including managed
  swap, state preservation and identical-command convergence, while advancing
  signed lifecycle authority to release sequence 25 and security epoch 1.
...
- Preserve the complete optimized Go-managed core, modular Wallet, signer,
  Mining, task-ledger and digest-bound optional component architecture.
- Recover an exact incomplete Hosting security transaction across immutable
  release identities while retaining strict channel, platform, operator,
  Tailscale, signer, privilege and host-state boundaries.
- Keep Protected Local behavior unchanged while advancing the signed lifecycle
  authority to release sequence 24 and security epoch 1.

## 0.1.76-rc.115

- Preserve the complete optimized Go-managed core, modular Wallet, signer,
  Mining, task-ledger and digest-bound optional component architecture.
- Bind public Hosting onboarding to the Hosting profile without presenting a
  Local/Hosting choice, preserve interactive completion beyond machine-phase
  deadlines, and honor the root-verified host-security capability boundary.
- Keep Protected Local behavior unchanged while advancing the signed lifecycle
  authority to release sequence 23 and security epoch 1.

## 0.1.76-rc.114

- Preserve the complete optimized Go-managed core, modular Wallet, signer,
  Mining, task-ledger and digest-bound optional component architecture.
- Classify the complete Debian package-status contract during fresh Hosting
  prerequisite provisioning and prove it with the real Ubuntu package database.
- Keep Protected Local behavior unchanged while advancing the signed lifecycle
  authority to release sequence 22 and security epoch 1.
...
- Preserve the complete optimized Go-managed core, modular Wallet, signer,
  Mining, task-ledger and digest-bound optional component architecture.
- Provision and verify the ACL prerequisite before fresh Hosting lifecycle
  initialization, including hosts where Tailscale is not installed yet.
- Keep Protected Local behavior unchanged while advancing the signed lifecycle
  authority to release sequence 20 and security epoch 1.

## 0.1.76-rc.111

- Preserve the complete optimized Go-managed core, modular Wallet, signer,
  Mining, task-ledger and digest-bound optional component architecture.
- Converge only the exact interrupted legacy Hosting updater projection while
  continuing to reject altered, mixed or owner-runtime installation state.
- Keep Protected Local behavior unchanged while advancing the signed lifecycle
  authority to release sequence 19 and security epoch 1.

## 0.1.76-rc.110

- Preserve the complete optimized Go-managed core, modular Wallet, signer,
  Mining, task-ledger and digest-bound optional component architecture.
- Recover only the exact canonical updater-only residue left by an interrupted
  fresh Hosting bootstrap while continuing to reject altered or mixed state.
- Keep Protected Local behavior unchanged while advancing the signed lifecycle
  authority to release sequence 18 and security epoch 1.
...
- Make the signed Go lifecycle the sole managed Local and Hosting install and
  update authority, including deterministic legacy signer-v2 custody migration.
- Add cached serial and concurrent LOCAL0 acceptance with exact rollback,
  restart, preservation, identity, and `Already current` receipts.
- Remove npm publication from managed releases and keep the root workspace
  private.
...
- Build release archives from a validated deterministic manifest with
  unsupported-entry rejection and per-entry progress receipts.
- Remove redundant runtime archives, legacy fixed-name application archives,
  and fabricated branch-platform copies from candidate inventory.
- Build independent native release families concurrently while preserving
  exact commit, tree, and lockfile cache identity.
...
- Measure archive liveness from raw tar input before gzip so active release
  builds cannot be killed only because the compressor buffers output.
...
- Build offline production artifacts from the frozen lockfile and content-addressed
  pnpm store without consulting registry metadata or including dev-only packages.
...
- Complete the canonical schema-1 owner-predecessor migration and bind Local
  and Hosting branch proof to the shared lifecycle acceptance contract.

## 0.1.76-rc.77

- Bind lifecycle root integrity before candidate allocation and retain the
  static-bootstrap Local and Hosting transaction redesign.
...
- Keep lifecycle compatibility evidence complete through rc.63.
...
- Bind pre-tag candidate descriptors to exact protected `main`, then require
  the immutable owner tag only after packaged P1 passes.
...
- Return `Already current` from durable release and live service identities
  without replaying expensive product-acceptance probes.
...
- Converge supported legacy and managed Local installations through one
  topology-, capability-, and schema-driven lifecycle takeover.
- Preserve Wallet, signer, Mining, network, plugin, and configuration state
  across failed activation, exact rollback, same-command retry, and restart.
- Keep ordinary update logic version-neutral and reject unsafe or unknown-newer
  control state without mutation.
