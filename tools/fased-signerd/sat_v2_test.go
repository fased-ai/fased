...
func TestSignerV2SATGeneratedManifestAndSemanticValidatorsAreComplete(t *testing.T) {
	const expected = "abortEmptyCycle,cancelBondUnlock,cancelBondUnlockV3,claimBondEpochRewardsV3,claimBondStakingRewards,claimCycleRewards,claimCycleRewardsBatch,claimProtocolDistributorSat,claimProtocolTreasury,claimUnallocatedStakingRewards,closeCommitPhase,closeResolvedCycleArtifacts,closeResolvedCycleRegistryPage,closeResolvedMinerCycleState,commitCycle,compactPendingCycleRange,depositMinerCapital,distributeCyclePage,finalizeBondUnlock,finalizeBondUnlockV3,finalizeCycleSettlement,increaseBondPosition,increaseBondPositionV3,initMinerCapital,initializeCycle,openBondPosition,openCycle,openDispute,refillRegistryReserveFromTreasury,registerBondEpochPositionV3,releaseUnrevealedCommit,republishEpochRoots,requestBondUnlock,requestBondUnlockV3,resolveDispute,retargetUnlock,revealCycle,scoreCyclePage,sealCycleEntropy,setActiveCommit,settleCyclePage,syncBondStakingPosition,syncBondStakingRewards,topUpRegistryReserve,updateBondTierPolicy,validatorAttestation,withdrawMinerCapital"
	actions := sortedSATActionsV2()
	if len(actions) != 47 || strings.Join(actions, ",") != expected {
