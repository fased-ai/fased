...
func mustValidate(req request, cfg signerConfig) error {
	...
	case "v2.network.put", "v2.network.bootstrap", "v2.network.setPrimary", "v2.network.repairMigratedPrimary", "v2.policy.put", "v2.policy.tighten", "v2.policy.activateBaseline", "v2.wallet.create", "v2.wallet.import", "v2.wallet.importLegacy", "v2.wallet.recovery.export", "v2.wallet.recovery.import", "v2.wallet.exportRaw", "v2.wallet.rotation.create", "v2.wallet.rotation.commit", "v2.execute", "v2.review.get", "v2.review.prepare", "v2.review.execute", "v2.operation.get", "v2.operation.reconcile", "v2.satLookup.binding.get", "v2.satCommitment.allocate", "v2.satCommitment.binding.get":
		if len(req.Request) == 0 || req.Chain != "" || strings.TrimSpace(req.WalletID) == "" {
			return errors.New("invalid signer request")
		}
	case "getAddresses":
...
func parseArgs() signerConfig {
	...
	cfg.rateLimit = map[string]int{
		...
		"v2.network.repairMigratedPrimary": getenvInt("FASED_WALLET_LOCAL_SIGNER_RATE_NETWORK", 6),
		...
		"v2.wallet.recovery.export":        getenvInt("FASED_WALLET_LOCAL_SIGNER_RATE_RECOVERY_ADMIN", 6),
		"v2.wallet.recovery.import":        getenvInt("FASED_WALLET_LOCAL_SIGNER_RATE_RECOVERY_ADMIN", 6),
		"v2.wallet.exportRaw":              getenvInt("FASED_WALLET_LOCAL_SIGNER_RATE_RECOVERY_ADMIN", 3),
