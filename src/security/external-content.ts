...
export function wrapExternalContent(content: string, options: WrapExternalContentOptions): string {
  ...
  const sanitizedSender = sender ? sanitizeExternalMetadataValue(sender) : "";
  const sanitizedSubject = subject ? sanitizeExternalMetadataValue(subject) : "";

  if (sanitizedSender) {
    metadataLines.push(`From: ${sanitizedSender}`);
  }
  if (sanitizedSubject) {
    metadataLines.push(`Subject: ${sanitizedSubject}`);
  }
