...
describe("security fix", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "fased-security-fix-suite-"));
  });
  ...
  it.each([
    ...
    "preserves the trusted %s application group on shared runtime state",
    async (_label, profileEnv) => {
      if (isWindows) {
        return;
      }
