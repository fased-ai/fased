  ...
  if (stateDirPerms.ok) {
    ...
    if (stateDirPerms.worldWritable) {
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; other users can write into your FasedAgent state.`,
      ...
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; group users can write into your FasedAgent state.`,
        remediation: formatPermissionRemediation({
          targetPath: params.stateDir,
          perms: stateDirPerms,
          isDir: true,
...
function collectGatewayConfigFindings(
  ...
  if (cfg.gateway?.controlUi?.allowInsecureAuth === true) {
    findings.push({
      ...
      detail:
        "gateway.controlUi.allowInsecureAuth=true allows localhost HTTP Control UI connections without device identity.",
      remediation:
        "Disable it or switch to HTTPS (Tailscale Serve) when using remote browser access.",
    });
  }
  ...
  const wallet = resolveWalletRuntimeConfig(cfg, env);
  if (wallet.enabled) {
    if (wallet.policy.directSigning) {
      const weakSolanaCaps =
        wallet.policy.solana.caps.maxPerTx <= 0n || wallet.policy.solana.caps.maxDaily <= 0n;
      if (weakSolanaCaps) {
        findings.push({
          checkId: "wallet.automated_execution_weak_caps",
          severity: "critical",
          title: "Wallet automated execution enabled with missing/zero caps",
          detail:
            "wallet.runtime.policy.directSigning=true enables background/agent execution, but one or more maxPerTx/maxDaily caps are unset or zero.",
          remediation:
            "Set non-zero wallet.runtime.policy.solana maxPerTx and maxDaily values before enabling automated execution.",
        });
      }
    }
    if (wallet.toolAccess.mode !== "owner-only") {
      findings.push({
        checkId: "wallet.tool_access_wide",
        severity: "warn",
        title: "Wallet tool access is broader than owner-only",
        detail: `wallet.runtime.toolAccess.mode="${wallet.toolAccess.mode}" allows non-owner agent tool calls.`,
        remediation:
          "Use wallet.runtime.toolAccess.mode=owner-only for managed/public deployments.",
      });
    }
    if (wallet.mode !== "managed" && wallet.service.host !== "127.0.0.1") {
      findings.push({
        checkId: "wallet.external_non_localhost",
        severity: "critical",
        title: "External wallet service is not localhost-bound",
        detail: `wallet.runtime.service.host="${wallet.service.host}" is reachable beyond loopback.`,
        remediation:
          "Bind wallet.runtime.service.host to 127.0.0.1 or use a local Unix socket proxy.",
      });
    }
  }
...
function collectBrowserControlFindings(
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${resolveConfigPath()} and re-run "${formatCliCommand("fased security audit --deep")}".`,
    });
    return findings;
  }
...
function collectLoggingFindings(cfg: FasedAgentConfig): SecurityAuditFinding[] {
...
function collectElevatedFindings(cfg: FasedAgentConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export async function runSecurityAudit(opts: SecurityAuditOptions): Promise<SecurityAuditReport> {
  ...
  if (deep?.gateway?.attempted && !deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("fased status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("fased security audit --deep")}".`,
    });
  }
