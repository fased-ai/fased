...
function stubChannelPlugin(params: {
  ...
  label: string;
  resolveAccount: (cfg: FasedAgentConfig, accountId: string | null | undefined) => unknown;
...
describe("security audit", () => {
  ...
  const withChannelSecurityStateDir = async (fn: (tmp: string) => Promise<void>) => {
    ...
    await withEnvAsync(
      { FASED_STATE_DIR: channelSecurityStateDir },
      async () => await fn(channelSecurityStateDir),
    );
  };

  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "fased-security-audit-"));
  ...
  it("flags non-loopback bind without auth as critical", async () => {
    ...
    const prevToken = process.env.FASED_GATEWAY_TOKEN;
    const prevPassword = process.env.FASED_GATEWAY_PASSWORD;
    ...
    try {
      ...
      if (prevPassword === undefined) {
        ...
        process.env.FASED_GATEWAY_PASSWORD = prevPassword;
      }
    }
  });
  ...
  it("warns for risky safeBinTrustedDirs entries", async () => {
    const riskyGlobalTrustedDirs =
      process.platform === "win32"
        ...
        : ["/usr/local/bin", "/tmp/fased-safe-bins"];
    const cfg: FasedAgentConfig = {
      tools: {
        exec: {
  ...
  it("warns when sandbox browser containers have missing or stale hash labels", async () => {
    ...
    const execDockerRawFn = (async (args: string[]) => {
      if (args[0] === "ps") {
        return {
          stdout: Buffer.from("fased-sbx-browser-old\nfased-sbx-browser-missing-hash\n"),
          stderr: Buffer.alloc(0),
          code: 0,
        };
      }
      if (args[0] === "inspect" && args.at(-1) === "fased-sbx-browser-old") {
        return {
      ...
      if (args[0] === "inspect" && args.at(-1) === "fased-sbx-browser-missing-hash") {
        return {
    ...
    }) as unknown as NonNullable<SecurityAuditOptions["execDockerRawFn"]>;
    ...
    expect(staleEpoch?.detail).toContain("fased-sbx-browser-old");
  });
  ...
  it("flags sandbox browser containers with non-loopback published ports", async () => {
    ...
    const execDockerRawFn = (async (args: string[]) => {
      if (args[0] === "ps") {
        return {
          stdout: Buffer.from("fased-sbx-browser-exposed\n"),
          stderr: Buffer.alloc(0),
          code: 0,
        };
      }
      if (args[0] === "inspect" && args.at(-1) === "fased-sbx-browser-exposed") {
        return {
      ...
      if (args[0] === "port" && args.at(-1) === "fased-sbx-browser-exposed") {
        return {
  ...
  it("uses symlink target permissions for config checks", async () => {
    ...
    const targetConfigPath = path.join(tmp, "managed-fased.json");
  ...
  it("scores X-Real-IP fallback risk by gateway exposure", async () => {
    const trustedProxyCfg = (trustedProxies: string[]): FasedAgentConfig => ({
      gateway: {
        bind: "loopback",
  ...
  it("warns when multiple DM senders share the main session", async () => {
    const cfg: FasedAgentConfig = { session: { dmScope: "main" } };
  ...
  it("warns when Discord allowlists contain name-based entries", async () => {
    await withChannelSecurityStateDir(async (tmp) => {
      ...
      expect(finding?.detail).toContain("~/.fased/credentials/discord-allowFrom.json:team.owner");
  ...
  it("adds probe_failed warnings for deep probe failure modes", async () => {
    const cfg: FasedAgentConfig = { gateway: { mode: "local" } };
    const cases: Array<{
      name: string;
  ...
  it("flags hooks token reuse of the gateway env token as critical", async () => {
    const prevToken = process.env.FASED_GATEWAY_TOKEN;
    process.env.FASED_GATEWAY_TOKEN = "shared-gateway-token-1234567890";
    const cfg: FasedAgentConfig = {
  ...
  it("scores hooks request sessionKey override by gateway exposure", async () => {
    ...
    } satisfies NonNullable<FasedAgentConfig["hooks"]>;
    const cases: Array<{
      name: string;
      cfg: FasedAgentConfig;
      expectedSeverity: "warn" | "critical";
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await audit(cfg, {
      stateDir: "/Users/test/Dropbox/.fased",
      configPath: "/Users/test/Dropbox/.fased/fased.json",
    });
  ...
  it("flags group/world-readable config include files", async () => {
    ...
    try {
      const cfg: FasedAgentConfig = { logging: { redactSensitive: "off" } };
      const user = "DESKTOP-TEST\\Tester";
  ...
  it("warns when install records drift from installed package versions", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({ name: "@fased/voice-call", version: "9.9.9" }),
      "utf-8",
    );
    await fs.writeFile(
      path.join(hookDir, "package.json"),
      JSON.stringify({ name: "@fased/test-hooks", version: "8.8.8" }),
      "utf-8",
    );
  ...
  describe("maybeProbeGateway auth selection", () => {
    ...
    const makeProbeEnv = (env?: { token?: string; password?: string }) => {
      ...
      if (env?.token !== undefined) {
        probeEnv.FASED_GATEWAY_TOKEN = env.token;
      }
      if (env?.password !== undefined) {
        probeEnv.FASED_GATEWAY_PASSWORD = env.password;
      }
      return probeEnv;
    };
