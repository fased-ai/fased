...
function maybeRepairUnenrolledWalletApprovalAuth(cfg: FasedAgentConfig): {
  config: FasedAgentConfig;
  changes: string[];
} {
  if (cfg.wallet?.approvalAuth?.mode !== "webauthn") {
    return { config: cfg, changes: [] };
  }
  const snapshot = readWalletApprovalAuthSnapshot(process.env, cfg);
  if (snapshot.passkeyCount > 0) {
    return { config: cfg, changes: [] };
  }
  const repaired = structuredClone(cfg);
  repaired.wallet = repaired.wallet ?? {};
  repaired.wallet.approvalAuth = repaired.wallet.approvalAuth ?? {};
  repaired.wallet.approvalAuth.mode = "none";
  return {
    config: repaired,
    changes: ["- Disabled wallet.approvalAuth.mode because no Wallet Control Passkey is enrolled."],
  };
}
...
function stripUnknownConfigKeys(config: FasedAgentConfig): {
  config: FasedAgentConfig;
  removed: string[];
} {
  const parsed = FasedAgentSchema.safeParse(config);
  if (parsed.success) {
...
function noteOpencodeProviderOverrides(cfg: FasedAgentConfig) {
...
export function collectMissingDefaultAccountBindingWarnings(cfg: FasedAgentConfig): string[] {
...
async function maybeRepairTelegramAllowFromUsernames(cfg: FasedAgentConfig): Promise<{
  config: FasedAgentConfig;
  changes: string[];
}> {
...
function scanDiscordNumericIdEntries(cfg: FasedAgentConfig): DiscordNumericIdHit[] {
...
function maybeRepairDiscordNumericIds(cfg: FasedAgentConfig): {
  config: FasedAgentConfig;
  changes: string[];
} {
...
function scanMutableAllowlistEntries(cfg: FasedAgentConfig): MutableAllowlistHit[] {
 ...
 */
function maybeRepairOpenPolicyAllowFrom(cfg: FasedAgentConfig): {
  config: FasedAgentConfig;
  changes: string[];
} {
...
async function maybeRepairAllowlistPolicyAllowFrom(cfg: FasedAgentConfig): Promise<{
  config: FasedAgentConfig;
  changes: string[];
}> {
  const channels = cfg.channels;
 ...
 * file support.
 */
function detectEmptyAllowlistPolicy(cfg: FasedAgentConfig): string[] {
  const channels = cfg.channels;
  if (!channels || typeof channels !== "object") {
    return [];
  }
  ...
  const checkAccount = (
    ...
    warnings.push(
      `- ${prefix}.dmPolicy is "allowlist" but allowFrom is empty — all DMs will be blocked. Add sender IDs to ${prefix}.allowFrom, or run "${formatCliCommand("fased doctor --fix")}" to auto-migrate from pairing store when entries exist.`,
    );
  };
...
function collectExecSafeBinScopes(cfg: FasedAgentConfig): ExecSafeBinScopeRef[] {
...
function scanExecSafeBinCoverage(cfg: FasedAgentConfig): ExecSafeBinCoverageHit[] {
...
function scanExecSafeBinTrustedDirHints(cfg: FasedAgentConfig): ExecSafeBinTrustedDirHintHit[] {
...
function maybeRepairExecSafeBinProfiles(cfg: FasedAgentConfig): {
  config: FasedAgentConfig;
  changes: string[];
  warnings: string[];
} {
...
function scanLegacyToolsBySenderKeys(cfg: FasedAgentConfig): LegacyToolsBySenderKeyHit[] {
  const hits: LegacyToolsBySenderKeyHit[] = [];
  collectLegacyToolsBySenderKeyHits(cfg, [], hits);
  return hits;
}

function maybeRepairLegacyToolsBySenderKeys(cfg: FasedAgentConfig): {
  config: FasedAgentConfig;
  changes: string[];
} {
  const next = structuredClone(cfg);
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  let cfg: FasedAgentConfig = baseCfg;
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(
        `Run "${formatCliCommand("fased doctor --fix")}" to apply compatibility migrations.`,
      );
    }
  }
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("fased doctor --fix")}" to apply these changes.`);
    }
  }
  ...
  const approvalAuthRepair = maybeRepairUnenrolledWalletApprovalAuth(candidate);
  if (approvalAuthRepair.changes.length > 0) {
    pendingChanges = true;
    candidate = approvalAuthRepair.config;
    if (shouldRepair) {
      cfg = approvalAuthRepair.config;
      note(approvalAuthRepair.changes.join("\n"), "Doctor changes");
    } else {
      note(
        [
          ...approvalAuthRepair.changes,
          `- Run "${formatCliCommand("fased doctor --fix")}" to apply this repair.`,
        ].join("\n"),
        "Doctor warnings",
      );
      fixHints.push(`Run "${formatCliCommand("fased doctor --fix")}" to apply these changes.`);
  ...
  if (shouldRepair) {
    ...
    if (discordHits.length > 0) {
      note(
        [
          ...
          `- Discord IDs must be strings; run "${formatCliCommand("fased doctor --fix")}" to convert numeric IDs to quoted strings.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (allowFromScan.changes.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand("fased doctor --fix")}" to add missing allowFrom wildcards.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (toolsBySenderHits.length > 0) {
      ...
      note(
        [
          ...
          `- Run "${formatCliCommand("fased doctor --fix")}" to migrate legacy keys to typed id: entries.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (safeBinCoverage.length > 0) {
      ...
      lines.push(
        `- Run "${formatCliCommand("fased doctor --fix")}" to scaffold missing custom safeBinProfiles entries.`,
      );
      note(lines.join("\n"), "Doctor warnings");
    }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "fased doctor --fix" to remove these keys.');
    }
  }
