...
const SEARCH_TOOL = "https://docs.fased.ai/mcp.SearchFasedAgent";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.fased.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('fased docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.fased.ai/");
      runtime.log(`Search: ${formatCliCommand('fased docs "your query"')}`);
    }
    return;
  }
