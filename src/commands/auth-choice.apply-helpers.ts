  ...
  while (true) {
    ...
    if (source === "env") {
      ...
      await params.prompter.note(
        `Validated environment variable ${envVar}. FasedAgent will store a reference, not the key value.`,
        "Reference validated",
      );
    ...
    const externalProviders = Object.entries(
      params.config.secrets?.providers ?? ({} as Record<string, SecretProviderConfig>),
    ).filter(([, provider]) => provider?.source === "file" || provider?.source === "exec");
    ...
    try {
      ...
      await params.prompter.note(
        `Validated ${providerEntry.source} reference ${selectedProvider}:${id}. FasedAgent will store a reference, not the key value.`,
        "Reference validated",
      );
      return { ref, resolvedValue };
    } catch (error) {
    ...
    options: [
      {
        ...
        hint: "Stores the key directly in FasedAgent config",
      },
      {
        value: "ref",
