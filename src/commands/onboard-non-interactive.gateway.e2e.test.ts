...
describe("onboard (non-interactive): gateway and remote auth", () => {
  const prev = {
    ...
    skipBrowser: process.env.FASED_SKIP_BROWSER_CONTROL_SERVER,
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "fased-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.FASED_STATE_DIR = prev.stateDir;
    process.env.FASED_CONFIG_PATH = prev.configPath;
    process.env.FASED_SKIP_CHANNELS = prev.skipChannels;
    process.env.FASED_SKIP_GMAIL_WATCHER = prev.skipGmail;
    process.env.FASED_SKIP_CRON = prev.skipCron;
    process.env.FASED_SKIP_CANVAS_HOST = prev.skipCanvas;
    process.env.FASED_SKIP_BROWSER_CONTROL_SERVER = prev.skipBrowser;
    process.env.FASED_GATEWAY_TOKEN = prev.token;
    process.env.FASED_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "fased");
    ...
    const cfg = JSON.parse(await fs.readFile(configPath, "utf8")) as {
      gateway?: { auth?: import("../config/types.gateway.js").GatewayAuthConfig };
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.FASED_STATE_DIR = stateDir;
    process.env.FASED_CONFIG_PATH = path.join(stateDir, "fased.json");
    ...
    const cfg = JSON.parse(await fs.readFile(configPath, "utf8")) as {
      gateway?: {
        bind?: string;
        port?: number;
        auth?: import("../config/types.gateway.js").GatewayAuthConfig;
      };
    };
