...
describe("applyAuthChoice", () => {
  ...
  async function setupTempState() {
    ...
    const env = await setupAuthTestEnv("fased-auth-");
  ...
  it("uses existing env API keys for selected providers", async () => {
    const scenarios: Array<{
      ...
      },
      {
        ...
        expectedModel: "vercel-ai-gateway/openai/gpt-5.5",
      },
      {
        authChoice: "ai-gateway-api-key",
        envKey: "AI_GATEWAY_API_KEY",
      ...
      config: {
        secrets: {
          providers: {
            filemain: {
              source: "file",
              path: "/tmp/fased-missing-secrets.json",
              mode: "json",
            },
          },
        },
  ...
  it("keeps existing default model for explicit provider keys when setDefaultModel=false", async () => {
    const scenarios: Array<{
      ...
      },
      {
        ...
        expectedOverride: "opencode/gpt-5.5",
        expectProviderConfigUndefined: "opencode-zen",
      },
    ];
    for (const scenario of scenarios) {
  ...
  it("ignores legacy LiteLLM oauth profiles when selecting litellm-api-key", async () => {
    ...
    const authProfilePath = authProfilePathForAgent(requireFasedAgentAgentDir());
    await fs.writeFile(
      authProfilePath,
      JSON.stringify(
        {
  ...
  it("writes Chutes OAuth credentials when selecting chutes (remote/manual)", async () => {
    ...
    const text: WizardPrompter["text"] = vi.fn(async (params) => {
      if (params.message.includes("Paste the redirect URL")) {
        const urlMatch = params.message.match(/https?:\/\/\S+/)?.[0] ?? "";
    ...
    expect(text).toHaveBeenCalledWith(
      expect.objectContaining({
        message: expect.stringContaining("Open this URL in your LOCAL browser:"),
      }),
    );
    expect(text).toHaveBeenCalledWith(
      expect.objectContaining({
        message: expect.stringContaining("https://api.chutes.ai/idp/authorize"),
      }),
    );
    ...
    expect(result.agentModelOverride).toBe("chutes/google/gemma-4-31B-turbo-TEE");
    expect(result.config.models?.providers?.chutes?.baseUrl).toBe("https://llm.chutes.ai/v1");
  ...
  it("rejects removed Qwen portal OAuth", async () => {
    await setupTempState();
  ...
  it("writes portal OAuth credentials for supported plugin providers", async () => {
    const scenarios: Array<{
