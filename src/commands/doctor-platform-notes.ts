...
function hasConfigGatewayCreds(cfg: FasedAgentConfig): boolean {
  const localToken =
...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  const tokenEntries = [["FASED_GATEWAY_TOKEN", await getenv("FASED_GATEWAY_TOKEN")]] as const;
  const passwordEntries = [
    ["FASED_GATEWAY_PASSWORD", await getenv("FASED_GATEWAY_PASSWORD")],
  ] as const;
  ...
  const lines = [
    ...
    envToken && envTokenKey
      ...
      : undefined,
    envPassword
      ? `- \`${envPasswordKey ?? "FASED_GATEWAY_PASSWORD"}\` is set; it overrides config passwords.`
      : undefined,
