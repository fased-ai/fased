...
function resolveGatewayAuthToken(
  ...
  const envToken = env.FASED_GATEWAY_TOKEN;
...
export async function maybeRepairGatewayServiceConfig(
  ...
  const audit = await auditGatewayServiceConfig({
    ...
    expectedPort: resolveGatewayPort(cfg, process.env),
  });
...
export async function maybeScanExtraGatewayServices(
  ...
  if (legacyServices.length > 0) {
    ...
    if (shouldRemove) {
      ...
      if (removed.length > 0) {
        runtime.log("Legacy gateway services removed. Installing FasedAgent gateway next.");
      }
    }
  }
