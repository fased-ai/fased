...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "fased-agent-" });
}

function mockConfig(
  home: string,
  storePath: string,
  agentOverrides?: Partial<NonNullable<NonNullable<FasedAgentConfig["agents"]>["defaults"]>>,
  telegramOverrides?: Partial<NonNullable<NonNullable<FasedAgentConfig["channels"]>["telegram"]>>,
  agentsList?: NonNullable<NonNullable<FasedAgentConfig["agents"]>["list"]>,
) {
...
describe("agentCommand", () => {
  ...
  it("does not duplicate lifecycle end events from embedded runs", async () => {
    await withTempHome(async (home) => {
      const store = path.join(home, "sessions.json");
      mockConfig(home, store);

      const lifecycleEvents: Array<{ runId: string; phase?: string }> = [];
      const stop = onAgentEvent((evt) => {
      ...
      vi.mocked(runEmbeddedPiAgent).mockImplementationOnce(async (params) => {
        const runId = (params as { runId?: string } | undefined)?.runId ?? "run";
        const data = { phase: "end", endedAt: Date.now() };
        (
          params as {
            onAgentEvent?: (evt: { stream: string; data: Record<string, unknown> }) => void;
          }
        ).onAgentEvent?.({ stream: "lifecycle", data });
      ...
      await agentCommand({ message: "hi", to: "+1555", runId: "run-liveness-end" }, runtime);
      stop();

      const matching = lifecycleEvents.filter(
        (evt) => evt.runId === "run-liveness-end" && evt.phase === "end",
      );
      expect(matching).toHaveLength(1);
      expect(getAgentRunContext("run-liveness-end")).toBeUndefined();
    });
  });

  it("emits lifecycle error and clears run context when embedded run fails", async () => {
    await withTempHome(async (home) => {
      const store = path.join(home, "sessions.json");
      mockConfig(home, store);

      const lifecycleEvents: Array<{ runId: string; phase?: string; error?: string }> = [];
      const stop = onAgentEvent((evt) => {
        if (evt.stream !== "lifecycle") {
          return;
        }
        lifecycleEvents.push({
          runId: evt.runId,
          phase: typeof evt.data?.phase === "string" ? evt.data.phase : undefined,
          error: typeof evt.data?.error === "string" ? evt.data.error : undefined,
        });
      });

      vi.mocked(runEmbeddedPiAgent).mockRejectedValueOnce(new Error("runner failed"));

      await expect(
        agentCommand({ message: "hi", to: "+1555", runId: "run-liveness-error" }, runtime),
      ).rejects.toThrow("runner failed");
      stop();

      const matching = lifecycleEvents.filter(
        (evt) => evt.runId === "run-liveness-error" && evt.phase === "error",
      );
      expect(matching).toHaveLength(1);
      expect(matching[0]?.error).toContain("runner failed");
      expect(getAgentRunContext("run-liveness-error")).toBeUndefined();
    });
  });
  ...
  it("does not emit strict-agentic warnings by default", async () => {
    await withTempHome(async (home) => {
      vi.mocked(runEmbeddedPiAgent).mockResolvedValue({
        meta: {
          durationMs: 5,
      ...
      expect(runtime.error).not.toHaveBeenCalledWith(
        expect.stringContaining("[agent:strict-agentic]"),
      );
      expect(runtime.log).toHaveBeenCalledWith("No reply from agent.");
    });
  });

  it("emits a strict-agentic diagnostic warning when warning mode is enabled", async () => {
    const previousMode = process.env.FASED_STRICT_AGENTIC_MODE;
    process.env.FASED_STRICT_AGENTIC_MODE = "warn";
    try {
      await withTempHome(async (home) => {
        vi.mocked(runEmbeddedPiAgent).mockResolvedValue({
          meta: {
        ...
        try {
          await agentCommand({ message: "hi", to: "+1555", runId: "run-strict" }, runtime);
        } finally {
          stop();
        }
        ...
        expect(runtime.error).toHaveBeenCalledWith(
          expect.stringContaining(
            "[agent:strict-agentic] mode=warn classification=empty reason=empty_result action=warn run=run-strict",
          ),
        );
      ...
      if (previousMode === undefined) {
        delete process.env.FASED_STRICT_AGENTIC_MODE;
      } else {
        process.env.FASED_STRICT_AGENTIC_MODE = previousMode;
      }
    }
  });

  it("emits strict-agentic diagnostics from warning-only config policy", async () => {
    await withTempHome(async (home) => {
      vi.mocked(runEmbeddedPiAgent).mockResolvedValue({
        meta: {
          durationMs: 5,
      ...
      const store = path.join(home, "sessions.json");
      mockConfig(home, store, { strictAgentic: { mode: "warn" } });
      const events: Array<{ stream: string; data?: Record<string, unknown> }> = [];
      const stop = onAgentEvent((evt) => {
        events.push(evt);
      });
      try {
        await agentCommand({ message: "hi", to: "+1555", runId: "run-strict-config" }, runtime);
      } finally {
        stop();
      }
      ...
      expect(runtime.error).toHaveBeenCalledWith(
        expect.stringContaining(
          "[agent:strict-agentic] mode=warn classification=empty reason=empty_result action=warn run=run-strict-config",
        ),
      );
      expect(events).toContainEqual(
        expect.objectContaining({
          runId: "run-strict-config",
          stream: "diagnostic",
          data: expect.objectContaining({
            phase: "strict-agentic-warning",
            classification: "empty",
  ...
  it("lets per-agent strict-agentic policy override default warning policy", async () => {
    await withTempHome(async (home) => {
      vi.mocked(runEmbeddedPiAgent).mockResolvedValue({
        meta: {
          durationMs: 5,
      ...
      const store = path.join(home, "sessions.json");
      mockConfig(home, store, { strictAgentic: { mode: "warn" } }, undefined, [
        { id: "ops", strictAgentic: { mode: "off" } },
      ]);

      await agentCommand({ message: "hi", agentId: "ops", runId: "run-strict-off" }, runtime);
