...
describe("getHealthSnapshot", () => {
  ...
  it("omits probe details when sensitive health fields are not allowed", async () => {
    ...
    const snap = await getHealthSnapshot({ timeoutMs: 25, includeSensitive: false });
    const telegram = snap.channels.telegram as {
      configured?: boolean;
      probe?: unknown;
    ...
    expect(telegram.configured).toBe(true);
    expect(telegram.probe).toBeUndefined();
    expect(telegram.lastProbeAt).toBeTruthy();
    expect(calls.some((c) => c.includes("/getMe"))).toBe(true);
  });

  it("preserves runtime-backed account state in health snapshots", async () => {
    testConfig = { channels: { telegram: { botToken: "t-1" } } };
    testStore = {};
    vi.stubEnv("DISCORD_BOT_TOKEN", "");
    vi.stubEnv("TELEGRAM_BOT_TOKEN", "");
      ...
      runtimeSnapshot: {
        channels: {
          telegram: {
            ...
            baseUrl: "https://user:pass@example.com/bot",
          },
        },
        channelAccounts: {},
      },
    ...
    expect(telegram.lastOutboundAt).toBeNull();
    expect(telegram.lastError).toBe("runtime unavailable");
    expect(telegram.baseUrl).toBe("https://example.com/bot");
    expect(telegram.probe).toBeUndefined();
  });

  it("treats telegram.tokenFile as configured", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "fased-health-"));
