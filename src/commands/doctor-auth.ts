...
function pruneAuthProfiles(
  cfg: FasedAgentConfig,
  profileIds: Set<string>,
): { next: FasedAgentConfig; changed: boolean } {
...
export async function maybeRemoveDeprecatedCliAuthProfiles(
  ...
  if (deprecated.has(CLAUDE_CLI_PROFILE_ID)) {
    lines.push(
      `- ${CLAUDE_CLI_PROFILE_ID} (Anthropic): use setup-token → ${formatCliCommand("fased models auth setup-token")}`,
    );
  }
  if (deprecated.has(CODEX_CLI_PROFILE_ID)) {
    lines.push(
      `- ${CODEX_CLI_PROFILE_ID} (OpenAI sign-in): use OAuth → ${formatCliCommand(
...
function formatAuthIssueHint(issue: AuthIssue): string | null {
  if (issue.provider === "anthropic" && issue.profileId === CLAUDE_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand("fased models auth setup-token")} or ${formatCliCommand(
      "fased configure",
    )}.`;
  }
  if (issue.provider === "openai-codex" && issue.profileId === CODEX_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand(
      "fased models auth login --provider openai-codex",
    )} or ${formatCliCommand("fased configure")}.`;
  }
  return `Re-auth via \`${formatCliCommand("fased configure")}\` or \`${formatCliCommand("fased onboard")}\`.`;
}
