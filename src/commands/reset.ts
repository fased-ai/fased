...
export async function resetCommand(runtime: RuntimeEnv, opts: ResetOptions) {
      ...
      options: [
        {
          value: "config",
          label: "Config only",
          hint: "fased.json; preserves wallets and Tailscale access",
        },
        {
          value: "config+creds+sessions",
          label: "Config + credentials + sessions",
          hint: "preserves wallets, workspace, and Tailscale access",
        },
        {
          value: "full",
          label: "Full reset",
          hint: "state dir + workspace, but preserves wallet material",
        },
      ],
  ...
  if (interactive && !opts.yes) {
    const ok = await confirm({
      message: stylePromptMessage(
        `Proceed with ${scope} reset? Wallet material and Tailscale device access are preserved.`,
      ),
    });
    if (isCancel(ok) || !ok) {
      cancel(stylePromptTitle("Reset cancelled.") ?? "Reset cancelled.");
  ...
  if (scope === "config+creds+sessions") {
    ...
    runtime.log(`Next: ${formatCliCommand("fased onboard --install-daemon")}`);
    return;
  }
