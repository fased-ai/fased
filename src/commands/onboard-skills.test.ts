...
vi.mock("../infra/skills-remote.js", () => ({
  getRemoteSkillEligibility: vi.fn(() => ({ hasBin: vi.fn(() => true) })),
}));
...
describe("setupSkills", () => {
  it("does not recommend Homebrew when user skips installing brew-backed deps", async () => {
    ...
    await setupSkills({} as FasedAgentConfig, "/tmp/ws", runtime, prompter);

    expect(buildWorkspaceSkillStatus).toHaveBeenCalledWith(
      "/tmp/ws",
      expect.objectContaining({
        eligibility: { remote: vi.mocked(getRemoteSkillEligibility).mock.results[0]?.value },
      }),
    );
