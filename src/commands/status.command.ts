...
export async function statusCommand(
  ...
  const gatewayPairingGuidance = (() => {
    const errorText = gatewayProbe?.error ?? "";
    const closeReason = gatewayProbe?.close?.reason ?? "";
    const combined = `${errorText}\n${closeReason}`;
    if (!/pairing required|not_paired/i.test(combined)) {
      return null;
    }
    const requestId = (() => {
      const match = combined.match(/\(requestId:\s*([^)]+)\)/i);
    ...
    return [
      ...
      ...(requestId
        ? [`Approve this request: ${formatCliCommand(`fased devices approve ${requestId}`)}`]
        : []),
      `Approve latest request: ${formatCliCommand("fased devices approve --latest")}`,
      `Review pending devices: ${formatCliCommand("fased devices list")}`,
    ];
  })();
  ...
  const agentsValue = (() => {
    const pending =
      agentStatus.bootstrapPendingCount > 0
        ? `${agentStatus.bootstrapPendingCount} bootstrap files pending`
        : "no bootstrap files";
  ...
  runtime.log(theme.heading("FasedAgent status"));
  runtime.log("");
  ...
  if (gatewayPairingGuidance) {
    runtime.log("");
    runtime.log(theme.heading("Gateway pairing"));
    for (const line of gatewayPairingGuidance) {
      runtime.log(line);
    }
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("fased security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("fased security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.fased.ai/faq");
  runtime.log("Troubleshooting: https://docs.fased.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("fased status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("fased logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("fased status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("fased gateway probe")}`);
  }
}
