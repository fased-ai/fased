import {
  cleanAgentModelProviders,
  normalizeModelProviderId,
} from "../agents/agent-model-providers.js";
import {
  listAgentEntries,
  resolveAgentDir,
...
type AgentTaskModels = AgentEntry["taskModels"];
type AgentModelProviders = AgentEntry["modelProviders"];

export type AgentIdentity = AgentIdentityFile;
export { listAgentEntries };

function normalizeAgentTaskModels(taskModels: AgentTaskModels | null | undefined): AgentTaskModels {
  if (!taskModels) {
    return undefined;
  }
  const next: NonNullable<AgentTaskModels> = {};
  for (const key of ["cheapCheck", "strong", "escalation", "coding", "summarizer"] as const) {
    const value = taskModels[key]?.trim();
    if (value) {
      next[key] = value;
    }
  }
...
function resolveAgentName(cfg: FasedAgentConfig, agentId: string) {
...
function resolveAgentModel(cfg: FasedAgentConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: FasedAgentConfig): AgentSummary[] {
...
export function applyAgentConfig(
  ...
  const nextEntry: AgentEntry = {
    ...
    ...(params.activeModelProvider !== undefined
      ? params.activeModelProvider
        ? { activeModelProvider: normalizeModelProviderId(params.activeModelProvider) }
        : {}
      : {}),
    ...(params.model ? { model: params.model } : {}),
  };
  if (params.activeModelProvider === null || params.activeModelProvider === "") {
    delete nextEntry.activeModelProvider;
  }
  if (params.modelProviders !== undefined) {
    const modelProviders = cleanAgentModelProviders(params.modelProviders ?? undefined);
    if (modelProviders) {
      nextEntry.modelProviders = modelProviders;
    } else {
      delete nextEntry.modelProviders;
    }
  }
  if (params.taskModels !== undefined) {
    const taskModels = normalizeAgentTaskModels(params.taskModels);
    if (taskModels) {
      nextEntry.taskModels = taskModels;
    } else {
      delete nextEntry.taskModels;
    }
  }
  const nextList = [...list];
  if (index >= 0) {
