...
function renderBlockingSystemGatewayServices(services: ExtraGatewayService[]): string {
  return [
    "System-level Fased gateway service detected while the user gateway service is not installed.",
    ...services.map((svc) => `- ${svc.label} (${svc.detail})`),
    "Fased will not install a second user-level gateway service automatically.",
    "Run `fased gateway status --deep` or `fased doctor --deep` to inspect duplicate services.",
  ].join("\n");
}
...
export async function maybeRepairGatewayDaemon(params: {
    ...
    if (params.cfg.gateway?.mode !== "remote") {
      ...
      if (install) {
        ...
        const { programArguments, workingDirectory, environment } = await buildGatewayInstallPlan({
          env: process.env,
          port,
          token: params.cfg.gateway?.auth?.token ?? process.env.FASED_GATEWAY_TOKEN,
  ...
  if (process.platform === "darwin") {
    const label = resolveGatewayLaunchAgentLabel(process.env.FASED_PROFILE);
    note(
      `LaunchAgent loaded; stopping requires "${formatCliCommand("fased gateway stop")}" or launchctl bootout gui/$UID/${label}.`,
      "Gateway",
    );
  }
