...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "FASED_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("FASED_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv FASED_GATEWAY_TOKEN");
    expect(message).not.toContain("FASED_GATEWAY_PASSWORD");
  });
