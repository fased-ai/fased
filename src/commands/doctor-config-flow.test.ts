...
describe("doctor config flow", () => {
  ...
  it("repairs stale wallet non-Solana policy and chain support from old installs", async () => {
      ...
      config: {
        wallet: {
          ...
          runtime: {
            ...
            policy: {
              [legacyPolicyKey]: { maxPerTx: "1", maxDaily: "2" },
    ...
    expect(result.cfg.wallet?.keystore?.chainSupport).toEqual(["solana"]);
    expect(result.cfg.wallet?.runtime?.chains).toEqual(["solana"]);
    expect(
      (result.cfg.wallet?.runtime?.policy as Record<string, unknown> | undefined)?.[
        legacyPolicyKey
      ],
    ).toBeUndefined();
  });

  it("disables stale passkey approval mode when no Wallet Control Passkey is enrolled", async () => {
    const result = await runDoctorConfigWithInput({
      repair: true,
      config: {
        wallet: {
    ...
    expect(result.cfg.wallet?.approvalAuth).toEqual({
      mode: "none",
      challengeTtlSeconds: 300,
      grantTtlSeconds: 120,
    });
