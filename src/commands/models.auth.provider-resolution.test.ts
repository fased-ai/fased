...
describe("resolveRequestedLoginProviderOrThrow", () => {
  ...
  it("throws when requested provider is not loaded", () => {
    ...
    expect(() =>
      resolveRequestedLoginProviderOrThrow(loadedProviders, "retired-provider"),
    ).toThrowError(
      'Unknown provider "retired-provider". Loaded providers: google-gemini-cli, qwen-portal. Verify plugins via `fased plugins list --json`.',
    );
  });
});
