...
export function collectWorkspaceDirs(cfg: FasedAgentConfig | undefined): string[] {
...
async function removeStateDir(
  stateDir: string,
  runtime: RuntimeEnv,
  opts?: { dryRun?: boolean; preserveWalletDir?: boolean },
): Promise<void> {
  if (!opts?.preserveWalletDir) {
  ...
  for (const entry of entries) {
    if (entry === "wallet") {
      runtime.log(
        `${opts?.dryRun ? "[dry-run] preserve" : "Preserved"} ${shortenHomeInString(path.join(stateDir, entry))}`,
      );
      continue;
    }
    await removePath(path.join(stateDir, entry), runtime, {
      dryRun: opts?.dryRun,
      label: path.join(stateDir, entry),
    });
  }
}
...
export async function removeStateAndLinkedPaths(
  ...
  await removeStateDir(cleanup.stateDir, runtime, {
    dryRun: opts?.dryRun,
    preserveWalletDir: opts?.preserveWalletDir,
  });
