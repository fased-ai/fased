...
function mockLocalPathExistsOnly() {
  vi.mocked(fs.existsSync).mockImplementation((value) =>
    String(value).endsWith(`${path.sep}extensions${path.sep}zalo`),
  );
}
...
async function runInitialValueForChannel(channel: "dev" | "beta") {
  ...
  const cfg: FasedAgentConfig = { update: { channel } };
...
describe("ensureOnboardingPluginInstalled", () => {
  it("installs from npm and enables the plugin", async () => {
    ...
    const cfg: FasedAgentConfig = { plugins: { allow: ["other"] } };
    ...
    expect(result.cfg.plugins?.installs?.zalo?.spec).toBe("@fased/zalo");
    expect(result.cfg.plugins?.installs?.zalo?.installPath).toBe("/tmp/zalo");
    expect(installPluginFromNpmSpec).toHaveBeenCalledWith(
      expect.objectContaining({ spec: "@fased/zalo", expectedIntegrity: "sha512-zalo" }),
    );
  });
  ...
  it("uses local path when selected", async () => {
    ...
    expect(result.cfg.plugins?.entries?.zalo?.enabled).toBe(true);
    expect(result.cfg.plugins?.allow).toEqual(["zalo"]);
  });

  it("uses local-only bundled entries without offering an undefined npm install", async () => {
    const runtime = makeRuntime();
    const select = vi.fn(async () => "local");
    const prompter = makePrompter({ select: select as WizardPrompter["select"] });
    const cfg: FasedAgentConfig = {};
    mockLocalPathExistsOnly();
    ...
    const call = (select.mock.calls as unknown as Array<[params: { options?: unknown[] }]>)[0];
    expect(call?.[0]?.options).toEqual(
      expect.arrayContaining([expect.objectContaining({ value: "local" })]),
    );
    expect(JSON.stringify(call?.[0]?.options)).not.toContain("undefined");
    expect(installPluginFromNpmSpec).not.toHaveBeenCalled();
    expect(result.installed).toBe(true);
    expect(result.cfg.plugins?.entries?.zalo?.enabled).toBe(true);
    expect(result.cfg.plugins?.load?.paths ?? []).toEqual([]);
  });
