...
export async function promptGatewayConfig(
  ...
  // Detect Tailscale binary only after incompatible auth modes have been resolved.
  if (tailscaleMode !== "off") {
    const tailscaleBin = await findTailscaleBinary();
    if (!tailscaleBin) {
      throw new Error(
        `${getTailscaleMissingBinNoteLines().join("\n")}\n\nInstall and authenticate Tailscale, then rerun gateway configuration.`,
      );
    }
  }
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: FasedAgent trusts user identity from a reverse proxy.",
        ...
        "Docs: https://docs.fased.ai/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );
