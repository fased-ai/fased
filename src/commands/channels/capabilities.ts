...
function normalizeChannelLookupKey(raw: string): string {
  return raw.trim().toLowerCase();
}

function listCatalogFallbackChannels(): CatalogFallbackChannel[] {
  const resolved = new Map<string, CatalogFallbackChannel>();
  for (const meta of listChatChannels()) {
    const dock = getChannelDock(meta.id);
    resolved.set(meta.id, {
      id: meta.id,
      support: dock?.capabilities,
    });
  }
...
function findCatalogFallbackChannel(rawChannel: string): CatalogFallbackChannel | undefined {
  const normalized = normalizeChannelLookupKey(rawChannel);
  if (!normalized) {
    return undefined;
  }
  const coreId = normalizeChatChannelId(normalized);
  if (coreId) {
    const dock = getChannelDock(coreId);
    return { id: coreId, support: dock?.capabilities };
  }
  for (const entry of listChannelPluginCatalogEntries()) {
    if (entry.id.toLowerCase() === normalized) {
      return { id: entry.id };
    }
    if ((entry.meta.aliases ?? []).some((alias) => alias.trim().toLowerCase() === normalized)) {
      return { id: entry.id };
    }
  }
  return undefined;
...
function buildCatalogFallbackReport(params: {
  channel: CatalogFallbackChannel;
  accountId?: string;
}): ChannelCapabilitiesReport {
  return {
    channel: params.channel.id,
    accountId: params.accountId?.trim() || "default",
    configured: false,
    enabled: false,
    support: params.channel.support,
    actions: params.channel.support ? ["send", "broadcast"] : undefined,
  };
}
...
export async function channelsCapabilitiesCommand(
  ...
  if (!rawChannel || rawChannel === "all") {
    ...
    for (const channel of listCatalogFallbackChannels()) {
      if (seen.has(channel.id)) {
        continue;
      }
      seen.add(channel.id);
      reports.push(buildCatalogFallbackReport({ channel, accountId: accountOverride }));
    }
  } else {
    ...
    if (plugin) {
      ...
      const channel = findCatalogFallbackChannel(rawChannel);
      if (channel) {
        reports.push(buildCatalogFallbackReport({ channel, accountId: accountOverride }));
      }
    }
