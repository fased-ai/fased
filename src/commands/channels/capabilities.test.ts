...
describe("channelsCapabilitiesCommand", () => {
  ...
  it("prints Slack bot + user scopes when user token is configured", async () => {
    const plugin = buildPlugin({
      ...
      probe: { ok: true, bot: { name: "fased" }, team: { name: "team" } },
    });
  ...
  it("falls back to catalog channels when no channel plugins are active", async () => {
    vi.mocked(listChannelPlugins).mockReturnValue([]);
    vi.mocked(getChannelPlugin).mockReturnValue(undefined);

    await channelsCapabilitiesCommand({ channel: "all", json: true }, runtime);

    const payload = JSON.parse(logs.join("\n")) as {
      channels: Array<{
        channel: string;
        configured?: boolean;
        support?: { chatTypes?: string[] };
      }>;
    };
    const telegram = payload.channels.find((entry) => entry.channel === "telegram");
    const wecom = payload.channels.find((entry) => entry.channel === "wecom");
    expect(telegram).toMatchObject({
      channel: "telegram",
      configured: false,
      support: { chatTypes: ["direct", "group", "channel", "thread"] },
    });
    expect(wecom).toBeUndefined();
  });

  it("resolves an advertised core channel even when its plugin is inactive", async () => {
    vi.mocked(listChannelPlugins).mockReturnValue([]);
    vi.mocked(getChannelPlugin).mockReturnValue(undefined);

    await channelsCapabilitiesCommand({ channel: "telegram", json: true }, runtime);
