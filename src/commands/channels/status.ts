...
import { type FasedAgentConfig, readConfigFileSnapshot } from "../../config/config.js";
...
function isInstallableCatalogEntry(entry: ChannelPluginCatalogEntry): boolean {
  return entry.catalogSource === "external-catalog" || entry.catalogSource === "official-catalog";
}

function appendInstallableChannelCatalogLines(lines: string[]) {
  const entries = listChannelPluginCatalogEntries().filter(isInstallableCatalogEntry);
  if (entries.length === 0) {
    return;
  }
  lines.push("Installable channel catalog:");
  for (const entry of entries) {
    lines.push(`- ${formatChannelPluginCatalogStatusLine(entry)}`);
  }
}
...
export function formatGatewayChannelsStatusLines(payload: Record<string, unknown>): string[] {
  ...
  appendInstallableChannelCatalogLines(lines);
  lines.push("");
  ...
  if (issues.length > 0) {
    ...
    lines.push(`- Run: ${formatCliCommand("fased doctor")}`);
    lines.push("");
  }
  lines.push(
