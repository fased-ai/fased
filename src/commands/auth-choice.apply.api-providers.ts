...
import {
  ...
  applyQwenCodingPlanConfig,
  applyQwenCodingPlanProviderConfig,
  applyQwenConfig,
...
const SIMPLE_API_KEY_PROVIDER_FLOWS: Partial<Record<AuthChoice, SimpleApiKeyProviderFlow>> = {
  "ai-gateway-api-key": {
    ...
    promptMessage: "Enter Vercel AI API key",
  ...
  "chutes-api-key": {
    ...
    noteMessage: [
      "Chutes exposes open-source models through an OpenAI-compatible API.",
      "Create an API key from your Chutes account settings.",
    ].join("\n"),
    noteTitle: "Chutes",
  },
  "moonshot-api-key": {
  ...
  "qwen-coding-plan-api-key": {
    ...
    expectedProviders: ["qwen-coding-plan"],
    envLabel: "BAILIAN_CODING_PLAN_API_KEY",
    promptMessage: "Enter Qwen Coding Plan API key",
    setCredential: setQwenCodingPlanApiKey,
    defaultModel: QWEN_CODING_PLAN_DEFAULT_MODEL_REF,
    applyDefaultConfig: applyQwenCodingPlanConfig,
    applyProviderConfig: applyQwenCodingPlanProviderConfig,
    noteDefault: QWEN_CODING_PLAN_DEFAULT_MODEL_REF,
    noteMessage: [
      "Qwen Coding Plan uses Alibaba Cloud's coding endpoint.",
      "Use the sk-sp... Coding Plan key from Alibaba Cloud Model Studio.",
    ].join("\n"),
    noteTitle: "Qwen Coding Plan",
  },
  "qwen-api-key": {
    provider: "qwen",
    profileId: "qwen:default",
    expectedProviders: ["qwen"],
    envLabel: "DASHSCOPE_API_KEY",
    promptMessage: "Enter Qwen DashScope API key",
    setCredential: setQwenApiKey,
    defaultModel: QWEN_DEFAULT_MODEL_REF,
    applyDefaultConfig: applyQwenConfig,
    applyProviderConfig: applyQwenProviderConfig,
    noteDefault: QWEN_DEFAULT_MODEL_REF,
    noteMessage: [
      "Qwen DashScope uses Alibaba's OpenAI-compatible endpoint.",
      "Use the sk... pay-as-you-go API key from DashScope/Qwen Cloud.",
    ].join("\n"),
    noteTitle: "Qwen",
  },
  "synthetic-api-key": {
...
export async function applyAuthChoiceApiProviders(
  ...
  if (authChoice === "litellm-api-key") {
    ...
    if (!hasCredential) {
      await ensureApiKeyFromOptionEnvOrPrompt({
        ...
        noteMessage:
          "LiteLLM provides a unified OpenAI-compatible API to your configured proxy models.\nUse a LiteLLM virtual key or master key.\nDefault API base URL is http://localhost:4000/v1",
        noteTitle: "LiteLLM",
      });
      hasCredential = true;
    }
  ...
  if (authChoice === "cloudflare-ai-gateway-api-key") {
    ...
    const ensureAccountGateway = async () => {
      ...
      if (!gatewayId) {
        const value = await params.prompter.text({
          message: "Enter Cloudflare Gateway ID",
    ...
    await ensureApiKeyFromOptionEnvOrPrompt({
      ...
      promptMessage: "Enter Cloudflare AI API key",
      normalize: normalizeApiKeyInput,
      validate: validateApiKeyInput,
      prompter: params.prompter,
      setCredential: async (apiKey, mode) =>
