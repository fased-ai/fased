...
export function printModelTable(
  ...
  for (const row of rows) {
    ...
    const featuresLabel = pad(truncate(row.features.join("+") || "-", FEATURES_PAD), FEATURES_PAD);
    ...
    const coloredFeatures = colorize(
      rich,
      row.features.includes("private-net blocked") ? theme.warn : theme.muted,
      featuresLabel,
    );
