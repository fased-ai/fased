...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveFasedAgentAgentDir();
  ...
  const catalogStatus = opts.plain
    ? undefined
    : buildModelCatalogStatus({
        catalog: await loadModelCatalog({ config: cfg }),
        cfg,
        providerExtensionCatalog: await loadProviderExtensionCatalogIndex(),
      });
  ...
  if (catalogStatus) {
    const sourceSummary = Object.entries(catalogStatus.sourceCounts)
      .toSorted(([left], [right]) => left.localeCompare(right))
      .map(([source, count]) => `${source}=${count}`)
      .join(", ");
    const catalogWarnings =
      catalogStatus.providerExtensionCatalog.errorEntries +
      catalogStatus.providerExtensionCatalog.skippedUntrustedEntries;
    runtime.log("");
    runtime.log(colorize(rich, theme.heading, "Catalog overview"));
    runtime.log(
      `${label("Models")}${colorize(rich, theme.muted, ":")} ${colorize(
        rich,
        theme.info,
        `${catalogStatus.totalModels} across ${catalogStatus.totalProviders} providers`,
      )}`,
    );
    runtime.log(
      `${label("Sources")}${colorize(rich, theme.muted, ":")} ${colorize(
        rich,
        sourceSummary ? theme.accent : theme.muted,
        sourceSummary || "-",
      )}`,
    );
    runtime.log(
      `${label("Provider index")}${colorize(rich, theme.muted, ":")} ${colorize(
        rich,
        catalogWarnings > 0 ? theme.warn : theme.success,
        `${catalogStatus.providerExtensionCatalog.loadedEntries} loaded, ${catalogWarnings} warnings`,
      )}`,
    );
    runtime.log(
      `${label("Deferred upstream")}${colorize(rich, theme.muted, ":")} ${colorize(
        rich,
        catalogStatus.providerExtensionManifest.deferredProviderIds.length
          ? theme.warn
          : theme.muted,
        catalogStatus.providerExtensionManifest.deferredProviderIds.length
          ? catalogStatus.providerExtensionManifest.deferredProviderIds.join(", ")
          : "-",
      )}`,
    );
  }
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("fased models auth setup-token")}\` or \`${formatCliCommand("fased configure")}\`.`
          : `Run \`${formatCliCommand("fased configure")}\` or set an API key env var.`;
