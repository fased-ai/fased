...
function normalizeAnthropicLoginChoice(raw?: string): AuthChoice | null {
  const normalized = normalizeProviderId(raw ?? "");
  if (!normalized) {
    return null;
  }
  if (normalized === "oauth" || normalized === "anthropic-oauth") {
    return "anthropic-oauth";
  }
  if (normalized === "setup-token" || normalized === "token" || normalized === "claude-cli") {
    return "token";
  }
  if (normalized === "api-key" || normalized === "apikey" || normalized === "apiKey") {
    return "apiKey";
  }
  throw new Error(`Unknown Anthropic auth method "${raw}". Use oauth, setup-token, or api-key.`);
}
...
export function resolveRequestedLoginProviderOrThrow(
  ...
  throw new Error(
    `Unknown provider "${requested}". Loaded providers: ${availableText}. Verify plugins via \`${formatCliCommand("fased plugins list --json")}\`.`,
  );
}
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  const requestedProviderId = opts.provider ? normalizeProviderId(opts.provider) : "";
  if (requestedProviderId === "anthropic") {
    const prompter = createClackPrompter();
    const authChoice =
      normalizeAnthropicLoginChoice(opts.method) ??
      ((await prompter.select({
        message: "Anthropic auth method",
        options: ANTHROPIC_PROVIDER_MANIFEST.methods.map((method) => ({
          value: method.id,
          label: method.label,
          hint: method.id === "token" ? "Paste a token from `claude setup-token`" : undefined,
        })),
      })) as AuthChoice);
    ...
    const result = await applyAuthChoice({
      ...
      setDefaultModel: opts.setDefault === true,
      opts: {},
    });
    await updateConfig(() => result.config);
    logConfigUpdated(runtime);
    runtime.log(`Anthropic auth configured with ${authChoice}.`);
    return;
  }
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("fased plugins install")}\`.`,
    );
  }
