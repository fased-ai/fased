...
export async function loadValidConfigOrThrow(): Promise<FasedAgentConfig> {
...
export async function updateConfig(
  mutator: (cfg: FasedAgentConfig) => FasedAgentConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: FasedAgentConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: FasedAgentConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("fased agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
