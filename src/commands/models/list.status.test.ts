...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveFasedAgentAgentDir: vi.fn().mockReturnValue("/tmp/fased-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/fased-agent"),
    ...
    resolveAuthStorePathForDisplay: vi.fn().mockReturnValue("/tmp/fased-agent/auth-profiles.json"),
    ...
    loadProviderExtensionCatalogIndex: vi.fn().mockResolvedValue({
      providers: {},
      entries: [
        {
          id: "fased-openai",
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveFasedAgentAgentDir: mocks.resolveFasedAgentAgentDir,
}));
...
vi.mock("../../agents/provider-extension-catalog-index.js", () => ({
  loadProviderExtensionCatalogIndex: mocks.loadProviderExtensionCatalogIndex,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/fased-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveFasedAgentAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.catalog.sourceCounts).toEqual({
      "current-preview": 1,
      "provider-index": 1,
    });
    expect(payload.catalog.providerExtensionCatalog).toMatchObject({
      totalEntries: 1,
      loadedEntries: 1,
      errorEntries: 0,
      loadedProviderIds: [],
    });
    expect(payload.catalog.providerExtensionManifest.deferredProviderIds).toEqual([]);
    expect(payload.auth.storePath).toBe("/tmp/fased-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/fased-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/fased-agent-custom");
