...
export function normalizeLegacyConfigValues(cfg: FasedAgentConfig): {
  config: FasedAgentConfig;
  changes: string[];
} {
  const changes: string[] = [];
  let next: FasedAgentConfig = cfg;
  ...
  const singleAccountTopLevelKeys = [
    "botToken",
    "token",
    "appToken",
    "signingSecret",
  ...
  const singleAccountMoveTriggerKeys = singleAccountTopLevelKeys.filter(
    (key) => key !== "streaming" && key !== "nativeStreaming",
  );

  const moveTopLevelSingleAccountValues = (params: {
    provider: "telegram" | "slack" | "discord";
    entry: Record<string, unknown>;
  }): { entry: Record<string, unknown>; changed: boolean } => {
    const rawAccounts = params.entry.accounts;
    if (!isRecord(rawAccounts) || isRecord(rawAccounts.default)) {
      return { entry: params.entry, changed: false };
    }
    if (!singleAccountMoveTriggerKeys.some((key) => params.entry[key] !== undefined)) {
      return { entry: params.entry, changed: false };
    }

    const defaultAccount: Record<string, unknown> = {};
    let updated: Record<string, unknown> = params.entry;
    for (const key of singleAccountTopLevelKeys) {
      if (updated[key] !== undefined) {
        defaultAccount[key] = updated[key];
        const { [key]: _ignored, ...rest } = updated;
        updated = rest;
      }
    }
    ...
    changes.push(
      `Moved channels.${params.provider} single-account top-level values into channels.${params.provider}.accounts.default.`,
    );
  ...
  const normalizeProvider = (provider: "telegram" | "slack" | "discord") => {
    ...
    const streaming = normalizeStreamingAliasesForProvider({
      provider,
      entry: updated,
      pathPrefix: `channels.${provider}`,
    });
    ...
    const defaultAccountMove = moveTopLevelSingleAccountValues({ provider, entry: updated });
    updated = defaultAccountMove.entry;
    changed = changed || defaultAccountMove.changed;
  ...
  const browser = next.browser as Record<string, unknown> | undefined;
  const ssrfPolicy = browser?.ssrfPolicy;
  if (isRecord(browser) && isRecord(ssrfPolicy) && ssrfPolicy.allowPrivateNetwork !== undefined) {
    const allowPrivateNetwork = ssrfPolicy.allowPrivateNetwork === true;
    const { allowPrivateNetwork: _ignored, ...restSsrfPolicy } = ssrfPolicy;
    next = {
      ...next,
      browser: {
        ...browser,
    ...
    changes.push(
      `Moved browser.ssrfPolicy.allowPrivateNetwork → browser.ssrfPolicy.dangerouslyAllowPrivateNetwork (${allowPrivateNetwork}).`,
    );
  }
...
export const normalizeCompatibilityConfigValues = normalizeLegacyConfigValues;
