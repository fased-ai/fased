...
beforeAll(() => {
  envSnapshot = captureEnv(["FASED_PROFILE"]);
  process.env.FASED_PROFILE = "isolated";
});
...
vi.mock("../infra/fased-root.js", () => ({
  resolveFasedAgentPackageRoot: vi.fn().mockResolvedValue("/tmp/fased"),
  resolveFasedAgentPackageRootSync: vi.fn().mockReturnValue("/tmp/fased"),
}));
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.fased.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.fased.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = runtimeLogMock.mock.calls.map((c: unknown[]) => String(c[0]));
    expect(logs.some((l: string) => l.includes("FasedAgent status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l: string) =>
          l.includes("fased status --all") ||
          l.includes("fased --profile isolated status --all") ||
          l.includes("fased status --all") ||
          l.includes("fased --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.FASED_GATEWAY_TOKEN = "abcd1234";
    try {
