...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under FASED_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("fased-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    ...
    process.env.FASED_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "fased-oauth-nosync-"));
    process.env.FASED_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside FASED_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "fased-oauth-external-"));
    process.env.FASED_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* FASED_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to FASED_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("fased-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
...
describe("applyAuthProfileConfig", () => {
  ...
  it("creates provider order when switching from legacy oauth to api_key without explicit order", () => {
    const next = applyAuthProfileConfig(
      {
        auth: {
          profiles: {
            "openrouter:legacy": { provider: "openrouter", mode: "oauth" },
          },
        },
      },
      {
    ...
    expect(next.auth?.order?.openrouter).toEqual(["openrouter:default", "openrouter:legacy"]);
  });
    ...
    const next = applyAuthProfileConfig(
      {
        auth: {
          profiles: {
            "openrouter:legacy": { provider: "openrouter", mode: "api_key" },
          },
        },
      },
      {
