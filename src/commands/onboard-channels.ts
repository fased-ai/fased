...
type OnboardingChannelEntry = {
  id: ChannelChoice;
  meta: ChannelMeta;
};

function listOnboardingChannelEntries(cfg: FasedAgentConfig): {
  entries: OnboardingChannelEntry[];
  installedPlugins: ReturnType<typeof listChannelPlugins>;
  catalogEntries: ReturnType<typeof listChannelPluginCatalogEntries>;
  catalogById: Map<ChannelChoice, ReturnType<typeof listChannelPluginCatalogEntries>[number]>;
} {
  ...
  const coreIds = new Set(core.map((meta) => meta.id));
  const workspaceDir = resolveAgentWorkspaceDir(cfg, resolveDefaultAgentId(cfg));
  const catalogEntries = listChannelPluginCatalogEntries({ workspaceDir }).filter(
    (entry) => !installedIds.has(entry.id) && !coreIds.has(entry.id),
  );
  ...
  return {
    entries: Array.from(metaById, ([id, meta]) => ({ id: id as ChannelChoice, meta })),
    installedPlugins,
    catalogEntries,
    catalogById: new Map(catalogEntries.map((entry) => [entry.id as ChannelChoice, entry])),
  };
}
...
async function collectChannelStatus(params: {
  ...
  const { entries, installedPlugins, catalogEntries, catalogById } = listOnboardingChannelEntries(
    params.cfg,
  );
  ...
  const fallbackStatuses = entries
    .filter((entry) => !statusByChannel.has(entry.id) && !catalogById.has(entry.id))
    .map((entry) => {
  ...
  const catalogStatuses = catalogEntries.map((entry) => ({
    channel: entry.id,
    configured: false,
    statusLines: [formatChannelPluginCatalogStatusLine(entry)],
    selectionHint: formatChannelPluginCatalogSelectionHint(entry),
    quickstartScore: 0,
  }));
...
async function noteChannelPrimer(
  ...
  await prompter.note(
    [
      ...
      `Approve with: ${formatCliCommand("fased pairing approve <channel> <code>")}`,
      'Public DMs require dmPolicy="open" + allowFrom=["*"].',
      "Multi-user DMs: run: " +
        formatCliCommand('fased config set session.dmScope "per-channel-peer"') +
  ...
  const selectPolicy = async (policy: ChannelOnboardingDmPolicy) => {
    await prompter.note(
      [
        ...
        `Approve: ${formatCliCommand(`fased pairing approve ${policy.channel} <code>`)}`,
...
export async function setupChannels(
  ...
  const primerChannels = listOnboardingChannelEntries(next).entries.map((entry) => ({
    id: entry.id,
    label: entry.meta.label,
    blurb: entry.meta.blurb,
  }));
  if (!options?.skipPrimerNote) {
  ...
  const getChannelEntries = () => {
    const { entries, catalogEntries: catalog, catalogById } = listOnboardingChannelEntries(next);
    return {
      entries,
      catalog,
      catalogById,
      ...
      if (adapter) {
        await prompter.note(
          `${channel} plugin not available (continuing with onboarding). If the channel still doesn't work after setup, run \`${formatCliCommand(
            "fased plugins list",
          )}\` and \`${formatCliCommand("fased plugins enable " + channel)}\`, then restart the gateway.`,
          "Channel setup",
        );
    ...
    const choice = (await prompter.select({
      ...
      options: [
        ...buildSelectionOptions(entries),
        {
          value: "__skip__",
          label: "Skip for now",
          hint: `You can add channels later via \`${formatCliCommand("fased channels add")}\``,
        },
      ],
