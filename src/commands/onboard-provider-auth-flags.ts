...
type OnboardProviderAuthOptionKey = keyof Pick<
  ...
  | "qianfanApiKey"
  | "qwenApiKey"
  | "qwenCodingPlanApiKey"
  | "volcengineApiKey"
  | "byteplusApiKey"
>;

export type OnboardProviderAuthFlag = {
  optionKey: OnboardProviderAuthOptionKey | (string & {});
...
export const ONBOARD_PROVIDER_AUTH_FLAGS: ReadonlyArray<OnboardProviderAuthFlag> = [
  ...
  },
  {
    ...
    description: "Cloudflare AI API key",
  },
  {
  ...
  },
  {
    ...
    cliFlag: "--qwen-coding-plan-api-key",
    cliOption: "--qwen-coding-plan-api-key <key>",
    description: "Qwen Coding Plan API key",
  },
  {
...
function mapManifestFlag(flag: ProviderOnboardAuthFlag): OnboardProviderAuthFlag {
  return {
    optionKey: flag.optionKey,
    authChoice: flag.authChoice as AuthChoice,
    cliFlag: flag.cliFlag as `--${string}`,
    cliOption: flag.cliOption as `--${string} <key>`,
    description: flag.description,
  };
}

export function resolveOnboardProviderAuthFlags(params?: {
  config?: FasedAgentConfig;
  workspaceDir?: string;
  env?: NodeJS.ProcessEnv;
}): OnboardProviderAuthFlag[] {
  const merged: OnboardProviderAuthFlag[] = [...ONBOARD_PROVIDER_AUTH_FLAGS];
  const seen = new Set(merged.map((flag) => `${String(flag.optionKey)}::${flag.cliFlag}`));

  for (const manifestFlag of resolveManifestProviderOnboardAuthFlags({
    config: params?.config,
    workspaceDir: params?.workspaceDir,
    env: params?.env,
    includeUntrustedWorkspacePlugins: false,
  })) {
    const mapped = mapManifestFlag(manifestFlag);
    const dedupeKey = `${String(mapped.optionKey)}::${mapped.cliFlag}`;
