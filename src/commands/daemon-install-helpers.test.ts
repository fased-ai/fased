...
describe("resolveGatewayDevMode", () => {
  it("detects dev mode for src ts entrypoints", () => {
    expect(resolveGatewayDevMode(["node", "/Users/me/fased/src/cli/index.ts"])).toBe(true);
    expect(resolveGatewayDevMode(["node", "C:\\Users\\me\\fased\\src\\cli\\index.ts"])).toBe(true);
    expect(resolveGatewayDevMode(["node", "/Users/me/fased/dist/cli/index.js"])).toBe(false);
  });
});
...
function mockNodeGatewayPlanFixture(
  ...
  const {
    ...
    supported = true,
    warning,
    serviceEnvironment = { FASED_PORT: "3000" },
...
describe("buildGatewayInstallPlan", () => {
  it("uses provided nodePath and returns plan", async () => {
    ...
    expect(plan.environment).toEqual({ FASED_PORT: "3000" });
  ...
  it("pins the managed service node binary into environment PATH", async () => {
    ...
    mocks.resolveGatewayProgramArguments.mockResolvedValue({
      programArguments: ["/opt/fased/node/v23.3.0/bin/node", "dist/entry.js"],
      workingDirectory: "/Users/me",
    });
  ...
  it("merges config env vars into the environment", async () => {
    ...
    expect(plan.environment.FASED_PORT).toBe("3000");
  ...
  it("passes explicit startupMode overrides through to program argument resolution", async () => {
    mockNodeGatewayPlanFixture();
...
describe("resolveGatewayStartupMode", () => {
  it("defaults to gateway when federation auto-connect is enabled without explicit managed mode", () => {
...
describe("gatewayInstallErrorHint", () => {
  it("returns platform-specific hints", () => {
    ...
    expect(gatewayInstallErrorHint("linux")).toMatch(
      /(?:fased|fased)( --profile isolated)? gateway install/,
    );
  });
});
