...
const requireAgentDir = () => {
  const agentDir = process.env.FASED_AGENT_DIR;
  if (!agentDir) {
    throw new Error("FASED_AGENT_DIR not set");
  }
  return agentDir;
};
...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under FASED_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "fased-oauth-"));
    process.env.FASED_STATE_DIR = tempStateDir;
    process.env.FASED_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.FASED_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to FASED_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "fased-minimax-"));
    process.env.FASED_STATE_DIR = tempStateDir;
    process.env.FASED_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.FASED_AGENT_DIR;
...
describe("applyMinimaxApiConfig", () => {
  ...
  it("sets reasoning for the current default model", () => {
  ...
  it("adds model alias", () => {
    const cfg = applyMinimaxApiConfig({}, "MiniMax-M2.7");
    expect(cfg.agents?.defaults?.models?.["minimax/MiniMax-M2.7"]?.alias).toBe("Minimax");
  });
...
describe("applyZaiConfig", () => {
  ...
  it("sets correct primary model", () => {
    ...
    expect(resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model)).toBe("zai/glm-5");
  });
  ...
  it("supports CN endpoint", () => {
    ...
    expect(resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model)).toBe("zai/glm-4.7-flash");
  });

  it("supports CN endpoint with glm-4.7-flashx", () => {
    const cfg = applyZaiConfig({}, { endpoint: "coding-cn", modelId: "glm-4.7-flashx" });
    expect(cfg.models?.providers?.zai?.baseUrl).toBe(ZAI_CODING_CN_BASE_URL);
    expect(resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model)).toBe("zai/glm-4.7-flashx");
  });
});
...
describe("applyXiaomiConfig", () => {
  it("adds Xiaomi provider with correct settings", () => {
    ...
    expect(resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model)).toBe("xiaomi/mimo-v2.5-pro");
  });
...
describe("applyOpencodeZenProviderConfig", () => {
  it("adds allowlist entry for the default model", () => {
    ...
    expect(Object.keys(models)).toContain("opencode/gpt-5.5");
  });
  ...
  it("preserves existing alias for the default model", () => {
    const cfg = applyOpencodeZenProviderConfig({
      agents: {
        defaults: {
          models: {
            "opencode/gpt-5.5": { alias: "My GPT" },
          },
        },
      },
    });
    expect(cfg.agents?.defaults?.models?.["opencode/gpt-5.5"]?.alias).toBe("My GPT");
  });
});
...
describe("applyOpencodeZenConfig", () => {
  it("sets correct primary model", () => {
    ...
    expect(resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model)).toBe("opencode/gpt-5.5");
  });
