...
import { hasEmittedCliBanner } from "../cli/banner.js";
import type { FasedAgentConfig } from "../config/config.js";
import { CONFIG_PATH, resolveStateDir } from "../config/config.js";
...
export function summarizeExistingConfig(config: FasedAgentConfig): string {
  const rows: string[] = [];
...
export function printWizardHeader(runtime: RuntimeEnv) {
  if (hasEmittedCliBanner() || process.env.FASED_INSTALLER_ONBOARD === "1") {
    return;
  }
  const header = [formatFasedDisplayLine(), ""].join("\n");
  runtime.log(header);
}
...
export async function ensureWorkspaceAndSessions(
  workspaceDir: string,
  runtime: RuntimeEnv,
  options?: { skipBootstrap?: boolean; agentId?: string; quietLogs?: boolean },
) {
...
export async function handleOnboardingRepair(scope: OnboardRepairScope, runtime: RuntimeEnv) {
  if (scope === "auth" || scope === "auth+sessions") {
    await moveToTrash(path.join(CONFIG_DIR, "credentials"), runtime);
    await moveToTrash(resolveAuthStorePath(), runtime);
  }

  if (scope === "sessions" || scope === "auth+sessions") {
    const sessionDirs = new Set<string>([
      resolveSessionTranscriptsDirForAgent(),
      ...(await listAgentSessionDirs(resolveStateDir())),
    ]);
...
export function resolveControlUiLinks(params: {
  ...
  const browserHost = host === "127.0.0.1" ? "localhost" : host;
  return {
    httpUrl: `http://${browserHost}:${port}${uiPath}`,
