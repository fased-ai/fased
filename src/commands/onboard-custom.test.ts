    ...
    const result = await runPromptCustomApi(prompter, {
      secrets: {
        providers: {
          filemain: {
            source: "file",
            path: "/tmp/fased-missing-provider.json",
            mode: "json",
          },
        },
      },
...
describe("applyCustomApiConfig", () => {
  ...
  it("requires explicit private-network opt-in for local custom providers", () => {
    expect(() =>
      applyCustomApiConfig({
        config: {},
    ...
    expect(result.config.models?.providers?.["custom-127-0-0-1-11434"]?.request).toEqual({
      allowPrivateNetwork: true,
    });
  });
});
...
describe("parseNonInteractiveCustomApiFlags", () => {
  ...
  it("preserves explicit private-network opt-in", () => {
    const result = parseNonInteractiveCustomApiFlags({
      baseUrl: "http://127.0.0.1:11434/v1",
      modelId: "llama3",
      allowPrivateNetwork: true,
