...
const resolveAuthAgentDir = (agentDir?: string) => agentDir ?? resolveFasedAgentAgentDir();
...
export async function setQwenApiKey(
  ...
  upsertAuthProfile({
    ...
    credential: buildApiKeyCredential("qwen", key, undefined, options),
    agentDir: resolveAuthAgentDir(agentDir),
  });
}

export async function setQwenCodingPlanApiKey(
  key: SecretInput,
  agentDir?: string,
  options?: ApiKeyStorageOptions,
) {
  upsertAuthProfile({
    ...
    credential: buildApiKeyCredential("qwen-coding-plan", key, undefined, options),
    agentDir: resolveAuthAgentDir(agentDir),
  });
}
