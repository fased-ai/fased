...
async function withOnboardEnv(
  ...
  const configPath = path.join(tempHome, "fased.json");
    ...
    await withEnvAsync(
      {
        ...
        FASED_GATEWAY_PASSWORD: undefined,
...
describe("onboard (non-interactive): provider auth", () => {
  ...
  it("stores MiniMax API key and uses global baseUrl by default", async () => {
    await withOnboardEnv("fased-onboard-minimax-", async (env) => {
  ...
  it("supports MiniMax CN API endpoint auth choice", async () => {
    await withOnboardEnv("fased-onboard-minimax-cn-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("stores Z.AI API key and uses global baseUrl by default", async () => {
    await withOnboardEnv("fased-onboard-zai-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("supports Z.AI CN coding endpoint auth choice", async () => {
    await withOnboardEnv("fased-onboard-zai-cn-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("stores xAI API key and sets default model", async () => {
    await withOnboardEnv("fased-onboard-xai-", async (env) => {
      ...
      expect(cfg.agents?.defaults?.model?.primary).toBe("xai/grok-4.3");
  ...
  it("infers Mistral auth choice from --mistral-api-key and sets default model", async () => {
    await withOnboardEnv("fased-onboard-mistral-infer-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
      ...
      expect(cfg.agents?.defaults?.model?.primary).toBe("mistral/mistral-medium-3.5");
      await expectApiKeyProfile({
  ...
  it("stores Volcano Engine API key and sets default model", async () => {
    await withOnboardEnv("fased-onboard-volcengine-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("infers BytePlus auth choice from --byteplus-api-key and sets default model", async () => {
    await withOnboardEnv("fased-onboard-byteplus-infer-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it("stores Qwen API keys and sets route-compatible default models", async () => {
    await withOnboardEnv("fased-onboard-qwen-api-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
        authChoice: "qwen-api-key",
        qwenApiKey: "dashscope-test-key",
      });
    ...
    await withOnboardEnv("fased-onboard-qwen-plan-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
        authChoice: "qwen-coding-plan-api-key",
        qwenCodingPlanApiKey: "coding-plan-test-key",
      });
      ...
      await expectApiKeyProfile({
        ...
        key: "coding-plan-test-key",
      });
    });
  });

  it("stores Vercel AI Gateway API key and sets default model", async () => {
    await withOnboardEnv("fased-onboard-ai-gateway-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
      ...
      expect(cfg.agents?.defaults?.model?.primary).toBe("vercel-ai-gateway/openai/gpt-5.5");
      await expectApiKeyProfile({
  ...
  it("stores token auth profile", async () => {
    await withOnboardEnv("fased-onboard-token-", async ({ configPath, runtime }) => {
  ...
  it("stores OpenAI API key and sets OpenAI default model", async () => {
    await withOnboardEnv("fased-onboard-openai-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
  ...
  it.each([
    {
      name: "anthropic",
      prefix: "fased-onboard-ref-flag-anthropic-",
      authChoice: "apiKey",
    ...
    {
      name: "openai",
      prefix: "fased-onboard-ref-flag-openai-",
      authChoice: "openai-api-key",
    ...
    {
      name: "openrouter",
      prefix: "fased-onboard-ref-flag-openrouter-",
    ...
    {
      name: "xai",
      prefix: "fased-onboard-ref-flag-xai-",
      authChoice: "xai-api-key",
      optionKey: "xaiApiKey",
    ...
    {
      name: "volcengine",
      prefix: "fased-onboard-ref-flag-volcengine-",
      authChoice: "volcengine-api-key",
    ...
    {
      name: "byteplus",
      prefix: "fased-onboard-ref-flag-byteplus-",
    ...
    {
      name: "qwen",
      prefix: "fased-onboard-ref-flag-qwen-",
      authChoice: "qwen-api-key",
      optionKey: "qwenApiKey",
      flagName: "--qwen-api-key",
      envVar: "DASHSCOPE_API_KEY",
    },
    {
      name: "qwen-coding-plan",
      prefix: "fased-onboard-ref-flag-qwen-plan-",
      authChoice: "qwen-coding-plan-api-key",
      optionKey: "qwenCodingPlanApiKey",
      flagName: "--qwen-coding-plan-api-key",
      envVar: "BAILIAN_CODING_PLAN_API_KEY",
    },
  ])(
  ...
  it("stores the detected env alias as keyRef for opencode ref mode", async () => {
    await withOnboardEnv("fased-onboard-ref-opencode-alias-", async ({ runtime }) => {
      await withEnvAsync(
        {
  ...
  it("rejects vLLM auth choice in non-interactive mode", async () => {
    await withOnboardEnv("fased-onboard-vllm-non-interactive-", async ({ runtime }) => {
      await expect(
  ...
  it.each([
    {
      ...
      prefix: "fased-onboard-cf-gateway-",
      options: {
    ...
    },
    {
      ...
      prefix: "fased-onboard-cf-gateway-infer-",
      options: {},
    },
  ...
  it("infers Together auth choice from --together-api-key and sets default model", async () => {
    await withOnboardEnv("fased-onboard-together-infer-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
      ...
      expect(cfg.agents?.defaults?.model?.primary).toBe("together/moonshotai/Kimi-K2.6");
      await expectApiKeyProfile({
  ...
  it("infers QIANFAN auth choice from --qianfan-api-key and sets default model", async () => {
    await withOnboardEnv("fased-onboard-qianfan-infer-", async (env) => {
      const cfg = await runOnboardingAndReadConfig(env, {
      ...
      expect(cfg.agents?.defaults?.model?.primary).toBe("qianfan/ernie-5.1");
      await expectApiKeyProfile({
  ...
  it("configures a custom provider from non-interactive flags", async () => {
    await withOnboardEnv("fased-onboard-custom-provider-", async ({ configPath, runtime }) => {
      await runNonInteractiveOnboardingWithDefaults(runtime, {
        authChoice: "custom-api-key",
  ...
  it("infers custom provider auth choice from custom flags", async () => {
    await withOnboardEnv(
      "fased-onboard-custom-provider-infer-",
  ...
  it("requires explicit private-network opt-in for non-interactive custom providers", async () => {
    await withOnboardEnv("fased-onboard-custom-provider-private-deny-", async ({ runtime }) => {
      await expect(
        runNonInteractiveOnboardingWithDefaults(runtime, {
          authChoice: "custom-api-key",
          customBaseUrl: "https://models.custom.local/v1",
      ...
      ).rejects.toThrow(/allow-private-network|private/i);
    });
  });

  it("uses CUSTOM_API_KEY env fallback for non-interactive custom provider auth", async () => {
    await withOnboardEnv(
      "fased-onboard-custom-provider-env-fallback-",
      async ({ configPath, runtime }) => {
  ...
  it("stores CUSTOM_API_KEY env ref for non-interactive custom provider auth in ref mode", async () => {
    await withOnboardEnv(
      "fased-onboard-custom-provider-env-ref-",
      async ({ configPath, runtime }) => {
        process.env.CUSTOM_API_KEY = "custom-env-key";
        await runCustomLocalNonInteractive(runtime, {
          secretInputMode: "ref",
  ...
  it("fails fast for custom provider ref mode when --custom-api-key is set but CUSTOM_API_KEY env is missing", async () => {
    await withOnboardEnv("fased-onboard-custom-provider-ref-flag-", async ({ runtime }) => {
  ...
  it("uses matching profile fallback for non-interactive custom provider auth", async () => {
    await withOnboardEnv(
      "fased-onboard-custom-provider-profile-fallback-",
      async ({ configPath, runtime }) => {
        upsertAuthProfile({
  ...
  it("fails custom provider auth when compatibility is invalid", async () => {
    await withOnboardEnv("fased-onboard-custom-provider-invalid-compat-", async ({ runtime }) => {
      await expect(
        runNonInteractiveOnboardingWithDefaults(runtime, {
          authChoice: "custom-api-key",
          customBaseUrl: "https://models.custom.local/v1",
  ...
  it("fails custom provider auth when explicit provider id is invalid", async () => {
    await withOnboardEnv("fased-onboard-custom-provider-invalid-id-", async ({ runtime }) => {
      await expect(
        runNonInteractiveOnboardingWithDefaults(runtime, {
          authChoice: "custom-api-key",
          customBaseUrl: "https://models.custom.local/v1",
  ...
  it("fails inferred custom auth when required flags are incomplete", async () => {
    await withOnboardEnv("fased-onboard-custom-provider-missing-required-", async ({ runtime }) => {
      await expect(
        runNonInteractiveOnboardingWithDefaults(runtime, {
          customApiKey: "custom-test-key",
          skipSkills: true,
