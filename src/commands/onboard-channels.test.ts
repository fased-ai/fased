...
describe("setupChannels", () => {
  ...
  it("QuickStart uses single-select (no multiselect) and doesn't prompt for Telegram token when WhatsApp is chosen", async () => {
    ...
    await setupChannels({} as FasedAgentConfig, runtime, prompter, {
      skipConfirm: true,
  ...
  it("uses active plugin metadata in the channel primer before static core metadata", async () => {
    const telegram = createChannelTestPluginBase({
      id: "telegram",
      label: "Telegram Current",
    });
    const plugin = {
      ...telegram,
      meta: {
        ...telegram.meta,
        blurb: "current catalog blurb.",
      },
    };
    ...
    const primer = note.mock.calls.find(([, title]) => title === "How channels work")?.[0] ?? "";
    expect(String(primer)).toContain("Telegram Current: current catalog blurb.");
    expect(String(primer)).not.toContain("Telegram: quickest setup path");
  });

  it("does not duplicate core channels that also have extension catalog entries", async () => {
    const note = vi.fn(async (_message?: string, _title?: string) => {});
    const select = vi.fn(async () => "__done__");
    const { multiselect, text } = createUnexpectedPromptGuards();
    ...
    expect(String(status).match(/^Google Chat:/gm) ?? []).toHaveLength(1);
    expect(String(status)).not.toContain("Google Chat: install plugin to enable");
  });

  it("shows catalog source in channel status and picker hints", async () => {
    const note = vi.fn(async (_message?: string, _title?: string) => {});
    ...
    const options = (quickstartCall?.[0].options ?? []) as Array<{ value: string; hint?: string }>;
  ...
  it("uses configureInteractive skip without mutating selection/account state", async () => {
    ...
    const runtime = createExitThrowingRuntime();
    try {
      const cfg = await setupChannels({} as FasedAgentConfig, runtime, prompter, {
        skipConfirm: true,
        quickstartDefaults: true,
  ...
  it("applies configureInteractive result cfg/account updates", async () => {
    ...
    const selection = vi.fn();
    const onAccountId = vi.fn();
    const configureInteractive = vi.fn(async ({ cfg }: { cfg: FasedAgentConfig }) => ({
      cfg: {
        ...cfg,
        channels: {
          ...cfg.channels,
  ...
  it("uses configureWhenConfigured when channel is already configured", async () => {
    ...
    const selection = vi.fn();
    const onAccountId = vi.fn();
    const configureWhenConfigured = vi.fn(async ({ cfg }: { cfg: FasedAgentConfig }) => ({
      cfg: {
        ...cfg,
        channels: {
          ...cfg.channels,
