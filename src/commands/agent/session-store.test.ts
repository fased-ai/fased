...
describe("updateSessionStoreAfterAgentRun", () => {
  it("preserves ACP metadata when caller has a stale session snapshot", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "fased-session-store-"));
  ...
  it("persists compaction tokensAfter when provider usage is unavailable", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "fased-session-store-"));
    const storePath = path.join(dir, "sessions.json");
    const sessionKey = `agent:codex:explicit:${randomUUID()}`;
    const sessionId = randomUUID();
    ...
    const persisted = loadSessionStore(storePath, { skipCache: true })[sessionKey];
    expect(persisted?.totalTokens).toBe(21_225);
  ...
  it("ignores non-finite compaction tokensAfter values", async () => {
    ...
    const persisted = loadSessionStore(storePath, { skipCache: true })[sessionKey];
    expect(persisted?.totalTokens).toBe(12_000);
    expect(persisted?.totalTokensFresh).toBe(true);
  });
});
