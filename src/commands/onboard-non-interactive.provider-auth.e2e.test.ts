...
function formatRuntimeErrorArg(value: unknown): string {
...
function captureEnv(): EnvSnapshot {
  return {
    ...
    skipChannels: process.env.FASED_SKIP_CHANNELS,
    skipGmail: process.env.FASED_SKIP_GMAIL_WATCHER,
    skipCron: process.env.FASED_SKIP_CRON,
    skipCanvas: process.env.FASED_SKIP_CANVAS_HOST,
    token: process.env.FASED_GATEWAY_TOKEN,
    password: process.env.FASED_GATEWAY_PASSWORD,
    customApiKey: process.env.CUSTOM_API_KEY,
    disableConfigCache: process.env.FASED_DISABLE_CONFIG_CACHE,
  };
}
...
function restoreEnv(prev: EnvSnapshot): void {
  ...
  restoreEnvVar("FASED_STATE_DIR", prev.stateDir);
  restoreEnvVar("FASED_CONFIG_PATH", prev.configPath);
  restoreEnvVar("FASED_SKIP_CHANNELS", prev.skipChannels);
  restoreEnvVar("FASED_SKIP_GMAIL_WATCHER", prev.skipGmail);
  restoreEnvVar("FASED_SKIP_CRON", prev.skipCron);
  restoreEnvVar("FASED_SKIP_CANVAS_HOST", prev.skipCanvas);
  restoreEnvVar("FASED_GATEWAY_TOKEN", prev.token);
  restoreEnvVar("FASED_GATEWAY_PASSWORD", prev.password);
  restoreEnvVar("CUSTOM_API_KEY", prev.customApiKey);
  restoreEnvVar("FASED_DISABLE_CONFIG_CACHE", prev.disableConfigCache);
}
...
async function withOnboardEnv(
  ...
  process.env.FASED_STATE_DIR = tempHome;
  process.env.FASED_CONFIG_PATH = configPath;

  const runtime: RuntimeMock = {
    log: () => {},
    error: (...args: unknown[]) => {
      throw new Error(formatRuntimeErrorArg(args[0]));
    },
    exit: (code: number) => {
...
describe("onboard (non-interactive): provider auth", () => {
  it("stores MiniMax API key and uses global baseUrl by default", async () => {
    await withOnboardEnv("fased-onboard-minimax-", async ({ configPath, runtime }) => {
  ...
  it("supports MiniMax CN API endpoint auth choice", async () => {
    await withOnboardEnv("fased-onboard-minimax-cn-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores Z.AI API key and uses global baseUrl by default", async () => {
    await withOnboardEnv("fased-onboard-zai-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("supports Z.AI CN coding endpoint auth choice", async () => {
    await withOnboardEnv("fased-onboard-zai-cn-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores xAI API key and sets default model", async () => {
    await withOnboardEnv("fased-onboard-xai-", async ({ configPath, runtime }) => {
  ...
  it("stores Vercel AI Gateway API key and sets default model", async () => {
    await withOnboardEnv("fased-onboard-ai-gateway-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores OpenAI API key and sets OpenAI default model", async () => {
    await withOnboardEnv("fased-onboard-openai-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores LiteLLM API key and sets default model", async () => {
    await withOnboardEnv("fased-onboard-litellm-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("infers Together auth choice from --together-api-key and sets default model", async () => {
    await withOnboardEnv("fased-onboard-together-infer-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("infers QIANFAN auth choice from --qianfan-api-key and sets default model", async () => {
    await withOnboardEnv("fased-onboard-qianfan-infer-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
