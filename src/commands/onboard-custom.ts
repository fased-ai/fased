...
export type CustomApiCompatibilityChoice = CustomApiCompatibility | "unknown";
...
export type CustomApiErrorCode =
  ...
  | "invalid_base_url"
  | "private_network_requires_opt_in"
  | "invalid_model_id"
  | "invalid_provider_id"
  | "invalid_alias";
...
export async function detectCustomApiCompatibility(params: {
  baseUrl: string;
  apiKey?: string;
  modelId: string;
}): Promise<CustomApiCompatibility | null> {
  ...
  return anthropicProbe.ok ? "anthropic" : null;
}
...
async function promptBaseUrlAndKey(params: {
  ...
  if (isPrivateNetworkBaseUrl(baseUrl)) {
    allowPrivateNetwork = await params.prompter.confirm({
      message:
        "This model provider URL is local/private. Allow model requests to reach private-network addresses?",
      initialValue: true,
    });
    if (!allowPrivateNetwork) {
      throw new CustomApiError(
        "private_network_requires_opt_in",
        "Private model provider URLs require explicit private-network access confirmation.",
      );
    }
  }
  ...
  const resolvedApiKey = await ensureApiKeyFromEnvOrPrompt({
    ...
    setCredential: async (apiKey: SecretInput) => {
      apiKeyInput = apiKey;
    },
  });
  return {
...
async function applyCustomApiRetryChoice(params: {
  ...
  let { baseUrl, apiKey, resolvedApiKey, modelId, allowPrivateNetwork } = params.current;
  if (params.retryChoice === "baseUrl" || params.retryChoice === "both") {
    ...
    allowPrivateNetwork = retryInput.allowPrivateNetwork;
  }
  if (params.retryChoice === "model" || params.retryChoice === "both") {
    modelId = await promptCustomApiModelId(params.prompter);
  }
  return { baseUrl, apiKey, resolvedApiKey, modelId, allowPrivateNetwork };
}
...
export function applyCustomApiConfig(params: ApplyCustomApiConfigParams): CustomApiResult {
  ...
  if (isPrivateNetworkBaseUrl(baseUrl) && !allowPrivateNetwork) {
    throw new CustomApiError(
      "private_network_requires_opt_in",
      [
        "Custom provider base URL is private or local.",
        "Use --allow-private-network in non-interactive mode, or confirm private-network access in the wizard.",
      ].join(" "),
    );
  }
...
export async function promptCustomApiConfig(params: {
  ...
  let allowPrivateNetwork = baseInput.allowPrivateNetwork;
    ...
    if (!compatibility) {
      ...
      if (openaiProbe.ok) {
        ...
        if (anthropicProbe.ok) {
          ...
          ({ baseUrl, apiKey, resolvedApiKey, modelId, allowPrivateNetwork } =
            await applyCustomApiRetryChoice({
              prompter,
              config,
              secretInputMode: params.secretInputMode,
              retryChoice,
              current: { baseUrl, apiKey, resolvedApiKey, modelId, allowPrivateNetwork },
            }));
          continue;
        }
      }
