 ...
 * Runs as part of `fased doctor` — config-only, no network calls.
 */
  ...
  if (resolved.provider !== "auto") {
    if (resolved.provider === "local") {
      ...
      note(
        [
          ...
          `- Switch to a remote provider: ${formatCliCommand("fased config set agents.defaults.memorySearch.provider openai")}`,
          "",
          `Verify: ${formatCliCommand("fased memory status --deep")}`,
        ].join("\n"),
        "Memory search",
      );
      return;
    ...
    note(
      [
        ...
        `- Configure credentials: ${formatCliCommand("fased configure --section model")}`,
        `- To disable: ${formatCliCommand("fased config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("fased memory status --deep")}`,
      ].join("\n"),
      "Memory search",
