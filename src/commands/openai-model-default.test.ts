...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: FasedAgentConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: FasedAgentConfig) => { changed: boolean; next: FasedAgentConfig };
  defaultModel: string;
  overrideConfig: FasedAgentConfig;
  alreadyDefaultConfig: FasedAgentConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    const defaultModel = "vercel-ai-gateway/openai/gpt-5.5";
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: FasedAgentConfig) => config,
      applyDefaultConfig: (config: FasedAgentConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: FasedAgentConfig = { agents: { defaults: {} } };
...
describe("applyOpenAICodexModelDefault", () => {
  ...
  it("keeps OpenAI sign-in models on the openai-codex compatibility route", () => {
    const cfg: FasedAgentConfig = {
      agents: {
        defaults: {
          model: {
    ...
    expect(applied.next.agents?.defaults).not.toHaveProperty("agentRuntime");
  });
