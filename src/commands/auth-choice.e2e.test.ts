...
describe("applyAuthChoice", () => {
  ...
  it("does not override the default model when selecting opencode-zen without setDefaultModel", async () => {
    ...
    expect(result.agentModelOverride).toBe("opencode/gpt-5.5");
  });
  ...
  it("writes Chutes OAuth credentials when selecting chutes (remote/manual)", async () => {
    ...
    const text: WizardPrompter["text"] = vi.fn(async (params) => {
      if (params.message === "Paste the redirect URL") {
        const lastLog = (runtime.log as ReturnType<typeof vi.fn>).mock.calls.at(-1)?.[0];
  ...
  it("rejects discontinued Qwen portal OAuth", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "fased-auth-"));
    process.env.FASED_STATE_DIR = tempStateDir;
    process.env.FASED_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.FASED_AGENT_DIR;
