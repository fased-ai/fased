...
async function runGatewayHealthCheck(params: {
  ...
  const token = params.cfg.gateway?.auth?.token ?? process.env.FASED_GATEWAY_TOKEN;
  const password = params.cfg.gateway?.auth?.password ?? process.env.FASED_GATEWAY_PASSWORD;
    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from fased.json",
        },
      ],
...
async function promptWebToolsConfig(
  ...
  note(
    [
      ...
      "Choose a managed provider, then store its API key or use an environment variable.",
  ...
  const enableSearch = guardCancel(
    await confirm({
      ...
      initialValue: existingSearch?.enabled ?? Boolean(existingSearch?.provider),
    }),
    runtime,
  );

  let workingConfig: FasedAgentConfig = {
    ...nextConfig,
    tools: {
      ...nextConfig.tools,
      web: {
  ...
  if (enableSearch) {
    const { setupSearch } = await import("./onboard-search.js");
  ...
  return {
    ...
    tools: {
      ...
      web: {
        ...
        search: workingConfig.tools?.web?.search,
        fetch: nextFetch,
      },
    },
  };
...
export async function runConfigureWizard(
  ...
  try {
    ...
    intro(opts.command === "update" ? "Update" : "Configure");
    const prompter = createClackPrompter();

    const snapshot = await readConfigFileSnapshot();
    const baseConfig: FasedAgentConfig = snapshot.valid ? snapshot.config : {};
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("fased doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    let gatewayToken: string | undefined =
      ...
      process.env.FASED_GATEWAY_TOKEN;
    ...
    const newPassword = nextConfig.gateway?.auth?.password ?? process.env.FASED_GATEWAY_PASSWORD;
    const oldPassword = baseConfig.gateway?.auth?.password ?? process.env.FASED_GATEWAY_PASSWORD;
    const token = nextConfig.gateway?.auth?.token ?? process.env.FASED_GATEWAY_TOKEN;
    ...
    note(
      [
        ...
        gatewayStatusLine,
        "Docs: https://docs.fased.ai/web/control-ui",
      ].join("\n"),
      "Control UI",
    );
