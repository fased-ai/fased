...
export async function noteSecurityWarnings(cfg: FasedAgentConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("fased security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("fased configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("fased config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("fased doctor --fix")} to generate a token`,
              `  Or set token directly: ${formatCliCommand(
                "fased config set gateway.auth.mode token",
              )}`,
            ];
      warnings.push(
        `- CRITICAL: Gateway bound to ${bindDescriptor} without authentication.`,
        `  Anyone on your network (or internet if port-forwarded) can fully control your agent.`,
        `  Fix: ${formatCliCommand("fased config set gateway.bind loopback")}`,
        ...authFixLines,
        `- Safer remote access: keep the gateway loopback-only and forward it over SSH:`,
        `  ${formatCliCommand(
          `ssh -N -L ${resolveGatewayPort(cfg)}:127.0.0.1:${resolveGatewayPort(cfg)} <user>@<host>`,
        )}`,
      );
    } else {
  ...
  if (cfg.approvals?.exec && cfg.approvals.exec.enabled === false) {
    warnings.push(
      "- approvals.exec.enabled=false disables approval forwarding only; host-local exec approval policy still lives in exec-approvals.json.",
      `  Inspect or update it with: ${formatCliCommand("fased approvals get --gateway")}`,
    );
  }
