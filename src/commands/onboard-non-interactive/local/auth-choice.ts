...
export async function applyNonInteractiveAuthChoice(params: {
  ...
  if (authChoice === "qwen-coding-plan-api-key") {
    const resolved = await resolveApiKey({
      provider: "qwen-coding-plan",
      cfg: baseConfig,
      flagValue: opts.qwenCodingPlanApiKey,
    ...
    if (
      !(await maybeSetResolvedApiKey(resolved, (value) =>
        setQwenCodingPlanApiKey(value, undefined, apiKeyStorageOptions),
      ))
    ) {
      return null;
    }
    ...
    return applyQwenCodingPlanConfig(nextConfig);
  }
  ...
  if (authChoice === "qwen-api-key") {
    ...
    if (
      !(await maybeSetResolvedApiKey(resolved, (value) =>
        setQwenApiKey(value, undefined, apiKeyStorageOptions),
      ))
    ) {
      return null;
    }
    ...
    return applyQwenConfig(nextConfig);
  }
  ...
  const applyMoonshotApiKeyChoice = async (
    applyConfig: (cfg: FasedAgentConfig) => FasedAgentConfig,
  ): Promise<FasedAgentConfig | null> => {
    const resolved = await resolveApiKey({
      provider: "moonshot",
      cfg: baseConfig,
  ...
  if (authChoice === "custom-api-key") {
    try {
      ...
      const result = applyCustomApiConfig({
        ...
        providerId: customAuth.providerId,
        allowPrivateNetwork: customAuth.allowPrivateNetwork,
      });
  ...
  const pluginProviderConfig = await applyNonInteractivePluginProviderApiKeyChoice({
    authChoice,
    opts,
    runtime,
    baseConfig,
  ...
  if (pluginProviderConfig !== undefined) {
    return pluginProviderConfig;
  }
