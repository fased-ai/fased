...
describe("agents helpers", () => {
  ...
  it("applyAgentConfig updates task model roles", () => {
    const cfg: FasedAgentConfig = {
      agents: {
        list: [{ id: "work", taskModels: { cheapCheck: "openrouter/old" } }],
      },
    };
    ...
    expect(next.agents?.list?.find((agent) => agent.id === "work")?.taskModels).toEqual({
      cheapCheck: "openrouter/cheap",
      escalation: "openrouter/strong",
    });
  });
  ...
  it("applyAgentConfig still normalizes legacy provider-scoped model settings", () => {
    const cfg: FasedAgentConfig = {
      agents: {
        list: [{ id: "work", activeModelProvider: "openai" }],
      },
    };
    ...
    const work = next.agents?.list?.find((agent) => agent.id === "work");
    expect(work?.activeModelProvider).toBe("openrouter");
    expect(work?.modelProviders).toEqual({
      openrouter: {
        profileId: "openrouter:work",
        primary: "openrouter/qwen/qwen3.6-flash",
        fallbacks: ["openrouter/z-ai/glm-5.1"],
  ...
  it("applyAgentConfig clears legacy provider-scoped model settings", () => {
    const cfg: FasedAgentConfig = {
      agents: {
        list: [
          {
    ...
    const work = next.agents?.list?.find((agent) => agent.id === "work");
    expect(work?.activeModelProvider).toBeUndefined();
    expect(work?.modelProviders).toBeUndefined();
  });
