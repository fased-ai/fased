...
vi.mock("../plugins/loader.js", () => ({
  loadFasedAgentPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
beforeEach(() => {
  ...
  setStdinTty(true);
  originalStateDir = process.env.FASED_STATE_DIR;
  originalUpdateInProgress = process.env.FASED_UPDATE_IN_PROGRESS;
  process.env.FASED_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "fased-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalUpdateInProgress === undefined) {
    ...
    process.env.FASED_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
