...
describe("formatTuiHeaderConnectionUrl", () => {
  it("redacts credentials and sensitive query params from gateway URLs", () => {
    expect(
      formatTuiHeaderConnectionUrl("wss://user:pass@example.com/ws?token=secret&room=main"),
    ).toBe("wss://***:***@example.com/ws?token=***&room=main");
  });
});
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run fased devices list");
    expect(state.pairingHint).toContain("fased devices list");
  });
