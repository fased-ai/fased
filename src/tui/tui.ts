...
export function formatTuiHeaderConnectionUrl(url: string): string {
...
export function resolveGatewayDisconnectState(reason?: string): {
  ...
  if (/pairing required/i.test(reasonLabel)) {
    return {
      ...
      activityStatus: "pairing required: run fased devices list",
      pairingHint:
        "Pairing required. Run `fased devices list`, approve your request ID, then reconnect.",
    };
  }
  return {
    connectionStatus: `gateway disconnected: ${reasonLabel}`,
...
export async function runTui(opts: TuiOptions) {
  ...
  const updateHeader = () => {
    ...
    header.setText(
      theme.header(
        `fased tui - ${formatTuiHeaderConnectionUrl(client.connection.url)} - agent ${agentLabel} - session ${sessionLabel}`,
      ),
    );
  };
