...
describe("resolveGatewayConnection", () => {
  ...
  it("uses FASED_GATEWAY_TOKEN for local mode", () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });

    withEnv({ FASED_GATEWAY_TOKEN: "env-token" }, () => {
      const result = resolveGatewayConnection({});
  ...
  it("prefers FASED_GATEWAY_PASSWORD over remote password fallback", () => {
    ...
    withEnv({ FASED_GATEWAY_PASSWORD: "env-pass" }, () => {
      const result = resolveGatewayConnection({});
