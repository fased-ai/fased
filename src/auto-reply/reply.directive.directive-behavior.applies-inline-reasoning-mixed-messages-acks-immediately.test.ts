...
describe("directive behavior", () => {
  ...
  it("covers think status and /thinking xhigh support matrix", async () => {
    await withTempHome(async (home) => {
      ...
      for (const model of ["openai-codex/gpt-5.3-codex", "openai/gpt-5.2"]) {
      ...
      expect(unsupportedModelTexts).toContain(
        'Thinking level "xhigh" is only supported for openai/gpt-5.5, openai/gpt-5.5-pro, openai/gpt-5.4, openai/gpt-5.4-pro, openai/gpt-5.4-mini, openai/gpt-5.4-nano, openai/gpt-5.2, openai-codex/gpt-5.5, openai-codex/gpt-5.4, openai-codex/gpt-5.3-codex, github-copilot/gpt-5.2-codex or github-copilot/gpt-5.2.',
      );
      expect(runEmbeddedPiAgent).not.toHaveBeenCalled();
    });
  });
  ...
  it("treats skill commands as reserved for model aliases", async () => {
    await withTempHome(async (home) => {
      const workspace = path.join(home, "fased");
      await writeSkill({
