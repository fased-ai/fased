...
type AgentDefaults = NonNullable<NonNullable<FasedAgentConfig["agents"]>["defaults"]>;
...
type NormalizedAuthMode = "api-key" | "oauth" | "token" | "mixed" | "unknown";
...
const readUsageFromSessionLog = (
  ...
  | undefined => {
  // Transcripts are stored at the session file path (fallback: ~/.fased/sessions/<SessionId>.jsonl)
  if (!sessionId) {
    return undefined;
  }
  let logPath: string;
...
export function buildStatusMessage(args: StatusArgs): string {
  ...
  const configuredFallbacks = (() => {
    ...
    if (modelConfig && typeof modelConfig === "object" && Array.isArray(modelConfig.fallbacks)) {
  ...
  const versionLine = `FasedAgent ${VERSION}${commit ? ` (${commit})` : ""}`;
...
export function buildHelpMessage(cfg?: FasedAgentConfig): string {
  const lines = ["ℹ️ Help", ""];
