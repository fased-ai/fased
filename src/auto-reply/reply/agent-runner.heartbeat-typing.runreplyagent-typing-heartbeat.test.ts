...
function requirePayload(payload: { text?: string } | undefined): { text?: string } {
  if (!payload) {
    throw new Error("Expected runReplyAgent to return a payload");
  }
  return payload;
}
...
type StateEnvSnapshot = {
  FASED_STATE_DIR: string | undefined;
};

function snapshotStateEnv(): StateEnvSnapshot {
  return { FASED_STATE_DIR: process.env.FASED_STATE_DIR };
}

function restoreStateEnv(snapshot: StateEnvSnapshot) {
  if (snapshot.FASED_STATE_DIR === undefined) {
    delete process.env.FASED_STATE_DIR;
  } else {
    process.env.FASED_STATE_DIR = snapshot.FASED_STATE_DIR;
  }
}
...
describe("runReplyAgent typing (heartbeat)", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(tmpdir(), "fased-typing-heartbeat-"));
  });
  ...
  it("retries after compaction failure by resetting the session", async () => {
    await withTempStateDir(async (stateDir) => {
      ...
      const payload = requirePayload(Array.isArray(res) ? res[0] : res);
