...
export function resolveConfiguredAcpBackendId(cfg: FasedAgentConfig): string {
  return cfg.acp?.backend?.trim() || "acpx";
}

export function resolveAcpInstallCommandHint(cfg: FasedAgentConfig): string {
  ...
  if (backendId === "acpx") {
    ...
    if (existsSync(localPath)) {
      return `fased plugins install ${localPath}`;
    }
    return "fased plugins install @fased/acpx";
  }
