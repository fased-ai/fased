...
export async function prependSystemEvents(params: {
  ...
  const resolveSystemEventTimezone = (cfg: FasedAgentConfig) => {
  ...
  const formatSystemEventTimestamp = (ts: number, cfg: FasedAgentConfig) => {
  ...
  systemLines.push(
    ...queued
      .map((event) => {
        ...
        return `${prefix}: [${formatSystemEventTimestamp(event.ts, params.cfg)}] ${compacted}`;
      })
...
export async function ensureSkillSnapshot(params: {
  ...
  if (process.env.FASED_TEST_FAST === "1") {
