...
describe("matchesMentionWithExplicit", () => {
  const mentionRegexes = [/\bfased\b/i];
  ...
  it("resolves known variables, aliases, and case-insensitive tokens", () => {
    const cases = [
      ...
      },
      {
        ...
        values: { identityName: "FasedAgent" },
        expected: "[FasedAgent]",
      },
      {
      ...
      },
      {
        ...
        values: {
          identityName: "FasedAgent",
          provider: "anthropic",
          model: "claude-opus-4-5",
          thinkingLevel: "high",
        },
        expected: "[FasedAgent] anthropic/claude-opus-4-5 (think:high)",
      },
    ] as const;
    for (const testCase of cases) {
...
describe("createReplyReferencePlanner", () => {
  it("plans references correctly for off/first/all modes", () => {
    ...
    expect(offPlanner.peek()).toBeUndefined();
    ...
    expect(existingIdPlanner.peek()).toBe("thread-1");
