...
function resolveActiveNamedSessionKey(params: {
  ...
  const normalizedCommand = params.commandBody.trim().toLowerCase();
  // /session commands must operate on the route/base entry so list/new/switch can
  // update the active child pointer. Other commands and normal chat turns run in
  // the active named session when one is selected.
  if (/^\/session(?:\s|$)/.test(normalizedCommand)) {
    return baseSessionKey;
  }
  const activeSessionKey = params.store[baseSessionKey]?.activeSessionKey?.trim().toLowerCase();
  if (!activeSessionKey || activeSessionKey === baseSessionKey) {
    return baseSessionKey;
  }
  return params.store[activeSessionKey] ? activeSessionKey : baseSessionKey;
}
...
function resolveParentForkMaxTokens(cfg: FasedAgentConfig): number {
...
export async function initSessionState(params: {
  ...
  let persistedModelOverrideSource: "auto" | "user" | undefined;
  ...
  const baseSessionKey = resolveSessionKey(sessionScope, sessionCtxForState, mainKey);
  sessionKey = resolveActiveNamedSessionKey({
    baseSessionKey,
    store: sessionStore,
  ...
  sessionEntry = {
    ...
    baseSessionKey: sessionKey !== baseSessionKey ? baseSessionKey : baseEntry?.baseSessionKey,
