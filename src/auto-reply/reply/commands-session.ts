...
import { handleNamedSessionControlCommand } from "./commands-control.js";
...
function resolveSessionCommandUsage() {
  return "Usage: /session list | /session new <name> | /session switch <name|main> | /session current | /session idle <duration|off> | /session max-age <duration|off>";
}
...
export const handleSessionCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  const namedSessionResult = await handleNamedSessionControlCommand(params);
  if (namedSessionResult) {
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting FasedAgent in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerFasedAgentRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting FasedAgent via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
