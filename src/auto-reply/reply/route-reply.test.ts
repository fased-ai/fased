...
describe("routeReply", () => {
  ...
  it("passes replyToId to WhatsApp sends for visible replies", async () => {
    mocks.sendMessageWhatsApp.mockClear();
    await routeReply({
      payload: { text: "hi", replyToId: "wa-msg-1" },
      channel: "whatsapp",
      to: "+15551234567",
      cfg: {} as never,
    });
    expect(mocks.sendMessageWhatsApp).toHaveBeenCalledWith(
      "+15551234567",
      "hi",
      expect.objectContaining({ replyToId: "wa-msg-1", verbose: false }),
    );
  });

  it("routes WhatsApp group visible replies to the group JID", async () => {
    mocks.sendMessageWhatsApp.mockClear();
    await routeReply({
      payload: { text: "hi group", replyToId: "group-msg-1" },
      channel: "whatsapp",
