...
const mocks = vi.hoisted(() => ({
  routeReply: vi.fn(
    async (_params: unknown): Promise<{ ok: boolean; messageId?: string; error?: string }> => ({
      ok: true,
      messageId: "mock",
    }),
  ),
...
const ttsMocks = vi.hoisted(() => {
  ...
  return {
    ...
    resolveTtsConfig: vi.fn((_cfg: FasedAgentConfig) => ({ mode: "final" })),
  };
});
...
const emptyConfig = {} as FasedAgentConfig;
...
async function listTasksViaGateway(params: Record<string, unknown>): Promise<TaskListResult> {
  ...
  await tasksHandlers["tasks.list"]({
    ...
    respond,
    context: {
      cronStorePath: "dispatch-from-config-cron-runs.json",
      cron: {},
    },
...
describe("dispatchReplyFromConfig", () => {
  ...
  it("mirrors failed origin route delivery into the channel task ledger", async () => {
    setNoAbort();
    mocks.routeReply.mockReset();
    mocks.routeReply.mockResolvedValueOnce({
      ok: false,
      error: "telegram send failed",
    } as Awaited<ReturnType<typeof mocks.routeReply>>);
    createRunningTaskRun({
      runtime: "channel",
      runId: "channel-route-fail",
      sourceId: "channel:telegram:msg-route-fail",
      ownerKey: "agent:main:slack:direct:123",
      requesterSessionKey: "agent:main:slack:direct:123",
      sessionKey: "agent:main:slack:direct:123",
      agentId: "main",
      channel: "telegram",
      taskKind: "channel-triggered-agent",
      task: "reply to routed channel message",
    ...
    const task = listTaskRecords({ source: "channel" }).tasks[0];
    expect(task).toMatchObject({
      status: "succeeded",
      deliveryStatus: "not_delivered",
      terminalSummary: "Channel reply delivery failed.",
      delivery: {
        channel: "telegram",
        target: "telegram:999",
        error: "final route: telegram send failed",
      },
      metadata: {
        dispatchRoute: "origin",
        dispatchErrors: ["final route: telegram send failed"],
        replyCounts: { final: 0, block: 0, tool: 0 },
      },
    });
    ...
    expect(ledger.summary.bySource.channel).toBe(1);
  ...
  it("emits diagnostics when enabled", async () => {
    setNoAbort();
    const cfg = { diagnostics: { enabled: true } } as FasedAgentConfig;
    const dispatcher = createDispatcher();
    const ctx = buildTestCtx({
      Provider: "slack",
      Surface: "slack",
  ...
  it("gates source progress callbacks when source delivery is message-tool-only", async () => {
    setNoAbort();
