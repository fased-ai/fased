...
function resolveLedgerChannel(ctx: TemplateContext): string | undefined {
...
function compactLedgerTaskText(value: string): string {
...
function loadedSkillNames(params: FollowupRun): string[] | undefined {
  const names = params.run.skillsSnapshot?.skills
...
export async function runReplyAgent(params: {
  ...
  const channelLedgerChannel = isHeartbeat ? undefined : resolveLedgerChannel(sessionCtx);
  const channelLedgerTarget = resolveOriginMessageTo({
    originatingTo: sessionCtx.OriginatingTo,
    to: sessionCtx.To,
  });
  const buildChannelLedgerDelivery = (): TaskRecord["delivery"] | undefined => {
    if (!channelLedgerChannel && !channelLedgerTarget) {
      return undefined;
    }
    return {
      ...(channelLedgerChannel ? { channel: channelLedgerChannel } : {}),
      ...(channelLedgerTarget ? { target: channelLedgerTarget } : {}),
    };
  };
  let channelLedgerRunId: string | undefined;
  const completeChannelLedgerRun = (
    summary: string,
    opts?: { deliveryStatus?: TaskDeliveryStatus; delivery?: TaskRecord["delivery"] },
  ) => {
    if (!channelLedgerRunId) {
      return;
    }
    completeTaskRunByRunId({
      runId: channelLedgerRunId,
      summary,
      deliveryStatus: opts?.deliveryStatus ?? "not_applicable",
      delivery: opts?.delivery,
    });
    channelLedgerRunId = undefined;
  };
  const channelLedgerOpts: GetReplyOptions | undefined = channelLedgerChannel
    ? {
        ...opts,
        onAgentRunStart: (runId) => {
          opts?.onAgentRunStart?.(runId);
          channelLedgerRunId = runId;
          const messageId = String(
            sessionCtx.MessageSidFull ?? sessionCtx.MessageSid ?? followupRun.messageId ?? runId,
          ).trim();
          createRunningTaskRun({
            runtime: "channel",
            sourceId: `channel:${channelLedgerChannel}:${messageId || runId}`,
            ownerKey: sessionKey,
            requesterSessionKey: sessionKey,
            sessionKey,
            agentId: followupRun.run.agentId,
            channel: channelLedgerChannel,
            runId,
            label: `Channel task from ${channelLedgerChannel}`,
            task: compactLedgerTaskText(commandBody || followupRun.prompt),
            deliveryStatus: "pending",
            delivery: buildChannelLedgerDelivery(),
            startedAt: Date.now(),
            taskKind: "channel-triggered-agent",
            model: followupRun.run.model,
            provider: followupRun.run.provider,
            loadedSkills: loadedSkillNames(followupRun),
            scopeKind: "channel",
            metadata: {
  ...
  try {
    ...
    if (runOutcome.kind === "final") {
      completeChannelLedgerRun("Channel task ended before model reply.");
    ...
    if (payloadArray.length === 0) {
      completeChannelLedgerRun("Channel task completed with no source reply.");
    ...
    if (replyPayloads.length === 0) {
      completeChannelLedgerRun("Channel task completed with no deliverable reply.");
      return finalizeWithFollowup(undefined, queueKey, runFollowupTurn);
    }
    ...
    if (autoCompactionCompleted) {
      ...
      if (sessionKey) {
        ...
        try {
          const contextContent = await readPostCompactionContext(workspaceDir);
          if (contextContent) {
    ...
    completeChannelLedgerRun(`Prepared ${finalPayloads.length} channel reply payload(s).`, {
      deliveryStatus: "pending",
      delivery: buildChannelLedgerDelivery(),
    });
    ...
    if (channelLedgerRunId) {
      failTaskRunByRunId({
        runId: channelLedgerRunId,
        status: "failed",
        summary: "Channel task ended before ledger completion.",
