...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fased-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop fased!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@fased_bot", { botUsername: "fased_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@fased_bot", { botUsername: "fased_bot" })).toBe(true);
