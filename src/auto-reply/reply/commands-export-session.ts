...
function writeNewDefaultExportFile(filePath: string, html: string): string {
  for (let suffix = 1; suffix <= 100; suffix++) {
    const candidate = suffix === 1 ? filePath : addCollisionSuffix(filePath, suffix);
    try {
      fs.writeFileSync(candidate, html, { encoding: "utf-8", flag: "wx" });
      return candidate;
    } catch (error) {
...
export async function buildExportSessionReply(params: HandleCommandsParams): Promise<ReplyPayload> {
  ...
  if (args.outputPath) {
    ...
    outputPath = writeNewDefaultExportFile(outputPath, html);
  }
