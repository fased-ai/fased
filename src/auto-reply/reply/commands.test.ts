...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "fased-commands-"));
...
describe("/compact command", () => {
  ...
  it("routes manual compaction with explicit trigger and context metadata", async () => {
    const cfg = {
      commands: { text: true },
      channels: { whatsapp: { allowFrom: ["*"] } },
      session: { store: "/tmp/fased-session-store.json" },
    } as FasedAgentConfig;
    const params = buildParams("/compact: focus on decisions", cfg, {
      From: "+15550001",
      To: "+15550002",
    });
    const agentDir = "/tmp/fased-agent-compact";
...
describe("handleCommands /status model fallbacks", () => {
  it("shows per-agent fallback overrides in status output", async () => {
    const cfg = {
      commands: { text: true },
    ...
    const params = buildParams("/status", cfg);
    params.sessionKey = "agent:kira:main";
    ...
    expect(result.shouldContinue).toBe(false);
    expect(result.reply?.text).toContain("Fallbacks: google/gemini-2.5-flash");
    expect(result.reply?.text).not.toContain("Fallbacks: anthropic/claude-sonnet-4-6");
  });

  it("keeps default fallback config when agent has no fallback override", async () => {
    const cfg = {
      commands: { text: true },
      session: { mainKey: "main", scope: "per-sender" },
      agents: {
    ...
    expect(result.shouldContinue).toBe(false);
    expect(result.reply?.text).toContain("Fallbacks: anthropic/claude-sonnet-4-6");
  });

  it("treats empty per-agent fallback override as disabling inherited fallbacks", async () => {
    const cfg = {
      commands: { text: true },
      session: { mainKey: "main", scope: "per-sender" },
      agents: {
    ...
    expect(result.shouldContinue).toBe(false);
    expect(result.reply?.text).not.toContain("Fallbacks:");
  });
});
...
describe("handleCommands /allowlist", () => {
  ...
  it("removes DM allowlist entries from canonical allowFrom and deletes legacy dm.allowFrom", async () => {
    ...
    for (const testCase of cases) {
      ...
      const written = writeConfigFileMock.mock.calls.at(-1)?.[0] as FasedAgentConfig;
