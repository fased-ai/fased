...
describe("runReplyAgent heartbeat followup guard", () => {
  ...
  it("records channel-triggered agent runs in the task ledger", async () => {
    await withStateDirEnv("fased-channel-ledger-", async () => {
      ...
      expect(task).toMatchObject({
        ...
        status: "succeeded",
        channel: "telegram",
        sourceId: "channel:telegram:msg-42",
...
describe("runReplyAgent typing (heartbeat)", () => {
  ...
  it("queues post-compaction context refresh before scheduling the retry followup", async () => {
    await withTempStateDir(async (stateDir) => {
      await fs.writeFile(
      ...
      const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(stateDir);
      const storePath = path.join(stateDir, "sessions", "sessions.json");
      const sessionEntry: SessionEntry = { sessionId: "session", updatedAt: Date.now() };
      const sessionStore = { main: sessionEntry };
      const observedOrder: string[] = [];

      vi.mocked(scheduleFollowupDrain).mockImplementationOnce(() => {
        const queuedEvents = peekSystemEvents("main");
        observedOrder.push(
          queuedEvents.some((event) => event.includes("[Post-compaction context refresh]"))
      ...
      expect(observedOrder).toEqual(["post-compaction-context-refresh"]);
      expect(peekSystemEvents("main").join("\n")).toContain("memory/today.md");
    });
  });
...
describe("runReplyAgent memory flush", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(tmpdir(), "fased-memory-flush-"));
  });
