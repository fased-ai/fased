...
export async function dispatchReplyFromConfig(params: {
  ...
  const syncChannelTaskDispatch = (params: {
    deliveryStatus: TaskDeliveryStatus;
    summary: string;
    replyCounts: Record<string, number>;
    route: "origin" | "dispatcher";
    errors?: string[];
  }) => {
    if (!sessionKey) {
      return;
    ...
    const deliveryChannel =
      params.route === "origin" ? originatingChannel : (currentSurface ?? channel);
    const deliveryTarget = params.route === "origin" ? originatingTo : chatId;
    updateLatestChannelTaskDelivery({
      sessionKey,
      ...
      delivery: {
        ...
        ...(params.deliveryStatus === "delivered" ? { deliveredAt: Date.now() } : {}),
        ...(params.errors?.[0] ? { error: params.errors[0] } : {}),
      },
      metadata: {
        dispatchRoute: params.route,
        replyCounts: params.replyCounts,
        ...(params.errors && params.errors.length > 0 ? { dispatchErrors: params.errors } : {}),
      },
    });
  };
  ...
  const noteRouteError = (context: string, error?: string) => {
    const message = error?.trim() || "unknown error";
    routeErrors.push(`${context}: ${message}`);
  };
  ...
  const sendPayloadAsync = async (
    ...
    if (!result.ok) {
      noteRouteError("tool/block route", result.error);
  ...
  try {
    ...
    if (fastAbort.handled) {
      ...
      if (shouldRouteToOriginating && originatingChannel && originatingTo) {
        ...
        if (!result.ok) {
          noteRouteError("abort route", result.error);
          logVerbose(
    ...
    const suppressSourceProgressCallbacks =
      params.replyOptions?.sourceReplyDeliveryMode === "message_tool_only";
    const shouldForwardSourceProgressCallback =
      !suppressSourceProgressCallbacks ||
    ...
    const replyResult = await (params.replyResolver ?? getReplyFromConfig)(
      ctx,
      {
        ...
        onToolStart: onToolStartFromReplyOptions
          ? async (payload) => {
              if (shouldForwardSourceProgressCallback) {
    ...
    for (const reply of replies) {
      ...
      if (shouldRouteToOriginating && originatingChannel && originatingTo) {
        ...
        if (!result.ok) {
          noteRouteError("final route", result.error);
          logVerbose(
      ...
      try {
        ...
        if (ttsSyntheticReply.mediaUrl) {
          ...
          if (shouldRouteToOriginating && originatingChannel && originatingTo) {
            ...
            if (!result.ok) {
              noteRouteError("tts route", result.error);
              logVerbose(
    ...
    const counts = dispatcher.getQueuedCounts();
    counts.final += routedFinalCount;
    const anyQueued = queuedFinal || counts.final > 0 || counts.block > 0 || counts.tool > 0;
    const dispatchRoute = shouldRouteToOriginating ? "origin" : "dispatcher";
    const deliveryStatus: TaskDeliveryStatus = anyQueued
      ? shouldRouteToOriginating
        ? "delivered"
        : "pending"
      : "not_delivered";
    syncChannelTaskDispatch({
      deliveryStatus,
      summary: anyQueued
        ? shouldRouteToOriginating
          ? "Final reply delivered to originating channel."
          : "Final reply queued to channel dispatcher."
        : routeErrors.length > 0
          ? "Channel reply delivery failed."
          : "No channel reply was queued.",
      replyCounts: counts,
      route: dispatchRoute,
      errors: routeErrors,
    });
    ...
    syncChannelTaskDispatch({
      ...
      summary: "Channel dispatch failed before delivery.",
      replyCounts: counts,
      route: shouldRouteToOriginating ? "origin" : "dispatcher",
