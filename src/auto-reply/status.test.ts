...
describe("buildStatusMessage", () => {
  it("summarizes agent readiness and context usage", () => {
    ...
    expect(normalized).toContain("FasedAgent");
  ...
  it("shows configured fallback models when provided", () => {
    const text = buildStatusMessage({
      ...
      sessionEntry: { sessionId: "configured-fallbacks", updatedAt: 0 },
      sessionKey: "agent:main:main",
      sessionScope: "per-sender",
      queue: { mode: "collect", depth: 0 },
      modelAuth: "api-key",
  ...
  it("omits configured fallbacks line when no fallbacks are configured", () => {
    const text = buildStatusMessage({
      agent: {
        model: "anthropic/claude-opus-4-6",
      },
      sessionEntry: { sessionId: "no-configured-fallbacks", updatedAt: 0 },
      sessionKey: "agent:main:main",
      sessionScope: "per-sender",
      queue: { mode: "collect", depth: 0 },
      modelAuth: "api-key",
  ...
  it("keeps provider prefix from configured model", () => {
    ...
    expect(normalizeTestText(text)).toContain("Model: anthropic/claude-sonnet-4-6");
  });
