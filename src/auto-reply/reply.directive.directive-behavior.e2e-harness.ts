...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(
    ...
    {
      env: {
        FASED_AGENT_DIR: (home) => path.join(home, ".fased", "agent"),
        PI_CODING_AGENT_DIR: (home) => path.join(home, ".fased", "agent"),
      },
      prefix: "fased-reply-",
    },
  );
...
export function makeRestrictedElevatedDisabledConfig(home: string): FasedAgentConfig {
  return {
    agents: {
      defaults: {
        model: "anthropic/claude-opus-4-5",
