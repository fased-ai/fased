...
export function isCommandEnabled(cfg: FasedAgentConfig, commandKey: string): boolean {
...
export function resolveNativeCommandName(
...
function toNativeCommandSpec(command: ChatCommandDefinition, provider?: string): NativeCommandSpec {
  return {
    name: resolveNativeCommandName(command, provider) ?? command.key,
...
export function findCommandByNativeName(
  ...
  return getChatCommands().find(
    (command) =>
      command.scope !== "text" &&
      resolveNativeCommandName(command, provider)?.toLowerCase() === normalized,
  );
}
...
function resolveDefaultCommandContext(cfg?: FasedAgentConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as FasedAgentConfig),
...
export function getCommandDetection(_cfg?: FasedAgentConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: FasedAgentConfig) {
