...
type HomeEnvSnapshot = {
  ...
  FASED_AGENT_DIR: string | undefined;
...
function snapshotHomeEnv(): HomeEnvSnapshot {
  return {
    ...
    FASED_AGENT_DIR: process.env.FASED_AGENT_DIR,
...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    await fs.mkdir(path.join(home, ".fased", "agents", "main", "sessions"), { recursive: true });
    const envSnapshot = snapshotHomeEnv();
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.FASED_STATE_DIR = path.join(home, ".fased");
    process.env.FASED_AGENT_DIR = path.join(home, ".fased", "agent");
    process.env.PI_CODING_AGENT_DIR = path.join(home, ".fased", "agent");
