...
export function formatLinkUrlForLog(raw: string): string {
  try {
...
export function formatLinkCommandForLog(argv: string[], rawUrl: string): string {
  const safeUrl = formatLinkUrlForLog(rawUrl);
  return argv
    .map((part) => (part === rawUrl ? safeUrl : part.split(rawUrl).join(safeUrl)))
    .join(" ");
}
...
async function runCliEntry(params: {
  ...
  if (shouldLogVerbose()) {
    logVerbose(`Link understanding via CLI: ${formatLinkCommandForLog(argv, params.url)}`);
  }
...
async function runLinkEntries(params: {
  ...
  for (const entry of params.entries) {
    try {
      ...
      if (shouldLogVerbose()) {
        logVerbose(
          `Link understanding failed for ${formatLinkUrlForLog(params.url)}: ${formatErrorMessage(
            err,
          )}`,
        );
      }
  ...
  if (lastError && shouldLogVerbose()) {
    logVerbose(`Link understanding exhausted for ${formatLinkUrlForLog(params.url)}`);
  }
  return null;
}
