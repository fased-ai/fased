...
function mergeSignalAccountConfig(cfg: FasedAgentConfig, accountId: string): SignalAccountConfig {
...
export function resolveSignalAccount(params: {
  ...
  const configured = Boolean(merged.account?.trim() || merged.httpUrl?.trim());
  return {
    accountId,
    enabled,
...
export function listEnabledSignalAccounts(cfg: FasedAgentConfig): ResolvedSignalAccount[] {
