...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.fasedCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.fasedCanvasA2UIAction &&
        typeof window.fasedCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.fasedSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("fased:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android FasedAgent node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.fasedSendUserAction === "function"
        ? window.fasedSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.FASED_SKIP_CANVAS_HOST)) {
    return true;
  }
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>FasedAgent Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
