...
} from "../gateway/canvas-capability.js";
import { detectMime } from "../media/mime.js";
import { resolveFileWithinRoot } from "./file-resolver.js";

export const A2UI_PATH = "/__fased__/a2ui";

export const CANVAS_HOST_PATH = "/__fased__/canvas";

export const CANVAS_WS_PATH = "/__fased__/ws";
...
(() => {
  ...
  // Works on:
  // - iOS: window.webkit.messageHandlers.fasedCanvasA2UIAction.postMessage(...)
  // - Android: window.fasedCanvasA2UIAction.postMessage(...)
  const handlerNames = ["fasedCanvasA2UIAction"];
  ...
  globalThis.FasedAgent = globalThis.FasedAgent ?? {};
  globalThis.FasedAgent.postMessage = postToNode;
  globalThis.FasedAgent.sendUserAction = sendUserAction;
  globalThis.fasedPostMessage = postToNode;
  globalThis.fasedSendUserAction = sendUserAction;

  try {
    const search = new URLSearchParams(location.search);
    const cap =
      search.get(${JSON.stringify(CANVAS_CAPABILITY_QUERY_PARAM)}) ||
      search.get(${JSON.stringify(LEGACY_CANVAS_CAPABILITY_QUERY_PARAM)});
    const proto = location.protocol === "https:" ? "wss" : "ws";
    const capQuery =
      cap ? "?" + ${JSON.stringify(CANVAS_CAPABILITY_QUERY_PARAM)} + "=" + encodeURIComponent(cap) : "";
