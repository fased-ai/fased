...
describe("canvas host state dir defaults", () => {
  it("uses FASED_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("fased-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("FasedAgent Canvas");
      } finally {
        await handler.close();
      }
    });
