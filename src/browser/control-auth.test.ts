...
describe("ensureBrowserControlAuth", () => {
  async function expectNoAutoGeneratedAuth(cfg: FasedAgentConfig): Promise<void> {
    const result = await ensureBrowserControlAuth({
      cfg,
      env: { FASED_BROWSER_AUTO_AUTH: "1" },
    });
  ...
  describe("trusted-proxy mode without trusted proxy options", () => {
    it("should not auto-generate token when auth mode is trusted-proxy", async () => {
      const cfg: FasedAgentConfig = {
        gateway: {
          auth: {
