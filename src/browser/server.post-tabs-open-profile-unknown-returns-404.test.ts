...
describe("profile CRUD endpoints", () => {
  ...
  it("validates profile create/delete endpoints", async () => {
    ...
    const createDuplicate = await realFetch(`${base}/profiles/create`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: "fased" }),
    });
    ...
    const sensitiveCdpUrl = "http://user:pass@10.0.0.42:9222/json?token=abc&launch=headless";
    const redactedCdpUrl =
      "http://redacted:redacted@10.0.0.42:9222/json?token=redacted&launch=redacted";

    const createRemote = await realFetch(`${base}/profiles/create`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: "remote", cdpUrl: sensitiveCdpUrl }),
    });
    ...
    expect(createRemoteBody.cdpUrl).toBe(redactedCdpUrl);
    ...
    const deleteDefault = await realFetch(`${base}/profiles/fased`, {
      method: "DELETE",
    });
