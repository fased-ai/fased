...
import {
  isChromeCdpReady,
  isChromeReachable,
  launchFasedAgentChrome,
  resolveFasedAgentUserDataDir,
  stopFasedAgentChrome,
} from "./chrome.js";
import type { ResolvedBrowserProfile } from "./config.js";
import { redactBrowserCdpUrl, resolveProfile } from "./config.js";
import {
...
function createProfileContext(
  ...
  const ensureBrowserAvailable = async (): Promise<void> => {
    ...
    const cdpUrlForDisplay = redactBrowserCdpUrl(profile.cdpUrl);

    if (isExtension && remoteCdp) {
      throw new Error(
        `Profile "${profile.name}" uses driver=extension but cdpUrl is not loopback (${cdpUrlForDisplay}).`,
      );
    }
    ...
    if (isExtension) {
      if (!httpReachable) {
        ...
        if (await isHttpReachable(1200)) {
          ...
          throw new Error(
            `Chrome extension relay for profile "${profile.name}" is not reachable at ${cdpUrlForDisplay}.`,
          );
        }
      }
      ...
      throw new Error(
        `Chrome extension relay is running, but no tab is connected. Click the FasedAgent Chrome extension icon on a tab to attach it (profile "${profile.name}").`,
      );
    }
    ...
    if (!httpReachable) {
      ...
      if (current.resolved.attachOnly || remoteCdp) {
        throw new Error(
          remoteCdp
            ? `Remote CDP for profile "${profile.name}" is not reachable at ${cdpUrlForDisplay}.`
            : `Browser attachOnly is enabled and profile "${profile.name}" is not running.`,
        );
      }
      const launched = await launchFasedAgentChrome(current.resolved, profile);
    ...
    if (!profileState.running) {
      throw new Error(
        `Port ${profile.cdpPort} is in use for profile "${profile.name}" but not by fased. ` +
    ...
    await stopFasedAgentChrome(profileState.running);
    setProfileRunning(null);

    const relaunched = await launchFasedAgentChrome(current.resolved, profile);
      ...
      if (profile.driver === "extension") {
        throw new Error(
          `tab not found (no attached Chrome tabs for profile "${profile.name}"). ` +
            "Click the FasedAgent Browser Relay toolbar icon on the tab you want to control (badge ON).",
        );
      }
  ...
  const resetProfile = async () => {
    if (profile.driver === "extension") {
      ...
      return { moved: false, from: redactBrowserCdpUrl(profile.cdpUrl) };
    }
    if (!profile.cdpIsLoopback) {
      throw new Error(
  ...
  const listProfiles = async (): Promise<ProfileStatus[]> => {
    ...
    for (const name of Object.keys(current.resolved.profiles)) {
      ...
      result.push({
        ...
        cdpUrl: redactBrowserCdpUrl(profile.cdpUrl),
        color: profile.color,
        running,
        tabCount,
