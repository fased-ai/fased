...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.FASED_GATEWAY_PORT = prevGatewayPort;
}
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/fased" }));
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          fased: { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchFasedAgentChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.prevGatewayPort = process.env.FASED_GATEWAY_PORT;
  process.env.FASED_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.FASED_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.FASED_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayPassword === undefined) {
    ...
    process.env.FASED_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
