import type { BrowserConfig, BrowserProfileConfig, FasedAgentConfig } from "../config/config.js";
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "fased" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_FASED_BROWSER_COLOR;
  }
...
export function redactBrowserCdpUrl(raw: string): string {
  try {
    ...
    for (const key of new Set(parsed.searchParams.keys())) {
      ...
      for (const value of values) {
        parsed.searchParams.append(key, value ? "redacted" : "");
      }
    }
...
/**
 * Ensure the default "fased" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_FASED_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_FASED_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an FasedAgent browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for fased/fased CDP).
...
export function resolveBrowserConfig(
  ...
  const enabled = cfg?.enabled ?? DEFAULT_FASED_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "fased";
