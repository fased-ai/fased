...
export function resolveRelayAcceptedTokensForPort(port: number): string[] {
  ...
  if (!gatewayToken) {
    throw new Error(
      "extension relay requires gateway auth token (set gateway.auth.token or FASED_GATEWAY_TOKEN)",
    );
  }
...
export async function probeAuthenticatedFasedAgentRelay(params: {
  baseUrl: string;
  ...
  try {
    ...
    return browserName === FASED_RELAY_BROWSER;
  } catch {
    return false;
  } finally {
    clearTimeout(timer);
