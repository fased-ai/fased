...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredFasedAgentTmpDir: vi.fn(() => "/tmp/fased"),
}));
vi.mock("../infra/tmp-fased-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredFasedAgentTmpDir.mockReturnValue("/tmp/fased");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredFasedAgentTmpDir.mockReturnValue("/tmp/fased-preferred");
    const { res, outPath } = await waitForImplicitDownloadOutput({
      downloadUrl: "https://example.com/file.bin",
      suggestedFilename: "file.bin",
    });
    expect(typeof outPath).toBe("string");
    const expectedRootedDownloadsDir = path.join(path.sep, "tmp", "fased-preferred", "downloads");
    const expectedDownloadsTail = `${path.join("tmp", "fased-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredFasedAgentTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.join(path.sep, "tmp", "fased-preferred", "downloads"),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "fased-preferred", "downloads")}${path.sep}`),
    );
  });
