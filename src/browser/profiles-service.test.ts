...
vi.mock("./chrome.js", () => ({
  resolveFasedAgentUserDataDir: vi.fn(() => "/tmp/fased-test/fased/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveFasedAgentUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "fased-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveFasedAgentUserDataDir).mockReturnValue(userDataDir);
