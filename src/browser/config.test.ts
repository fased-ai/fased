...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and fased-orange color", () => {
    ...
    expect(fased?.driver).toBe("fased");
    expect(fased?.cdpPort).toBe(18800);
    expect(fased?.cdpUrl).toBe("http://127.0.0.1:18800");
    expect(resolved.remoteCdpTimeoutMs).toBe(1500);
    expect(resolved.remoteCdpHandshakeTimeoutMs).toBe(3000);
  });

  it("derives default ports from FASED_GATEWAY_PORT when unset", () => {
    withEnv({ FASED_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const fased = resolveProfile(resolved, "fased");
      expect(fased?.cdpPort).toBe(19012);
      expect(fased?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });

  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ FASED_GATEWAY_PORT: undefined }, () => {
      ...
      const fased = resolveProfile(resolved, "fased");
      expect(fased?.cdpPort).toBe(19022);
      expect(fased?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "fased");
  ...
  it("redacts CDP credentials for display", () => {
    expect(
      redactBrowserCdpUrl(
        'https://user:pass@browserless.example/chrome?token=abc&launch={"headless":false}',
      ),
    ).toBe("https://redacted:redacted@browserless.example/chrome?token=redacted&launch=redacted");
  });
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        fased: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("fased");
  });
