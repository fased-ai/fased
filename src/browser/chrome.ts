...
import {
  decorateFasedAgentProfile,
  ensureProfileCleanExit,
  isProfileDecorated,
} from "./chrome.profile-decoration.js";
import type { ResolvedBrowserConfig, ResolvedBrowserProfile } from "./config.js";
import { DEFAULT_FASED_BROWSER_COLOR, DEFAULT_FASED_BROWSER_PROFILE_NAME } from "./constants.js";
...
export function resolveFasedAgentUserDataDir(profileName = DEFAULT_FASED_BROWSER_PROFILE_NAME) {
...
export async function launchFasedAgentChrome(
  ...
  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_FASED_BROWSER_COLOR).toUpperCase(),
  );
  ...
  log.info(
    `fased browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
...
export async function stopFasedAgentChrome(running: RunningChrome, timeoutMs = 2500) {
