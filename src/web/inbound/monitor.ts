...
function withGroupReplyContext(
  ...
  if (!params.group || !params.messageId) {
    return payload;
  }
  const existing =
    typeof payload === "object" && payload && "contextInfo" in payload
      ? (payload as { contextInfo?: Record<string, unknown> }).contextInfo
      : undefined;
  return {
    ...payload,
    contextInfo: {
...
export async function monitorWebInbox(options: {
  ...
  const getGroupMeta = async (jid: string) => {
    ...
    try {
      ...
      logVerbose(
        `Failed to fetch group metadata for ${redactIdentifier(jid)}: ${formatError(err)}`,
      );
