...
describe("createWebSendApi", () => {
  ...
  it("adds visible reply context when replyToId is provided", async () => {
    await api.sendMessage("+1555", "hello", undefined, undefined, { replyToId: "msg-123" });
    expect(sendMessage).toHaveBeenCalledWith(
      "1555@s.whatsapp.net",
      expect.objectContaining({
        text: "hello",
  ...
  it("adds visible reply context for group JID replies", async () => {
    await api.sendMessage("120363123456789@g.us", "hello group", undefined, undefined, {
  ...
  it("adds visible reply context to media payloads", async () => {
    const payload = Buffer.from("img");
    await api.sendMessage("+1555", "cap", payload, "image/jpeg", { replyToId: "msg-456" });
    expect(sendMessage).toHaveBeenCalledWith(
      "1555@s.whatsapp.net",
      expect.objectContaining({
        image: payload,
