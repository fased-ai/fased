...
export async function checkInboundAccessControl(params: {
  ...
  if (params.group && access.decision !== "allow") {
    if (access.reason === "groupPolicy=disabled") {
      ...
      logVerbose(
        `Blocked group message from ${params.senderE164 ? redactIdentifier(params.senderE164) : "unknown sender"} (groupPolicy: allowlist)`,
      );
    }
    return {
      allowed: false,
      ...
      if (suppressPairingReply) {
        ...
        if (created) {
          ...
          try {
            ...
            logVerbose(
              `whatsapp pairing reply failed for ${redactIdentifier(candidate)}: ${formatError(err)}`,
            );
          }
        }
      }
