...
describe("login-qr", () => {
  ...
  it("uses formatted errors for QR startup failures", async () => {
    createWaSocketMock.mockRejectedValueOnce(new Error("raw-token-value"));
    formatErrorMock.mockReturnValueOnce("redacted-error");

    const result = await startWebLoginWithQr({ timeoutMs: 5000 });

    expect(result.message).toBe("Failed to start WhatsApp login: redacted-error");
    expect(result.message).not.toContain("raw-token-value");
  });
});
