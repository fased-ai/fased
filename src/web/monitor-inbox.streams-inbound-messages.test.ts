...
describe("web monitor inbox", () => {
  ...
  it("quotes triggering group messages when sending text and media replies", async () => {
    const mediaPayload = {
    ...
    const onMessage = vi.fn(async (msg) => {
      await msg.reply("pong");
      await msg.sendMedia(mediaPayload);
    });
    ...
    expect(sock.sendMessage).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        text: "pong",
        contextInfo: expect.objectContaining(quotedContext),
      }),
    );
    expect(sock.sendMessage).toHaveBeenCalledWith(
      "12345-67890@g.us",
