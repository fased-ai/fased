  ...
  const sendWithRetry = async (fn: () => Promise<unknown>, label: string, maxAttempts = 3) => {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        logVerbose(
          `Retrying ${label} to ${redactedFrom} after failure (${attempt}/${maxAttempts - 1}) in ${backoffMs}ms: ${errText}`,
        );
    ...
    for (const [index, chunk] of textChunks.entries()) {
      ...
      if (!skipLog) {
        ...
        whatsappOutboundLog.debug(
          `Sent chunk ${index + 1}/${totalChunks} to ${redactedFrom} (${durationMs.toFixed(0)}ms)`,
        );
      }
    }
    replyLogger.info(
  ...
  for (const [index, mediaUrl] of mediaList.entries()) {
    ...
    try {
      ...
      whatsappOutboundLog.info(
        `Sent media reply to ${redactedFrom} (${(media.buffer.length / (1024 * 1024)).toFixed(2)}MB)`,
      );
      replyLogger.info(
        {
          correlationId: msg.id ?? newConnectionId(),
      ...
      replyLogger.warn(
        { err: formatError(err), mediaUrl: redactedMediaUrl },
