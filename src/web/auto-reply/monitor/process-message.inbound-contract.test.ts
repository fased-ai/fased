...
describe("web processMessage inbound contract", () => {
  beforeEach(async () => {
    ...
    sessionDir = await fs.mkdtemp(path.join(os.tmpdir(), "fased-process-message-"));
  ...
  it("defaults responsePrefix to identity name in self-chats when unset", async () => {
            ...
            list: [
              {
                id: "main",
                default: true,
                identity: { name: "FasedAgent", emoji: "🤖", theme: "matrix" },
              },
            ],
          },
          messages: {},
    ...
    expect(dispatcherOptions?.responsePrefix).toBe("[FasedAgent]");
  });
