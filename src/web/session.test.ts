...
function mockCredsJsonSpies(readContents: string) {
  const credsSuffix = path.join(".fased", "credentials", "whatsapp", "default", "creds.json");
...
describe("web session", () => {
  ...
  it("formatError redacts secrets in diagnostics", () => {
    const formatted = formatError(
      new Error(
        "failed https://example.test/path?token=super-secret-token-value Authorization: Bearer abcdefghijklmnopqrstuvwxyz",
      ),
    );

    expect(formatted).not.toContain("super-secret-token-value");
