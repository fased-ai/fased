...
function enqueueSaveCreds(
  ...
  credsSaveQueue = credsSaveQueue
    ...
    .catch((err) => {
      logger.warn({ error: formatError(err) }, "WhatsApp creds save queue error");
    });
}
...
export async function createWaSocket(
      ...
      try {
        ...
        if (connection === "close") {
          ...
          if (status === DisconnectReason.loggedOut) {
            console.error(
              danger(
                `WhatsApp session logged out. Run: ${formatCliCommand("fased channels login")}`,
              ),
            );
          }
        }
        ...
        sessionLogger.error({ error: formatError(err) }, "connection.update handler error");
      }
    },
  );
  ...
  if (sock.ws && typeof (sock.ws as unknown as { on?: unknown }).on === "function") {
    sock.ws.on("error", (err: Error) => {
      sessionLogger.error({ error: formatError(err) }, "WebSocket error");
    });
  }
...
function redactFormattedError(value: string): string {
...
export function formatError(err: unknown): string {
  if (err instanceof Error) {
    return redactFormattedError(err.message);
  }
  if (typeof err === "string") {
    return redactFormattedError(err);
  }
  if (!err || typeof err !== "object") {
    return redactFormattedError(String(err));
  }
  ...
  if (pieces.length > 0) {
    return redactFormattedError(pieces.join(" "));
  }
  return redactFormattedError(safeStringify(err));
}
