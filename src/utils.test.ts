...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDirSync("fased-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    withTempDirSync("fased-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", () => {
    withTempDirSync("fased-lid-a-", (first) => {
      withTempDirSync("fased-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers FASED_STATE_DIR over FASED_CONFIG_DIR", () => {
    const resolved = resolveConfigDir(
      {
        FASED_STATE_DIR: "/srv/fased-state",
        FASED_CONFIG_DIR: "/srv/fased-config",
      } as NodeJS.ProcessEnv,
      () => "/home/fased",
    );

    expect(resolved).toBe(path.resolve("/srv/fased-state"));
  });

  it("uses FASED_CONFIG_DIR when FASED_STATE_DIR is not set", () => {
    const resolved = resolveConfigDir(
      {
        FASED_CONFIG_DIR: "/srv/fased-config",
      } as NodeJS.ProcessEnv,
    ...
    expect(resolved).toBe(path.resolve("/srv/fased-config"));
  });

  it("prefers ~/.fased when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "fased-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers FASED_HOME over HOME", () => {
    vi.stubEnv("FASED_HOME", "/srv/fased-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/fased-home"));
...
describe("shortenHomePath", () => {
  it("uses $FASED_HOME prefix when FASED_HOME is set", () => {
    vi.stubEnv("FASED_HOME", "/srv/fased-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/fased-home")}/.fased/fased.json`)).toBe(
      "$FASED_HOME/.fased/fased.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $FASED_HOME replacement when FASED_HOME is set", () => {
    vi.stubEnv("FASED_HOME", "/srv/fased-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/fased-home")}/.fased/fased.json`),
    ).toBe("config: $FASED_HOME/.fased/fased.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/fased")).toBe(path.resolve(os.homedir(), "fased"));
  });
  ...
  it("prefers FASED_HOME for tilde expansion", () => {
    vi.stubEnv("FASED_HOME", "/srv/fased-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/fased")).toBe(path.resolve("/srv/fased-home", "fased"));
