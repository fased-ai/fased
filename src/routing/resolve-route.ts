...
function listAgents(cfg: FasedAgentConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: FasedAgentConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: FasedAgentConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<FasedAgentConfig, EvaluatedBindingsCache>();
