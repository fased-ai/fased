...
function hasEntryCredential(
  provider: Pick<
    PluginWebSearchProviderEntry,
    "id" | "envVars" | "getConfiguredCredentialValue" | "getCredentialValue" | "requiresCredential"
  >,
  ...
  const rawValue =
    configuredValue ?? provider.getCredentialValue?.(search as Record<string, unknown>);
  const configuredRef = coerceSecretRef(rawValue);
  if (configuredRef && configuredRef.source !== "env") {
    return true;
  }
  const fromConfig = normalizeSecretInput(rawValue);
...
export function listWebSearchProviders(params?: {
  ...
  return resolveRuntimeWebSearchProviders({ config: params?.config });
}

export function listConfiguredWebSearchProviders(params?: {
  config?: FasedAgentConfig;
}): PluginWebSearchProviderEntry[] {
  return resolvePluginWebSearchProviders({ config: params?.config });
}
...
export function resolveWebSearchProviderId(params: {
  ...
  const providers = sortWebSearchProvidersForAutoDetect(
    params.providers ?? resolvePluginWebSearchProviders({ config: params.config }),
  );
  const raw =
...
export function resolveWebSearchDefinition(
  ...
  const providers = sortWebSearchProvidersForAutoDetect(
    options?.preferRuntimeProviders
      ? resolveRuntimeWebSearchProviders({ config: options?.config })
      : resolvePluginWebSearchProviders({ config: options?.config }),
  ).filter(Boolean);
