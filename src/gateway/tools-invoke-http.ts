...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(getHeader(req, "x-fased-message-channel") ?? "");
  const accountId = getHeader(req, "x-fased-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-fased-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-fased-thread-id")?.trim() || undefined;
  ...
  const allTools = createFasedAgentTools({
    agentSessionKey: sessionKey,
    agentChannel: messageChannel ?? undefined,
    agentAccountId: accountId,
    agentTo,
    agentThreadId,
    requestSource: "gateway-http",
    config: cfg,
