...
export function buildGatewayConnectionDetails(
  options: { config?: FasedAgentConfig; url?: string; configPath?: string } = {},
  ...
  if (!isSecureWebSocketUrl(url)) {
    throw new Error(
      [
        ...
        "Doctor: fased doctor --fix",
        "Docs: https://docs.fased.ai/gateway/remote",
      ].join("\n"),
    );
  }
...
function resolveGatewayCredentials(
  ...
  options: { preferLocalConfigAuth?: boolean } = {},
): {
  token?: string;
  password?: string;
} {
...
function isGatewayConnectRequestOrAuthError(error: Error): boolean {
  const message = error.message.toLowerCase();
  return (
    error.name === "GatewayClientRequestError" ||
    message.startsWith("unauthorized") ||
    message.includes("missing scope:") ||
    message.includes("gateway token") ||
    message.includes("gateway password") ||
    message.includes("tls fingerprint") ||
    message.includes("security error")
  );
}

function formatGatewayConnectTransportError(
  error: Error,
  connectionDetails: GatewayConnectionDetails,
): string {
  ...
  return await new Promise<T>((resolve, reject) => {
    ...
    const client = new GatewayClient({
      ...
      onConnectError: (err) => {
        ...
        ignoreClose = true;
        client.stop();
        if (isGatewayConnectRequestOrAuthError(err)) {
          stop(err);
          return;
        }
        stop(
          new GatewayTransportError({
            kind: "closed",
            message: formatGatewayConnectTransportError(err, params.connectionDetails),
      ...
      onClose: (code, reason) => {
        ...
        stop(
          new GatewayTransportError({
            kind: "closed",
            message: formatGatewayCloseError(code, reason, params.connectionDetails),
            connectionDetails: params.connectionDetails,
            code,
            reason,
          }),
    ...
    const timer = setTimeout(() => {
      ...
      stop(
        new GatewayTransportError({
          kind: "timeout",
          message: formatGatewayTimeoutError(timeoutMs, params.connectionDetails),
          connectionDetails: params.connectionDetails,
          timeoutMs,
        }),
      );
...
async function callGatewayWithScopes<T = Record<string, unknown>>(
  ...
  const { token, password } = resolveGatewayCredentials(context, {
    preferLocalConfigAuth: options.preferLocalConfigAuth,
  });
...
export async function callGatewayScoped<T = Record<string, unknown>>(
  ...
  const preferLocalConfigAuth =
    opts.mode !== GATEWAY_CLIENT_MODES.CLI && opts.clientName !== GATEWAY_CLIENT_NAMES.CLI;
  return await callGatewayWithScopes(opts, opts.scopes, { preferLocalConfigAuth });
}
...
export async function callGatewayCli<T = Record<string, unknown>>(
  ...
  return await callGatewayWithScopes(opts, scopes, { preferLocalConfigAuth: false });
}
...
export async function callGatewayLeastPrivilege<T = Record<string, unknown>>(
  ...
  return await callGatewayWithScopes(opts, scopes, { preferLocalConfigAuth: true });
}
