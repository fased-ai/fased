...
function normalizeHookMapping(
  ...
  return {
    ...
    workflowDefinitionId: mapping.workflowDefinitionId,
    ...
    notifyPolicy: mapping.notifyPolicy,
    transform,
  };
}
...
function buildActionFromMapping(
  ...
  if (mapping.action === "workflow") {
    return {
      ...
      action: {
        kind: "workflow",
        triggerId: mapping.id,
        workflowDefinitionId: renderOptional(mapping.workflowDefinitionId, ctx) ?? "",
...
function mergeAction(
  ...
  defaultAction: "wake" | "agent" | "workflow",
): HookMappingResult {
  if (!override) {
  ...
  if (kind === "workflow") {
    ...
    const workflowDefinitionId =
      typeof override.workflowDefinitionId === "string"
        ? override.workflowDefinitionId
        : (baseWorkflow?.workflowDefinitionId ?? "");
    return validateAction({
      kind: "workflow",
      triggerId: baseWorkflow?.triggerId,
      workflowDefinitionId,
      name: override.name ?? baseWorkflow?.name,
      agentId: override.agentId ?? baseWorkflow?.agentId,
      sessionKey: override.sessionKey ?? baseWorkflow?.sessionKey,
      notifyPolicy: override.notifyPolicy ?? baseWorkflow?.notifyPolicy,
    });
  }
  ...
  return validateAction({
    ...
    notifyPolicy: override.notifyPolicy ?? baseAgent?.notifyPolicy,
  });
}
...
function validateAction(action: HookAction): HookMappingResult {
  ...
  if (action.kind === "workflow") {
    if (!action.workflowDefinitionId?.trim()) {
      return { ok: false, error: "hook mapping requires workflowDefinitionId" };
    }
    return { ok: true, action };
  }
