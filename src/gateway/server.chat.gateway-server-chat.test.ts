...
describe("gateway server chat", () => {
  ...
  test("handles chat send and history flows", async () => {
    ...
    try {
      ...
      const controlHistoryPath = path.join(historyDir, "sess-main.jsonl");
      await fs.writeFile(
        controlHistoryPath,
        [
          JSON.stringify({
      ...
      const controlRes = await rpcReq<{ messages?: unknown[] }>(ws, "chat.history", {
        sessionKey: "main",
      });
      expect(controlRes.ok).toBe(true);
      const roleAndText = (controlRes.payload?.messages ?? [])
        .map((message) => {
          ...
          const text =
            typeof entry.text === "string"
              ...
              : Array.isArray(entry.content) &&
                  entry.content[0] &&
                  typeof entry.content[0] === "object" &&
                  typeof (entry.content[0] as { text?: unknown }).text === "string"
                ? ((entry.content[0] as { text: string }).text ?? "")
                : "";
          return `${role}:${text}`;
        })
  ...
  test("routes chat.send slash commands without agent runs", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "fased-gw-"));
    try {
