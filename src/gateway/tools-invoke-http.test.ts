...
let lastCreateFasedAgentToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/fased-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateFasedAgentToolsContext?.agentTo,
          agentThreadId: lastCreateFasedAgentToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createFasedAgentTools: (ctx: Record<string, unknown>) => {
      lastCreateFasedAgentToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateFasedAgentToolsContext = undefined;
});
...
describe("POST /tools/invoke", () => {
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-fased-message-to": "channel:24514",
        "x-fased-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
  ...
  it("does not let gateway.tools.allow bypass the effective agent tool policy", async () => {
    cfg = {
      ...cfg,
      agents: {
        list: [{ id: "main", default: true, tools: { deny: ["gateway"] } }],
      },
      gateway: { tools: { allow: ["gateway"] } },
    };
  ...
  it("normalizes null HTTP tool args to an empty object", async () => {
    cfg = {
      ...cfg,
      agents: {
        list: [{ id: "main", default: true, tools: { allow: ["tools_invoke_args_echo"] } }],
      },
    };
    ...
    const body = await res.json();
    expect(body.ok).toBe(true);
    expect(body.result).toEqual({ ok: true, args: {} });
  });
