...
describe("gateway e2e", () => {
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "fased-gw-mock-home-"));
      ...
      const workspaceDir = path.join(tempHome, "fased");
  ...
  it("runs wizard over ws and writes auth token config", { timeout: 90_000 }, async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "fased-wizard-home-"));
    process.env.HOME = tempHome;
