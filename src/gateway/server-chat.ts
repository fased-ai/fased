...
import {
  ...
  broadcastSessionToolEvent,
} from "./server-session-events.js";
import type { SessionEventSubscriberRegistry } from "./session-event-subscribers.js";
...
export function createAgentEventHandler({
  ...
  const emitChatDelta = (
    ...
    if (isSuppressedControlReplyText(cleaned)) {
      return;
    }
    if (isSuppressedControlReplyLeadFragment(cleaned)) {
      return;
    }
