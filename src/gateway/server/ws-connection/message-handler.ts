        ...
        if (enforceOriginCheckForAnyClient || isControlUi || isWebchat) {
          ...
          if (!originCheck.ok) {
            ...
            markHandshakeFailure("origin-mismatch", {
              ...
              host: originRequestHost ?? requestHost ?? "n/a",
          ...
          if (originCheck.matchedBy === "host-header-fallback") {
            logWsControl.warn(
              `security warning: websocket origin accepted via Host-header fallback conn=${connId} host=${originRequestHost ?? requestHost ?? "n/a"} origin=${requestOrigin ?? "n/a"}`,
            );
          }
        }
        ...
        const helloOk = {
          ...
          auth: {
            ...(deviceToken
              ...
              : {}),
            role: deviceToken?.role ?? role,
            scopes: deviceToken?.scopes ?? scopes,
          },
          policy: {
