...
export const CANVAS_CAPABILITY_PATH_PREFIX = "/__fased__/cap";
export const CANVAS_CAPABILITY_QUERY_PARAM = "fased_cap";
export const LEGACY_CANVAS_CAPABILITY_QUERY_PARAM = "oc_cap";
...
export function normalizeCanvasScopedUrl(rawUrl: string): NormalizedCanvasScopedUrl {
  ...
  if (url.pathname.startsWith(prefix)) {
    ...
    if (slashIndex <= 0) {
      ...
      if (!capabilityFromPath || !canonicalPath.startsWith("/")) {
        ...
        if (
          !url.searchParams.has(CANVAS_CAPABILITY_QUERY_PARAM) &&
          !url.searchParams.has(LEGACY_CANVAS_CAPABILITY_QUERY_PARAM)
        ) {
  ...
  const capability =
    ...
    normalizeCapability(url.searchParams.get(LEGACY_CANVAS_CAPABILITY_QUERY_PARAM));
  return {
