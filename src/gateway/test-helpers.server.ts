...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "fased-gateway-home-"));
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    tempConfigRoot = await fs.mkdtemp(path.join(tempHome, "fased-test-"));
  } else {
    tempConfigRoot = path.join(tempHome, ".fased-test");
...
const CONNECT_CHALLENGE_NONCE_KEY = "__fasedTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__fasedTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["FASED_GATEWAY_TOKEN"]);
  const prev = process.env.FASED_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.FASED_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.FASED_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
  ...
  const defaultPassword =
    opts?.skipDefaultAuth === true
      ...
      : typeof (testState.gatewayAuth as { password?: unknown } | undefined)?.password === "string"
        ...
        : process.env.FASED_GATEWAY_PASSWORD;
