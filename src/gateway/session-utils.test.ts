...
function createSingleAgentAvatarConfig(workspace: string): FasedAgentConfig {
  return {
...
describe("gateway session utils", () => {
  const originalCombinedStoreCacheTtl = process.env.FASED_SESSION_COMBINED_STORE_CACHE_TTL_MS;
  const originalSessionStoreCacheTtl = process.env.FASED_SESSION_CACHE_TTL_MS;

  beforeEach(() => {
    clearSessionStoreCacheForTest();
    clearCombinedSessionStoreCacheForTest();
    process.env.FASED_SESSION_COMBINED_STORE_CACHE_TTL_MS = "60000";
    process.env.FASED_SESSION_CACHE_TTL_MS = "60000";
  });

  afterEach(() => {
    clearSessionStoreCacheForTest();
    clearCombinedSessionStoreCacheForTest();
    if (originalCombinedStoreCacheTtl === undefined) {
      delete process.env.FASED_SESSION_COMBINED_STORE_CACHE_TTL_MS;
    } else {
      process.env.FASED_SESSION_COMBINED_STORE_CACHE_TTL_MS = originalCombinedStoreCacheTtl;
    }
    if (originalSessionStoreCacheTtl === undefined) {
      delete process.env.FASED_SESSION_CACHE_TTL_MS;
    } else {
      process.env.FASED_SESSION_CACHE_TTL_MS = originalSessionStoreCacheTtl;
    }
    vi.restoreAllMocks();
  });
  ...
  test("loadCombinedSessionStoreForGateway reuses unchanged combined stores without leaking mutations", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "session-utils-combined-cache-"));
    const storeTemplate = path.join(root, "{agentId}", "sessions.json");
    const mainStorePath = path.join(root, "main", "sessions.json");
    const opsStorePath = path.join(root, "ops", "sessions.json");
    fs.mkdirSync(path.dirname(mainStorePath), { recursive: true });
    fs.mkdirSync(path.dirname(opsStorePath), { recursive: true });
    fs.writeFileSync(
      mainStorePath,
      JSON.stringify({
        main: { sessionId: "main-session", updatedAt: 1, displayName: "Main session" },
      }),
      "utf8",
    );
    fs.writeFileSync(
      opsStorePath,
      JSON.stringify({
        work: { sessionId: "ops-session", updatedAt: 2, displayName: "Ops session" },
      }),
      "utf8",
    );
    const cfg = {
    ...
    const readsAfterFirst = readSpy.mock.calls.filter(([filePath]) =>
      [mainStorePath, opsStorePath].includes(String(filePath)),
    ).length;
    expect(first.store["agent:ops:work"]?.displayName).toBe("Ops session");
    first.store["agent:ops:work"].displayName = "Mutated caller copy";

    const second = loadCombinedSessionStoreForGateway(cfg);
    const readsAfterSecond = readSpy.mock.calls.filter(([filePath]) =>
      [mainStorePath, opsStorePath].includes(String(filePath)),
    ).length;

    expect(readsAfterSecond).toBe(readsAfterFirst);
    expect(second.store["agent:ops:work"]?.displayName).toBe("Ops session");
  });

  test("loadCombinedSessionStoreForGateway invalidates when store metadata changes even if mtime is preserved", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "session-utils-combined-stale-"));
    const storePath = path.join(root, "sessions.json");
    fs.writeFileSync(
      storePath,
      JSON.stringify({
        main: { sessionId: "old-session", updatedAt: 1, displayName: "Old session" },
      }),
      "utf8",
    );
    const cfg = {
    ...
    const first = loadCombinedSessionStoreForGateway(cfg);
    expect(first.store["agent:main:main"]?.displayName).toBe("Old session");

    const originalStat = fs.statSync(storePath);
    fs.writeFileSync(
      storePath,
      JSON.stringify({
        main: {
          ...
          updatedAt: 2,
          displayName: "New session with a larger payload",
        },
      }),
      "utf8",
    );
    fs.utimesSync(storePath, originalStat.atime, originalStat.mtime);

    const second = loadCombinedSessionStoreForGateway(cfg);
    expect(second.store["agent:main:main"]?.sessionId).toBe("new-session");
    expect(second.store["agent:main:main"]?.displayName).toBe("New session with a larger payload");
  });
});
...
describe("resolveSessionModelRef", () => {
  ...
  test("uses the Agent primary model when a session belongs to that Agent", () => {
    const cfg = {
      agents: {
        defaults: {
        ...
        list: [
          {
            id: "research",
            model: { primary: "openrouter/qwen/qwen3.6-flash" },
          },
        ],
      },
    } as FasedAgentConfig;

    const resolved = resolveSessionModelRef(cfg, undefined, "research");
...
describe("listSessionsFromStore search", () => {
  ...
  test("includes loaded skill snapshots in session rows", () => {
    const result = listSessionsFromStore({
      cfg: baseCfg,
      storePath: "/tmp/sessions.json",
      store: {
        "agent:main:work-project": {
          ...
          skillsSnapshot: {
            prompt: "skills",
            skills: [{ name: "Wallet" }, { name: "Mining" }, { name: "Search" }],
    ...
    expect(result.sessions[0].skills).toEqual({
      count: 3,
      names: ["Wallet", "Mining", "Search"],
      skillFilter: ["Wallet", "Mining"],
    });
  });
  ...
  test("exposes compaction checkpoint summaries without transcript paths", () => {
    const now = Date.now();
    const store: Record<string, SessionEntry> = {
      "agent:main:main": {
        sessionId: "sess-main",
        ...
        compactionCheckpoints: [
          {
            ...
            tokensBefore: 1200,
            tokensAfter: 600,
            summary: "Before context trim",
            preCompaction: {
              sessionId: "sess-main",
              sessionFile: "/tmp/secret.checkpoint.jsonl",
            },
            postCompaction: {
              sessionId: "sess-main",
    ...
    expect(result.sessions[0]?.compactionCheckpointCount).toBe(1);
    expect(result.sessions[0]?.compactionCheckpoints).toEqual([
      {
        checkpointId: "checkpoint-1",
        createdAt: now,
        reason: "manual",
