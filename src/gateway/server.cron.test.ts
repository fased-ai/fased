...
async function cleanupCronTestRun(params: {
  ...
  if (params.prevSkipCron === undefined) {
    delete process.env.FASED_SKIP_CRON;
    return;
  }
  process.env.FASED_SKIP_CRON = params.prevSkipCron;
}
...
async function setupCronTestRun(params: {
  ...
  const prevSkipCron = process.env.FASED_SKIP_CRON;
  process.env.FASED_SKIP_CRON = "0";
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), params.tempPrefix));
  testState.cronStorePath = path.join(dir, "cron", "jobs.json");
  testState.sessionConfig = params.sessionConfig;
  testState.sessionStorePath = params.sessionStore
    ? path.join(dir, "sessions", "sessions.json")
    : undefined;
  ...
  if (testState.sessionStorePath) {
    await fs.mkdir(path.dirname(testState.sessionStorePath), { recursive: true });
    await fs.writeFile(testState.sessionStorePath, JSON.stringify({}, null, 2));
  }
...
describe("gateway server cron", () => {
  ...
  test("handles cron CRUD, normalization, and patch semantics", { timeout: 20_000 }, async () => {
    ...
    try {
      ...
      expect((runRes.payload as { ran?: unknown } | null)?.ran).toBe(true);
      await waitForCondition(async () => {
        const runsRes = await rpcReq(ws, "cron.runs", { id: routeJobId, limit: 10 });
        const entries = (runsRes.payload as { entries?: unknown } | null)?.entries;
        return (
          Array.isArray(entries) &&
          entries.some((entry) => {
            const summary = (entry as { summary?: unknown }).summary;
            return typeof summary === "string" && summary.includes("cron route check");
          })
        );
  ...
  test(
    "runs the session-owned task lifecycle through gateway RPC",
    { timeout: 20_000 },
    async () => {
      const { prevSkipCron, dir } = await setupCronTestRun({
        tempPrefix: "fased-gw-cron-task-life-",
        sessionConfig: { mainKey: "main" },
        sessionStore: true,
        cronEnabled: false,
      });
      ...
      try {
        const sessionKey = "agent:main:webchat:direct:lifecycle-smoke";
        const sessionRes = await rpcReq<{ ok: true; key: string; entry?: { label?: string } }>(
          ws,
          "sessions.patch",
          {
            key: sessionKey,
            label: "Lifecycle smoke",
          },
        );
        expect(sessionRes.ok).toBe(true);
        expect(sessionRes.payload?.key).toBe(sessionKey);

        const sessionsRes = await rpcReq<{
          sessions?: Array<{ key: string; label?: string; displayName?: string }>;
        }>(ws, "sessions.list", { includeGlobal: false, includeUnknown: false });
        expect(sessionsRes.ok).toBe(true);
        const sessionRow = sessionsRes.payload?.sessions?.find((row) => row.key === sessionKey);
        expect(sessionRow?.label).toBe("Lifecycle smoke");
        expect(sessionRow?.displayName).toBe("Lifecycle smoke");

        const addRes = await rpcReq<{
          id?: string;
          name?: string;
          agentId?: string;
          sessionKey?: string;
          ...
          executionPolicy?: {
            ...
            memoryScope?: string;
            skillScope?: string;
            modelPolicy?: { mode?: string };
          };
        }>(ws, "cron.add", {
          name: "Lifecycle smoke task",
          enabled: true,
          agentId: "main",
          sessionKey,
          schedule: { kind: "every", everyMs: 60_000 },
          sessionTarget: "isolated",
          wakeMode: "next-heartbeat",
          payload: { kind: "agentTurn", message: "Check task lifecycle" },
          delivery: { mode: "none" },
          executionPolicy: {
        ...
        const listRes = await rpcReq<{
          jobs?: Array<{ id?: string; sessionKey?: string; agentId?: string; name?: string }>;
        }>(ws, "cron.list", { includeDisabled: true, query: "Lifecycle smoke" });
        expect(listRes.ok).toBe(true);
        const listedJob = listRes.payload?.jobs?.find((job) => job.id === jobId);
        ...
        const updateRes = await rpcReq<{
          ...
          executionPolicy?: { executionMode?: string; modelPolicy?: { mode?: string } };
        }>(ws, "cron.update", {
          id: jobId,
          patch: {
            name: "Lifecycle smoke task edited",
            schedule: { kind: "every", everyMs: 120_000 },
            payload: { kind: "agentTurn", message: "Check edited task lifecycle" },
            delivery: { mode: "none" },
            executionPolicy: {
              triggerKind: "schedule",
              executionMode: "no-model",
        ...
        cronIsolatedRun.mockResolvedValueOnce({
          status: "ok",
          summary: "Check edited task lifecycle",
          policy: {
        ...
        await waitForCondition(async () => {
          const runsRes = await rpcReq<{ entries?: Array<{ jobId?: string }> }>(ws, "cron.runs", {
            id: jobId,
            limit: 10,
          });
          return Boolean(
            runsRes.ok && runsRes.payload?.entries?.some((candidate) => candidate.jobId === jobId),
          );
        }, CRON_WAIT_TIMEOUT_MS);
        const runsRes = await rpcReq<{
          entries?: Array<{
            jobId?: string;
            status?: string;
            summary?: string;
        ...
        }>(ws, "cron.runs", { id: jobId, limit: 10 });
        expect(runsRes.ok).toBe(true);
        const runEntry = runsRes.payload?.entries?.find((candidate) => candidate.jobId === jobId);
        ...
        expect(runEntry?.summary).toContain("Check edited task lifecycle");

        const pauseRes = await rpcReq<{ enabled?: boolean }>(ws, "cron.update", {
          id: jobId,
          patch: { enabled: false },
        });
        expect(pauseRes.ok).toBe(true);
        expect(pauseRes.payload?.enabled).toBe(false);

        const resumeRes = await rpcReq<{ enabled?: boolean }>(ws, "cron.update", {
          id: jobId,
          patch: { enabled: true },
        });
        expect(resumeRes.ok).toBe(true);
        expect(resumeRes.payload?.enabled).toBe(true);

        const removeRes = await rpcReq<{ removed?: boolean }>(ws, "cron.remove", { id: jobId });
        expect(removeRes.ok).toBe(true);

        const afterRemoveRes = await rpcReq<{ jobs?: Array<{ id?: string }> }>(ws, "cron.list", {
          includeDisabled: true,
          query: "Lifecycle smoke",
        });
        expect(afterRemoveRes.ok).toBe(true);
        expect(afterRemoveRes.payload?.jobs?.some((job) => job.id === jobId)).toBe(false);
      } finally {
        await cleanupCronTestRun({
          ws,
          server,
  ...
  test("writes task policy telemetry to cron run history", async () => {
    const { prevSkipCron, dir } = await setupCronTestRun({
      tempPrefix: "fased-gw-cron-policy-log-",
      cronEnabled: false,
    });
    ...
    try {
      ...
      type PolicyTelemetry = {
        ...
        memoryScope?: unknown;
        skillScope?: unknown;
        modelPolicyMode?: unknown;
      };
      let policy: PolicyTelemetry | undefined;
      await waitForCondition(async () => {
  ...
  test("posts webhooks for delivery mode and legacy notify fallback only when summary exists", async () => {
    ...
    const { prevSkipCron, dir } = await setupCronTestRun({
      tempPrefix: "fased-gw-cron-webhook-",
      cronEnabled: false,
