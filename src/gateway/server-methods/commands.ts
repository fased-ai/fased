...
import { getPluginCommandSpecs, listPluginCommands } from "../../plugins/commands.js";
...
function mapCommand(params: {
  ...
  const nativeName =
    cmd.scope === "text" ? undefined : (resolveNativeCommandName(cmd, provider) ?? cmd.key);
  return {
...
function buildPluginCommandEntries(params: { nameSurface: CommandNameSurface }): CommandEntry[] {
  const pluginTextSpecs = listPluginCommands();
  const pluginNativeSpecs = getPluginCommandSpecs();

  if (params.nameSurface === "text") {
    return pluginTextSpecs.map((spec) => ({
      name: spec.name,
