...
describe("push.test handler", () => {
  ...
  it("sends push test when registration and auth are available", async () => {
    ...
    const { respond, invoke, logGateway } = createInvokeParams({
      nodeId: "ios-node-1",
      title: "Wake",
      body: "Ping",
    });
    ...
    expect(logGateway.info).toHaveBeenCalledWith(
      expect.stringContaining(
        "security audit: mutating-admin-rpc method=push.test outcome=succeeded",
      ),
    );
    const auditLine = String(logGateway.info.mock.calls.at(-1)?.[0] ?? "");
    expect(auditLine).toContain("nodeId=ios-node-1");
    expect(auditLine).toContain("environment=sandbox");
    expect(auditLine).not.toContain("Wake");
    expect(auditLine).not.toContain("Ping");
    expect(auditLine).not.toContain("abcd");
    expect(auditLine).not.toContain("PRIVATE KEY");
  });
});
