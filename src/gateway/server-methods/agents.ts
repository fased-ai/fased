...
async function resolveAgentWorkspaceFilePath(params: {
  ...
  if (candidateLstat.isSymbolicLink()) {
    ...
    try {
      ...
      return {
        kind: "invalid",
        requestPath,
        reason: error instanceof Error ? error.message : "symlink target escapes workspace root",
      };
    }
  ...
  try {
    ...
    return {
      kind: "invalid",
      requestPath,
      reason: error instanceof Error ? error.message : "file path escapes workspace root",
    };
  }
  return { kind: "ready", requestPath, ioPath: targetReal, workspaceReal };
}
...
function resolveTaskModelsParam(value: unknown):
  | {
      cheapCheck?: string;
      strong?: string;
      escalation?: string;
  ...
  for (const key of ["cheapCheck", "strong", "escalation", "coding", "summarizer"] as const) {
    const resolved = resolveOptionalStringParam(record[key]);
    if (resolved) {
      next[key] = resolved;
    }
  }
...
function resolveModelProvidersParam(value: unknown) {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return undefined;
  }
  const out: NonNullable<Parameters<typeof cleanAgentModelProviders>[0]> = {};
  for (const [providerId, raw] of Object.entries(value as Record<string, unknown>)) {
    if (!raw || typeof raw !== "object" || Array.isArray(raw)) {
      continue;
    }
    const record = raw as Record<string, unknown>;
    const taskModels = resolveTaskModelsParam(record.taskModels);
    out[providerId] = {
      ...(resolveOptionalStringParam(record.profileId)
        ? { profileId: resolveOptionalStringParam(record.profileId) }
        : {}),
      ...(resolveOptionalStringParam(record.primary)
        ? { primary: resolveOptionalStringParam(record.primary) }
        : {}),
  ...
  return cleanAgentModelProviders(out) ?? {};
}
...
export const agentsHandlers: GatewayRequestHandlers = {
  ...
  "agents.update": async ({ params, respond }) => {
    ...
    const activeModelProvider =
      "activeModelProvider" in params
        ? resolveOptionalStringParam(params.activeModelProvider) || null
        : undefined;
    const modelProviders =
      "modelProviders" in params ? resolveModelProvidersParam(params.modelProviders) : undefined;
    const taskModels =
      "taskModels" in params ? resolveTaskModelsParam(params.taskModels) : undefined;
    const avatar = resolveOptionalStringParam(params.avatar);
    ...
    const nextConfig = applyAgentConfig(cfg, {
      ...
      ...(activeModelProvider !== undefined ? { activeModelProvider } : {}),
      ...(modelProviders !== undefined ? { modelProviders } : {}),
      ...(model ? { model } : {}),
      ...(taskModels !== undefined ? { taskModels } : {}),
    });
