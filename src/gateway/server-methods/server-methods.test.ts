...
describe("exec approval handlers", () => {
  ...
  type ExecApprovalListArgs = Parameters<ExecApprovalHandlers["exec.approval.list"]>[0];
  ...
  async function listExecApprovals(params: {
    ...
    return params.handlers["exec.approval.list"]({
      params: {},
      respond: params.respond as unknown as ExecApprovalListArgs["respond"],
      context: {} as ExecApprovalListArgs["context"],
      client: null,
  ...
  it("lists and gets pending approval records without resolving them", async () => {
    const { handlers, respond, context } = createExecApprovalFixture();
    ...
    expect(respond).toHaveBeenCalledWith(
      true,
      expect.objectContaining({ status: "accepted", id: "approval-123" }),
      undefined,
    );
    ...
    await getExecApproval({ handlers, id: "approval-123", respond: getRespond });
    ...
    const listAfterResolveRespond = vi.fn();
    await listExecApprovals({ handlers, respond: listAfterResolveRespond });
    expect(listAfterResolveRespond).toHaveBeenCalledWith(true, [], undefined);

    const getAfterResolveRespond = vi.fn();
    await getExecApproval({ handlers, id: "approval-123", respond: getAfterResolveRespond });
    expect(getAfterResolveRespond).toHaveBeenCalledWith(
      false,
      undefined,
...
describe("logs.tail", () => {
  ...
  it("falls back to latest rolling log file when today is missing", async () => {
    const tempDir = await fsPromises.mkdtemp(path.join(os.tmpdir(), "fased-logs-"));
    const older = path.join(tempDir, "fased-2026-01-20.log");
    const newer = path.join(tempDir, "fased-2026-01-21.log");
    ...
    setLoggerOverride({ file: path.join(tempDir, "fased-2026-01-22.log") });
