...
describe("gateway agent handler", () => {
  ...
  it("registers active agent RPC runs for chat.abort", async () => {
    primeMainAgentRun();
    ...
    const runId = "test-agent-rpc-abort-register";
    await invokeAgent(
      {
        message: "long task",
        agentId: "main",
    ...
    expect(active?.sessionId).toBe("existing-session-id");
    expect(active?.controller.signal.aborted).toBe(false);
    expect(capturedSignal).toBe(active?.controller.signal);
  });

  it("chat.abort by runId aborts an active agent RPC run and clears its entry", async () => {
    primeMainAgentRun();
    let capturedSignal: AbortSignal | undefined;
    mocks.agentCommand.mockImplementationOnce((opts: { abortSignal?: AbortSignal }) => {
      capturedSignal = opts.abortSignal;
    ...
    const context = makeContext();
    const runId = "test-agent-rpc-abort-runid";
    await invokeAgent(
      {
        message: "long task",
        agentId: "main",
    ...
    await vi.waitFor(() => expect(context.chatAbortControllers.has(runId)).toBe(true));
  ...
  it("cleans active agent RPC abort entries after successful completion", async () => {
    primeMainAgentRun();
    ...
    const context = makeContext();
    const runId = "test-agent-rpc-abort-cleanup";
    await invokeAgent(
      {
        message: "short task",
        agentId: "main",
