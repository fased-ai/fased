...
function serializeExecApprovalRecord(
  record: NonNullable<ReturnType<ExecApprovalManager["getSnapshot"]>>,
) {
  return {
    id: record.id,
...
export function createExecApprovalHandlers(
  ...
  return {
    "exec.approval.get": async ({ params, respond }) => {
      ...
      respond(true, serializeExecApprovalRecord(snapshot), undefined);
    },
    "exec.approval.list": async ({ params, respond }) => {
      ...
      respond(true, manager.listPendingRecords().map(serializeExecApprovalRecord), undefined);
    },
