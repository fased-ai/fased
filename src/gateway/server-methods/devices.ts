...
function shouldReturnRotatedDeviceToken(params: {
  callerDeviceId?: string;
  targetDeviceId: string;
}): boolean {
  const callerDeviceId = params.callerDeviceId?.trim();
  const targetDeviceId = params.targetDeviceId.trim();
  return Boolean(callerDeviceId && targetDeviceId && callerDeviceId === targetDeviceId);
}
  ...
  "device.token.rotate": async ({ params, respond, context, client }) => {
    ...
    respond(
      true,
      {
        deviceId,
        role: entry.role,
        ...(shouldReturnRotatedDeviceToken({
          callerDeviceId: client?.connect?.device?.id,
