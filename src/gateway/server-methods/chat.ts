...
function extractAssistantHistoryText(message: unknown): string | undefined {
  if (!message || typeof message !== "object") {
    return undefined;
  }
  const entry = message as { role?: unknown; text?: unknown; content?: unknown };
...
function shouldDropAssistantHistoryMessage(message: unknown): boolean {
  ...
  const text = extractAssistantHistoryText(message);
...
function buildOversizedHistoryPlaceholder(message?: unknown): Record<string, unknown> {
  ...
  return {
    ...
    __fased: { truncated: true, reason: "oversized" },
  };
}
...
function appendAssistantTranscriptMessage(params: {
  ...
  if (appended.ok && appended.message) {
    emitSessionTranscriptUpdate({
      ...
      ...(typeof appended.messageId === "string" ? { messageId: appended.messageId } : {}),
    });
  }
  return appended;
}
...
export const chatHandlers: GatewayRequestHandlers = {
  ...
  "chat.send": async ({ params, respond, context, client }) => {
    ...
    try {
      ...
      // Regression guard for short-lived reconnect ordering.
  ...
  "chat.inject": async ({ params, respond, context, client }) => {
    ...
    const target = resolveGatewaySessionStoreTarget({ cfg, key: rawSessionKey });
    ...
    const entry = storeKey ? store[storeKey] : undefined;
    const storePath = target.storePath;
    const sessionKey = target.canonicalKey;
    const sessionId = entry?.sessionId;
    if (!sessionId || !storePath) {
      logMutatingAdminRpcAudit({
        ...
        method: "chat.inject",
        outcome: "denied",
        details: { sessionKey: rawSessionKey, reason: "session_not_found" },
      });
    ...
    if (!appended.ok || !appended.messageId || !appended.message) {
      logMutatingAdminRpcAudit({
        ...
        method: "chat.inject",
        outcome: "failed",
        details: { sessionKey, reason: "transcript_write_failed" },
      });
      respond(
        false,
        undefined,
    ...
    logMutatingAdminRpcAudit({
      ...
      method: "chat.inject",
      outcome: "succeeded",
      details: { sessionKey, messageId: appended.messageId },
    });
