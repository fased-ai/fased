...
const mocks = vi.hoisted(() => ({
  ...
  isWorkspaceOnboardingCompleted: vi.fn(async () => false),
...
vi.mock("../../agents/workspace.js", async () => {
  ...
  return {
    ...
    isWorkspaceOnboardingCompleted: mocks.isWorkspaceOnboardingCompleted,
  };
});
...
function mockWorkspaceStateRead(params: {
  ...
  if (params.errorCode) {
    mocks.isWorkspaceOnboardingCompleted.mockRejectedValueOnce(createErrnoError(params.errorCode));
    return;
  }
  if (typeof params.rawContent === "string") {
    mocks.isWorkspaceOnboardingCompleted.mockResolvedValueOnce(false);
    return;
  }
  mocks.isWorkspaceOnboardingCompleted.mockResolvedValueOnce(
    Boolean(params.onboardingCompletedAt?.trim()),
  );
...
beforeEach(() => {
  ...
  mocks.isWorkspaceOnboardingCompleted.mockResolvedValue(false);
...
describe("agents.update", () => {
  ...
  it("updates per-agent task model roles", async () => {
    const { promise } = makeCall("agents.update", {
      agentId: "test-agent",
      taskModels: {
