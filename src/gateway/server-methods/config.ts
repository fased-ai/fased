...
function parseValidateConfigFromRawOrRespond(
  ...
  snapshot: Awaited<ReturnType<typeof readConfigFileSnapshot>>,
  respond: RespondFn,
): { config: FasedAgentConfig; schema: ConfigSchemaResponse } | null {
...
function loadSchemaWithPlugins(): ConfigSchemaResponse {
  ...
  // Schema reads must not activate plugin runtime state. Use manifest metadata
  // only so config.schema/config.get can run repeatedly without loading or
  // replacing the active plugin registry.
...
export const configHandlers: GatewayRequestHandlers = {
  ...
  "config.schema.lookup": ({ params, respond, context }) => {
    ...
    const schema = loadSchemaWithPlugins();
    const result = lookupConfigSchema(schema, (params as { path: string }).path);
    if (!result) {
      respond(
        false,
        undefined,
    ...
    if (!validateConfigSchemaLookupResult(result)) {
      respond(
        ...
        errorShape(ErrorCodes.UNAVAILABLE, "config.schema.lookup returned invalid payload", {
          details: { errors: validateConfigSchemaLookupResult.errors ?? [] },
        }),
      );
      context.logGateway.warn(
        `config.schema.lookup returned invalid payload for path=${(params as { path: string }).path}`,
      );
      return;
    }
