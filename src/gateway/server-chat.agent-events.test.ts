...
import { createSessionEventSubscriberRegistry } from "./session-event-subscribers.js";
...
describe("agent event handler", () => {
  ...
  it("broadcasts redacted session.tool events to session subscribers", () => {
    const { broadcastToConnIds, sessionEventSubscribers, handler } = createHarness({
      resolveSessionKeyForRun: () => "session-tool-subscriber",
    });

    sessionEventSubscribers.subscribe("conn-session");
    registerAgentRunContext("run-tool-session", {
      sessionKey: "session-tool-subscriber",
      verboseLevel: "on",
    });
  ...
  it("broadcasts sessions.changed lifecycle events to session subscribers", () => {
    const { broadcastToConnIds, sessionEventSubscribers, handler } = createHarness({
      resolveSessionKeyForRun: () => "session-lifecycle",
    });
    sessionEventSubscribers.subscribe("conn-session");

    emitFallbackLifecycle({ handler, runId: "run-lifecycle" });

    expect(broadcastToConnIds).toHaveBeenCalledWith(
      "sessions.changed",
      expect.objectContaining({
        sessionKey: "agent:main:session-lifecycle",
        phase: "fallback",
        runId: "run-lifecycle",
