...
describe("runPaidFederatedContentSummarize", () => {
  ...
  it("explains locked split-key Agent wallet payment failures", async () => {
    ...
    const result = await runPaidFederatedContentSummarize(
      ...
      },
      {
        ...
        readWalletProviderRegistry: () => ({
          version: 1,
          providers: {
            "embedded-keystore": { enabled: false, updatedAt: "2026-04-09T00:00:00.000Z" },
            "local-socket-signer": { enabled: true, updatedAt: "2026-04-09T00:00:00.000Z" },
    ...
    expect(result.reason).toContain("Buyer Agent wallet is locked by split-key custody");
    expect(result.reason).toContain("No funds were moved");
    expect(result.reason).not.toContain("provide a passkey approval token");
  });
