...
describe("gateway auth browser hardening", () => {
  ...
  test("rejects non-local browser origins for non-control-ui clients even with proxy headers", async () => {
    testState.gatewayAuth = { mode: "token", token: "secret" };
    await withGatewayServer(async ({ port }) => {
  ...
  test("accepts hosted Tailscale dashboard session token when websocket upgrade omits forwarded host", async () => {
    testState.gatewayAuth = { mode: "token", token: "secret" };
    testState.gatewayControlUi = { allowInsecureAuth: true };
    const { writeConfigFile } = await import("../config/config.js");
    await writeConfigFile({
  ...
  test("accepts hosted dashboard session token under default Control UI device policy", async () => {
    testState.gatewayAuth = { mode: "token", token: "secret" };
    const { writeConfigFile } = await import("../config/config.js");
    await writeConfigFile({
      gateway: {
    ...
    await withGatewayServer(async ({ port }) => {
      ...
      try {
        ...
        const { identity, device } = await createSignedDevice({
          ...
          identityPath: path.join(os.tmpdir(), `fased-browser-device-${randomUUID()}.json`),
          nonce: String(nonce ?? ""),
        });
