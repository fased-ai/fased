...
function resolveGatewayAuthFromConfig(params: {
  ...
  const hasExplicitAuth =
    authConfig.mode !== undefined ||
    (typeof authConfig.token === "string" && authConfig.token.trim().length > 0) ||
    (typeof authConfig.password === "string" && authConfig.password.trim().length > 0) ||
    authConfig.trustedProxy !== undefined;
  if (!hasExplicitAuth) {
...
function shouldPersistGeneratedToken(params: {
  ...
  if (params.authOverride?.mode !== undefined) {
    return false;
  }
