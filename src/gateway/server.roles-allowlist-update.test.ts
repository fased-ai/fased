...
const connectNodeClient = async (params: {
  ...
  const token = process.env.FASED_GATEWAY_TOKEN;
  if (!token) {
    throw new Error("FASED_GATEWAY_TOKEN is required for node test clients");
  }
...
describe("gateway node command allowlist", () => {
  ...
  test("rejects reconnect metadata spoof for paired node devices", async () => {
    ...
    const deviceIdentityPath = path.join(
      os.tmpdir(),
      `fased-spoof-test-device-${Date.now()}-${Math.random().toString(36).slice(2)}.json`,
    );
