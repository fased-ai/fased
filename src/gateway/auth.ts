...
import { resolveControlUiSessionCookie } from "./server-http.js";

export type ResolvedGatewayAuthMode = "none" | "token" | "password" | "trusted-proxy";

export type ResolvedGatewayAuth = {
  mode: ResolvedGatewayAuthMode;
  modeSource?: "config" | "env" | "default";
  token?: string;
  password?: string;
...
type SessionTokenVerifier = (params: {
  token: string;
  req?: IncomingMessage;
...
export function isLocalDirectRequest(
  ...
  // Local connections (loopback) are trusted if there are no proxy headers,
  ...
  // If we're behind a trusted proxy, a connection from the proxy to the gateway
  // (often on 127.0.0.1 or a LAN IP) is considered local if the proxy source itself is trusted.
  if (isTrustedProxyAddress(remoteAddr, trustedProxies)) {
    return true;
  }
  if (allowRealIpFallback && isLoopbackAddress(remoteAddr)) {
    return true;
  }
  return false;
}
...
export function resolveGatewayAuth(params: {
  ...
  const token = authConfig.token ?? env.FASED_GATEWAY_TOKEN ?? undefined;
  const password = authConfig.password ?? env.FASED_GATEWAY_PASSWORD ?? undefined;
...
export function assertGatewayAuthConfigured(auth: ResolvedGatewayAuth): void {
  if (auth.mode === "token" && !auth.token) {
    ...
    throw new Error(
      "gateway auth mode is token, but no token was configured (set gateway.auth.token or FASED_GATEWAY_TOKEN)",
    );
  }
...
export { authorizeGatewayConnect as authorizeHttpGatewayConnect };
export { authorizeGatewayConnect as authorizeWsControlUiGatewayConnect };
...
export async function authorizeGatewayConnect(params: {
  ...
  clientIp?: string;
  /** Allow fallback to req.socket.remoteAddress if proxy-aware resolution fails. */
  allowRealIpFallback?: boolean;
  ...
  const trySessionTokenAuth = async (): Promise<GatewayAuthResult | null> => {
    if (sessionAuthResolved !== undefined) {
      return sessionAuthResolved;
    }
    const token = connectAuth?.token?.trim() || (req ? resolveControlUiSessionCookie(req) : "");
    if (!token || !auth.sessionTokenVerifier) {
      sessionAuthResolved = null;
      return null;
    }
    const verified = await auth.sessionTokenVerifier({
      token,
      req,
      trustedProxies,
    });
  ...
  if (auth.mode === "token") {
    if (!auth.token) {
      const sessionAuth = await trySessionTokenAuth();
      if (sessionAuth) {
        return sessionAuth;
      }
