...
async function readUntilFirstSseDataLine(
  reader: ReadableStreamDefaultReader<Uint8Array>,
  decoder: TextDecoder,
): Promise<{ data: string; text: string }> {
  let text = "";
  for (;;) {
    const chunk = await withTimeout(reader.read(), 3_000, "waiting for first SSE data line");
    if (chunk.done) {
      throw new Error("stream ended before first SSE data line");
    }
    text += decoder.decode(chunk.value, { stream: true });
    const [first] = parseSseDataLines(text);
    if (first) {
...
async function readRemainingStreamText(
  ...
  let text = "";
  for (;;) {
    const chunk = await withTimeout(reader.read(), 3_000, "waiting for SSE stream completion");
    if (chunk.done) {
    ...
    try {
...

      {
        await expectAgentSessionKeyMatch({
          body: { model: "fased", messages: [{ role: "user", content: "hi" }] },
          headers: { "x-fased-agent-id": "beta" },
          matcher: /^agent:beta:/,
        });
      }
...

      {
        await expectAgentSessionKeyMatch({
          ...
          headers: { "x-fased-agent-id": "alpha" },
          matcher: /^agent:alpha:/,
        });
      }

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "fased", messages: [{ role: "user", content: "hi" }] },
          {
            "x-fased-agent-id": "beta",
            "x-fased-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
...

      {
        ...
        const choice0 = (json.choices as Array<Record<string, unknown>>)[0] ?? {};
        const msg = (choice0.message as Record<string, unknown> | undefined) ?? {};
        expect(msg.content).toBe("No response from FasedAgent.");
      }
    ...
    try {
      {
        ...
        const agentResult = new Promise<unknown>((resolve) => {
          resolveAgent = resolve;
        });
        const finishAgent = () => {
          if (!resolvedAgent) {
            resolvedAgent = true;
            resolveAgent({ payloads: [{ text: "fallback hello" }] });
          }
        };
        agentCommand.mockClear();
        agentCommand.mockImplementationOnce((async () => await agentResult) as never);
        ...
        try {
          ...
          const first = await readUntilFirstSseDataLine(reader, decoder);
          const firstChunk = JSON.parse(first.data) as Record<string, unknown>;
          const firstChoice =
            ((firstChunk.choices as Array<Record<string, unknown>> | undefined) ?? [])[0] ?? {};
          expect(firstChunk.object).toBe("chat.completion.chunk");
          expect(firstChoice.finish_reason).toBeNull();
          expect(firstChoice.delta).toEqual({ role: "assistant" });

          finishAgent();
          const text = first.text + (await readRemainingStreamText(reader, decoder));
          finishedStream = true;
          ...
          const allContent = jsonChunks
            ...
            .join("");
          expect(allContent).toBe("fallback hello");
        } finally {
          finishAgent();
          if (reader && !finishedStream) {
...

      {
        ...
        const firstChoice =
          ((jsonChunks[0]?.choices as Array<Record<string, unknown>> | undefined) ?? [])[0] ?? {};
        expect(firstChoice.finish_reason).toBeNull();
        expect(firstChoice.delta).toEqual({ role: "assistant" });
        const allContent = jsonChunks
          .slice(1)
