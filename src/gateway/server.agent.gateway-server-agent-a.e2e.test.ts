...
function lastAgentCommandCall(): Record<string, unknown> {
  const calls = (agentCommand as unknown as { mock: { calls: unknown[][] } }).mock.calls;
...
describe("gateway server agent", () => {
  test("agent marks implicit delivery when lastTo is stale", async () => {
    ...
    const call = lastAgentCommandCall();
