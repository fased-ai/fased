...
type StartMode = "hello" | "close" | "connect-error-close" | "silent";
let startMode: StartMode = "hello";
let closeCode = 1006;
let closeReason = "";
let connectError = new Error("connect failed");
...
describe("buildGatewayConnectionDetails", () => {
  ...
  it("throws for insecure ws:// remote URLs (CWE-319)", () => {
    ...
    expect((thrown as Error).message).toContain("fased doctor --fix");
  });
...
describe("callGateway error details", () => {
  ...
  it("includes connection details when the gateway closes", async () => {
    ...
    expect((err as { kind?: unknown }).kind).toBe("closed");
  });
  ...
  it("marks timeout failures as gateway transport errors", async () => {
    startMode = "silent";
    setLocalLoopbackGatewayConfig();
    ...
    expect(isGatewayTransportError(err)).toBe(true);
    expect((err as { kind?: unknown }).kind).toBe("timeout");
  });

  it("surfaces gateway connect auth errors without transport fallback classification", async () => {
    startMode = "connect-error-close";
    connectError = Object.assign(new Error("unauthorized: gateway token mismatch"), {
      name: "GatewayClientRequestError",
    });
    setLocalLoopbackGatewayConfig();

    let err: unknown;
    await callGateway({ method: "health", timeoutMs: 10 }).catch((caught) => {
      err = caught;
    });

    expect(err).toBe(connectError);
    expect(isGatewayTransportError(err)).toBe(false);
  });
...
describe("callGateway url override auth requirements", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["FASED_GATEWAY_TOKEN", "FASED_GATEWAY_PASSWORD"]);
    resetGatewayCallMocks();
  ...
  it("throws when url override is set without explicit credentials", async () => {
    ...
    process.env.FASED_GATEWAY_PASSWORD = "env-password";
    loadConfig.mockReturnValue({
      gateway: {
        mode: "local",
...
describe("callGateway password resolution", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["FASED_GATEWAY_PASSWORD", "FASED_GATEWAY_TOKEN"]);
    resetGatewayCallMocks();
  ...
  it.each([
    ...
    {
      label: "prefers local config password over env password for backend self-calls",
    ...
    if (envPassword !== undefined) {
      process.env.FASED_GATEWAY_PASSWORD = envPassword;
    }
  ...
  it("keeps CLI calls env-first for local gateway credentials", async () => {
    process.env.FASED_GATEWAY_TOKEN = "env-token";
    process.env.FASED_GATEWAY_PASSWORD = "env-password";
    loadConfig.mockReturnValue({
      gateway: {
    ...
    expect(lastClientOptions?.password).toBe("env-password");
  });

  it("prefers local config token over stale env token for backend self-calls", async () => {
    process.env.FASED_GATEWAY_TOKEN = "stale-env-token";
    loadConfig.mockReturnValue({
      gateway: {
        mode: "local",
        bind: "loopback",
