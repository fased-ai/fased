...
beforeAll(async () => {
  previousToken = process.env.FASED_GATEWAY_TOKEN;
...
afterAll(async () => {
  ...
  if (previousToken === undefined) {
    ...
    process.env.FASED_GATEWAY_TOKEN = previousToken;
  }
});
...
describe("gateway server health/presence", () => {
  ...
  test("agent events stream with seq", { timeout: 8000 }, async () => {
    ...
    const payload = evt.payload as { runId?: string; data?: { msg?: unknown } };
    expect(payload.runId).toBe(runId);
    expect(typeof evt.seq).toBe("number");
    expect(payload.data?.msg).toBe("hi");
  ...
  test("presence broadcast reaches multiple clients", { timeout: 8000 }, async () => {
    ...
    for (const evt of events) {
      const presence = (evt.payload as { presence?: unknown[] } | undefined)?.presence ?? [];
  ...
  test("presence includes client fingerprint", async () => {
    const identityPath = path.join(os.tmpdir(), `fased-device-${randomUUID()}.json`);
    const identity = loadOrCreateDeviceIdentity(identityPath);
    const token = process.env.FASED_GATEWAY_TOKEN?.trim() || undefined;
    const role = "operator";
    const scopes: string[] = ["operator.admin"];
    const signedAtMs = Date.now();
    const nonce = "nonce-fingerprint";
  ...
  test("cli connections are not tracked as instances", async () => {
    ...
    const entries = presenceRes.payload as unknown as Array<Record<string, unknown>>;
