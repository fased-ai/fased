...
import {
  ...
  taskExplicitlyUsesWebSearch,
} from "../cron/access-block.js";
import {
  deliverCronIsolatedAgentTurnResult,
  runCronGraphDataNode,
...
function buildWebSearchAccessBlock(params: {
  provider: string;
  detectedAtMs: number;
}): CronTaskAccessBlock {
  if (params.provider === "brave") {
    return missingBraveSearchAccessBlock({
      source: "preflight",
      detectedAtMs: params.detectedAtMs,
    });
  }
  return {
    code: `missing_${params.provider}_api_key`,
    service: "web_search",
    reason: `Missing ${params.provider} API key for web_search.`,
...
function resolveWebSearchPreflightBlock(
  cfg: ReturnType<typeof loadConfig>,
): CronTaskAccessBlock | undefined {
  const search = cfg.tools?.web?.search;
  const provider =
    typeof search?.provider === "string" && search.provider.trim()
      ? search.provider.trim().toLowerCase()
      : "brave";
  if (provider === "perplexity") {
    const perplexity = search?.perplexity as { apiKey?: unknown } | undefined;
    const ready = Boolean(
      normalizeSecretInput(perplexity?.apiKey) ||
      normalizeSecretInput(process.env.PERPLEXITY_API_KEY) ||
      normalizeSecretInput(process.env.OPENROUTER_API_KEY),
    );
    return ready ? undefined : buildWebSearchAccessBlock({ provider, detectedAtMs: Date.now() });
  }
  if (provider === "grok") {
    const grok = search?.grok as { apiKey?: unknown } | undefined;
    const ready = Boolean(
      normalizeSecretInput(grok?.apiKey) || normalizeSecretInput(process.env.XAI_API_KEY),
    );
    return ready ? undefined : buildWebSearchAccessBlock({ provider, detectedAtMs: Date.now() });
  }
  if (provider === "gemini") {
    const gemini = search?.gemini as { apiKey?: unknown } | undefined;
    const ready = Boolean(
      normalizeSecretInput(gemini?.apiKey) || normalizeSecretInput(process.env.GEMINI_API_KEY),
    );
    return ready ? undefined : buildWebSearchAccessBlock({ provider, detectedAtMs: Date.now() });
  }
  if (provider === "kimi") {
    const kimi = search?.kimi as { apiKey?: unknown } | undefined;
    const ready = Boolean(
  ...
  const ready = Boolean(
    normalizeSecretInput(search?.apiKey) || normalizeSecretInput(process.env.BRAVE_API_KEY),
  );
  return ready
    ? undefined
    : buildWebSearchAccessBlock({ provider: "brave", detectedAtMs: Date.now() });
}

function resolveCheapCheckModelRolePreflightBlock(params: {
  cfg: ReturnType<typeof loadConfig>;
  job: CronJob;
}): CronTaskAccessBlock | undefined {
  if (params.job.executionPolicy?.planner?.strategy !== "cheap-model") {
    return undefined;
  }
  if (taskExplicitModelRef(params.job)) {
    return undefined;
  }
  const role = resolveTaskModelRole({
    cfg: params.cfg,
...
export function buildGatewayCronService(params: {
  ...
  const cronEnabled = process.env.FASED_SKIP_CRON !== "1" && params.cfg.cron?.enabled !== false;
  ...
  const cron = new CronService({
    ...
    runIsolatedAgentJob: async ({ job, message, abortSignal, deferDelivery, graphContext }) => {
    ...
    runGraphNodeHandler: async ({ job, message, nodeId, nodeKind, graphContext, abortSignal }) => {
      const { agentId, cfg: runtimeConfig } = resolveCronAgent(job.agentId);
      return await runCronGraphDataNode({
        cfg: runtimeConfig,
        job,
        message,
        nodeId,
    ...
    deliverIsolatedAgentJobResult: async ({ job, runId, result, abortSignal }) => {
      const { agentId, cfg: runtimeConfig } = resolveCronAgent(job.agentId);
      return await deliverCronIsolatedAgentTurnResult({
        cfg: runtimeConfig,
        deps: params.deps,
        job,
        runId,
    ...
    preflightJobAccess: (job) => {
      const runtimeConfig = loadConfig();
      const modelRoleBlock = resolveCheapCheckModelRolePreflightBlock({
        cfg: runtimeConfig,
        job,
      });
      if (modelRoleBlock) {
      ...
      return taskExplicitlyUsesWebSearch(job)
        ? resolveWebSearchPreflightBlock(runtimeConfig)
        : undefined;
    },
