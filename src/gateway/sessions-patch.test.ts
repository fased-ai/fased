...
async function applySubagentModelPatch(cfg: FasedAgentConfig) {
...
describe("gateway sessions patch", () => {
  ...
  test("accepts model refs from authenticated providers even when the model allowlist is narrow", async () => {
    ...
    const res = await applySessionsPatchToStore({
      ...
      store,
      storeKey: "agent:main:main",
      patch: { key: "agent:main:main", model: "openai/gpt-5.5" },
      loadGatewayModelCatalog: async () => [
  ...
  test("accepts standard signed-in provider model refs when the gateway catalog is stale", async () => {
    ...
    const res = await applySessionsPatchToStore({
      ...
      store,
      storeKey: "agent:main:main",
      patch: { key: "agent:main:main", model: "openrouter/mistralai/mistral-small-2603" },
      loadGatewayModelCatalog: async () => [
        { provider: "openrouter", id: "openrouter/auto", name: "OpenRouter Auto" },
      ],
