...
const { createFasedAgentTools } = await import("../agents/fased-tools.js");
...
type SessionSendTool = ReturnType<typeof createFasedAgentTools>[number];
...
beforeAll(async () => {
  envSnapshot = captureEnv(["FASED_GATEWAY_PORT", "FASED_GATEWAY_TOKEN"]);
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("FASED_CONFIG_PATH missing in gateway test environment");
      }
