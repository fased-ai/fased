...
beforeAll(async () => {
  prevGatewayPort = process.env.FASED_GATEWAY_PORT;
  prevGatewayToken = process.env.FASED_GATEWAY_TOKEN;
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.FASED_GATEWAY_PORT = String(gatewayPort);
  process.env.FASED_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send gateway loopback", () => {
  it("returns reply when lifecycle ends before agent.wait", async () => {
    ...
    (
      spy as unknown as { mockImplementation: (fn: (opts: unknown) => Promise<void>) => void }
    ...
    const tool = createFasedAgentTools().find((candidate) => candidate.name === "sessions_send");
    if (!tool) {
    ...
    const firstCall = (spy.mock.calls as unknown as [unknown][])[0]?.[0] as
