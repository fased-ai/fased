...
export function drainNodePendingWork(
  nodeId: string,
  opts: { maxItems?: number; includeDefaultStatus?: boolean; nowMs?: number } = {},
): { revision: number; items: NodePendingWorkItem[]; hasMore: boolean } {
...
export function acknowledgeNodePendingWork(params: { nodeId: string; itemIds: string[] }): {
  ...
  const nodeId = params.nodeId.trim();
  if (!nodeId) {
    return { revision: 0, removedItemIds: [], remainingCount: 0 };
  }
  ...
  const remainingCount = state.itemsById.size;
  pruneStateIfEmpty(nodeId, state);
  return { revision: state.revision, removedItemIds, remainingCount };
}
