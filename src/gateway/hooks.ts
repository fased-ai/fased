...
export function resolveHooksConfig(cfg: FasedAgentConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: FasedAgentConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-fased-token"] === "string" ? req.headers["x-fased-token"].trim() : "";
  if (headerToken) {
    return headerToken;
  }
  return undefined;
...
export type HookWorkflowDispatchPayload = {
...
const VALID_HOOK_NOTIFY_POLICIES = new Set<TaskNotifyPolicy>([
  "silent",
  "done_only",
  "state_changes",
]);
...
export function normalizeAgentPayload(payload: Record<string, unknown>):
  ...
  const triggerIdRaw = payload.triggerId;
  const triggerId =
    typeof triggerIdRaw === "string" && triggerIdRaw.trim() ? triggerIdRaw.trim() : undefined;
  ...
  const notifyPolicyRaw = payload.notifyPolicy;
  const notifyPolicy =
    typeof notifyPolicyRaw === "string" &&
    VALID_HOOK_NOTIFY_POLICIES.has(notifyPolicyRaw as TaskNotifyPolicy)
      ? (notifyPolicyRaw as TaskNotifyPolicy)
      : undefined;
  if (notifyPolicyRaw !== undefined && !notifyPolicy) {
    return { ok: false, error: "notifyPolicy must be silent|done_only|state_changes" };
  }
