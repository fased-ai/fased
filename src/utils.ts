...
export function resolveConfigDir(
  ...
  const override = env.FASED_STATE_DIR?.trim() || env.FASED_CONFIG_DIR?.trim();
  if (override) {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.FASED_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$FASED_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via FASED_STATE_DIR or FASED_CONFIG_DIR.
