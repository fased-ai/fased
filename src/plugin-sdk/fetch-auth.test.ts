...
describe("fetchWithBearerAuthScopeFallback", () => {
  it("normalizes request headers before retrying with bearer auth", async () => {
    ...
    const firstHeaders = fetchFn.mock.calls[0]?.[1]?.headers as object;
    const secondHeaders = fetchFn.mock.calls[1]?.[1]?.headers as Headers;
    expect(new Headers(firstHeaders as HeadersInit).get("x-trace")).toBe("1");
    expect(Object.getOwnPropertySymbols(firstHeaders)).toHaveLength(0);
    expect(secondHeaders.get("authorization")).toBe("Bearer token-1");
    expect(secondHeaders.get("x-trace")).toBe("1");
  });
