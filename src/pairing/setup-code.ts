...
function resolveGatewayPort(cfg: FasedAgentConfig, env: NodeJS.ProcessEnv): number {
...
function resolveAuth(cfg: FasedAgentConfig, env: NodeJS.ProcessEnv): ResolveAuthResult {
  const mode = cfg.gateway?.auth?.mode;
  const token = env.FASED_GATEWAY_TOKEN?.trim() || cfg.gateway?.auth?.token?.trim();
  const password = env.FASED_GATEWAY_PASSWORD?.trim() || cfg.gateway?.auth?.password?.trim();
