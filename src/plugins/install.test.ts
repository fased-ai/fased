...
vi.mock("../process/exec.js", () => execMock);
...
let skillScanner: typeof import("../security/skill-scanner.js");
    ...
    JSON.stringify(
      {
        ...
        fased: { extensions: params.extensions },
      },
      null,
      2,
    ),
...
beforeAll(async () => {
  ...
  skillScanner = await import("../security/skill-scanner.js");
});
...
describe("installPluginFromArchive", () => {
  ...
  it("rejects packages without fased.extensions", async () => {
    const result = await installArchivePackageAndReturnResult({
      packageJson: { name: "@fased/nope", version: "0.0.1" },
      outName: "bad.tgz",
    });
    expect(result.ok).toBe(false);
    if (result.ok) {
  ...
  it("installs packages with fased.extensions", async () => {
    const result = await installArchivePackageAndReturnResult({
      packageJson: {
    ...
    expect(result.pluginId).toBe("fased-plugin-demo");
    expect(result.extensions).toEqual(["./dist/index.js"]);
  });

  it("infers package entries from fased.plugin.json and package main", async () => {
  ...
  it("accepts legacy package ids from expected plugin aliases", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    fs.mkdirSync(path.join(pluginDir, "dist"), { recursive: true });
    fs.writeFileSync(
      path.join(pluginDir, "package.json"),
    ...
    const result = await installPluginFromDir({
      ...
      expectedPluginIds: ["fased-plugin-yuanbao"],
      logger: { info: () => {}, warn: () => {} },
    });
...
describe("installPluginFromDir", () => {
  it("reports source-only TypeScript extension entries in dry-run package review", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    fs.mkdirSync(path.join(pluginDir, "src"), { recursive: true });
    fs.writeFileSync(
      path.join(pluginDir, "package.json"),
      JSON.stringify({
        name: "@fased/source-only-plugin",
        version: "1.0.0",
        fased: { extensions: ["./src/index.ts"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(path.join(pluginDir, "src", "index.ts"), "export {};", "utf-8");
    ...
    expect(result.review?.runtimeWarnings).toEqual([
  ...
  it("strips workspace devDependencies before npm install", async () => {
    ...
    expect(manifest.devDependencies?.fased).toBeUndefined();
    expect(manifest.devDependencies?.vitest).toBe("^3.0.0");
  });

  it("uses fased.plugin.json id as install key when it differs from package name", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    fs.mkdirSync(path.join(pluginDir, "dist"), { recursive: true });
    fs.writeFileSync(
      path.join(pluginDir, "package.json"),
      JSON.stringify({
        name: "@fased/cognee-fased",
        version: "0.0.1",
        fased: { extensions: ["./dist/index.js"] },
      }),
      "utf-8",
    ...
    expect(
      infoMessages.some((msg) =>
        msg.includes(
          'Plugin manifest id "memory-cognee" differs from npm package name "cognee-fased"',
        ),
      ),
    ).toBe(true);
  });
...
describe("installPluginFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@fased/voice-call@0.0.1");
    expect(result.npmResolution?.integrity).toBe("sha512-plugin-test");

    expectSingleNpmPackIgnoreScriptsCall({
      calls: run.mock.calls,
      expectedSpec: "@fased/voice-call@0.0.1",
    });
