...
export function formatIMessageTargetForLog(raw: string): string {
  try {
    const target = parseIMessageTarget(raw);
    switch (target.kind) {
      case "chat_id":
        return "chat_id:<redacted>";
      case "chat_guid":
        return "chat_guid:<redacted>";
      case "chat_identifier":
        return "chat_identifier:<redacted>";
      case "handle":
        return `${target.service}:<redacted>`;
    }
  } catch {
    return "<redacted>";
  }
