...
describe("deliverReplies", () => {
  ...
  it("redacts delivery targets in routine logs", async () => {
    await deliverReplies({
      replies: [{ text: "hello" }],
      target: "sms:+15555550123",
      client,
      accountId: "acct-4",
      runtime,
      maxBytes: 2048,
    ...
    expect(runtime.log).toHaveBeenCalledWith("imessage: delivered reply to sms:<redacted>");
    expect(runtime.log).not.toHaveBeenCalledWith(expect.stringContaining("+15555550123"));
  });
});
