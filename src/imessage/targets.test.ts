...
describe("imessage targets", () => {
  ...
  it("redacts targets for logs", () => {
    expect(formatIMessageTargetForLog("chat_id:42")).toBe("chat_id:<redacted>");
    expect(formatIMessageTargetForLog("chat_guid:iMessage;-;+15555550123")).toBe(
      "chat_guid:<redacted>",
    );
    expect(formatIMessageTargetForLog("sms:+15555550123")).toBe("sms:<redacted>");
    expect(formatIMessageTargetForLog("user@example.com")).toBe("auto:<redacted>");
  });
});
