...
describe("monitorSlackProvider tool results", () => {
        ...
        list: [
          {
            id: "main",
            default: true,
            identity: { name: "FasedAgent", theme: "space agent", emoji: "🤖" },
          },
          {
            id: "rich",
  ...
  it("updates assistant thread status when replies start", async () => {
    replyMock.mockImplementation(async (_ctx, opts) => {
      await (opts as { onReplyStart?: () => Promise<void> })?.onReplyStart?.();
  ...
  it("accepts channel messages when mentionPatterns match", async () => {
    ...
    expect((replyMock.mock.calls[0][0] as { WasMentioned: boolean }).WasMentioned).toBe(true);
  });
  ...
  it("accepts channel messages without mention when channels.slack.requireMention is false", async () => {
    ...
    expect(replyMock).toHaveBeenCalledTimes(1);
    expect((replyMock.mock.calls[0][0] as { WasMentioned: boolean }).WasMentioned).toBe(false);
    expect(sendMock).toHaveBeenCalledTimes(1);
  });
