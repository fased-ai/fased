...
describe("Slack native command argument menus", () => {
  ...
  beforeAll(async () => {
    ...
    argMenuHandler = requireHandler(harness.actions, "fased_cmdarg", "arg-menu action");
    argMenuOptionsHandler = requireHandler(harness.options, "fased_cmdarg", "arg-menu options");
  });
  ...
  it("registers options handlers without losing app receiver binding", async () => {
    ...
    expect(testHarness.actions.has("fased_cmdarg")).toBe(true);
    expect(testHarness.options.has("fased_cmdarg")).toBe(true);
  ...
  it("shows a static_select menu when choices exceed button row size", async () => {
    ...
    expect(element?.action_id).toBe("fased_cmdarg");
  ...
  it("shows an external_select menu when choices exceed static_select options max", async () => {
    ...
    expect(element?.action_id).toBe("fased_cmdarg");
    expect(blockId).toContain("fased_cmdarg_ext:");
    const token = (blockId ?? "").slice("fased_cmdarg_ext:".length);
