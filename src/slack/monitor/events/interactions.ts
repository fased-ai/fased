...
// Prefix for FasedAgent-generated action IDs to scope our handler
...
export function registerSlackInteractionEvents(params: { ctx: SlackMonitorContext }) {
  ...
  // Handle Block Kit button clicks from FasedAgent-generated messages
  // Only matches action_ids that start with our prefix to avoid interfering
  // with other Slack integrations or future features
  ctx.app.action(new RegExp(`^${FASED_ACTION_PREFIX}`), async (args: SlackActionMiddlewareArgs) => {
  ...
  // Handle FasedAgent modal submissions with callback_ids scoped by our prefix.
