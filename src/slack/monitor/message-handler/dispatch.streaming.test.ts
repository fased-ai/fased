...
describe("slack native streaming thread hint", () => {
  ...
  it("does not treat a top-level self thread_ts as a real thread when replyToMode=off", () => {
    expect(
      resolveSlackStreamingThreadHint({
        replyToMode: "off",
        incomingThreadTs: "1000.4",
        messageTs: "1000.4",
        isThreadReply: false,
      }),
    ).toBeUndefined();
  ...
  it("uses message ts for top-level progress when replyToMode=all", () => {
    expect(
      resolveSlackStreamingThreadHint({
        replyToMode: "all",
        incomingThreadTs: "1000.5",
        messageTs: "1000.5",
        isThreadReply: false,
      }),
    ).toBe("1000.5");
