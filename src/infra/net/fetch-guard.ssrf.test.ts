...
import { fetchWithSsrFGuard, resolveEnvHttpProxyAgentOptions } from "./fetch-guard.js";
...
describe("fetchWithSsrFGuard hardening", () => {
  ...
  it("normalizes symbolic header dictionaries before guarded fetch dispatch", async () => {
    ...
    const [, seenInit] = fetchImpl.mock.calls[0] as unknown as [string, RequestInit];
    expect(new Headers(seenInit.headers).get("x-trace")).toBe("1");
    expect(Object.getOwnPropertySymbols(seenInit.headers as object)).toHaveLength(0);
...
describe("resolveEnvHttpProxyAgentOptions", () => {
  it("uses ALL_PROXY as the HTTP and HTTPS proxy fallback", () => {
    expect(
      resolveEnvHttpProxyAgentOptions({
        ALL_PROXY: "socks5://proxy.example:1080",
      }),
    ).toEqual({
      httpProxy: "socks5://proxy.example:1080",
      httpsProxy: "socks5://proxy.example:1080",
    });
  });

  it("uses HTTP_PROXY as the HTTPS fallback before ALL_PROXY", () => {
    expect(
      resolveEnvHttpProxyAgentOptions({
      ...
      httpsProxy: "http://http-proxy.example:8080",
    });
  });

  it("prefers lower-case proxy env vars over upper-case values", () => {
    expect(
      resolveEnvHttpProxyAgentOptions({
      ...
      httpsProxy: "http://lower-https.example:8443",
    });
  });
});
