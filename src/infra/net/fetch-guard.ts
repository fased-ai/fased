...
export type EnvHttpProxyAgentOptions = {
  httpProxy?: string;
  httpsProxy?: string;
};
...
function resolveEnvProxyUrl(
  ...
  const lower = normalizeProxyEnvValue(env[`${protocol}_proxy`]);
  const upper = normalizeProxyEnvValue(env[`${protocol.toUpperCase()}_PROXY`]);
  return (lower !== undefined ? lower : upper) ?? undefined;
}

function resolveEnvAllProxyUrl(env: NodeJS.ProcessEnv): string | undefined {
  const lower = normalizeProxyEnvValue(env.all_proxy);
  const upper = normalizeProxyEnvValue(env.ALL_PROXY);
  return (lower !== undefined ? lower : upper) ?? undefined;
}

export function resolveEnvHttpProxyAgentOptions(
  env: NodeJS.ProcessEnv = process.env,
): EnvHttpProxyAgentOptions | undefined {
  const allProxy = resolveEnvAllProxyUrl(env);
  const httpProxy = resolveEnvProxyUrl("http", env) ?? allProxy;
  const httpsProxy = resolveEnvProxyUrl("https", env) ?? httpProxy;
  const options: EnvHttpProxyAgentOptions = {
    ...(httpProxy ? { httpProxy } : {}),
...
function hasEnvProxyConfigured(): boolean {
  return resolveEnvHttpProxyAgentOptions() !== undefined;
}
...
export async function fetchWithSsrFGuard(params: GuardedFetchOptions): Promise<GuardedFetchResult> {
  ...
  let currentInit = params.init
    ? normalizeRequestInitHeadersForFetch({ ...params.init })
    : undefined;
  let redirectCount = 0;
  ...
  while (true) {
    ...
    try {
      ...
      if (params.proxy === "env" && hasEnvProxyConfigured()) {
        dispatcher = new EnvHttpProxyAgent(resolveEnvHttpProxyAgentOptions());
