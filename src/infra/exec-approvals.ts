...
export const DEFAULT_EXEC_APPROVAL_ALLOWED_DECISIONS = [
  "allow-once",
  "allow-always",
  "deny",
...
const DEFAULT_SOCKET = "~/.fased/exec-approvals.sock";
const DEFAULT_FILE = "~/.fased/exec-approvals.json";
...
export function resolveExecApprovalsFromFile(params: {
  ...
  const sourceFor = (key: keyof ExecApprovalsDefaults) =>
    agent[key] !== undefined
      ? `agents.${agentKey}.${key}`
      : wildcard[key] !== undefined
        ? `agents.*.${key}`
  ...
  return {
    ...
    agent: resolvedAgent,
    agentSources: {
      security: sourceFor("security"),
      ask: sourceFor("ask"),
      askFallback: sourceFor("askFallback"),
      autoAllowSkills: sourceFor("autoAllowSkills"),
    },
    allowlist,
    file,
  };
...
export function resolveExecApprovalAllowedDecisions(): readonly ExecApprovalDecision[] {
  return DEFAULT_EXEC_APPROVAL_ALLOWED_DECISIONS;
}
