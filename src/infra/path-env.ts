...
function candidateBinDirs(opts: EnsureFasedAgentPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `fased` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "fased");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.FASED_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "fased"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `fased` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureFasedAgentCliOnPath(opts: EnsureFasedAgentPathOpts = {}) {
  if (isTruthyEnvValue(process.env.FASED_PATH_BOOTSTRAPPED)) {
    return;
  }
