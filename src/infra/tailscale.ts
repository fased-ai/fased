...
export async function getTailscaleBinary(): Promise<string> {
  const forcedBinary = process.env.FASED_TEST_TAILSCALE_BINARY?.trim();
  if (forcedBinary) {
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for FasedAgent. You can keep running the web gateway without it: \`${formatCliCommand("fased gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
...
export async function enableTailscaleServe(port: number, exec: typeof runExec = runExec) {
  ...
  await execWithSudoFallback(
    exec,
    tailscaleBin,
    ["serve", "--bg", "--yes", `http://127.0.0.1:${port}`],
    {
      maxBuffer: 200_000,
      timeoutMs: 15_000,
    },
