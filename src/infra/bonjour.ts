...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.FASED_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "FasedAgent";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(FasedAgent\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  const hostnameRaw =
    process.env.FASED_MDNS_HOSTNAME?.trim() || process.env.FASED_MDNS_HOSTNAME?.trim() || "fased";
  const hostname =
    hostnameRaw
      .replace(/\.local$/i, "")
      .split(".")[0]
