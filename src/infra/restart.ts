...
export function triggerFasedAgentRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(process.env.FASED_SYSTEMD_UNIT, process.env.FASED_PROFILE);
  ...
  const label =
    process.env.FASED_LAUNCHD_LABEL || resolveGatewayLaunchAgentLabel(process.env.FASED_PROFILE);
