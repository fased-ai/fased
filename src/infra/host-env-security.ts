...
function sortUniqueUpper(values: Iterable<string>): string[] {
  return Array.from(new Set(Array.from(values, (value) => value.toUpperCase()))).toSorted((a, b) =>
    a.localeCompare(b),
  );
}

const BLOCKED_EVERYWHERE_KEYS = sortUniqueUpper(
  HOST_ENV_SECURITY_POLICY.blockedEverywhereKeys ?? HOST_ENV_SECURITY_POLICY.blockedKeys ?? [],
);
const BLOCKED_OVERRIDE_ONLY_KEYS = sortUniqueUpper(
  HOST_ENV_SECURITY_POLICY.blockedOverrideOnlyKeys ??
    HOST_ENV_SECURITY_POLICY.blockedOverrideKeys ??
    [],
);
const ALLOWED_INHERITED_OVERRIDE_ONLY_KEYS = new Set(
  sortUniqueUpper(HOST_ENV_SECURITY_POLICY.allowedInheritedOverrideOnlyKeys ?? []),
);
const BLOCKED_INHERITED_KEYS = sortUniqueUpper([
  ...BLOCKED_EVERYWHERE_KEYS,
  ...BLOCKED_OVERRIDE_ONLY_KEYS.filter((key) => !ALLOWED_INHERITED_OVERRIDE_ONLY_KEYS.has(key)),
]);

export const HOST_DANGEROUS_ENV_KEY_VALUES: readonly string[] =
  Object.freeze(BLOCKED_EVERYWHERE_KEYS);
export const HOST_DANGEROUS_ENV_PREFIXES: readonly string[] = Object.freeze(
  sortUniqueUpper(HOST_ENV_SECURITY_POLICY.blockedPrefixes),
);
export const HOST_DANGEROUS_INHERITED_ENV_KEY_VALUES: readonly string[] =
  Object.freeze(BLOCKED_INHERITED_KEYS);
...
export const HOST_DANGEROUS_OVERRIDE_ENV_PREFIXES: readonly string[] = Object.freeze(
  sortUniqueUpper(HOST_ENV_SECURITY_POLICY.blockedOverridePrefixes ?? []),
);
...
export function sanitizeHostExecEnv(params?: {
  ...
  for (const [rawKey, value] of Object.entries(baseEnv)) {
    ...
    if (!key || isDangerousHostInheritedEnvVarName(key)) {
      continue;
    }
    merged[key] = value;
  }
