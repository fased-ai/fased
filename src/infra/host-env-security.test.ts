...
describe("isDangerousHostEnvVarName", () => {
  it("matches dangerous keys and prefixes case-insensitively", () => {
    ...
    expect(isDangerousHostEnvVarName("VIMINIT")).toBe(true);
    expect(isDangerousHostEnvVarName("TF_VAR_SECRET")).toBe(false);
    expect(isDangerousHostEnvVarName("GITHUB_TOKEN")).toBe(false);
    expect(isDangerousHostEnvVarName("FOO")).toBe(false);
  });
});

describe("isDangerousHostInheritedEnvVarName", () => {
  it("blocks inherited high-risk keys while allowing trusted runtime context keys", () => {
    expect(isDangerousHostInheritedEnvVarName("VIMINIT")).toBe(true);
    expect(isDangerousHostInheritedEnvVarName("GITHUB_TOKEN")).toBe(true);
    expect(isDangerousHostInheritedEnvVarName("DATABASE_URL")).toBe(true);
    expect(isDangerousHostInheritedEnvVarName("TF_VAR_secret")).toBe(false);
...
describe("sanitizeHostExecEnv", () => {
  ...
  it("removes expanded dangerous inherited keys while preserving allowed inherited context", () => {
    const env = sanitizeHostExecEnv({
      baseEnv: {
        PATH: "/usr/bin:/bin",
        HOME: "/home/user",
...
describe("isDangerousHostEnvOverrideVarName", () => {
  it("matches override-only blocked keys case-insensitively", () => {
    ...
    expect(isDangerousHostEnvOverrideVarName("GITHUB_TOKEN")).toBe(true);
    expect(isDangerousHostEnvOverrideVarName("TF_VAR_SECRET")).toBe(true);
...
describe("shell wrapper exploit regression", () => {
  it("blocks SHELLOPTS/PS4 chain after sanitization", async () => {
    ...
    const marker = path.join(os.tmpdir(), `fased-ps4-marker-${process.pid}-${Date.now()}`);
    try {
