...
const cfg = {} as unknown as FasedAgentConfig;
...
describe("system events (session routing)", () => {
  ...
  it("formats untrusted queued events with an explicit untrusted prefix", async () => {
    enqueueSystemEvent("Exec denied (node=n1): rm -rf /", {
    ...
    expect(result).toMatch(
      /^System \(untrusted\): \[[^\]]+\] Exec denied \(node=n1\): rm -rf \/\n\nhello$/,
    );
  });
});
