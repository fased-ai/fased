...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.FASED_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*fased-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "fased";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "fased");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "fased-gw");
  ...
  records.push(`_fased-gw._tcp IN PTR ${instanceLabel}._fased-gw._tcp`);
  records.push(`${instanceLabel}._fased-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._fased-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; fased-content-hash: ${contentHash}\n${contentBody}`;
}
