...
let ensureFasedAgentCliOnPath: typeof import("./path-env.js").ensureFasedAgentCliOnPath;

describe("ensureFasedAgentCliOnPath", () => {
  const envKeys = [
    "PATH",
    "FASED_PATH_BOOTSTRAPPED",
  ...
  beforeAll(async () => {
    ({ ensureFasedAgentCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling fased exists", () => {
    const tmp = abs("/tmp/fased-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "fased");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.FASED_PATH_BOOTSTRAPPED;
  ...
  it("is idempotent", () => {
    ...
    process.env.FASED_PATH_BOOTSTRAPPED = "1";
    ensureFasedAgentCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/fased-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "fased");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/fased-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "fased");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "fased");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/fased-path/case-linuxbrew");
