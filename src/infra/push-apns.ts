...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const inlineKeyRaw =
    normalizeNonEmptyString(env.FASED_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.FASED_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set FASED_APNS_PRIVATE_KEY_P8 or FASED_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading FASED_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
