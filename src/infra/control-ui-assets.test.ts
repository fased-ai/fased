...
vi.mock("./fased-root.js", () => ({
  resolveFasedAgentPackageRoot: vi.fn(async () => null),
  resolveFasedAgentPackageRootSync: vi.fn(() => null),
}));
...
let fasedRoot: typeof import("./fased-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    fasedRoot = await import("./fased-root.js");
  });
  ...
  it("uses resolveFasedAgentPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/fased");
    (
      fasedRoot.resolveFasedAgentPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/fased"))).resolves.toBe(
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "fased" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "fased.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-fased");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/fased-bundle");
    (
      fasedRoot.resolveFasedAgentPackageRootSync as unknown as ReturnType<typeof vi.fn>
