...
import { formatOutboundTargetForLog, redactOutboundTargetInText } from "./outbound/format.js";
...
async function deliverToTargets(params: {
  ...
  const deliveries = params.targets.map(async (target) => {
    ...
    try {
      ...
      log.error(
        `exec approvals: failed to deliver to ${formatOutboundTargetForLog(
          channel,
          target.to,
        )}: ${redactOutboundTargetInText(formatErrorMessage(err), channel, target.to)}`,
      );
    }
  });
