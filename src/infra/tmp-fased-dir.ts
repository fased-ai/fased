...
export const POSIX_FASED_TMP_DIR = "/tmp/fased";
const TMP_DIR_ACCESS_MODE = fs.constants.W_OK | fs.constants.X_OK;

type ResolvePreferredFasedAgentTmpDirOptions = {
...
export function resolvePreferredFasedAgentTmpDir(
  options: ResolvePreferredFasedAgentTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "fased" : `fased-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[fased] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback FasedAgent temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback FasedAgent temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_FASED_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_FASED_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_FASED_TMP_DIR)) {
      return POSIX_FASED_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_FASED_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_FASED_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_FASED_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_FASED_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_FASED_TMP_DIR;
