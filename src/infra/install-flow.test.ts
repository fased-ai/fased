...
describe("resolveExistingInstallPath", () => {
  ...
  beforeEach(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "fased-install-flow-"));
  });
...
describe("withExtractedArchiveRoot", () => {
  ...
  it("extracts archive and passes root directory to callback", async () => {
    const tmpRoot = path.join(path.sep, "tmp", "fased-install-flow");
    ...
    expect(withTempDirSpy).toHaveBeenCalledWith("fased-plugin-", expect.any(Function));
  ...
  it("accepts flat skill archives when a root marker is present", async () => {
    const tmpRoot = path.join(path.sep, "tmp", "fased-install-flow");
    const archivePath = path.join(path.sep, "tmp", "skill.zip");
    const extractDir = path.join(tmpRoot, "extract");
    vi.spyOn(installSource, "withTempDir").mockImplementation(async (_prefix, fn) => {
      await fs.mkdir(extractDir, { recursive: true });
      await fs.writeFile(path.join(extractDir, "SKILL.md"), "---\nname: flat\n---\n");
      return await fn(tmpRoot);
    });
    vi.spyOn(archive, "extractArchive").mockResolvedValue(undefined);
    const resolveRootSpy = vi.spyOn(archive, "resolvePackedRootDir");

    const onExtracted = vi.fn(async (rootDir: string) => ({ ok: true as const, rootDir }));
    const result = await withExtractedArchiveRoot({
      archivePath,
      tempDirPrefix: "fased-skill-",
      timeoutMs: 1000,
      rootMarkers: ["SKILL.md"],
      onExtracted,
    });

    expect(resolveRootSpy).not.toHaveBeenCalled();
    expect(onExtracted).toHaveBeenCalledWith(extractDir);
    expect(result).toEqual({
      ok: true,
      rootDir: extractDir,
    });
  ...
  it("returns extract failure when extraction throws", async () => {
    vi.spyOn(installSource, "withTempDir").mockImplementation(
      async (_prefix, fn) => await fn("/tmp/fased-install-flow"),
    );
