...
export function findPathKey(env: Record<string, string | undefined>): string {
  if (Object.hasOwn(env, "PATH")) {
    return "PATH";
  }
  const pathLikeKey = Object.keys(env).find((key) => key.toLowerCase() === "path");
  return pathLikeKey ?? "PATH";
}
