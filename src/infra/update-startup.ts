...
export async function runGatewayUpdateCheck(params: {
  ...
  const shouldRunUpdateHints = params.cfg.update?.checkOnStart === true;
  ...
  if (cmp != null && cmp < 0) {
    ...
    if (shouldRunUpdateHints && shouldNotify) {
      params.log.info(
        `update available (${tag}): v${resolved.version} (current v${VERSION}). Run: ${formatCliCommand("fased update")}`,
      );
