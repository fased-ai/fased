...
import { formatOutboundTargetForLog, redactOutboundTargetInText } from "./format.js";
...
export async function recoverPendingDeliveries(opts: {
  ...
  for (const entry of pending) {
    ...
    try {
      ...
      recovered += 1;
      opts.log.info(
        `Recovered delivery ${entry.id} to ${formatOutboundTargetForLog(entry.channel, entry.to)}`,
      );
    } catch (err) {
      const errMsg = redactOutboundTargetInText(formatErrorMessage(err), entry.channel, entry.to);
