...
vi.mock("../../media/runtime-service.js", async () => {
  const actual = await vi.importActual<typeof import("../../media/runtime-service.js")>(
    "../../media/runtime-service.js",
  );
  return {
    ...actual,
...
describe("runMessageAction sandboxed media validation", () => {
  ...
  it("allows media paths under preferred FasedAgent tmp root", async () => {
    const tmpRoot = resolvePreferredFasedAgentTmpDir();
    ...
    try {
      ...
      const hostTmpOutsideFasedAgent = path.join(os.tmpdir(), "outside-fased", "test-media.png");
      await expect(
        runMessageAction({
          cfg: slackConfig,
          action: "send",
          params: {
            channel: "slack",
            target: "#C12345678",
            media: hostTmpOutsideFasedAgent,
            message: "",
          },
          sandboxRoot: sandboxDir,
          dryRun: true,
...
describe("runMessageAction components parsing", () => {
  const handleAction = vi.fn(
    async ({
      ...
      jsonResult({
        ...
        paramsRequesterSenderId: params.requesterSenderId ?? null,
        paramsSenderIsOwner: params.senderIsOwner ?? null,
        requesterSenderId: requesterSenderId ?? null,
        senderIsOwner: senderIsOwner ?? null,
  ...
  it("parses presentation and delivery JSON strings before plugin dispatch", async () => {
    const presentation = {
      title: "Deploy status",
      tone: "info",
      blocks: [{ type: "text", text: "Ready" }],
    };
    const delivery = { pin: { enabled: true, notify: false } };
    const result = await runMessageAction({
      cfg: {} as FasedAgentConfig,
      action: "send",
      params: {
    ...
    expect(result.kind).toBe("send");
    expect(handleAction).toHaveBeenCalled();
    expect(result.payload).toMatchObject({ ok: true, presentation, delivery });
  });
  ...
  it("uses trusted requester context and removes caller-supplied trust params", async () => {
    const result = await runMessageAction({
      cfg: {} as FasedAgentConfig,
      action: "send",
      params: {
    ...
    expect(result.payload).toMatchObject({
      ok: true,
      paramsRequesterSenderId: null,
      paramsSenderIsOwner: null,
      requesterSenderId: "trusted-sender",
      senderIsOwner: true,
      sessionId: "trusted-session",
    });
