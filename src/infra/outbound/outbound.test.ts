...
import {
  ...
  formatOutboundTargetForLog,
  redactOutboundTargetInText,
} from "./format.js";
import {
...
describe("delivery-queue", () => {
  ...
  describe("recoverPendingDeliveries", () => {
    ...
    it("moves entries to failed/ immediately on permanent delivery errors", async () => {
      ...
      expect(warnMessages).not.toContain("user:abc");
      expect(warnMessages).toContain("msteams:us...bc");
    });
...
describe("formatOutboundTargetForLog", () => {
  it("masks outbound delivery targets for operational logs", () => {
    expect(formatOutboundTargetForLog("whatsapp", "+15551234567")).toBe("whatsapp:+1...67");
    expect(formatOutboundTargetForLog("slack", "U123")).toBe("slack:<redacted>");
    expect(formatOutboundTargetForLog("telegram", "   ")).toBe("telegram:<empty>");
  });

  it("redacts known outbound targets repeated inside error text", () => {
    const message = redactOutboundTargetInText(
      "No conversation reference found for user:abc",
      "msteams",
      "user:abc",
    );

    expect(message).toBe("No conversation reference found for msteams:us...bc");
  });
});
...
describe("resolveOutboundSessionRoute", () => {
  ...
  it("resolves provider-specific session routes", async () => {
    const perChannelPeerCfg = { session: { dmScope: "per-channel-peer" } } as FasedAgentConfig;
