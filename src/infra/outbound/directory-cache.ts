...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: FasedAgentConfig | null = null;
  ...
  get(key: string, cfg: FasedAgentConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: FasedAgentConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  clear(cfg?: FasedAgentConfig): void {
    this.cache.clear();
    if (cfg) {
  ...
  private resetIfConfigChanged(cfg: FasedAgentConfig): void {
