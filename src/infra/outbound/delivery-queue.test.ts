...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "fased-dq-test-"));
});
...
describe("recoverPendingDeliveries", () => {
  ...
  it("redacts sensitive delivery errors before storing and logging them", async () => {
    await enqueueDelivery(
      { channel: "slack", to: "channel:C123456", payloads: [{ text: "x" }] },
      tmpDir,
    );
    ...
    const deliver = vi
      .fn()
      .mockRejectedValue(new Error(`request failed for channel:C123456 API_KEY=${secret}`));
    const log = { info: vi.fn(), warn: vi.fn(), error: vi.fn() };
    ...
    const entries = await loadPendingDeliveries(tmpDir);
    expect(entries).toHaveLength(1);
    expect(entries[0].lastError).not.toContain(secret);
    expect(entries[0].lastError).not.toContain("channel:C123456");
    const warnMessages = log.warn.mock.calls.map(([message]) => String(message)).join("\n");
    expect(warnMessages).not.toContain(secret);
    expect(warnMessages).not.toContain("channel:C123456");
  });
  ...
  it("redacts delivery targets in recovery logs", async () => {
    await enqueueDelivery(
      { channel: "whatsapp", to: "+15551234567", payloads: [{ text: "a" }] },
      tmpDir,
    );
    ...
    const messages = log.info.mock.calls.map(([message]) => String(message)).join("\n");
    expect(messages).toContain("whatsapp:+1...67");
    expect(messages).not.toContain("+15551234567");
  });
  ...
  it("defers entries when backoff exceeds the recovery budget", async () => {
    ...
    expect(deliver).not.toHaveBeenCalled();
    expect(result).toEqual({ recovered: 0, failed: 0, skippedMaxRetries: 0, deferredBackoff: 1 });
