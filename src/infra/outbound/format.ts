...
export function formatOutboundTargetForLog(channel: string, to?: string | null): string {
  return `${channel}:${redactOutboundTargetForLog(to)}`;
}

export function redactOutboundTargetInText(
  text: string,
  channel: string,
  to?: string | null,
): string {
  const normalizedTarget = to?.trim().replace(/\s+/g, " ");
  if (!normalizedTarget || normalizedTarget.length < 5) {
    return text;
  }
  return text.split(normalizedTarget).join(formatOutboundTargetForLog(channel, normalizedTarget));
}

function redactOutboundTargetForLog(value?: string | null): string {
