...
async function resolveChannel(cfg: FasedAgentConfig, params: Record<string, unknown>) {
...
const UNTRUSTED_REQUESTER_CONTEXT_PARAM_KEYS = [
  "requesterAccountId",
  "requesterSenderId",
  "requesterSenderName",
...
function removeCallerSuppliedRequesterContext(params: Record<string, unknown>): void {
  for (const key of UNTRUSTED_REQUESTER_CONTEXT_PARAM_KEYS) {
    delete params[key];
  }
}
...
async function handleSendAction(ctx: ResolvedActionContext): Promise<MessageActionRunResult> {
  ...
  const hasPresentation = params.presentation != null && typeof params.presentation === "object";
  const caption = readStringParam(params, "caption", { allowEmpty: true }) ?? "";
  let message =
    readStringParam(params, "message", {
      required: !mediaHint && !hasCard && !hasComponents && !hasPresentation,
      allowEmpty: true,
    }) ?? "";
  if (message.includes("\\n")) {
...
export async function runMessageAction(
  ...
  removeCallerSuppliedRequesterContext(params);
  ...
  parsePresentationParam(params);
  parseDeliveryParam(params);
