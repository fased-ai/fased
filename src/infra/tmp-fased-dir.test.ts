...
import { POSIX_FASED_TMP_DIR, resolvePreferredFasedAgentTmpDir } from "./tmp-fased-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredFasedAgentTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `fased-${uid}`);
}
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredFasedAgentTmpDir({
    accessSync,
    chmodSync,
...
describe("resolvePreferredFasedAgentTmpDir", () => {
  it("prefers /tmp/fased when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_FASED_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/fased when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_FASED_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/fased when /tmp/fased is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/fased when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/fased is a symlink", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => true,
      uid: 501,
  ...
  it("falls back when /tmp/fased is not owned by the current user", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 0,
  ...
  it("falls back when /tmp/fased is group/other writable", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 501,
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback FasedAgent temp dir/);
  });
