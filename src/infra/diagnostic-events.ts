...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __fasedDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__fasedDiagnosticEventsState) {
    globalStore.__fasedDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__fasedDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: FasedAgentConfig): boolean {
