...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "fased-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["FASED_STATE_DIR", "NODE_ENV", "VITEST"]);
    ...
    if (!loaded) {
      ({ resolveFasedAgentPackageRoot } = await import("./fased-root.js"));
  ...
  function mockPackageUpdateStatus(tag = "latest", version = "2.0.0") {
    vi.mocked(resolveFasedAgentPackageRoot).mockResolvedValue("/opt/fased");
  ...
  async function runUpdateCheckAndReadState(channel: "stable" | "beta") {
    ...
    await runGatewayUpdateCheck({
      cfg: { update: { channel, checkOnStart: true } },
      log,
      isNixMode: false,
      allowInTests: true,
    });
  ...
  it("hydrates cached update from persisted state during throttle window", async () => {
    ...
    await runGatewayUpdateCheck({
      cfg: { update: { channel: "stable", checkOnStart: true } },
      log: { info: vi.fn() },
      isNixMode: false,
      allowInTests: true,
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/fased/dist/entry.js"];
    try {
      await runGatewayUpdateCheck({
        cfg: {
          update: {
