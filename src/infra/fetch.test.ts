...
describe("wrapFetchWithAbortSignal", () => {
  it("normalizes plain header dictionaries before delegated fetch", async () => {
    ...
    await wrapped("https://example.com", { headers: headers as HeadersInit });

    expect(new Headers(seenHeaders).get("x-trace")).toBe("1");
    expect(Object.getOwnPropertySymbols(seenHeaders as object)).toHaveLength(0);
  });
