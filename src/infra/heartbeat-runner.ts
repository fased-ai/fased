...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: FasedAgentConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: FasedAgentConfig) {
...
export function resolveHeartbeatPrompt(cfg: FasedAgentConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: FasedAgentConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: FasedAgentConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
