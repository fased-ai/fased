...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveFasedAgentPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "fased" + dist/control-ui/index.html
...
export async function ensureControlUiAssetsBuilt(
  ...
  runtime.log("Preparing Control UI assets (auto-installing UI dependencies if needed)…");
