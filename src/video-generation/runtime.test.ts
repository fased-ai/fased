...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    getVideoGenerationProvider: vi.fn<
      (providerId: string, config?: FasedAgentConfig) => VideoGenerationProvider | undefined
    >(() => undefined),
    isFailoverError: vi.fn<(err: unknown) => boolean>(() => false),
    listVideoGenerationProviders: vi.fn<(config?: FasedAgentConfig) => VideoGenerationProvider[]>(
      () => [],
    ),
...
describe("video-generation runtime", () => {
  ...
  it("generates videos through the active video-generation provider", async () => {
    ...
    const result = await generateVideo({
      ...
      prompt: "animate a product scene",
      agentDir: "/tmp/agent",
      authStore,
    });
  ...
  it("lists runtime video-generation providers through the provider registry", () => {
    ...
    expect(listRuntimeVideoGenerationProviders()).toEqual(providers);
  });

  it("normalizes requested durations to provider-supported values", async () => {
