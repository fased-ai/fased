...
async function main() {
  ...
  if (smokeTest) {
    try {
      ...
      console.error(`Relay smoke test failed (${smokeTest}):`, formatUncaughtError(err));
      process.exit(1);
    }
  }
  ...
  const { ensureFasedAgentCliOnPath } = await import("../infra/path-env.js");
  ...
  process.on("uncaughtException", (error) => {
    console.error("[fased] Uncaught exception:", formatUncaughtError(error));
    process.exit(1);
  });
...
void main().catch((err) => {
  console.error("[fased] Relay failed:", formatUncaughtError(err));
  process.exit(1);
});
