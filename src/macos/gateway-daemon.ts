...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `fased --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.FASED_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ?? process.env.FASED_GATEWAY_BIND ?? cfg.gateway?.bind ?? "loopback";
  const bind =
    bindRaw === "loopback" ||
    bindRaw === "lan" ||
    bindRaw === "auto" ||
  ...
  const request = (action: "stop" | "restart", signal: string) => {
    ...
    void (async () => {
      try {
        ...
        defaultRuntime.error(`gateway: shutdown error: ${formatErrorMessage(err)}`);
      } finally {
        if (forceExitTimer) {
        ...
        if (isRestart) {
          ...
          if (respawn.mode === "spawned" || respawn.mode === "supervised") {
            ...
            if (respawn.mode === "failed") {
              ...
              defaultRuntime.log("gateway: restart mode in-process restart (FASED_NO_RESPAWN)");
            }
            shuttingDown = false;
            restartResolver?.();
          }
  ...
  try {
    try {
      ...
      if (err instanceof GatewayLockError) {
        defaultRuntime.error(`Gateway start blocked: ${formatErrorMessage(err)}`);
        process.exit(1);
      }
      throw err;
    }
    ...
    while (true) {
      ...
      try {
        ...
        cleanupSignals();
        defaultRuntime.error(`Gateway failed to start: ${formatErrorMessage(err)}`);
        process.exit(1);
      }
...
void main().catch((err) => {
  console.error("[fased] Gateway daemon failed:", formatUncaughtError(err));
  process.exit(1);
});
