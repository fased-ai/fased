...
describe("dispatchTelegramMessage draft streaming", () => {
  ...
  it("keeps forum topic media delivery visible when tool status progress starts", async () => {
    ...
    dispatchReplyWithBufferedBlockDispatcher.mockImplementation(
      async ({ dispatcherOptions, replyOptions }) => {
        await replyOptions?.onPartialReply?.({ text: "Checking pool state" });
        await replyOptions?.onToolStart?.({ name: "exec", phase: "start" });
        await dispatcherOptions.deliver(
          { mediaUrl: "file:///tmp/fased-chart.png" },
          {
            kind: "final",
          },
        );
    ...
    await dispatchWithContext({
      context: createContext({
        ...
        statusReactionController: statusReactionController as unknown as NonNullable<
          TelegramMessageContext["statusReactionController"]
        >,
      }),
    });
    ...
    expect(deliverReplies).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        mediaLocalRoots: expect.arrayContaining([path.join(STATE_DIR, "workspace-work")]),
        replies: [expect.objectContaining({ mediaUrl: "file:///tmp/fased-chart.png" })],
      }),
    );
  ...
  it("shows topic progress drafts for message-tool-only Codex tool starts before status reactions resolve", async () => {
    ...
    dispatchReplyWithBufferedBlockDispatcher.mockImplementation(async ({ replyOptions }) => {
      ...
      await Promise.resolve();
      expect(draftStream.update.mock.calls.at(-1)?.[0]).toBe("Working\n`🛠️ Exec`");
      releaseSetTool?.();
    ...
    await dispatchWithContext({
      ...
      streamMode: "partial",
      telegramCfg: { streaming: "progress" },
    });
