...
describe("createTelegramBot", () => {
  ...
  it("sends replies without native reply threading", async () => {
    ...
    for (const call of sendMessageSpy.mock.calls) {
      expect(
        (call[2] as { reply_to_message_id?: unknown } | undefined)?.reply_to_message_id,
      ).toBeUndefined();
    }
  });
  it("honors replyToMode=first for threaded replies", async () => {
    ...
    expect((first?.[2] as { reply_to_message_id?: unknown } | undefined)?.reply_to_message_id).toBe(
      101,
    );
  ...
  it("honors replyToMode=all for threaded replies", async () => {
    ...
    expect(sendMessageSpy.mock.calls.length).toBeGreaterThan(1);
    for (const call of sendMessageSpy.mock.calls) {
      expect((call[2] as { reply_to_message_id?: unknown } | undefined)?.reply_to_message_id).toBe(
        101,
      );
    }
  });
