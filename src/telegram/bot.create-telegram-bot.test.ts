...
describe("createTelegramBot", () => {
  ...
  it("handles pairing DM flows for new and already-pending requests", async () => {
    ...
    for (const testCase of cases) {
      ...
      if (testCase.expectPairingText) {
        ...
        expect(pairingText, testCase.name).toContain("fased pairing approve telegram PAIRME12");
  ...
  async function dispatchMessage(params: {
    ...
    await handler({
      ...
      me: params.me ?? { username: "fased_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    });
  }
  ...
  it("honors threaded replies for replyToMode=first/all", async () => {
    for (const [mode, messageId] of [
      ...
      expect(
        (first?.[2] as { reply_to_message_id?: number } | undefined)?.reply_to_message_id,
      ).toBe(messageId);
      for (const call of rest) {
        const replyToId = (call[2] as { reply_to_message_id?: number } | undefined)
          ?.reply_to_message_id;
        expect(replyToId).toBe(mode === "all" ? messageId : undefined);
      }
    }
  });
  it("honors routed group activation from session store", async () => {
    const storeDir = fs.mkdtempSync(path.join(os.tmpdir(), "fased-telegram-"));
