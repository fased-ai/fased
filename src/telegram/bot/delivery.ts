  ...
  for (const reply of replies) {
    ...
    if (mediaList.length === 0) {
      ...
      for (let i = 0; i < chunks.length; i += 1) {
        ...
        const chunkReplyToMessageId =
          replyToMode === "all" || i === 0 ? replyToMessageIdForPayload : undefined;
    ...
    for (const mediaUrl of mediaList) {
      ...
      const replyToMessageId =
        replyToMode === "all" || isFirstMedia ? replyToMessageIdForPayload : undefined;
      ...
      if (pendingFollowUpText && isFirstMedia) {
        ...
        for (let i = 0; i < chunks.length; i += 1) {
          const chunk = chunks[i];
          const followUpReplyToMessageId =
            replyToMode === "all" ? replyToMessageIdForPayload : undefined;
          await sendTelegramText(bot, chatId, chunk.html, runtime, {
...
async function sendTelegramVoiceFallbackText(opts: {
  ...
  for (let i = 0; i < chunks.length; i += 1) {
    const chunk = chunks[i];
    const chunkReplyToId = opts.replyToMode === "all" || i === 0 ? opts.replyToId : undefined;
