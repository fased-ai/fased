...
import type { FasedAgentConfig, ReplyToMode, TelegramAccountConfig } from "../config/types.js";
...
function formatTelegramProgressToolName(name?: string): string {
...
function formatTelegramToolProgressDraft(payload: { name?: string }): string {
  return `${PROGRESS_DRAFT_LABEL}\n\`🛠️ ${formatTelegramProgressToolName(payload.name)}\``;
}

async function resolveStickerVisionSupport(cfg: FasedAgentConfig, agentId: string) {
  try {
...
export const dispatchTelegramMessage = async ({
  ...
  const pushToolProgressDraft = async (payload: { name?: string }) => {
    if (!progressDraftsEnabled || !answerLane.stream) {
      return;
    }
  ...
  try {
    ({ queuedFinal } = await dispatchReplyWithBufferedBlockDispatcher({
      ...
      replyOptions: {
        ...
        allowProgressCallbacksWhenSourceDeliverySuppressed: progressDraftsEnabled
          ? true
          : undefined,
        onPartialReply:
        ...
        onReasoningEnd: reasoningLane.stream
          ...
          : undefined,
        onToolStart:
          progressDraftsEnabled || statusReactionController
            ? async (payload) => {
