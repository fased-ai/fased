...
async function sendLinePairingReply(params: {
  ...
  logVerbose(`line pairing request sender=${formatLineIdForLog(senderId)}`);
  const idLabel = (() => {
    try {
  ...
  try {
    ...
    logVerbose(
      `line pairing reply failed for ${formatLineIdForLog(senderId)}: ${formatErrorMessage(err)}`,
    );
  }
  try {
...
async function shouldProcessLineEvent(
  ...
  if (isGroup) {
    if (groupConfig?.enabled === false) {
      logVerbose(`Blocked line group ${formatLineIdForLog(groupId ?? roomId)} (group disabled)`);
      return false;
    }
    if (typeof groupAllowOverride !== "undefined") {
      ...
      if (!isSenderAllowed({ allow: effectiveGroupAllow, senderId })) {
        logVerbose(
          `Blocked line group sender ${formatLineIdForLog(senderId)} (group allowFrom override)`,
        );
        return false;
      }
    }
    ...
    if (groupPolicy === "allowlist") {
      ...
      if (!isSenderAllowed({ allow: effectiveGroupAllow, senderId })) {
        logVerbose(
          `Blocked line group message from ${formatLineIdForLog(
            senderId,
  ...
  if (!dmAllowed) {
    if (dmPolicy === "pairing") {
      ...
      logVerbose(`Blocked line sender ${formatLineIdForLog(senderId)} (dmPolicy: ${dmPolicy})`);
    }
    return false;
  }
  ...
  if (message.type === "image" || message.type === "video" || message.type === "audio") {
    try {
      ...
      if (errMsg.includes("exceeds") && errMsg.includes("limit")) {
        logVerbose(`line: media exceeds size limit for message ${formatLineIdForLog(message.id)}`);
...
async function handleFollowEvent(event: FollowEvent, _context: LineHandlerContext): Promise<void> {
  ...
  logVerbose(`line: user ${formatLineIdForLog(userId)} followed`);
...
async function handleUnfollowEvent(
  ...
  const userId = event.source.type === "user" ? event.source.userId : undefined;
  logVerbose(`line: user ${formatLineIdForLog(userId)} unfollowed`);
}
...
async function handleJoinEvent(event: JoinEvent, _context: LineHandlerContext): Promise<void> {
  ...
  logVerbose(
    `line: bot joined ${groupId ? "group" : "room"} ${formatLineIdForLog(groupId ?? roomId)}`,
  );
}
...
async function handleLeaveEvent(event: LeaveEvent, _context: LineHandlerContext): Promise<void> {
  ...
  const roomId = event.source.type === "room" ? event.source.roomId : undefined;
  logVerbose(
    `line: bot left ${groupId ? "group" : "room"} ${formatLineIdForLog(groupId ?? roomId)}`,
  );
}
...
async function handlePostbackEvent(
  ...
  logVerbose(`line: received postback (${data.length} chars)`);
