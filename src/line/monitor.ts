...
export async function monitorLineProvider(
  ...
  const bot = createLineBot({
    ...
    onMessage: async (ctx) => {
      ...
      logVerbose(`line: received message from ${formatLineAddressForLog(ctxPayload.From)}`);
      ...
      try {
              ...
              const { replyTokenUsed: nextReplyTokenUsed } = await deliverLineAutoReply({
                ...
                deps: {
                  ...
                  onReplyError: (replyErr) => {
                    logVerbose(
                      `line: reply token failed, falling back to push: ${formatErrorMessage(
                        replyErr,
                      )}`,
                    );
                  },
        ...
        if (!queuedFinal) {
          logVerbose(
            `line: no response generated for message from ${formatLineAddressForLog(
              ctxPayload.From,
            )}`,
          );
        }
        ...
        if (replyToken) {
          try {
            ...
            runtime.error?.(danger(`line: error reply failed: ${formatErrorMessage(replyErr)}`));
          }
        }
      } finally {
        stopLoading?.();
  ...
  const stopHandler = () => {
    ...
    stopped = true;
    logVerbose(`line: stopping provider for account ${formatLineIdForLog(resolvedAccountId)}`);
    unregisterHttp();
    recordChannelRuntimeState({
      channel: "line",
      accountId: resolvedAccountId,
