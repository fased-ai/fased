...
function logLineHttpError(err: unknown, context: string): void {
  ...
  if (typeof body === "string") {
    ...
    logVerbose(`line: ${context} failed (${summary}): ${formatErrorMessage(body)}`);
  }
}
...
async function pushLineMessages(
  ...
  if (opts.verbose) {
    const logMessage =
      ...
      `line: pushed ${messages.length} messages to ${formatLineIdForLog(chatId)}`;
...
export async function sendMessageLine(
  ...
  if (opts.replyToken) {
    await replyLineMessages(opts.replyToken, messages, opts, {
      verboseMessage: () => `line: replied to ${formatLineIdForLog(chatId)}`,
    });
  ...
  return pushLineMessages(chatId, messages, opts, {
    verboseMessage: (resolvedChatId) =>
      `line: pushed message to ${formatLineIdForLog(resolvedChatId)}`,
  });
}
...
export async function pushImageMessage(
  ...
  return pushLineMessages(to, [createImageMessage(originalContentUrl, previewImageUrl)], opts, {
    verboseMessage: (chatId) => `line: pushed image to ${formatLineIdForLog(chatId)}`,
  });
}
...
export async function pushLocationMessage(
  ...
  return pushLineMessages(to, [createLocationMessage(location)], opts, {
    verboseMessage: (chatId) => `line: pushed location to ${formatLineIdForLog(chatId)}`,
  });
}
...
export async function pushFlexMessage(
  ...
  return pushLineMessages(to, [flexMessage], opts, {
    ...
    verboseMessage: (chatId) => `line: pushed flex message to ${formatLineIdForLog(chatId)}`,
  });
}
...
export async function pushTemplateMessage(
  ...
  return pushLineMessages(to, [template], opts, {
    verboseMessage: (chatId) => `line: pushed template message to ${formatLineIdForLog(chatId)}`,
  });
}
...
export async function showLoadingAnimation(
  ...
  try {
    ...
    logVerbose(`line: showing loading animation to ${formatLineIdForLog(chatId)}`);
  } catch (err) {
...
export async function getUserProfile(
  ...
  try {
    ...
    logVerbose(
      `line: failed to fetch profile for ${formatLineIdForLog(userId)}: ${formatErrorMessage(err)}`,
    );
    return null;
  }
}
