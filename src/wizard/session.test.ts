...
describe("WizardSession", () => {
  ...
  test("redacts sensitive error details", async () => {
    const session = new WizardSession(async () => {
      throw new Error("failed https://example.test/setup?token=super-secret-token-value");
    });
    ...
    expect(done.error).not.toContain("super-secret-token-value");
