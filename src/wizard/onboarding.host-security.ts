...
function formatLocalDeviceTailnetRequirementNote(): string {
  return [
    ...
    "Linux/Fedora: install Tailscale on your own computer first, then run `sudo tailscale up` there.",
    "Linux install command for your own computer: `curl -fsSL https://tailscale.com/install.sh | sh`",
...
function formatTailnetSshVerificationNote(target: TailnetSshTarget): string {
  ...
  return [
    ...
    "Linux/Fedora local computer: `curl -fsSL https://tailscale.com/install.sh | sh`, then `sudo tailscale up`.",
