...
function formatWizardSessionError(err: unknown): string {
  const message = err instanceof Error ? err.message : String(err);
  return redactSensitiveText(redactSensitiveUrlLikeString(message), { mode: "tools" });
}
  ...
  private async run(prompter: WizardPrompter) {
    try {
      ...
      if (err instanceof WizardCancelledError) {
        ...
        this.status = "error";
        this.error = formatWizardSessionError(err);
      }
    } finally {
      this.resolveStep(null);
    }
