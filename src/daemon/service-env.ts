...
export function buildServiceEnvironment(params: {
  ...
  const stateDir = env.FASED_STATE_DIR;
  const configPath = env.FASED_CONFIG_PATH;
  ...
  return {
    ...
    FASED_SYSTEMD_UNIT: systemdUnit,
    FASED_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    FASED_SERVICE_KIND: GATEWAY_SERVICE_KIND,
...
export function buildTaskWorkerServiceEnvironment(params: {
  env: Record<string, string | undefined>;
  name?: string;
  platform?: NodeJS.Platform;
}): Record<string, string | undefined> {
  ...
  return {
    ...
    ...proxyEnv,
    NODE_EXTRA_CA_CERTS: nodeCaCerts,
    FASED_PROFILE: env.FASED_PROFILE,
    FASED_STATE_DIR: stateDir,
    FASED_CONFIG_PATH: configPath,
    FASED_TASK_WORKER_NAME: params.name,
...
export function resolveGatewayMaxOldSpaceMb(
  params: {
  ...
  const raw = (env.FASED_GATEWAY_MAX_OLD_SPACE_MB ?? "").trim();
  const parsed = parseInt(raw, 10);
  if (!isNaN(parsed) && parsed > 0) {
    return parsed;
  }
  return params.fallbackMb ?? 1024;
}
