...
describe("auditGatewayServiceConfig", () => {
  ...
  it("accepts the managed startup script as a canonical hosted startup command", async () => {
    const audit = await auditGatewayServiceConfig({
      env: { HOME: "/tmp" },
      platform: "linux",
      command: {
  ...
  it("accepts gateway service ports that match the expected config port", async () => {
    ...
    expect(
      audit.issues.some((issue) => issue.code === SERVICE_AUDIT_CODES.gatewayPortMismatch),
    ).toBe(false);
  });
