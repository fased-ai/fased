...
function hasCanonicalGatewayStartup(programArguments?: string[]): boolean {
  ...
  const hasGateway = programArguments.some((arg) => arg === "gateway");
  if (hasGateway) {
    return true;
  }
  const managedIndex = programArguments.indexOf("managed");
  if (managedIndex === -1) {
    return false;
  }
...
function auditGatewayCommand(programArguments: string[] | undefined, issues: ServiceConfigIssue[]) {
  ...
  if (!hasCanonicalGatewayStartup(programArguments)) {
    issues.push({
      code: SERVICE_AUDIT_CODES.gatewayCommandMissing,
      message: "Service command does not include a supported gateway startup command",
      level: "aggressive",
    });
  }
}
...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service FASED_GATEWAY_TOKEN does not match gateway.auth.token in fased.json",
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `fased gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
