...
describe("resolvePreferredNodePath", () => {
  ...
  it("ignores execPath when it is not node", async () => {
    ...
    expect(result).toBe(darwinNode);
    expect(execFile).toHaveBeenCalledTimes(1);
    expect(execFile).toHaveBeenCalledWith(darwinNode, expect.any(Array), {
      encoding: "utf8",
    });
  });
  ...
  it("skips system node when it is too old", async () => {
    ...
    // Node 22.11.x is below minimum 22.14.0
...
describe("resolveSystemNodeInfo", () => {
  ...
  it("marks node unsupported when node:sqlite is missing", async () => {
    mockNodePathPresent(darwinNode);

    const execFile = vi.fn().mockResolvedValue({
      stdout: JSON.stringify({ version: "24.11.1", sqliteAvailable: false }),
      stderr: "",
    });
  ...
  it("renders a warning when system node is too old", () => {
    ...
    expect(warning).toContain("below the required Node 24 recommended / Node 22.14+");
