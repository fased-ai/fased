...
): Promise<{ version: string | null; sqliteAvailable: boolean }> {
  try {
    const { stdout } = await execFileImpl(
      nodePath,
      [
        "-e",
        "const out={version:process.versions.node,sqliteAvailable:false};try{require('node:sqlite');out.sqliteAvailable=true;}catch{}process.stdout.write(JSON.stringify(out));",
      ],
      {
        encoding: "utf8",
      },
    ...
    if (!value) {
      return { version: null, sqliteAvailable: false };
    }
    try {
      const parsed = JSON.parse(value) as { version?: unknown; sqliteAvailable?: unknown };
      return {
        version: typeof parsed.version === "string" ? parsed.version : null,
        sqliteAvailable: parsed.sqliteAvailable === true,
      };
    } catch {
      return { version: value, sqliteAvailable: true };
    }
  } catch {
    return { version: null, sqliteAvailable: false };
  }
...
export async function resolveSystemNodeInfo(params: {
  ...
  const nodeInfo = await resolveNodeRuntimeInfo(systemNode, params.execFile ?? execFileAsync);
  return {
    path: systemNode,
    version: nodeInfo.version,
    sqliteAvailable: nodeInfo.sqliteAvailable,
    supported: isSupportedNodeVersion(nodeInfo.version) && nodeInfo.sqliteAvailable,
  };
}
...
export function renderSystemNodeWarning(
  ...
  const reason = systemNode.sqliteAvailable
    ? "below the required Node 24 recommended / Node 22.14+ minimum"
    : "missing built-in node:sqlite support";
  return `System Node ${versionLabel} at ${systemNode.path} is ${reason}.${selectedLabel} Install Node 24 from nodejs.org, Homebrew, or NodeSource.`;
}
...
export async function resolvePreferredNodePath(params: {
  ...
  // Prefer the node that is currently running `fased gateway install`.
  ...
  if (currentExecPath && isNodeExecPath(currentExecPath, platform)) {
    ...
    const nodeInfo = await resolveNodeRuntimeInfo(currentExecPath, execFileImpl);
    if (isSupportedNodeVersion(nodeInfo.version) && nodeInfo.sqliteAvailable) {
