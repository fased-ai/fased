...
describe("getMinimalServicePathParts - Linux user directories", () => {
  ...
  it("excludes user bin directories when HOME is undefined on Linux", () => {
    ...
    expect(result).toEqual(["/usr/local/bin", "/usr/bin", "/bin", "/snap/bin"]);
...
describe("buildMinimalServicePath", () => {
  ...
  it("excludes Linux user directories when HOME is not in env", () => {
    ...
    expect(parts).toEqual(["/usr/local/bin", "/usr/bin", "/bin", "/snap/bin"]);
...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.FASED_GATEWAY_PORT).toBe("18789");
    expect(env.FASED_GATEWAY_TOKEN).toBe("secret");
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", FASED_PROFILE: "work" },
      port: 18789,
    });
    expect(env.FASED_SYSTEMD_UNIT).toBe("fased-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.FASED_LAUNCHD_LABEL).toBe("ai.fased.work");
    }
  });
