...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `fased-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      FASED_LAUNCHD_LABEL: `ai.fased.launchd-int-${testId}`,
      FASED_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
