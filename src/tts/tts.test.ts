...
const { _test, resolveTtsConfig, maybeApplyTtsToPayload, getTtsProvider, listTtsPersonas } = tts;
...
describe("tts", () => {
  ...
  describe("listTtsPersonas", () => {
    it("exposes configured provider voice presets without leaking secrets", () => {
      ...
      const personas = listTtsPersonas(config, "/tmp/fased-tts-personas-test.json");
      const active = personas.find((persona) => persona.active);
      expect(active?.id).toBe("openai:nova");
      expect(active?.model).toBe("tts-1");

      expect(personas.some((persona) => persona.id === "openai:alloy")).toBe(true);
      expect(personas).toContainEqual(
      ...
      expect(JSON.stringify(personas)).not.toContain("secret-openai-key");
      expect(JSON.stringify(personas)).not.toContain("secret-eleven-key");
    });
  });
  ...
  describe("maybeApplyTtsToPayload", () => {
    ...
    const withMockedAutoTtsFetch = async (
      ...
      const prevPrefs = process.env.FASED_TTS_PREFS;
      process.env.FASED_TTS_PREFS = `/tmp/tts-test-${Date.now()}.json`;
      ...
      try {
        ...
        process.env.FASED_TTS_PREFS = prevPrefs;
      }
    };

    const taggedCfg: FasedAgentConfig = {
      ...baseCfg,
      messages: {
        ...baseCfg.messages!,
