...
import { stripMarkdown } from "../text/strip-markdown.js";
...
export function resolveTtsPrefsPath(config: ResolvedTtsConfig): string {
  ...
  const envPath = process.env.FASED_TTS_PREFS?.trim();
  if (envPath) {
...
export function listTtsPersonas(config: ResolvedTtsConfig, prefsPath: string): TtsPersonaEntry[] {
  const activeProvider = getTtsProvider(config, prefsPath);
  const openaiConfigured = isTtsProviderConfigured(config, "openai");
  const elevenlabsConfigured = isTtsProviderConfigured(config, "elevenlabs");
  const edgeConfigured = isTtsProviderConfigured(config, "edge");

  const openaiPersonas: TtsPersonaEntry[] = OPENAI_TTS_VOICES.map((voice) => ({
    id: `openai:${voice}`,
    label: `OpenAI ${voice}`,
    description: `OpenAI ${config.openai.model} voice ${voice}.`,
    provider: "openai",
    configured: openaiConfigured,
    active: activeProvider === "openai" && config.openai.voice === voice,
    source: "built-in",
    model: config.openai.model,
    voice,
  }));

  const elevenlabsPersona: TtsPersonaEntry = {
    id: `elevenlabs:${config.elevenlabs.voiceId}`,
    label: "ElevenLabs configured voice",
    description: `Configured ElevenLabs voice ${config.elevenlabs.voiceId}.`,
    provider: "elevenlabs",
  ...
  const edgePersona: TtsPersonaEntry = {
    id: `edge:${config.edge.voice}`,
    label: `Edge ${config.edge.voice}`,
    description: `Microsoft Edge TTS voice ${config.edge.voice}.`,
    provider: "edge",
  ...
  return [...openaiPersonas, elevenlabsPersona, edgePersona];
}
  ...
  for (const provider of providers) {
    ...
    try {
      if (provider === "edge") {
        ...
        const tempRoot = resolvePreferredFasedAgentTmpDir();
