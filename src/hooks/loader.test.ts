...
describe("loader", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "fased-hooks-loader-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["FASED_BUNDLED_HOOKS_DIR"]);
    process.env.FASED_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
