...
homepage: https://docs.fased.ai/automation/hooks#command-logger
metadata:
  {
    "fased":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.fased/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.fased/logs/commands.log ~/.fased/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/fased`:
   ```
   /home/username/.fased/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
