...
async function resolveAvailableMemoryFilename(params: {
  ...
  const base = `${params.dateStr}-${params.slug}`;
  let suffix = 1;
  while (true) {
...
async function saveSessionMemoryNow(event: Parameters<HookHandler>[0]): Promise<void> {
  try {
    ...
    const cfg = context.cfg as FasedAgentConfig | undefined;
    ...
    // Generate descriptive slug from session only when explicitly enabled.
    ...
    // Write to new memory file after constraining the generated filename to memoryDir.
