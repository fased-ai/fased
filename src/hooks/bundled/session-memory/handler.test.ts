...
let flushSessionMemoryWritesForTest: () => Promise<void>;

beforeAll(async () => {
  const module = await import("./handler.js");
  handler = module.default;
  flushSessionMemoryWritesForTest = (
    module as typeof module & { flushSessionMemoryWritesForTest: () => Promise<void> }
  ).flushSessionMemoryWritesForTest;
});
...
async function runNewWithPreviousSession(params: {
  ...
  cfg?: (tempDir: string) => FasedAgentConfig;
  action?: "new" | "reset";
}): Promise<{ tempDir: string; files: string[]; memoryContent: string }> {
  const tempDir = await makeTempWorkspace("fased-session-memory-");
...
function makeSessionMemoryConfig(tempDir: string, messages?: number): FasedAgentConfig {
  return {
    agents: { defaults: { workspace: tempDir } },
  ...
  it("does not block reset command handling on opt-in model slug generation", async () => {
    ...
    const sessionFile = await writeWorkspaceFile({
      ...
      content: createMockSessionContent([
        { role: "user", content: "Investigate slow reset" },
