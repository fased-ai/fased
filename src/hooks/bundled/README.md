...
This directory contains hooks that ship with FasedAgent. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.fased/workspace`)
...
```bash
fased hooks enable bootstrap-extra-files
```
...
**Output**: `~/.fased/logs/commands.log`
...
- **Managed hooks**: `~/.fased/hooks/` (shared across workspaces)
...
Hooks can be configured in `~/.fased/fased.json`:
...
2. Restart gateway: `pkill -9 -f 'fased.*gateway' && pnpm fased gateway`
3. Enable the hook: `fased hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.fased.ai/automation/hooks
