...
const SECRET_ARG_NAMES = new Set(["--token", "--hook-token", "--push-token"]);
...
export function redactUrlSecret(value: string): string {
  return value.replace(
    /([?&](?:token|hookToken|pushToken|apiKey|key)=)[^&\s]+/gi,
    `$1${REDACTED_SECRET}`,
  );
}

export function redactGmailCommandArgs(args: readonly string[]): string[] {
  ...
  for (const arg of args) {
    ...
    if (SECRET_ARG_NAMES.has(flagName)) {
      if (equalsIndex === -1) {
        redacted.push(arg);
        redactNext = true;
      } else {
        redacted.push(`${flagName}=${REDACTED_SECRET}`);
      }
      continue;
    }

    redacted.push(redactUrlSecret(arg));
  }
  return redacted;
}
