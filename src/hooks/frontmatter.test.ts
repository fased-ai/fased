...
`;
    ...
    expect(parsed.fased.emoji).toBe("💾");
    expect(parsed.fased.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.fased.emoji).toBe("📝");
    expect(parsed.fased.events).toEqual(["command"]);
    expect(parsed.fased.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.fased.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"fased": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"fased": {"events": ["test"]}}');
  });
...
describe("resolveFasedAgentMetadata", () => {
  it("extracts fased metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveFasedAgentMetadata(frontmatter);
  ...
  it("returns undefined when fased key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        fased: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with FasedAgent" },
            { id: "npm", kind: "npm", package: "@fased/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@fased/hook");
  });
...
`;
    ...
    const fased = resolveFasedAgentMetadata(frontmatter);
    expect(fased).toBeDefined();
    expect(fased?.emoji).toBe("💾");
    expect(fased?.events).toEqual(["command:new", "command:reset"]);
    expect(fased?.requires?.config).toEqual(["workspace.dir"]);
    expect(fased?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const fased = resolveFasedAgentMetadata(frontmatter);
    expect(fased?.emoji).toBe("disk");
    expect(fased?.events).toEqual(["command:new"]);
  });
});
