...
export async function runGmailSetup(opts: GmailSetupOptions) {
  ...
  const redactedSummary = {
    ...summary,
    pushEndpoint: redactUrlSecret(pushEndpoint),
    hookToken: REDACTED_SECRET,
    pushToken: REDACTED_SECRET,
  };

  if (opts.json) {
    defaultRuntime.log(JSON.stringify(redactedSummary, null, 2));
    return summary;
  }
  ...
  defaultRuntime.log(`- push endpoint: ${redactUrlSecret(pushEndpoint)}`);
  defaultRuntime.log(`- hook url: ${hookUrl}`);
  defaultRuntime.log(`- config: ${displayPath(CONFIG_PATH)}`);
  defaultRuntime.log(`Next: ${formatCliCommand("fased webhooks gmail run")}`);
  return summary;
}
...
function spawnGogServe(cfg: GmailHookRuntimeConfig) {
  ...
  defaultRuntime.log(`Starting gog ${redactGmailCommandArgs(args).join(" ")}`);
