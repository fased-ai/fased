...
const fixtureRoot = path.join(os.tmpdir(), `fased-hook-install-${randomUUID()}`);
let tempDirIndex = 0;
...
let installHooksFromArchive: typeof import("./install.js").installHooksFromArchive;
let installHooksFromNpmSpec: typeof import("./install.js").installHooksFromNpmSpec;
let installHooksFromPath: typeof import("./install.js").installHooksFromPath;
let isAddressInUseError: typeof import("./gmail-watcher.js").isAddressInUseError;

beforeAll(async () => {
  vi.resetModules();
  ({ runCommandWithTimeout } = await import("../process/exec.js"));
  ({ installHooksFromArchive, installHooksFromNpmSpec, installHooksFromPath } =
    await import("./install.js"));
  ({ isAddressInUseError } = await import("./gmail-watcher.js"));
});
...
function createHookPackageFiles(params: {
  rootDir: string;
  packageName: string;
  hookName: string;
  hookEntry?: string;
  ...
  const packageJson = {
    ...
    version: "0.0.1",
    fased: { hooks: [params.hookEntry ?? `./hooks/${params.hookName}`] },
    ...(params.dependencies ? { dependencies: params.dependencies } : {}),
  };
  const hookDir = path.join(params.rootDir, "hooks", params.hookName);
  fs.mkdirSync(hookDir, { recursive: true });
  fs.writeFileSync(path.join(params.rootDir, "package.json"), JSON.stringify(packageJson));
  fs.writeFileSync(
    ...
    [
      ...
      'metadata: {"fased":{"events":["command:new"]}}',
      "---",
      "",
      `# ${params.hookName}`,
    ].join("\n"),
...
async function createHookPackageArchive(params: {
  kind: "zip" | "tar" | "tgz";
  packageName: string;
  hookName: string;
  hookEntry?: string;
  ...
  createHookPackageFiles({ rootDir: packageDir, ...params });

  if (params.kind === "zip") {
    const zip = new JSZip();
    function addDirToZip(dir: string, prefix: string) {
    ...
    addDirToZip(packageDir, "package");
    return await zip.generateAsync({ type: "nodebuffer", compression: "STORE" });
  }

  const archivePath = path.join(workDir, params.kind === "tgz" ? "hooks.tgz" : "hooks.tar");
  await tar.c(
    {
      cwd: workDir,
      file: archivePath,
      gzip: params.kind === "tgz",
    },
    ["package"],
  );
...
describe("installHooksFromArchive", () => {
  it.each([
    ...
    const contents = await createHookPackageArchive({
      kind: tc.name as "zip" | "tar",
      packageName: `@fased/${tc.expectedPackId}`,
      hookName: tc.expectedHook,
    });
    const fixture = writeArchiveFixture({ fileName: tc.fileName, contents });
  ...
  it.each([
    ...
    const fixture = writeArchiveFixture({ fileName: "hooks.tar", contents });
    const result = await installHooksFromArchive({
      archivePath: fixture.archivePath,
      hooksDir: fixture.hooksDir,
    });
...
describe("installHooksFromPath", () => {
  it("uses --ignore-scripts for dependency install", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@fased/test-hooks",
        version: "0.0.1",
        fased: { hooks: ["./hooks/one-hook"] },
  ...
  it("rejects hook pack entries that traverse outside package directory", async () => {
    ...
    expect(result.error).toContain("fased.hooks entry escapes package directory");
  });
  ...
  it("rejects hook pack entries that escape via symlink", async () => {
    ...
    expect(result.error).toContain("fased.hooks entry resolves outside package directory");
  });
});
...
describe("installHooksFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    const npmPackHooksBuffer = await createHookPackageArchive({
      kind: "tgz",
      packageName: "@fased/test-hooks",
      hookName: "one-hook",
    });
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@fased/test-hooks@0.0.1");
    ...
    expectSingleNpmPackIgnoreScriptsCall({
      ...
      expectedSpec: "@fased/test-hooks@0.0.1",
    });
