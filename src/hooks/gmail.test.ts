...
import { type FasedAgentConfig, DEFAULT_GATEWAY_PORT } from "../config/config.js";
import {
  buildDefaultHookUrl,
  buildTopicPath,
  parseTopicPath,
  redactGmailCommandArgs,
  redactUrlSecret,
...
describe("gmail hook config", () => {
  ...
  it("resolves runtime config with defaults", () => {
    ...
    if (result.ok) {
      expect(result.value.account).toBe("fased@gmail.com");
  ...
  it("redacts hook secrets from command args", () => {
    expect(
      redactGmailCommandArgs([
        "gmail",
        "watch",
        ...
        "--hook-url",
        "https://node.example/gmail?token=push-token",
      ]),
    ).toEqual([
      "gmail",
      "watch",
      ...
      "--hook-token=<redacted>",
      "--hook-url",
      "https://node.example/gmail?token=<redacted>",
    ]);
  });

  it("redacts token query params from hook URLs", () => {
    expect(redactUrlSecret("https://node.example/gmail?token=push-token&ok=1")).toBe(
      "https://node.example/gmail?token=<redacted>&ok=1",
    );
  });
});
