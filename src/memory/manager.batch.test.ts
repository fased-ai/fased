...
describeWithNodeSqlite("memory indexing with OpenAI batches", () => {
  ...
  function createBatchCfg(): FasedAgentConfig {
    return {
      agents: {
        defaults: {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "fased-mem-batch-"));
