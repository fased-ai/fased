...
describeWithNodeSqlite("memory manager atomic reindex", () => {
  let fixtureRoot = "";
  let caseId = 0;
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "fased-mem-atomic-"));
  });

  beforeEach(async () => {
    vi.stubEnv("FASED_TEST_MEMORY_UNSAFE_REINDEX", "0");
    resetEmbeddingMocks();
    shouldFail = false;
