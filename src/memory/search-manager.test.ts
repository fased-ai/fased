...
function createQmdCfg(agentId: string): FasedAgentConfig {
  return {
...
describe("getMemorySearchManager caching", () => {
  ...
  it("uses filesystem keyword fallback when builtin sqlite is unavailable", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fased-memory-fs-fallback-"));
    const workspace = path.join(root, "workspace");
    await fs.mkdir(workspace, { recursive: true });
    await fs.writeFile(path.join(workspace, "MEMORY.md"), "Alpha fallback memory line.", "utf-8");
    const cfg: FasedAgentConfig = {
      agents: {
        defaults: {
          workspace,
          memorySearch: {
            store: { path: path.join(root, "memory.sqlite"), vector: { enabled: false } },
    ...
    try {
      ...
      expect(results[0]).toMatchObject({
        path: "MEMORY.md",
        snippet: "Alpha fallback memory line.",
        source: "memory",
      });
    } finally {
