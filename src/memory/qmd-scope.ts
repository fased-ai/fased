...
export function resolveQmdScopeDenial(
  scope: ResolvedQmdConfig["scope"],
  sessionKey?: string,
): string | undefined {
  if (isQmdScopeAllowed(scope, sessionKey)) {
    return undefined;
  }
  const parsed = parseQmdSessionScope(sessionKey);
  const channel = parsed.channel ?? "unknown";
  const chatType = parsed.chatType ?? "unknown";
  const key = sessionKey?.trim() || "<none>";
  return `qmd memory scope denies session key ${key} (channel=${channel}, chatType=${chatType})`;
}
