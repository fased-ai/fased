...
export async function getMemorySearchManager(params: {
  ...
  try {
    ...
    if (isNodeSqliteUnavailable(message)) {
      const { FilesystemMemorySearchManager } = await import("./filesystem-manager.js");
      ...
      if (manager) {
        log.warn(`builtin sqlite memory unavailable; using filesystem fallback: ${message}`);
        return { manager };
      }
    }
...
function isNodeSqliteUnavailable(message: string): boolean {
  return (
    message.includes("missing node:sqlite") ||
    message.includes("No such built-in module: node:sqlite") ||
    message.includes("No known conditions for")
  );
}
