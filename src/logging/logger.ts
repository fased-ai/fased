...
export const DEFAULT_LOG_DIR = resolvePreferredFasedAgentTmpDir();
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "fased.log"); // legacy single-file path

const LOG_PREFIX = "fased";
...
function resolveSettings(): ResolvedSettings {
  let cfg: FasedAgentConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg) {
    try {
      const loaded = requireConfig?.("../config/config.js") as
        | {
            loadConfig?: () => FasedAgentConfig;
          }
        | undefined;
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.FASED_TEST_FILE_LOG !== "1" ? "silent" : "info";
      ...
      if (nextBytes > settings.maxFileBytes) {
        if (!warnedAboutSizeCap) {
          ...
          process.stderr.write(
            `[fased] log file size cap reached; suppressing writes file=${settings.file} maxFileBytes=${settings.maxFileBytes}\n`,
          );
        }
        return;
      }
