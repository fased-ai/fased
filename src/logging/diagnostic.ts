...
function formatDiagnosticIdentifierForLog(value: unknown): string {
  ...
  try {
    return redactIdentifier(JSON.stringify(value));
  } catch {
    return redactIdentifier(Object.prototype.toString.call(value));
  }
}

function formatDiagnosticErrorForLog(err: unknown): string {
  ...
  try {
    return redactSensitiveText(JSON.stringify(err));
  } catch {
...
export function logWebhookReceived(params: {
  ...
  if (diag.isEnabled("debug")) {
    diag.debug(
      `webhook received: channel=${params.channel} type=${params.updateType ?? "unknown"} chatId=${formatDiagnosticIdentifierForLog(
        params.chatId,
...
export function logWebhookProcessed(params: {
  ...
  if (diag.isEnabled("debug")) {
    diag.debug(
      ...
      } chatId=${formatDiagnosticIdentifierForLog(params.chatId)} duration=${
        params.durationMs ?? 0
      }ms processed=${webhookStats.processed}`,
    );
  }
  emitDiagnosticEvent({
...
export function logWebhookError(params: {
  ...
  diag.error(
    `webhook error: channel=${params.channel} type=${params.updateType ?? "unknown"} chatId=${formatDiagnosticIdentifierForLog(
      params.chatId,
    )} error="${redactSensitiveText(params.error)}" errors=${webhookStats.errors}`,
  );
  emitDiagnosticEvent({
    type: "webhook.error",
    channel: params.channel,
...
export function logMessageQueued(params: {
  ...
  if (diag.isEnabled("debug")) {
    diag.debug(
      `message queued: sessionId=${formatDiagnosticIdentifierForLog(
        state.sessionId,
      )} sessionKey=${formatDiagnosticIdentifierForLog(state.sessionKey)} source=${
        params.source
...
export function logMessageProcessed(params: {
  ...
  if (wantsLog) {
    const payload = `message processed: channel=${params.channel} chatId=${formatDiagnosticIdentifierForLog(
      params.chatId,
    )} messageId=${formatDiagnosticIdentifierForLog(
      params.messageId,
    )} sessionId=${formatDiagnosticIdentifierForLog(
      params.sessionId,
    )} sessionKey=${formatDiagnosticIdentifierForLog(params.sessionKey)} outcome=${
      params.outcome
    } duration=${params.durationMs ?? 0}ms${params.reason ? ` reason=${params.reason}` : ""}${
      params.error ? ` error="${redactSensitiveText(params.error)}"` : ""
    }`;
...
export function logSessionStateChange(
  ...
  if (!isProbeSession && diag.isEnabled("debug")) {
    diag.debug(
      `session state: sessionId=${formatDiagnosticIdentifierForLog(
        state.sessionId,
      )} sessionKey=${formatDiagnosticIdentifierForLog(state.sessionKey)} prev=${prevState} new=${
        params.state
      } reason="${params.reason ?? ""}" queueDepth=${state.queueDepth}`,
    );
  }
  emitDiagnosticEvent({
    type: "session.state",
...
export function logSessionStuck(params: SessionRef & { state: SessionStateValue; ageMs: number }) {
  const state = getDiagnosticSessionState(params);
  diag.warn(
    `stuck session: sessionId=${formatDiagnosticIdentifierForLog(
      state.sessionId,
    )} sessionKey=${formatDiagnosticIdentifierForLog(state.sessionKey)} state=${
      params.state
    } age=${Math.round(params.ageMs / 1000)}s queueDepth=${state.queueDepth}`,
  );
  emitDiagnosticEvent({
    type: "session.stuck",
    sessionId: state.sessionId,
...
export function logRunAttempt(params: SessionRef & { runId: string; attempt: number }) {
  diag.debug(
    `run attempt: sessionId=${formatDiagnosticIdentifierForLog(
      params.sessionId,
    )} sessionKey=${formatDiagnosticIdentifierForLog(
      params.sessionKey,
    )} runId=${formatDiagnosticIdentifierForLog(params.runId)} attempt=${params.attempt}`,
  );
  emitDiagnosticEvent({
    type: "run.attempt",
    sessionId: params.sessionId,
...
export function logToolLoopAction(
  ...
  const payload = `tool loop: sessionId=${formatDiagnosticIdentifierForLog(
    params.sessionId,
  )} sessionKey=${formatDiagnosticIdentifierForLog(params.sessionKey)} tool=${
    params.toolName
  } level=${params.level} action=${params.action} detector=${params.detector} count=${
    params.count
  }${params.pairedToolName ? ` pairedTool=${params.pairedToolName}` : ""} message="${redactSensitiveText(
    params.message,
  )}"`;
...
export function logActiveRuns() {
  const activeSessions = Array.from(diagnosticSessionStates.entries())
    ...
    .map(
      ([id, s]) =>
        `${formatDiagnosticIdentifierForLog(id)}(q=${s.queueDepth},age=${Math.round(
          (Date.now() - s.lastActivity) / 1000,
        )}s)`,
    );
