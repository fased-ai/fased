...
describe("normalizeCronJobCreate", () => {
  it("preserves task failure alert settings", () => {
    const normalized = normalizeCronJobCreate({
      name: "failure alert",
    ...
    const patch = normalizeCronJobPatch({ failureAlert: false }) as Record<string, unknown>;
    expect(patch.failureAlert).toBe(false);
  });
  ...
  it("normalizes task execution policy metadata", () => {
    const normalized = normalizeCronJobCreate({
      name: "policy",
      schedule: { kind: "every", everyMs: 60_000 },
      ...
      executionPolicy: {
        ...
        successCriteria: " report when complete ",
        executionMode: " SKILL-ONLY ",
        memoryScope: " search ",
        skillScope: " selected ",
        allowedSkills: [" wallet ", "wallet", "", 7],
        skillAction: {
          toolName: " wallet ",
        ...
        stop: {
          onSuccess: true,
          outputIncludes: [" done ", "done", "", 3],
          maxSuccessfulRuns: 2.8,
          maxTotalRuns: 4,
        },
        evaluator: {
          escalateOnSignal: true,
          signalIncludes: [" FASED_ESCALATE:true ", "FASED_ESCALATE:true", "", 3],
          maxEscalations: 1.8,
        },
        repairPolicy: {
          autoRetryReplacement: false,
          autoStopOptionalSources: true,
          maxAutoRepairsPerRun: 2.8,
          requireApprovalForPrimarySource: false,
        },
        planner: {
          source: "ignored",
          strategy: " CHEAP-MODEL ",
          rationale: " monitor loop ",
          confidence: " MEDIUM ",
          signals: [" monitor ", "monitor", "", 3],
        },
      },
    }) as unknown as Record<string, unknown>;
    ...
    expect(normalized.executionPolicy).toEqual({
      ...
      evaluator: {
        escalateOnSignal: true,
        signalIncludes: ["FASED_ESCALATE:true"],
        maxEscalations: 1,
      },
      repairPolicy: {
        autoRetryReplacement: false,
  ...
  it("normalizes workflow graph revisions without throwing", () => {
    const normalized = normalizeCronJobCreate({
      name: "policy graph",
      schedule: { kind: "every", everyMs: 60_000 },
      payload: { kind: "agentTurn", message: "check market" },
    ...
    expect(normalized.executionPolicy).toMatchObject({
      planner: {
        graph: {
          graphRevision: 2,
          parentRevision: 1,
...
describe("normalizeCronJobPatch", () => {
  ...
  it("allows clearing execution policy metadata", () => {
    const normalized = normalizeCronJobPatch({
      executionPolicy: null,
    }) as unknown as Record<string, unknown>;

    expect(normalized.executionPolicy).toBeNull();
  });
});
