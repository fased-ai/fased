...
import { isJobDue, nextWakeAtMs, recomputeNextRunsForMaintenance } from "./service/jobs.js";
...
describe("issue #13992 regression - cron jobs skip execution", () => {
  ...
  it("should repair zero nextRunAtMs during maintenance", () => {
    const now = Date.now();
  ...
  it("does not treat zero nextRunAtMs as scheduled or due", () => {
    const now = Date.now();
  ...
  it("should recover stuck running markers during maintenance", () => {
    const now = Date.now();
    ...
    expect(job.state.lastRunAtMs).toBe(stuckTime);
    expect(job.state.lastRecoveredRun).toEqual(
      expect.objectContaining({
        phase: "recovered",
        reason: "Recovered stale task run lease.",
      }),
    );
