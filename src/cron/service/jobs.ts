...
import { matchingTrustedSourcesForTask } from "../trusted-sources.js";
import type {
  CronDelivery,
  CronDeliveryPatch,
  CronJob,
...
import {
  ...
  hasActiveCronJobRun,
  isCronJobRunLeaseExpired,
  recoverCronJobRunLease,
} from "./run-lease.js";
import type { CronServiceState } from "./state.js";

const STUCK_RUN_MS = 2 * 60 * 60 * 1000;
const RUN_BUDGET_WINDOW_MS = 60 * 60 * 1000;

function payloadTaskText(payload: CronPayload): string {
  return payload.kind === "agentTurn" ? payload.message : payload.text;
}

function planJobTaskPolicy(params: {
  state?: CronServiceState;
  agentId?: string;
  sessionKey?: string;
  name?: string;
  payload: CronPayload;
  executionPolicy: CronJobCreate["executionPolicy"];
}) {
  ...
  return planTaskExecutionPolicy({
    ...
    trustedSources: matchingTrustedSourcesForTask({
      ...
      text: [params.name ?? "", payloadTaskText(params.payload)].join(" "),
    }),
  });
}
...
export function reserveCronJobRunBudget(job: CronJob, nowMs: number): string | undefined {
  const rawMaxRuns = job.executionPolicy?.budget?.maxRunsPerHour;
  if (typeof rawMaxRuns !== "number" || !Number.isFinite(rawMaxRuns)) {
    return undefined;
  }
  const maxRuns = rawMaxRuns <= 0 ? 0 : Math.max(1, Math.floor(rawMaxRuns));
  const safeNow = Math.max(0, Math.floor(nowMs));
  const windowStartedAt =
    typeof job.state.budgetWindowStartedAtMs === "number" &&
    Number.isFinite(job.state.budgetWindowStartedAtMs)
      ? Math.max(0, Math.floor(job.state.budgetWindowStartedAtMs))
      : undefined;
  const shouldResetWindow =
    windowStartedAt === undefined || safeNow - windowStartedAt >= RUN_BUDGET_WINDOW_MS;
  if (shouldResetWindow) {
    job.state.budgetWindowStartedAtMs = safeNow;
    job.state.budgetRunsInWindow = 0;
  }

  const currentRuns =
    typeof job.state.budgetRunsInWindow === "number" &&
    Number.isFinite(job.state.budgetRunsInWindow)
      ? Math.max(0, Math.floor(job.state.budgetRunsInWindow))
      : 0;
  if (currentRuns >= maxRuns) {
    return `Task run budget exceeded: ${currentRuns}/${maxRuns} runs used in the current hour.`;
  }
  job.state.budgetRunsInWindow = currentRuns + 1;
  return undefined;
}
...
export function assertSupportedJobSpec(job: Pick<CronJob, "sessionTarget" | "payload">) {
  if (job.sessionTarget === "main" && job.payload.kind !== "systemEvent") {
    throw new Error('main-session tasks require payload.kind="systemEvent"');
  }
  if (job.sessionTarget === "isolated" && job.payload.kind !== "agentTurn") {
    throw new Error('isolated tasks require payload.kind="agentTurn"');
  }
}
...
function normalizeJobTickState(params: { state: CronServiceState; job: CronJob; nowMs: number }): {
  ...
  if (isCronJobRunLeaseExpired({ job, nowMs, legacyStuckMs: STUCK_RUN_MS })) {
    state.deps.log.warn(
      {
        jobId: job.id,
        runningAtMs: job.state.runningAtMs,
        runId: job.state.activeRun?.runId,
        leaseExpiresAtMs: job.state.activeRun?.leaseExpiresAtMs,
      },
      "cron: recovered stale task run lease",
    );
    recoverCronJobRunLease(job, nowMs, "Recovered stale task run lease.");
    changed = true;
  }
...
export function createJob(state: CronServiceState, input: CronJobCreate): CronJob {
  ...
  const sessionKey = normalizeOptionalSessionKey((input as { sessionKey?: unknown }).sessionKey);
  const schedule =
  ...
  const job: CronJob = {
    ...
    executionPolicy: planJobTaskPolicy({
      state,
      agentId,
      sessionKey,
      name: input.name,
...
export function applyJobPatch(job: CronJob, patch: CronJobPatch) {
  ...
  if ("executionPolicy" in patch) {
    job.executionPolicy = patch.executionPolicy ?? undefined;
  }
  ...
  if (
    ...
    ("executionPolicy" in patch || patch.payload || "name" in patch)
  ) {
    job.executionPolicy =
      job.executionPolicy || job.payload.kind === "agentTurn"
        ? planTaskExecutionPolicy({
            name: job.name,
            message: payloadTaskText(job.payload),
...
export function isJobDue(job: CronJob, nowMs: number, opts: { forced: boolean }) {
  ...
  return (
    job.enabled && hasScheduledNextRunAtMs(job.state.nextRunAtMs) && nowMs >= job.state.nextRunAtMs
  );
}
