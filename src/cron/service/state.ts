...
import type {
  CronGraphNodeHandlerParams,
  CronGraphNodeHandlerResult,
  CronTaskGraphContextItem,
} from "../graph-context.js";
import type { CronTaskRunQueueSummary } from "../task-run-queue.js";
import type {
  CronDeliveryStatus,
  CronJob,
  CronJobCreate,
...
export type CronEvent = {
  jobId: string;
  action: "added" | "updated" | "removed" | "started" | "finished" | "blocked";
  runAtMs?: number;
  durationMs?: number;
...
export type CronServiceDeps = {
  ...
  runIsolatedAgentJob: (params: {
    ...
    /**
     * When true, execution should produce a replayable result but leave
     * outbound channel delivery to the durable queue's `deliver` step.
     */
  ...
  }) => Promise<
    {
      ...
      CronRunTelemetry
  >;
  runGraphNodeHandler?: (params: CronGraphNodeHandlerParams) => Promise<CronGraphNodeHandlerResult>;
  deliverIsolatedAgentJobResult?: (params: {
    job: CronJob;
    runId: string;
  ...
  }) => Promise<
    {
      ...
      CronRunTelemetry
  >;
  preflightJobAccess?: (job: CronJob) => CronTaskAccessBlock | undefined;
