...
import { queueCancel } from "./service/ops.js";
import { createCronServiceState, type CronEvent } from "./service/state.js";
import { DEFAULT_JOB_TIMEOUT_MS, executeJobCore, onTimer, runMissedJobs } from "./service/timer.js";
import { CRON_TASK_RUN_QUEUE_WORKER_STEP_ID, readCronTaskRunQueue } from "./task-run-queue.js";
...
async function startCronForStore(params: {
  storePath: string;
  cronEnabled?: boolean;
  nowMs?: CronServiceOptions["nowMs"];
...
describe("Cron issue regressions", () => {
  ...
  it("repairs zero nextRunAtMs on non-schedule updates", async () => {
    const store = await makeStorePath();
  ...
  it("hard-aborts active local queued runs when queue cancel targets the run id", async () => {
    vi.useRealTimers();
    const store = await makeStorePath();
    const scheduledAt = Date.parse("2026-02-15T13:00:00.000Z");
    const cronJob = createIsolatedRegressionJob({
    ...
    const state = createCronServiceState({
      ...
      runIsolatedAgentJob: vi.fn(async ({ abortSignal }) => {
        ...
        return { status: "ok" as const, summary: "late result should not apply" };
      }),
    });
    ...
    const runId = state.store?.jobs.find((entry) => entry.id === "cancel-active-run")?.state
      .activeRun?.runId;
    expect(runId).toBeTruthy();

    const cancel = await queueCancel(state, runId!, "Operator canceled run.");
    expect(cancel).toMatchObject({ ok: true, aborted: true });
    ...
    expect(observedAbortSignal?.reason).toBe("Operator canceled run.");
    expect(state.activeRunAbortControllers.size).toBe(0);
    const job = state.store?.jobs.find((entry) => entry.id === "cancel-active-run");
    expect(job?.state.activeRun).toBeUndefined();
    expect(job?.state.lastStatus).toBe("skipped");
    expect(job?.state.lastError).toBe("Operator canceled run.");

    const queue = await readCronTaskRunQueue({ storePath: store.storePath });
    const run = queue.runs.find((entry) => entry.runId === runId);
    expect(run).toEqual(expect.objectContaining({ status: "canceled" }));
    expect(run?.result?.summary).toBeUndefined();
    expect(run?.steps.find((step) => step.id === CRON_TASK_RUN_QUEUE_WORKER_STEP_ID)).toEqual(
      expect.objectContaining({ status: "canceled", error: "Operator canceled run." }),
    );
  });
  ...
  it("respects abort signals while retrying main-session wake-now heartbeat runs", async () => {
    ...
    const state = createCronServiceState({
      cronEnabled: true,
      storePath: "/tmp/fased-cron-abort-test/jobs.json",
      log: noopLogger,
      nowMs: () => Date.now(),
      enqueueSystemEvent,
      requestHeartbeatNow,
  ...
  it("skips runs after a task exhausts maxRunsPerHour budget", async () => {
    vi.useRealTimers();
    const store = await makeStorePath();
    let now = Date.parse("2026-02-06T10:00:00.000Z");
    const events: CronEvent[] = [];
    ...
    try {
      ...
      const finished = events.filter((evt) => evt.action === "finished");
      expect(runIsolatedAgentJob).toHaveBeenCalledOnce();
      expect(finished.at(-1)?.status).toBe("skipped");
      expect(finished.at(-1)?.error).toContain("run budget exceeded");
      expect(cron.getJob(job.id)?.state.budgetRunsInWindow).toBe(1);
    } finally {
      cron.stop();
    }
  });

  it("emits task execution policy telemetry for isolated runs", async () => {
    vi.useRealTimers();
    const store = await makeStorePath();
    ...
    try {
      ...
      const finished = events.filter((evt) => evt.action === "finished");
      expect(finished.at(-1)?.policy).toMatchObject({
        requestedExecutionMode: "no-model",
        effectiveExecutionMode: "no-model",
        memoryScope: "none",
        skillScope: "none",
  ...
  it("disables recurring tasks when stop policy matches run output", async () => {
    vi.useRealTimers();
    const store = await makeStorePath();
    const now = Date.parse("2026-02-06T10:00:00.000Z");
    const events: CronEvent[] = [];
    const runIsolatedAgentJob = vi.fn().mockResolvedValue({
      status: "ok",
      summary: "wallet check complete",
      outputText: "DONE: wallet balance checked",
      policy: {
        objective: "watch wallet",
        successCriteria: "stop when done",
        requestedExecutionMode: "agent-turn",
        effectiveExecutionMode: "agent-turn",
        stop: { outputIncludes: ["done"] },
      },
    }) as CronServiceOptions["runIsolatedAgentJob"];
    const cron = await startCronForStore({
      storePath: store.storePath,
    ...
    try {
      ...
      expect(stored?.state.stopReason).toBe("outputIncludes:done");
      const finished = events.filter((evt) => evt.action === "finished");
      expect(finished.at(-1)?.policy).toMatchObject({
        objective: "watch wallet",
        successCriteria: "stop when done",
  ...
  it("disables recurring tasks when success criteria is semantically complete", async () => {
    vi.useRealTimers();
    const store = await makeStorePath();
    const now = Date.parse("2026-02-06T10:00:00.000Z");
    const runIsolatedAgentJob = vi.fn().mockResolvedValue({
      status: "ok",
      summary: "Success criteria met: wallet report delivered.",
      outputText: "Task complete. No further action is needed.",
      policy: {
        objective: "watch wallet",
        successCriteria: "wallet report delivered",
        requestedExecutionMode: "agent-turn",
        effectiveExecutionMode: "agent-turn",
        stop: { maxTotalRuns: 10 },
      },
    }) as CronServiceOptions["runIsolatedAgentJob"];
    const cron = await startCronForStore({
      storePath: store.storePath,
    ...
    try {
      ...
      const stored = cron.getJob(job.id);
      expect(stored?.enabled).toBe(false);
      expect(stored?.state.stopReason).toBe("successCriteria");
    } finally {
      cron.stop();
    }
  });
