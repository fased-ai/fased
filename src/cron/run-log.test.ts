...
describe("cron run log", () => {
  ...
  it("appends JSONL and prunes by line count", async () => {
    await withRunLogDir("fased-cron-log-", async (dir) => {
  ...
  it("reads newest entries and filters by jobId", async () => {
    await withRunLogDir("fased-cron-log-read-", async (dir) => {
  ...
  it("ignores invalid and non-finished lines while preserving delivery fields", async () => {
    await withRunLogDir("fased-cron-log-filter-", async (dir) => {
      const logPath = path.join(dir, "runs", "job-1.jsonl");
  ...
  it("reads telemetry fields", async () => {
    await withRunLogDir("fased-cron-log-telemetry-", async (dir) => {
      const logPath = path.join(dir, "runs", "job-1.jsonl");
  ...
  it("reads task policy telemetry fields", async () => {
    await withRunLogDir("fased-cron-log-policy-", async (dir) => {
      const logPath = path.join(dir, "runs", "job-1.jsonl");
      ...
      await appendCronRunLog(logPath, {
        ...
        policy: {
          ...
          budget: { maxTokensPerRun: 1000, maxCostUsdPerRun: 0.05, maxRunsPerHour: 12 },
          planner: {
            source: "heuristic",
          ...
          sourceQuality: {
            bestSourceId: "source-fetch-gateway",
            bestScore: 0.93,
            lowQualityCount: 1,
            unavailableCount: 0,
          },
  ...
  it("cleans up pending-write bookkeeping after appends complete", async () => {
    await withRunLogDir("fased-cron-log-pending-", async (dir) => {
