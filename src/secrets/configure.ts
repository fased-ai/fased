...
function getSecretProviders(config: FasedAgentConfig): Record<string, SecretProviderConfig> {
...
function removeSecretProvider(config: FasedAgentConfig, providerAlias: string): boolean {
  if (!isRecord(config.secrets?.providers)) {
    return false;
  }
...
function buildCandidates(config: FasedAgentConfig): ConfigureCandidate[] {
...
async function configureProvidersInteractive(config: FasedAgentConfig): Promise<void> {
  while (true) {
...
export async function runSecretsConfigureInteractive(
  ...
  if (!params.providersOnly) {
    ...
    if (candidates.length === 0) {
      throw new Error("No configurable secret-bearing fields found in fased.json.");
    }
  ...
  const plan: SecretsApplyPlan = {
    ...
    generatedBy: "fased secrets configure",
