...
import { resolveStateDir, type FasedAgentConfig } from "../config/config.js";
...
function collectAuthStorePaths(config: FasedAgentConfig, stateDir: string): string[] {
...
export async function runSecretsAudit(
  ...
  const previousAuthStoreReadOnly = process.env.FASED_AUTH_STORE_READONLY;
  process.env.FASED_AUTH_STORE_READONLY = "1";
  try {
    ...
    const config = snapshot.valid ? snapshot.config : ({} as FasedAgentConfig);
    ...
    if (previousAuthStoreReadOnly === undefined) {
      delete process.env.FASED_AUTH_STORE_READONLY;
    } else {
      process.env.FASED_AUTH_STORE_READONLY = previousAuthStoreReadOnly;
    }
  }
}
