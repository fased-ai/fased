...
describe("secret ref resolver", () => {
  ...
  it("does not fall back to process.env when an explicit env is supplied", async () => {
    const previous = process.env.FASED_SECRET_RESOLVE_SHOULD_NOT_LEAK;
    process.env.FASED_SECRET_RESOLVE_SHOULD_NOT_LEAK = "host-secret";
    try {
      await expect(
        resolveSecretRefString(
          { source: "env", provider: "default", id: "FASED_SECRET_RESOLVE_SHOULD_NOT_LEAK" },
          {
            config: {},
            env: {},
          },
        ),
      ).rejects.toThrow('Environment variable "FASED_SECRET_RESOLVE_SHOULD_NOT_LEAK" is missing');
    } finally {
      if (previous === undefined) {
        delete process.env.FASED_SECRET_RESOLVE_SHOULD_NOT_LEAK;
      } else {
        process.env.FASED_SECRET_RESOLVE_SHOULD_NOT_LEAK = previous;
      }
    }
  });
  ...
  it("resolves file refs in json mode", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fased-secrets-resolve-file-"));
  ...
  it("resolves exec refs with protocolVersion 1 response", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fased-secrets-resolve-exec-"));
    cleanupRoots.push(root);
  ...
  it("supports non-JSON single-value exec output when jsonOnly is false", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fased-secrets-resolve-exec-plain-"));
    cleanupRoots.push(root);
  ...
  it("rejects symlink command paths unless allowSymlinkCommand is enabled", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fased-secrets-resolve-exec-link-"));
    cleanupRoots.push(root);
  ...
  it("handles Homebrew-style symlinked exec commands with args only when explicitly allowed", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fased-secrets-resolve-homebrew-"));
    cleanupRoots.push(root);
  ...
  it("checks trustedDirs against resolved symlink target", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fased-secrets-resolve-exec-link-"));
    const outside = await fs.mkdtemp(path.join(os.tmpdir(), "fased-secrets-resolve-exec-out-"));
    cleanupRoots.push(root);
  ...
  it("rejects exec refs when protocolVersion is not 1", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fased-secrets-resolve-exec-protocol-"));
    cleanupRoots.push(root);
  ...
  it("does not pass process.env values to exec providers when explicit env is supplied", async () => {
    if (process.platform === "win32") {
      return;
    }
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fased-secrets-resolve-exec-env-"));
    cleanupRoots.push(root);
    const scriptPath = path.join(root, "resolver-env.mjs");
    const previous = process.env.FASED_SECRET_EXEC_SHOULD_NOT_LEAK;
    process.env.FASED_SECRET_EXEC_SHOULD_NOT_LEAK = "host-secret";
    await writeSecureFile(
      scriptPath,
      [
        `#!${process.execPath}`,
        "const value = process.env.FASED_SECRET_EXEC_SHOULD_NOT_LEAK ?? 'missing';",
        "process.stdout.write(JSON.stringify({ protocolVersion: 1, values: { 'env/value': value } }));",
      ].join("\n"),
      0o700,
    );
    ...
    try {
      const value = await resolveSecretRefString(
        { source: "exec", provider: "execmain", id: "env/value" },
        {
          config: {
            secrets: {
              providers: {
                execmain: {
                  source: "exec",
                  command: scriptPath,
                  passEnv: ["FASED_SECRET_EXEC_SHOULD_NOT_LEAK"],
                },
              },
            },
          },
      ...
      if (previous === undefined) {
        delete process.env.FASED_SECRET_EXEC_SHOULD_NOT_LEAK;
      } else {
        process.env.FASED_SECRET_EXEC_SHOULD_NOT_LEAK = previous;
      }
    }
  });
  ...
  it("rejects exec refs when response omits requested id", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fased-secrets-resolve-exec-id-"));
    cleanupRoots.push(root);
  ...
  it("rejects exec refs with invalid JSON when jsonOnly is true", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fased-secrets-resolve-exec-json-"));
    cleanupRoots.push(root);
  ...
  it("supports file singleValue mode with id=value", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fased-secrets-resolve-single-value-"));
    cleanupRoots.push(root);
  ...
  it("times out file provider reads when timeoutMs elapses", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fased-secrets-resolve-timeout-"));
    cleanupRoots.push(root);
    const filePath = path.join(root, "secrets.json");
    await writeSecureFile(
      filePath,
