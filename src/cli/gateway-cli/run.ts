...
const GATEWAY_RUN_INHERITED_OPTIONS = [
  "port",
  "bind",
  "token",
  "auth",
...
function inheritGatewayRunOptions(opts: GatewayRunOpts, command: Command): GatewayRunOpts {
  const merged: Record<string, unknown> = { ...opts };
  for (const name of GATEWAY_RUN_INHERITED_OPTIONS) {
    const inherited = inheritOptionFromParent(command, name);
    if (inherited !== undefined) {
      merged[name] = inherited;
    }
  }
  return merged as GatewayRunOpts;
}
...
async function runGatewayCommand(opts: GatewayRunOpts) {
  ...
  if (opts.claudeCliLogs) {
    ...
    process.env.FASED_CLAUDE_CLI_LOG_OUTPUT = "1";
  }
  ...
  if (opts.rawStream) {
    process.env.FASED_RAW_STREAM = "1";
  }
  const rawStreamPath = toOptionString(opts.rawStreamPath);
  if (rawStreamPath) {
    process.env.FASED_RAW_STREAM_PATH = rawStreamPath;
  }
  ...
  if (!opts.allowUnconfigured && mode !== "local") {
    if (!configExists) {
      defaultRuntime.error(
        `Missing config. Run \`${formatCliCommand("fased setup")}\` or set gateway.mode=local (or pass --allow-unconfigured).`,
      );
    } else {
      defaultRuntime.error(
  ...
  if (resolvedAuthMode === "token" && !hasToken && !resolvedAuth.allowTailscale) {
    defaultRuntime.error(
      [
        ...
        "Set gateway.auth.token (or FASED_GATEWAY_TOKEN), or pass --token.",
        ...authHints,
      ]
        .filter(Boolean)
        .join("\n"),
  ...
  if (resolvedAuthMode === "password" && !hasPassword) {
    defaultRuntime.error(
      [
        ...
        "Set gateway.auth.password (or FASED_GATEWAY_PASSWORD), or pass --password.",
        ...authHints,
      ]
        .filter(Boolean)
        .join("\n"),
  ...
  if (bind !== "loopback" && !hasSharedSecret && resolvedAuthMode !== "trusted-proxy") {
    defaultRuntime.error(
      [
        ...
        "Set gateway.auth.token/password (or FASED_GATEWAY_TOKEN/FASED_GATEWAY_PASSWORD) or pass --token/--password.",
        ...authHints,
      ]
        .filter(Boolean)
        .join("\n"),
  ...
  try {
    ...
    if (
      ...
      defaultRuntime.error(
        `Gateway failed to start: ${errMessage}\nIf the gateway is supervised, stop it with: ${formatCliCommand("fased gateway stop")}`,
      );
      try {
...
export function addGatewayRunCommand(cmd: Command): Command {
  return cmd
    ...
    .option(
      "--token <token>",
      "Shared token required in connect.params.auth.token (default: FASED_GATEWAY_TOKEN env if set)",
    )
    ...
    .action(async (opts, command) => {
      await runGatewayCommand(inheritGatewayRunOptions(opts, command));
    });
}
