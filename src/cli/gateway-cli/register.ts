...
function requireAdminRpcConfirmation(opts: { yes?: boolean }, method: string) {
  if (opts.yes !== true) {
    throw new Error(`${method} is side-effecting; rerun with --yes to confirm`);
  }
}

function renderAdminRpcResult(method: string, result: unknown, json: boolean | undefined) {
  if (json) {
  ...
  const rich = isRich();
  defaultRuntime.log(
    `${colorize(rich, theme.heading, "Admin RPC")}: ${colorize(rich, theme.muted, method)}`,
  );
  defaultRuntime.log(
    colorize(rich, theme.muted, "Sanitized mutating-admin-rpc audit event written by the gateway."),
  );
  defaultRuntime.log(JSON.stringify(result, null, 2));
}
...
export function registerGatewayCli(program: Command) {
      ...
      .addHelpText(
        ...
        () =>
          `\n${theme.heading("Examples:")}\n${formatHelpExamples([
            ["fased gateway run", "Run the gateway in the foreground."],
            ["fased gateway status", "Show service status and probe reachability."],
            ["fased gateway discover", "Find local and wide-area gateway beacons."],
            ["fased gateway call health", "Call a gateway RPC method directly."],
          ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/gateway", "docs.fased.ai/cli/gateway")}\n`,
      ),
  );
  ...
  const admin = gateway
    .command("admin")
    .description("Operator-only side-effecting Gateway RPC controls");

  gatewayCallOpts(
    admin
      .command("inject-chat")
      .description("Inject an operator-labeled assistant message into a session")
      .requiredOption("--session-key <key>", "Session key")
      .requiredOption("--message <text>", "Message text")
      .option("--yes", "Confirm this side-effecting call", false)
      .action(async (opts, command) => {
        await runGatewayCommand(async () => {
          requireAdminRpcConfirmation(opts, "chat.inject");
          const rpcOpts = resolveGatewayRpcOptions(opts, command);
          const result = await callGatewayCli("chat.inject", rpcOpts, {
            sessionKey: requireTextOption(opts.sessionKey, "session key"),
            message: requireTextOption(opts.message, "message"),
            label: "operator-cli",
          });
          renderAdminRpcResult("chat.inject", result, rpcOpts.json);
        }, "Gateway admin chat inject failed");
      }),
  );
  ...
  gatewayCallOpts(
    admin
      .command("push-test")
      .description("Send a real APNs test push to a registered node")
      .requiredOption("--node-id <id>", "Registered node id")
      .option("--title <text>", "Push title", "Fased test push")
      .option("--body <text>", "Push body", "Operator test push")
      .option("--environment <env>", "APNs environment override (sandbox|production)")
      .option("--yes", "Confirm this side-effecting call", false)
      .action(async (opts, command) => {
        await runGatewayCommand(async () => {
          requireAdminRpcConfirmation(opts, "push.test");
          const rpcOpts = resolveGatewayRpcOptions(opts, command);
          const params: Record<string, unknown> = {
            nodeId: requireTextOption(opts.nodeId, "node id"),
            title: String(opts.title ?? "Fased test push"),
            body: String(opts.body ?? "Operator test push"),
          };
          if (typeof opts.environment === "string" && opts.environment.trim()) {
            params.environment = opts.environment.trim();
          }
          const result = await callGatewayCli("push.test", rpcOpts, params);
          renderAdminRpcResult("push.test", result, rpcOpts.json);
        }, "Gateway admin push test failed");
      }),
  );

  gatewayCallOpts(
    admin
      .command("web-login-start")
      .description("Start a web login QR flow for a provider account")
      .option("--account-id <id>", "Provider account id")
      .option("--yes", "Confirm this side-effecting call", false)
      .action(async (opts, command) => {
        await runGatewayCommand(async () => {
          requireAdminRpcConfirmation(opts, "web.login.start");
          const rpcOpts = resolveGatewayRpcOptions(opts, command);
          const accountId = typeof opts.accountId === "string" ? opts.accountId.trim() : "";
          const result = await callGatewayCli("web.login.start", rpcOpts, {
            ...(accountId ? { accountId } : {}),
            force: true,
          });
          renderAdminRpcResult("web.login.start", result, rpcOpts.json);
        }, "Gateway admin web login start failed");
      }),
  );

  gatewayCallOpts(
    admin
      .command("web-login-wait")
      .description("Wait for a web login flow and restart the provider account if connected")
      .option("--account-id <id>", "Provider account id")
      .option("--yes", "Confirm this side-effecting call", false)
      .action(async (opts, command) => {
        await runGatewayCommand(async () => {
          requireAdminRpcConfirmation(opts, "web.login.wait");
          const rpcOpts = resolveGatewayRpcOptions(opts, command);
          const accountId = typeof opts.accountId === "string" ? opts.accountId.trim() : "";
          const params = accountId ? { accountId } : {};
          const result = await callGatewayCli("web.login.wait", rpcOpts, params);
          renderAdminRpcResult("web.login.wait", result, rpcOpts.json);
        }, "Gateway admin web login wait failed");
      }),
  );
