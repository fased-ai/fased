...
vi.mock("../../config/config.js", () => ({
  getConfigPath: () => "/tmp/fased-test-missing-config.json",
  loadConfig: () => ({}),
...
vi.mock("../../gateway/auth.js", () => ({
  resolveGatewayAuth: (params: {
    ...
    mode:
      params.authConfig?.mode ??
      (params.authConfig?.password || params.env?.FASED_GATEWAY_PASSWORD
        ? "password"
        : params.authConfig?.token || params.env?.FASED_GATEWAY_TOKEN
          ? "token"
          : "none"),
    token: params.authConfig?.token ?? params.env?.FASED_GATEWAY_TOKEN,
    password: params.authConfig?.password ?? params.env?.FASED_GATEWAY_PASSWORD,
