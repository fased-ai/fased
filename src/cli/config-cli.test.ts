...
import type { ConfigFileSnapshot, FasedAgentConfig } from "../config/types.js";

/**
 * Test for issue #6070:
 * `fased config set/unset` must update snapshot.resolved (user config after $include/${ENV},
 * but before runtime defaults), so runtime defaults don't leak into the written config.
 */

const mockReadConfigFileSnapshot = vi.fn<() => Promise<ConfigFileSnapshot>>();
const mockWriteConfigFile = vi.fn<
  (cfg: FasedAgentConfig, options?: { unsetPaths?: string[][] }) => Promise<void>
>(async () => {});

vi.mock("../config/config.js", () => ({
  readConfigFileSnapshot: () => mockReadConfigFileSnapshot(),
  writeConfigFile: (cfg: FasedAgentConfig, options?: { unsetPaths?: string[][] }) =>
...
function setSnapshot(resolved: FasedAgentConfig, config: FasedAgentConfig) {
...
describe("config cli", () => {
  ...
  describe("config set - issue #6070", () => {
    it("preserves existing config keys when setting a new value", async () => {
      const resolved: FasedAgentConfig = {
        agents: {
      ...
      const runtimeMerged: FasedAgentConfig = {
        ...resolved,
        agents: {
          ...resolved.agents,
          defaults: {
