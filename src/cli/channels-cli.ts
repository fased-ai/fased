    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["fased channels list", "List configured channels and auth profiles."],
          ["fased channels status --probe", "Run channel status checks and probes."],
          [
            "fased channels add --channel telegram --token <token>",
            "Add or update a channel account non-interactively.",
          ],
          ["fased channels login --channel whatsapp", "Link a WhatsApp Web account."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink(
          "/cli/channels",
          "docs.fased.ai/cli/channels",
        )}\n`,
    );
