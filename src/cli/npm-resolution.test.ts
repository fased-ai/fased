...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@fased/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@fased/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@fased/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@fased/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @fased/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@fased/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@fased/plugin-alpha@1.2.3",
        installPath: "/tmp/fased/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@fased/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @fased/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@fased/plugin-alpha@latest",
      true,
      "/tmp/fased/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@fased/plugin-alpha@latest",
      installPath: "/tmp/fased/extensions/alpha",
      version: "1.2.3",
