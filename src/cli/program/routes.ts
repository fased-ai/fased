...
const routeMemoryStatus: RouteSpec = {
  ...
  run: async (argv) => {
    ...
    if (hasUnexpectedTrailingPositionals(argv, ["memory", "status"], [agent])) {
      return false;
    }
    const json = hasFlag(argv, "--json");
    const deep = hasFlag(argv, "--deep");
...
const routeMemoryDoctor: RouteSpec = {
  match: (path) => path[0] === "memory" && path[1] === "doctor",
  run: async (argv) => {
    const agent = getFlagValue(argv, "--agent");
    if (agent === null) {
      return false;
    }
    if (hasUnexpectedTrailingPositionals(argv, ["memory", "doctor"], [agent])) {
      return false;
    }
    const json = hasFlag(argv, "--json");
    const { runMemoryDoctorStatus } = await import("../memory-cli.js");
    await runMemoryDoctorStatus({ agent, json });
    return true;
  },
};
...
function hasUnexpectedTrailingPositionals(
  argv: string[],
  commandPath: string[],
  allowedTrailingValues: Array<string | null | undefined> = [],
): boolean {
  const allowed = new Set(
    allowedTrailingValues.filter((value): value is string => typeof value === "string"),
  );
  return getCommandPositionals(argv)
