...
const EXAMPLES = [
  ["fased models --help", "Show detailed help for the models command."],
  ["fased channels login --verbose", "Link personal WhatsApp Web and show QR + connection logs."],
  [
  ...
  [
    'fased message send --channel telegram --target @mychat --message "Hi"',
...
export function configureProgramHelp(program: Command, ctx: ProgramContext) {
  program
    ...
    .option(
      "--dev",
      "Dev profile: isolate state under ~/.fased-dev, default gateway port 19001, and shift derived ports (browser/canvas)",
    )
    .option(
      "--profile <name>",
      "Use a named profile (isolates FASED_STATE_DIR/FASED_CONFIG_PATH under ~/.fased-<name>)",
    )
    .option(
      "--log-level <level>",
  ...
  program.addHelpText("afterAll", ({ command }) => {
    ...
    const docs = formatDocsLink("/cli", "docs.fased.ai/cli");
