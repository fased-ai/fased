...
function isBareParentDefaultHelpInvocation(actionCommand: Command, argv: string[]): boolean {
  ...
  const [primary, extra] = getCommandPath(argv, 2);
  if (!primary || extra !== undefined) {
    return false;
  }
...
export function registerPreActionHooks(program: Command, programVersion: string) {
  program.hook("preAction", async (_thisCommand, actionCommand) => {
    ...
    if (hasHelpOrVersion(argv) || isBareParentDefaultHelpInvocation(actionCommand, argv)) {
      return;
    }
    const commandPath = getCommandPath(argv, 2);
    const hideBanner =
      isTruthyEnvValue(process.env.FASED_HIDE_BANNER) ||
    ...
    if (cliLogLevel) {
      process.env.FASED_LOG_LEVEL = cliLogLevel;
    }
    if (!verbose) {
