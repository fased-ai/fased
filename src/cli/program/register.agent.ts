...
${formatHelpExamples([
  ...
  ['fased agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'fased agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'fased agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['fased agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.fased.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.fased.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['fased agents set-identity --agent main --name "FasedAgent" --emoji "🤖"', "Set name + emoji."],
  ["fased agents set-identity --agent main --avatar avatars/fased.png", "Set avatar path."],
  [
    "fased agents set-identity --workspace ~/.fased/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "fased agents set-identity --identity-file ~/.fased/workspace/IDENTITY.md --agent main",
