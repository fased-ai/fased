...
const shouldRegisterPrimaryOnly = (argv: string[]) => {
  if (isTruthyEnvValue(process.env.FASED_DISABLE_LAZY_SUBCOMMANDS)) {
    return false;
  }
...
const shouldEagerRegisterSubcommands = (_argv: string[]) => {
  return isTruthyEnvValue(process.env.FASED_DISABLE_LAZY_SUBCOMMANDS);
};

const loadConfig = async (): Promise<FasedAgentConfig> => {
...
const entries: SubCliEntry[] = [
  ...
  {
    name: "cron",
    description: "Manage scheduled Agent tasks (compatibility alias)",
    hasSubcommands: true,
    register: async (program) => {
      const mod = await import("../cron-cli.js");
  ...
  {
    name: "docs",
    description: "Search the live FasedAgent docs",
    hasSubcommands: false,
    register: async (program) => {
