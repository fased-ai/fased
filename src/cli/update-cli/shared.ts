...
export function resolveGitInstallDir(): string {
  const override = process.env.FASED_GIT_DIR?.trim();
  if (override) {
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", FASED_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `FASED_GIT_DIR points at a non-git directory: ${params.dir}. Set FASED_GIT_DIR to an empty folder or an fased checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`FASED_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
