...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "fased-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "fased-bundled-skills-test-"));
    process.env.FASED_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
