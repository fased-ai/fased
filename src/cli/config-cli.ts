...
async function loadValidConfig(runtime: RuntimeEnv = defaultRuntime) {
  ...
  runtime.error(`Run \`${formatCliCommand("fased doctor")}\` to repair, then retry.`);
  runtime.exit(1);
  return snapshot;
}
...
export async function runConfigGet(opts: {
  ...
  try {
    ...
    const configToQuery = opts.unredacted ? snapshot.resolved : redactConfigObject(snapshot.config);
...
export function registerConfigCli(program: Command) {
  const cmd = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/config", "docs.fased.ai/cli/config")}\n`,
    )
    .option(
      "--section <section>",
  ...
  cmd
    ...
    .option("--unredacted", "Show sensitive values (unredacted)", false)
    .action(async (path: string, opts) => {
      await runConfigGet({
        path,
        json: Boolean(opts.json),
        unredacted: Boolean(opts.unredacted),
      });
    });
