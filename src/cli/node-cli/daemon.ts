...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("fased node install"), formatCliCommand("fased node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("fased node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    FASED_LOG_PREFIX: baseEnv.FASED_LOG_PREFIX ?? "node",
