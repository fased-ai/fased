    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["fased node run --host 127.0.0.1 --port 18789", "Run the node host in the foreground."],
          ["fased node status", "Check node host service status."],
          ["fased node install", "Install the node host service."],
          ["fased node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.fased.ai/cli/node")}\n`,
    );
