    ...
    cron
      ...
      .action(async (opts) => {
        try {
          ...
          printCronStatus(res as Parameters<typeof printCronStatus>[0], defaultRuntime);
        } catch (err) {
...
export function registerCronListCommand(cron: Command) {
  addGatewayClientOptions(
    cron
      ...
      .option("--all", "Include disabled tasks", false)
      .option("--json", "Output JSON", false)
      .action(async (opts) => {
        try {
...
export function registerCronAddCommand(cron: Command) {
  addGatewayClientOptions(
    cron
      ...
      .option("--disabled", "Create task disabled", false)
      ...
      .option("--agent <id>", "Agent id for this task")
      .option("--session <target>", "Session target (main|isolated)")
      .option(
        "--session-key <key>",
        "Session key for task routing (e.g. agent:my-agent:my-session)",
      )
      ...
      .option("--best-effort-deliver", "Do not fail the task if delivery fails", false)
      .option("--json", "Output JSON", false)
      .action(async (opts: GatewayRpcOpts & Record<string, unknown>, cmd?: Command) => {
        try {
          ...
          if (sessionTarget === "main" && payload.kind !== "systemEvent") {
            throw new Error("Main-session tasks require --system-event (systemEvent).");
          }
          if (sessionTarget === "isolated" && payload.kind !== "agentTurn") {
            throw new Error("Isolated tasks require --message (agentTurn).");
          }
          if (
