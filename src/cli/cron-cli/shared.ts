...
export async function warnIfCronSchedulerDisabled(opts: GatewayRpcOpts) {
  try {
    ...
    defaultRuntime.error(
      [
        "warning: task scheduler is disabled in the Gateway; tasks are saved but will not run automatically.",
...
export function printCronList(jobs: CronJob[], runtime = defaultRuntime) {
  if (jobs.length === 0) {
    runtime.log("No scheduled tasks.");
    return;
  }
...
function formatMaybeRelative(ms: number | null | undefined, nowMs: number) {
  return ms ? formatRelative(ms, nowMs) : "-";
}

export function printCronStatus(status: CronStatusSummary, runtime = defaultRuntime) {
  const now = Date.now();
  const enabled = status.enabled ? "enabled" : "disabled";
  const jobs = Number.isFinite(status.jobs) ? status.jobs : 0;
  runtime.log(
    [
      `Tasks ${enabled}`,
      `${jobs} job${jobs === 1 ? "" : "s"}`,
      `next ${formatMaybeRelative(status.nextWakeAtMs, now)}`,
    ].join(" · "),
  );

  const queue =
    (status as Partial<CronStatusSummary>).queue ??
    ({
      path: "",
      total: 0,
      queued: 0,
      ...
      activeRuns: [],
      recentRuns: [],
    } satisfies CronStatusSummary["queue"]);
  runtime.log(
    [
      "Queue",
      `${queue.queued} queued`,
      `${queue.running} running`,
      `${queue.expiredLeases} expired lease${queue.expiredLeases === 1 ? "" : "s"}`,
      `${queue.cancelRequested} cancel requested`,
      `${queue.terminal} finished`,
    ].join(" · "),
  );

  if (queue.workers.length === 0) {
    runtime.log("Workers: no active leases");
  } else {
    runtime.log("Workers:");
    for (const worker of queue.workers) {
      runtime.log(
        `- ${worker.workerId} · ${worker.running} active · ${worker.expired} expired · next lease ${formatMaybeRelative(
  ...
  if (queue.activeRuns.length > 0) {
    runtime.log("Active runs:");
    for (const run of queue.activeRuns.slice(0, 8)) {
      const lease = run.leaseOwner
        ? ` · leased by ${run.leaseOwner}${run.leaseExpired ? " (expired)" : ""}`
        : "";
      const control = run.leaseExpired
        ? ` · control: fased task clear-stale ${run.runId}`
        : ` · control: fased task cancel-run ${run.runId}`;
      runtime.log(
        `- ${run.jobName} · ${run.runId} · ${run.stepId} attempt ${run.attempt}/${run.maxAttempts}${lease}${control}`,
      );
    }
  }
  ...
  if (queue.path) {
    runtime.log(`Queue file: ${queue.path}`);
  }
}
