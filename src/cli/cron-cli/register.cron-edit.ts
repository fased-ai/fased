...
export function registerCronEditCommand(cron: Command) {
  addGatewayClientOptions(
    cron
      ...
      .option("--enable", "Enable task", false)
      .option("--disable", "Disable task", false)
      .option("--delete-after-run", "Delete one-shot task after it succeeds", false)
      .option("--keep-after-run", "Keep one-shot task after it succeeds", false)
      .option("--session <target>", "Session target (main|isolated)")
      .option("--agent <id>", "Set agent id")
      .option("--clear-agent", "Unset agent and use default", false)
      .option("--session-key <key>", "Set session key for task routing")
      ...
      .option("--thinking <level>", "Thinking level for agent tasks")
      .option("--model <model>", "Model override for agent tasks")
      ...
      .option("--best-effort-deliver", "Do not fail task if delivery fails")
      .option("--no-best-effort-deliver", "Fail task when delivery fails")
      .action(async (id, opts) => {
        try {
          if (opts.session === "main" && opts.message) {
            throw new Error(
              "Main-session tasks cannot use --message; use --system-event or --session isolated.",
            );
          }
          if (opts.session === "isolated" && opts.systemEvent) {
            throw new Error(
              "Isolated tasks cannot use --system-event; use --message or --session main.",
            );
          }
          ...
          if (opts.at) {
            ...
            if (existing.schedule.kind !== "cron") {
              throw new Error("Current task is not a cron schedule; use --cron to convert first");
            }
            const tz =
