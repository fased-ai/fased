...
const serviceReadCommand = vi.fn(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    FASED_STATE_DIR: "/tmp/fased-daemon",
    FASED_CONFIG_PATH: "/tmp/fased-daemon/fased.json",
  },
}));
...
const resolveStateDir = vi.fn((env: NodeJS.ProcessEnv) => env.FASED_STATE_DIR ?? "/tmp/fased-cli");
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.FASED_CONFIG_PATH ?? `${stateDir}/fased.json`;
});

vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/fased-daemon/");
    return {
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.FASED_STATE_DIR = "/tmp/fased-cli";
    process.env.FASED_CONFIG_PATH = "/tmp/fased-cli/fased.json";
