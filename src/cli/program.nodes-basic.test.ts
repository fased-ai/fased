...
describe("cli program (nodes basics)", () => {
  ...
  it("runs nodes list and merges paired rows with effective node.list metadata", async () => {
    const now = Date.now();
    callGateway.mockImplementation(async (...args: unknown[]) => {
      const opts = (args[0] ?? {}) as { method?: string };
      if (opts.method === "node.pair.list") {
    ...
    expect(callGateway).toHaveBeenCalledWith(expect.objectContaining({ method: "node.list" }));
    const output = getRuntimeOutput();
    expect(output).toContain("Live Name");
    expect(output).toContain("10.0.0.9");
    expect(output).not.toContain("Old Name");
    expect(output).not.toContain("secret-node-token");
  });
  ...
  it("runs nodes remove and calls node.pair.remove", async () => {
    callGateway.mockImplementation(async (...args: unknown[]) => {
      ...
      if (opts.method === "node.list") {
        return {
          nodes: [{ nodeId: "n1", displayName: "Node One", paired: true, connected: false }],
        };
      }
    ...
    await runProgram(["nodes", "remove", "--node", "Node One"]);
    ...
    expect(getRuntimeOutput()).toContain("node remove ok: n1");
  });
