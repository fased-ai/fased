...
describe("gateway-cli coverage", () => {
  ...
  it("registers gateway discover and prints json output", async () => {
    ...
    discoverGatewayBeacons.mockResolvedValueOnce([
      {
        instanceName: "Studio (FasedAgent)",
        displayName: "Studio",
        domain: "fased.internal.",
        host: "studio.fased.internal",
  ...
  it("routes confirmed admin chat inject through the dedicated CLI control", async () => {
    resetRuntimeCapture();
    callGateway.mockClear();
  ...
  it("requires --yes for side-effecting admin RPC CLI controls", async () => {
    resetRuntimeCapture();
    callGateway.mockClear();

    await expectGatewayExit(["gateway", "admin", "push-test", "--node-id", "ios-node-1"]);

    expect(callGateway).not.toHaveBeenCalled();
    expect(runtimeErrors.join("\n")).toContain("push.test is side-effecting");
  });

  it("routes confirmed admin web login controls without exposing tokens", async () => {
    resetRuntimeCapture();
    callGateway.mockClear();
  ...
  it("uses env/config port when --port is omitted", async () => {
    await withEnvOverride({ FASED_GATEWAY_PORT: "19001" }, async () => {
      resetRuntimeCapture();
