...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.fased.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + FasedAgent hooks")
    ...
    .option("--hook-url <url>", "FasedAgent hook URL")
    .option("--hook-token <token>", "FasedAgent hook token")
