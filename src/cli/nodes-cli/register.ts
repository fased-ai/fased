    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["fased nodes status", "List known nodes with live status."],
          ["fased nodes pairing pending", "Show pending node pairing requests."],
          ['fased nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["fased nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.fased.ai/cli/nodes")}\n`,
    );
