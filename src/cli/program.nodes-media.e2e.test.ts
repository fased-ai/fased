...
function asGatewayRequest(opts: unknown): { method?: string } {
  return (opts ?? {}) as { method?: string };
}
...
function mockCameraGateway(
  ...
  callGateway.mockImplementation(async (opts: unknown) => {
    const request = asGatewayRequest(opts);
...
describe("cli program (nodes media)", () => {
  ...
  it("runs nodes camera clip and prints one MEDIA path", async () => {
    ...
    expect(mediaPath).toMatch(/fased-camera-clip-front-.*\.mp4$/);
  ...
  it("runs nodes canvas snapshot and prints MEDIA path", async () => {
    ...
    const out = getFirstRuntimeLogLine();
    const mediaPath = out.replace(/^MEDIA:/, "").trim();
    expect(mediaPath).toMatch(/fased-canvas-snapshot-.*\.png$/);
  ...
  describe("URL-based payloads", () => {
    ...
    it("runs nodes camera snap with url payload", async () => {
      ...
      const out = getFirstRuntimeLogLine();
      const mediaPath = out.replace(/^MEDIA:/, "").trim();
      expect(mediaPath).toMatch(/fased-camera-snap-front-.*\.jpg$/);
