...
function formatAttachmentRefForLog(value?: string): string {
  const trimmed = value?.trim();
  if (!trimmed) {
    return "(unknown)";
  }
  if (/^https?:\/\//i.test(trimmed)) {
    return redactSensitiveUrlLikeString(trimmed);
  }
  const normalized = normalizeAttachmentPath(trimmed) ?? trimmed;
...
export class MediaAttachmentCache {
  ...
  private async ensureLocalStat(entry: AttachmentCacheEntry): Promise<number | undefined> {
    ...
    if (!isInboundPathAllowed({ filePath: entry.resolvedPath, roots: this.localPathRoots })) {
      ...
      if (shouldLogVerbose()) {
        logVerbose(
          `Blocked attachment path outside allowed roots: ${formatAttachmentRefForLog(entry.attachment.path ?? entry.attachment.url)}`,
        );
      }
      return undefined;
    }
    ...
    try {
      ...
      if (!isInboundPathAllowed({ filePath: canonicalPath, roots: canonicalRoots })) {
        ...
        if (shouldLogVerbose()) {
          logVerbose(
            `Blocked canonicalized attachment path outside allowed roots: ${formatAttachmentRefForLog(canonicalPath)}`,
          );
        }
        return undefined;
      }
      ...
      if (shouldLogVerbose()) {
        ...
        logVerbose(`Failed to read attachment ${entry.attachment.index + 1}: ${reason}`);
      }
      return undefined;
    }
  }
