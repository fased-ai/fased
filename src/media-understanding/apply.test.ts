...
const TEMP_MEDIA_PREFIX = "fased-media-";
...
function createGroqAudioConfig(): FasedAgentConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): FasedAgentConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): FasedAgentConfig {
  return {
...
describe("applyMediaUnderstanding", () => {
  ...
  beforeAll(async () => {
    const baseDir = resolvePreferredFasedAgentTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        FASED_AGENT_DIR: isolatedAgentDir,
