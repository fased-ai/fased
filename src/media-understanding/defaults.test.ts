...
describe("DEFAULT_AUDIO_MODELS", () => {
  ...
  it("uses the OpenAI transcription default for Codex audio", () => {
    expect(DEFAULT_AUDIO_MODELS["openai-codex"]).toBe("gpt-4o-mini-transcribe");
  });
});
...
describe("AUTO_AUDIO_KEY_PROVIDERS", () => {
  ...
  it("includes openai-codex auto key resolution", () => {
    expect(AUTO_AUDIO_KEY_PROVIDERS).toContain("openai-codex");
  });
});
