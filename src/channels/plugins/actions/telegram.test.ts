...
const handleTelegramAction = vi.fn<
  (action: unknown, cfg: unknown, opts: unknown) => Promise<{ ok: boolean }>
>(async () => ({ ok: true }));

vi.mock("../../../agents/tools/telegram-actions.js", () => ({
  handleTelegramAction: (action: unknown, cfg: unknown, opts: unknown) =>
    handleTelegramAction(action, cfg, opts),
}));

describe("telegramMessageActions", () => {
  it("excludes sticker actions when not enabled", () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as FasedAgentConfig;
  ...
  it("rejects non-integer messageId for edit before reaching telegram-actions", async () => {
    ...
    await expect(
      telegramMessageActions.handleAction!({
        ...
        params: {
          chatId: "123",
          messageId: "nope" as unknown as number,
          message: "Updated",
        },
        cfg,
        accountId: undefined,
  ...
  it("accepts numeric messageId and channelId for reactions", async () => {
    ...
    const callArg = handleTelegramAction.mock.calls[0]?.[0];
    const call = callArg as Record<string, unknown>;
