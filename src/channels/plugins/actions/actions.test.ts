...
function telegramCfg(): FasedAgentConfig {
  return { channels: { telegram: { botToken: "tok" } } } as FasedAgentConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: FasedAgentConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as FasedAgentConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as FasedAgentConfig;
...
function createSignalAccountOverrideCfg(): FasedAgentConfig {
  return {
    channels: {
      signal: {
  ...
  return {
    channels: {
      discord: {
        ...
        accounts: {
          moderator: { token: "d1", actions: accountActions },
        },
      },
    },
  } as FasedAgentConfig;
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as FasedAgentConfig;
  ...
  it("lists moderation when at least one account enables it", () => {
    const cases = [
      {
        channels: {
          discord: {
            accounts: {
              moderator: { token: "d1", actions: { moderation: true } },
            },
          },
        },
      },
    ...
    for (const channelConfig of cases) {
      const cfg = channelConfig as unknown as FasedAgentConfig;
      const actions = discordMessageActions.listActions?.({ cfg }) ?? [];
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as FasedAgentConfig,
        accountId: undefined,
        params: {
