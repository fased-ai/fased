...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: FasedAgentConfig, policy: ChannelAccessPolicy) => FasedAgentConfig;
  resolveAllowlist: (params: { cfg: FasedAgentConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: FasedAgentConfig; resolved: TResolved }) => FasedAgentConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: FasedAgentConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: FasedAgentConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: FasedAgentConfig, policy: ChannelAccessPolicy): FasedAgentConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: FasedAgentConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: FasedAgentConfig; resolved: string[] }) => {
      calls.push("apply");
