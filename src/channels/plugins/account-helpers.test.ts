...
function cfg(accounts?: Record<string, unknown> | null): FasedAgentConfig {
  if (accounts === null) {
    return { channels: { testchannel: {} } } as unknown as FasedAgentConfig;
  }
  if (accounts === undefined) {
    return {} as unknown as FasedAgentConfig;
  }
  return { channels: { testchannel: { accounts } } } as unknown as FasedAgentConfig;
}

describe("createAccountListHelpers", () => {
  describe("listConfiguredAccountIds", () => {
    it("returns empty for missing config", () => {
      expect(listConfiguredAccountIds({} as FasedAgentConfig)).toEqual([]);
    });
  ...
  describe("listAccountIds", () => {
    it('returns ["default"] for empty config', () => {
      expect(listAccountIds({} as FasedAgentConfig)).toEqual(["default"]);
    });
  ...
  describe("resolveDefaultAccountId", () => {
    ...
    it('returns "default" for empty config', () => {
      expect(resolveDefaultAccountId({} as FasedAgentConfig)).toBe("default");
    });
  });
});
