...
describe("signalOutbound", () => {
  ...
  it("preserves Signal group targets for sendText", async () => {
    const sendSignal = vi.fn().mockResolvedValue({ messageId: "sig-group-1", timestamp: 789 });
    const sendText = signalOutbound.sendText;
    expect(sendText).toBeDefined();
    ...
    expect(result).toEqual({ channel: "signal", messageId: "sig-group-1", timestamp: 789 });
  });

  it("preserves Signal group targets and media options for sendMedia", async () => {
    const sendSignal = vi.fn().mockResolvedValue({
      messageId: "sig-group-media-1",
      timestamp: 790,
    });
    const sendMedia = signalOutbound.sendMedia;
    expect(sendMedia).toBeDefined();
    ...
    const result = await sendMedia!({
      ...
      to: "signal:group:group-id",
      text: "caption",
      mediaUrl: "file:///tmp/signal-image.jpg",
      mediaLocalRoots: ["/tmp"],
      accountId: "work",
      deps: { sendSignal },
    });
