...
function buildCopilotTokenExchangeHeaders(githubToken: string): Record<string, string> {
  return {
...
export function deriveCopilotApiBaseUrlFromToken(token: string): string | null {
  ...
  // Copilot API clients expect converting proxy.* -> api.*.
...
export async function resolveCopilotApiToken(params: {
  ...
  const res = await fetchImpl(COPILOT_TOKEN_URL, {
    method: "GET",
    headers: buildCopilotTokenExchangeHeaders(params.githubToken),
  });
