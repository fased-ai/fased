...
describe("deliverDiscordReply", () => {
  ...
  it("propagates final text delivery failures", async () => {
    sendMessageDiscordMock.mockRejectedValueOnce(new Error("discord delivery failed"));

    await expect(
      deliverDiscordReply({
        replies: [{ text: "final reply" }],
        target: "channel:789",
        token: "token",
        runtime,
        textLimit: 2000,
  ...
  it("propagates bot delivery failures after a webhook fallback fails", async () => {
    const threadBindings = await createBoundThreadBindings();
    sendWebhookMessageDiscordMock.mockRejectedValueOnce(new Error("rate limited"));
    sendMessageDiscordMock.mockRejectedValueOnce(new Error("bot delivery failed"));
    ...
    ).rejects.toThrow("bot delivery failed");
