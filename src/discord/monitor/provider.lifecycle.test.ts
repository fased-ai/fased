...
const {
  ...
  return {
    ...
    getDiscordGatewayEmitterMock: vi.fn<() => EventEmitter | undefined>(() => undefined),
...
describe("runDiscordGatewayLifecycle", () => {
  ...
  const createLifecycleHarness = (params?: {
    ...
    const getStatus = vi.fn(() => ({ accountId: params?.accountId ?? "default" }));
  ...
  it("handles queued disallowed intents errors without waiting for gateway events", async () => {
    ...
    expect(lifecycleParams.setStatus).toHaveBeenCalledWith(
      expect.objectContaining({
        accountId: "default",
        connected: false,
        lastError: expect.stringContaining("missing privileged gateway intents"),
        lastDisconnect: expect.objectContaining({
          status: 4014,
          error: expect.stringContaining("4014"),
        }),
      }),
    );
    expectLifecycleCleanup({
  ...
  it("records gateway open and reconnect debug events in channel status", async () => {
    const { runDiscordGatewayLifecycle } = await import("./provider.lifecycle.js");
    ...
    harness.lifecycleParams.client = {
      getPlugin: vi.fn(() => ({
        emitter,
        disconnect,
        connect,
    ...
    await expect(runDiscordGatewayLifecycle(harness.lifecycleParams)).resolves.toBeUndefined();

    expect(harness.lifecycleParams.setStatus).toHaveBeenNthCalledWith(
      1,
      expect.objectContaining({
        accountId: "default",
        connected: true,
    ...
    expect(harness.lifecycleParams.setStatus).toHaveBeenNthCalledWith(
      ...
      expect.objectContaining({
        accountId: "default",
        connected: false,
        lastError: "WebSocket connection closed with code 1001",
        lastDisconnect: expect.objectContaining({
          error: "WebSocket connection closed with code 1001",
        }),
      }),
    );
