...
export async function runDiscordGatewayLifecycle(params: {
  ...
  const updateStatus = (patch: Omit<ChannelAccountSnapshot, "accountId">) => {
    params.setStatus?.({
      ...(params.getStatus?.() ?? { accountId: params.accountId }),
  ...
  const onGatewayDebug = (msg: unknown) => {
    ...
    if (!message.includes("WebSocket connection opened")) {
      if (
        ...
        message.includes("Reconnecting with backoff") ||
        message.includes("Attempting resume with backoff")
      ) {
        updateStatus({
          connected: false,
          lastDisconnect: {
  ...
  const logGatewayError = (err: unknown) => {
    ...
    if (params.isDisallowedIntentsError(err)) {
      ...
      updateStatus({
        ...
        lastError: "discord gateway closed with code 4014: missing privileged gateway intents",
      });
    ...
    updateStatus({
      ...
      lastError: `discord gateway error: ${message}`,
    });
    params.runtime.error?.(danger(`discord gateway error: ${message}`));
  };
