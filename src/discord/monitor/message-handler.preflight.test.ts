...
describe("preflightDiscordMessage", () => {
  ...
  it("routes authorized guild text commands through the mention gate", async () => {
    const guildId = "guild-command-gate";
    const channelId = "guild-command-channel";
    const text = "/steer worker keep digging";
    const client = {
    ...
    const result = await preflightDiscordMessage({
      ...
      } as import("../../config/config.js").FasedAgentConfig,
      discordConfig: {} as NonNullable<
        import("../../config/config.js").FasedAgentConfig["channels"]
      >["discord"],
      accountId: "default",
      token: "token",
    ...
    expect(result?.baseText).toBe(text);
  ...
  it("drops native Discord command echo messages before text-command routing", async () => {
    const guildId = "guild-command-echo";
    const channelId = "guild-command-echo-channel";
    const client = {
      fetchChannel: async (targetChannelId: string) => {
        if (targetChannelId === channelId) {
          return {
    ...
    const message = {
      id: "m-guild-command-echo-1",
      type: MessageType.ChatInputCommand,
      content: "/steer worker keep digging",
      timestamp: new Date().toISOString(),
      channelId,
      attachments: [],
      mentionedUsers: [],
  ...
  it("bypasses mention gating in bound threads for allowed bot senders", async () => {
    ...
    const result = await preflightDiscordMessage({
      ...
      discordConfig: {
        allowBots: true,
      } as NonNullable<import("../../config/config.js").FasedAgentConfig["channels"]>["discord"],
      accountId: "default",
      token: "token",
      runtime: {} as import("../../runtime.js").RuntimeEnv,
      botUserId: "fased-bot",
