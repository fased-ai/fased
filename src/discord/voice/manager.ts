...
function resolveVoiceTtsConfig(params: { cfg: FasedAgentConfig; override?: TtsConfig }): {
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredFasedAgentTmpDir(), "discord-voice-"));
