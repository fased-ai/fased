...
const toolResultMocks = vi.hoisted(() => ({
  sendMock: vi.fn(),
  reactMock: vi.fn(),
...
vi.mock("./send.js", () => ({
  sendMessageDiscord: (...args: unknown[]) => toolResultMocks.sendMock(...args),
  reactMessageDiscord: async (...args: unknown[]) => {
    toolResultMocks.reactMock(...args);
  },
}));
...
vi.mock("../auto-reply/dispatch.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    dispatchInboundMessage: (...args: unknown[]) => toolResultMocks.dispatchMock(...args),
    dispatchInboundMessageWithDispatcher: (...args: unknown[]) =>
      toolResultMocks.dispatchMock(...args),
...
vi.mock("../pairing/pairing-store.js", () => ({
  readChannelAllowFromStore: (...args: unknown[]) =>
    toolResultMocks.readAllowFromStoreMock(...args),
  upsertChannelPairingRequest: (...args: unknown[]) =>
    toolResultMocks.upsertPairingRequestMock(...args),
}));
...
vi.mock("../config/sessions.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    resolveStorePath: vi.fn(() => "/tmp/fased-sessions.json"),
    updateLastRoute: (...args: unknown[]) => toolResultMocks.updateLastRouteMock(...args),
...
function createDefaultThreadConfig(): LoadedConfig {
  return {
    ...
    session: { store: "/tmp/fased-sessions.json" },
...
describe("discord tool result dispatch", () => {
  it(
    ...
    async () => {
      const cfg = createMentionRequiredGuildConfig({
        messages: {
          responsePrefix: "PFX",
          groupChat: { mentionPatterns: ["\\bfased\\b"] },
        },
      });
      ...
      await handler(createGuildMessageEvent({ messageId: "m2", content: "fased: hello" }), client);
      ...
      createGuildMessageEvent({
        ...
        messagePatch: {
          referencedMessage: {
            ...
            mentionedUsers: [],
            mentionedRoles: [],
            author: { id: "bot-id", bot: true, username: "FasedAgent" },
          },
        },
        eventPatch: {
  ...
  it("treats forum threads as distinct sessions without channel payloads", async () => {
    ...
    const cfg = {
      agent: { model: "anthropic/claude-opus-4-5", workspace: "/tmp/fased" },
      session: { store: "/tmp/fased-sessions.json" },
      channels: {
        discord: {
          dm: { enabled: true, policy: "open" },
