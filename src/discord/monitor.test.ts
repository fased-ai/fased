...
describe("discord allowlist helpers", () => {
  it("normalizes slugs", () => {
    expect(normalizeDiscordSlug("Friends of FasedAgent")).toBe("friends-of-fased");
  ...
  it("matches ids by default and names only when enabled", () => {
    const allow = normalizeDiscordAllowList(
      ["123", "steipete", "Friends of FasedAgent"],
    ...
    expect(allowListMatches(allow, { name: "friends-of-fased" })).toBe(false);
    expect(allowListMatches(allow, { name: "steipete" }, { allowNameMatching: true })).toBe(true);
    expect(allowListMatches(allow, { name: "friends-of-fased" }, { allowNameMatching: true })).toBe(
      true,
    );
...
describe("discord guild/channel resolution", () => {
  it("resolves guild entry by id", () => {
    const guildEntries = makeEntries({
      "123": { slug: "friends-of-fased" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of FasedAgent"),
      guildEntries,
    });
    expect(resolved?.id).toBe("123");
    expect(resolved?.slug).toBe("friends-of-fased");
  });

  it("resolves guild entry by slug key", () => {
    const guildEntries = makeEntries({
      "friends-of-fased": { slug: "friends-of-fased" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of FasedAgent"),
      guildEntries,
  ...
  it("matches group DM allowlist", () => {
    expect(
      resolveGroupDmAllow({
        channels: ["fased-dm"],
        channelId: "1",
        channelName: "FasedAgent DM",
        channelSlug: "fased-dm",
      }),
    ).toBe(true);
    expect(
      resolveGroupDmAllow({
