...
import type { FasedAgentConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: FasedAgentConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: FasedAgentConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = FasedAgentSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as FasedAgentConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as FasedAgentConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as FasedAgentConfig,
  };
}
