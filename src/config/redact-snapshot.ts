...
function redactSensitiveObjectShape(
  ...
  for (const [key, item] of Object.entries(value)) {
    if (typeof item === "string" && !isEnvVarPlaceholder(item)) {
      ...
      result[key] = item.map((entry) => {
        if (typeof entry === "string" && !isEnvVarPlaceholder(entry)) {
          values.push(entry);
          return REDACTED_SENTINEL;
        }
        if (entry && typeof entry === "object" && !Array.isArray(entry)) {
          return redactSensitiveObjectShape(entry as Record<string, unknown>, values);
        }
        return entry;
      });
    } else if (item && typeof item === "object") {
      result[key] = redactSensitiveObjectShape(item as Record<string, unknown>, values);
    } else {
      result[key] = item;
    }
  }
 ...
 */
export const REDACTED_SENTINEL = "__FASED_REDACTED__";
  ...
  if (typeof obj === "object") {
    ...
    for (const [key, value] of Object.entries(obj as Record<string, unknown>)) {
      ...
      if (
        ...
        if (isWholeObjectSensitivePath(dotPath)) {
          collectSensitiveStrings(value, values);
          result[key] = REDACTED_SENTINEL;
        } else {
          result[key] = redactSensitiveObjectShape(value as Record<string, unknown>, values);
        }
      } else if (typeof value === "object" && value !== null) {
