...
export type ToolPolicyConfig = {
   ...
   *
   * Intended for additive configuration (e.g., "also allow shell") without forcing
...
export type MemorySearchConfig = {
  ...
  model?: string;
  /** Multimodal memory indexing options for providers that support media embeddings. */
  multimodal?: {
    enabled?: boolean;
  ...
  sync?: {
    ...
    sessions?: {
      ...
      /** Force reindexing after transcript compaction. */
...
export type ToolsConfig = {
  ...
  web?: {
    search?: {
      ...
      enabled?: boolean;
      /** Search provider id (built-in or plugin-registered). */
      provider?: string;
      /** Brave Search API key or SecretRef (optional; defaults to BRAVE_API_KEY env var). */
      apiKey?: SecretInput;
      ...
      /** OpenAI-Codex native Responses web_search configuration. */
      openaiCodex?: {
        /** Enable native Codex web_search injection. */
        enabled?: boolean;
        /** Use cached or live external web access. */
        mode?: "cached" | "live";
        /** Optional allowed domains for provider-side web search. */
        allowedDomains?: string[];
        /** Provider context size hint. */
        contextSize?: "low" | "medium" | "high";
        /** Optional provider-side user location hint. */
        userLocation?: {
          country?: string;
          region?: string;
          city?: string;
      ...
      /** DuckDuckGo-specific configuration (keyless fallback provider). */
      duckduckgo?: {
        /** Optional DuckDuckGo region such as "us-en". */
        region?: string;
        /** Safe search level. */
      ...
      exa?: {
        ...
        apiKey?: SecretInput;
        /** Optional Exa-compatible search endpoint or base URL. */
        baseUrl?: string;
        /** Exa search type. */
        type?: "auto" | "neural" | "fast" | "deep" | "deep-reasoning" | "instant";
      };
      /** Firecrawl search configuration (used when provider="firecrawl"). */
      firecrawl?: {
      ...
      tavily?: {
        ...
        apiKey?: SecretInput;
        /** Tavily base URL (default: https://api.tavily.com). */
        baseUrl?: string;
        /** Include Tavily answer text when available. */
  ...
  /** sessions_spawn attachment limits. */
  sessions_spawn?: {
    attachments?: {
      enabled?: boolean;
      maxFiles?: number;
