...
export const LEGACY_CONFIG_MIGRATIONS_PART_2: LegacyConfigMigration[] = [
  ...
  {
    id: "media.preserveFilenames-v2",
    describe: "Move top-level media.preserveFilenames to channels.msteams.preserveFilenames",
    apply: (raw, changes) => {
      const media = getRecord(raw.media);
      if (media?.preserveFilenames === undefined) {
        return;
      }

      const channels = ensureRecord(raw, "channels");
      const msteams = ensureRecord(channels, "msteams");
      if (msteams.preserveFilenames === undefined) {
        msteams.preserveFilenames = media.preserveFilenames;
        changes.push("Moved media.preserveFilenames → channels.msteams.preserveFilenames.");
      } else {
        changes.push(
      ...
      delete media.preserveFilenames;
