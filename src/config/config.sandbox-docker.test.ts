...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("fased-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "fased-sandbox-browser-global" },
      agentBrowser: { network: "fased-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("fased-sandbox-browser-agent");
  });
