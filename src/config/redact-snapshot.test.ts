...
import type { ConfigFileSnapshot } from "./types.fased.js";
...
type LooseConfig = {
  [key: string]: LooseConfig;
  [key: number]: LooseConfig;
};

function looseConfig(value: unknown): LooseConfig {
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.fased/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  it("redacts common secret field patterns across config sections", () => {
    ...
    const cfg = looseConfig(result.config);
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/fased/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/fased/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/fased/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/fased/nickserv-password.txt");
  ...
  it("uses uiHints to determine sensitivity", () => {
    ...
    const config = looseConfig(result.config);
    const custom = config.custom as unknown as Record<string, string>;
  ...
  it("keeps regex fallback for extension keys not covered by uiHints", () => {
    ...
    const config = looseConfig(redacted.config);
  ...
  it("contract-covers dynamic catchall/record paths for redact+restore", () => {
    const hints = mapSensitivePaths(FasedAgentSchema, "", {});
    const snapshot = makeSnapshot({
      env: {
