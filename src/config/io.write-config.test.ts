...
describe("config io write", () => {
  ...
  async function writeConfigAndCreateIo(params: {
    ...
    const configPath = path.join(params.home, ".fased", "fased.json");
  ...
  async function writeGatewayPatchAndReadLastAuditEntry(params: {
    ...
    const auditPath = path.join(params.home, ".fased", "logs", "config-audit.jsonl");
  ...
  async function writeRequiredConfigPlugin(params: { dir: string; id: string }) {
  ...
  it("persists caller changes onto resolved config without leaking runtime defaults", async () => {
    await withTempHome("fased-config-io-", async (home) => {
  ...
  it('shows actionable guidance for dmPolicy="open" without wildcard allowFrom', async () => {
    await withTempHome("fased-config-io-", async (home) => {
      ...
      const invalidConfig: FasedAgentConfig = {
        channels: {
          telegram: {
            dmPolicy: "open",
            allowFrom: [],
      ...
      await expect(io.writeConfigFile(invalidConfig)).rejects.toThrow(
        "fased config set channels.telegram.allowFrom '[\"*\"]'",
      );
      await expect(io.writeConfigFile(invalidConfig)).rejects.toThrow(
        'fased config set channels.telegram.dmPolicy "pairing"',
      );
    });
  });
  ...
  it("writes disabled plugin entries without requiring plugin config", async () => {
    await withTempHome("fased-config-io-", async (home) => {
      ...
      await writeRequiredConfigPlugin({ dir: pluginDir, id: "required-config-plugin" });
      ...
      expect(persisted.plugins?.entries?.["required-config-plugin"]).toEqual({
        enabled: false,
      });
    });
  });
  ...
  it("records gateway watch session markers in config audit entries", async () => {
    await withTempHome("fased-config-io-", async (home) => {
      const { last } = await writeGatewayPatchAndReadLastAuditEntry({
        ...
        env: {
          FASED_WATCH_MODE: "1",
          FASED_WATCH_SESSION: "watch-session-1",
          FASED_WATCH_COMMAND: "gateway --force",
