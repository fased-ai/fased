...
export type AgentTaskModelConfig = AgentTaskModelRolesConfig;
...
export type AgentDefaultsConfig = {
  ...
  /** Explicit task model slots. Tasks never infer provider-specific fallback models. */
  taskModels?: AgentTaskModelConfig;
  ...
  /** Legacy PDF model override used by PDF-capable tool registration tests. */
  ...
  /** Control whether bootstrap truncation warnings are injected into prompts. */
  ...
  /**
   * Strict-agentic completion policy. Warning mode reports runs that planned or
   * returned empty output without changing delivery/retry behavior.
   */
  strictAgentic?: {
    /** Default: off. "warn" emits sanitized diagnostics only. */
    mode?: "off" | "warn";
  };
  ...
  heartbeat?: {
    /** Heartbeat interval (duration string, default unit: minutes; default: 30m once heartbeat is configured). */
    every?: string;
  ...
  subagents?: {
    ...
    /** Default cross-agent allowlist for sessions_spawn. Agent-level config can override this. */
    allowAgents?: string[];
...
export type AgentCompactionConfig = {
  ...
  /** Safety timeout for provider-side compaction calls, in seconds. */
  timeoutSeconds?: number;
