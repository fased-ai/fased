...
describe("resolveStorePath", () => {
  ...
  it("uses FASED_HOME for tilde expansion", () => {
    ...
    const resolved = resolveStorePath("~/.fased/agents/{agentId}/sessions/sessions.json", {
      agentId: "research",
    });

    expect(resolved).toBe(
      path.resolve("/srv/fased-home/.fased/agents/research/sessions/sessions.json"),
    );
  });
});
...
describe("session path safety", () => {
  ...
  it("accepts absolute sessionFile paths that resolve within the sessions dir", () => {
    ...
    const resolved = resolveSessionFilePath(
      "sess-1",
      { sessionFile: "/tmp/fased/agents/main/sessions/abc-123.jsonl" },
      { sessionsDir },
    );
  ...
  it("accepts absolute sessionFile with topic suffix within the sessions dir", () => {
    ...
    const resolved = resolveSessionFilePath(
      "sess-1",
      { sessionFile: "/tmp/fased/agents/main/sessions/abc-123-topic-42.jsonl" },
      { sessionsDir },
    );
