...
function enforcedMaintenanceOptions(
  override: Partial<{
    mode: "warn" | "enforce";
    pruneAfterMs: number;
    maxEntries: number;
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "fased-pruning-integ-"));
  });
  ...
  beforeEach(async () => {
    ...
    savedCacheTtl = process.env.FASED_SESSION_CACHE_TTL_MS;
    process.env.FASED_SESSION_CACHE_TTL_MS = "0";
  ...
  afterEach(() => {
    ...
    if (savedCacheTtl === undefined) {
      ...
      process.env.FASED_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
  ...
  it("saveSessionStore prunes stale entries on write", async () => {
    ...
    await saveSessionStore(storePath, store, enforcedMaintenanceOptions());
  ...
  it("cleans up reset archives using resetArchiveRetention", async () => {
    ...
    await saveSessionStore(
      storePath,
      store,
      enforcedMaintenanceOptions({
        pruneAfterMs: 30 * DAY_MS,
        resetArchiveRetentionMs: 3 * DAY_MS,
      }),
    );
  ...
  it("does not archive external transcript paths when capping entries", async () => {
    const now = Date.now();
    const externalDir = await fs.mkdtemp(path.join(os.tmpdir(), "fased-external-cap-"));
  ...
  it("never deletes transcripts outside the agent sessions directory during budget cleanup", async () => {
    const now = Date.now();
    const externalDir = await fs.mkdtemp(path.join(os.tmpdir(), "fased-external-session-"));
    const externalTranscript = path.join(externalDir, "outside.jsonl");
