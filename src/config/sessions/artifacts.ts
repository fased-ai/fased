...
export function isSessionCompactionCheckpointArtifactName(fileName: string): boolean {
...
export function isPrimarySessionTranscriptFileName(fileName: string): boolean {
  ...
  return (
    !isSessionArchiveArtifactName(fileName) && !isSessionCompactionCheckpointArtifactName(fileName)
  );
}
