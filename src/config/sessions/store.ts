...
function getSessionStoreTtl(): number {
  return resolveCacheTtlMs({
    envValue: process.env.FASED_SESSION_CACHE_TTL_MS,
...
export async function drainSessionStoreLockQueuesForTest(): Promise<void> {
  while (LOCK_QUEUES.size > 0) {
    const paths = [...LOCK_QUEUES.keys()];
    await Promise.all(paths.map((storePath) => drainSessionStoreLockQueue(storePath)));
    if (paths.every((storePath) => !LOCK_QUEUES.get(storePath)?.pending.length)) {
      break;
    }
  }
}
...
/**
 * Resolve maintenance settings from fased.json (`session.maintenance`).
...
function settleSessionStoreLockTask(task: SessionStoreLockTask, settle: () => void): void {
  if (task.settled) {
    return;
  }
  task.settled = true;
...
function removePendingSessionStoreLockTask(
  queue: SessionStoreLockQueue | undefined,
  ...
  try {
    while (queue.pending.length > 0) {
      ...
      if (task.timeoutMs != null && remainingTimeoutMs <= 0) {
        settleSessionStoreLockTask(task, () => {
  ...
  const promise = new Promise<T>((resolve, reject) => {
    ...
    if (hasTimeout) {
      task.timeoutHandle = setTimeout(() => {
        const currentQueue = LOCK_QUEUES.get(storePath);
        removePendingSessionStoreLockTask(currentQueue, task);
        settleSessionStoreLockTask(task, () => {
          task.reject(lockTimeoutError(storePath));
        });
      }, timeoutMs);
