...
describe("session path safety", () => {
  ...
  it("resolves transcript path inside an explicit sessions dir", () => {
    const sessionsDir = "/tmp/fased/agents/main/sessions";
    const resolved = resolveSessionTranscriptPathInDir("sess-1", sessionsDir, "topic/a+b");

    expect(resolved).toBe(path.resolve(sessionsDir, "sess-1-topic-topic%2Fa%2Bb.jsonl"));
  });

  it("rejects sessionFile paths outside known agent sessions dirs", () => {
    const sessionsDir = "/tmp/fased/agents/main/sessions";

    expect(() =>
      resolveSessionFilePath(
        "sess-1",
        { sessionFile: "/tmp/fased/agents/work/not-sessions/abc-123.jsonl" },
        { sessionsDir },
      ),
  ...
  it("accepts symlink-alias session paths that resolve under the sessions dir", () => {
    ...
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "fased-symlink-session-"));
  ...
  it("rejects sessionFile symlinks that escape sessions dir", () => {
    if (process.platform === "win32") {
      return;
    }
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "fased-symlink-escape-"));
    ...
    try {
      ...
      expect(() =>
        resolveSessionFilePath("sess-1", { sessionFile: symlinkPath }, { sessionsDir }),
      ).toThrow(/within sessions directory/);
    } finally {
      fs.rmSync(tmpDir, { recursive: true, force: true });
    }
...
describe("session store lock (Promise chain mutex)", () => {
  ...
  beforeAll(async () => {
    lockFixtureRoot = await fsPromises.mkdtemp(path.join(os.tmpdir(), "fased-lock-test-"));
  });
