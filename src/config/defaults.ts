...
const DEFAULT_MODEL_INPUT: NonNullable<ModelDefinitionConfig["input"]> = ["text"];
...
function resolveAnthropicDefaultAuthMode(cfg: FasedAgentConfig): AnthropicAuthDefaultsMode | null {
...
function isAnthropicCacheRetentionTarget(
  ...
  return provider === "amazon-bedrock" && model.includes("anthropic.claude");
}
...
export function applyMessageDefaults(cfg: FasedAgentConfig): FasedAgentConfig {
...
export function applyTalkApiKey(config: FasedAgentConfig): FasedAgentConfig {
  ...
  if (hasTalkApiKey(active.config?.apiKey) || hasTalkApiKey(talk?.apiKey)) {
    return normalized;
  }
...
export function applyTalkConfigNormalization(config: FasedAgentConfig): FasedAgentConfig {
  return normalizeTalkConfig(config);
}

export function applyModelDefaults(cfg: FasedAgentConfig): FasedAgentConfig {
  let mutated = false;
  let nextCfg = cfg;
...
export function applyAgentDefaults(cfg: FasedAgentConfig): FasedAgentConfig {
...
export function applyLoggingDefaults(cfg: FasedAgentConfig): FasedAgentConfig {
...
export function applyContextPruningDefaults(cfg: FasedAgentConfig): FasedAgentConfig {
...
export function applyCompactionDefaults(cfg: FasedAgentConfig): FasedAgentConfig {
  const defaults = cfg.agents?.defaults;
  if (!defaults) {
    return cfg;
  }
