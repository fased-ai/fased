...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "fased-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ FASED_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-fased",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-fased#general");
  });
  ...
  it("derives session transcripts dir from FASED_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { FASED_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent absolute sessionFile paths", () => {
    const stateDir = path.resolve("/home/user/.fased");
  ...
  it("resolves cross-agent paths when FASED_STATE_DIR differs from stored paths", () => {
  ...
  it("rejects structural cross-root paths that traverse after sessions", () => {
    withStateDir(path.resolve("/different/state"), () => {
      const originalBase = path.resolve("/original/state");
  ...
  it("rejects structural cross-root paths nested below sessions", () => {
    withStateDir(path.resolve("/different/state"), () => {
      const originalBase = path.resolve("/original/state");
      ...
      expect(() =>
        resolveSessionFilePath("sess-1", { sessionFile: nested }, { agentId: "bot1" }),
      ).toThrow(/within sessions directory/);
    });
  });

  it("rejects sessionFile paths outside agent sessions directories", () => {
    withStateDir(path.resolve("/home/user/.fased"), () => {
      expect(() =>
        resolveSessionFilePath(
          "sess-1",
