...
import type { FasedAgentConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
const LEGACY_REMOVED_PLUGIN_IDS = [] as const;

function pruneLegacyRemovedPlugins(cfg: FasedAgentConfig): {
  ...
  for (const pluginId of LEGACY_REMOVED_PLUGIN_IDS) {
    const hasEntry = Boolean(next.plugins?.entries && pluginId in next.plugins.entries);
    const hasInstall = Boolean(next.plugins?.installs && pluginId in next.plugins.installs);
    const hasAllow = Array.isArray(next.plugins?.allow) && next.plugins.allow.includes(pluginId);
    const hasMemorySlot = next.plugins?.slots?.memory === pluginId;
    if (!hasEntry && !hasInstall && !hasAllow && !hasMemorySlot) {
      continue;
    }
    next = removePluginFromConfig(next, pluginId).config;
    removedPluginIds.push(pluginId);
  }
  return { config: next, removedPluginIds };
}
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  fased config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  fased config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: FasedAgentConfig,
  pathSegments: string[],
): { changed: boolean; next: FasedAgentConfig } {
...
function coerceConfig(value: unknown): FasedAgentConfig {
...
function stampConfigVersion(cfg: FasedAgentConfig): FasedAgentConfig {
...
function warnIfConfigFromFuture(cfg: FasedAgentConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareFasedAgentVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer FasedAgent (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as FasedAgentConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): FasedAgentConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as FasedAgentConfig, {
        env: deps.env,
      ...
      const allowlistRepair = repairInstalledPluginAllowlist(ownerDisplaySecretResolution.config);
      const cfgWithOwnerDisplaySecret = allowlistRepair.config;
      const repairedPluginIds = [
        ...new Set([
          ...preValidationAllowlistRepair.repairedPluginIds,
          ...allowlistRepair.repairedPluginIds,
        ]),
      ].toSorted((left, right) => left.localeCompare(right));
      if (prunedLegacyPlugins.removedPluginIds.length > 0) {
        const key = `${configPath}:legacy-removed-plugins`;
        if (!AUTO_LEGACY_REMOVED_PLUGIN_PERSIST_IN_FLIGHT.has(key)) {
          AUTO_LEGACY_REMOVED_PLUGIN_PERSIST_IN_FLIGHT.add(key);
      ...
      if (repairedPluginIds.length > 0) {
        const key = `${configPath}:installed-plugin-allowlist`;
        if (!AUTO_INSTALLED_PLUGIN_ALLOWLIST_REPAIR_PERSIST_IN_FLIGHT.has(key)) {
          AUTO_INSTALLED_PLUGIN_ALLOWLIST_REPAIR_PERSIST_IN_FLIGHT.add(key);
          deps.logger.warn(
            `Pinned installed plugins in plugins.allow: ${repairedPluginIds.join(", ")}`,
          );
          void writeConfigFile(cfgWithOwnerDisplaySecret, { expectedConfigPath: configPath })
            .catch((err) => {
              deps.logger.warn(
                `Failed to persist repaired plugins.allow at ${configPath}: ${String(err)}`,
              );
            })
            .finally(() => {
              AUTO_INSTALLED_PLUGIN_ALLOWLIST_REPAIR_PERSIST_IN_FLIGHT.delete(key);
            });
        }
      }
  ...
  async function readConfigFileSnapshotInternal(): Promise<ReadConfigFileSnapshotInternalResult> {
    ...
    try {
      ...
      const allowlistRepair = repairInstalledPluginAllowlist(snapshotConfig);
      const repairedSnapshotConfig = allowlistRepair.config;
      const repairedPluginIds = [
        ...new Set([
          ...preValidationAllowlistRepair.repairedPluginIds,
          ...allowlistRepair.repairedPluginIds,
      ...
      const repairedResolvedSnapshotConfig = coerceConfig(preValidationAllowlistRepair.config);
      if (prunedLegacyPlugins.removedPluginIds.length > 0) {
        const key = `${configPath}:legacy-removed-plugins`;
        if (!AUTO_LEGACY_REMOVED_PLUGIN_PERSIST_IN_FLIGHT.has(key)) {
          AUTO_LEGACY_REMOVED_PLUGIN_PERSIST_IN_FLIGHT.add(key);
      ...
      return {
        snapshot: {
          ...
          resolved: repairedResolvedSnapshotConfig,
          valid: true,
          config: repairedSnapshotConfig,
          hash,
          issues: [],
          warnings: validated.warnings,
          legacyIssues,
  ...
  async function writeConfigFileUnlocked(cfg: FasedAgentConfig, options: ConfigWriteOptions = {}) {
    ...
    const logConfigOverwrite = () => {
      if (deps.env.FASED_SUPPRESS_CONFIG_OVERWRITE_LOG === "1") {
        return;
      }
      if (!snapshot.exists) {
        return;
      ...
      const shouldLogInVitest = deps.env.FASED_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.FASED_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.FASED_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.FASED_WATCH_SESSION === "string" &&
        deps.env.FASED_WATCH_SESSION.trim().length > 0
          ? deps.env.FASED_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.FASED_WATCH_COMMAND === "string" &&
        deps.env.FASED_WATCH_COMMAND.trim().length > 0
          ? deps.env.FASED_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `FASED_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: FasedAgentConfig | null = null;
let runtimeConfigSourceSnapshot: FasedAgentConfig | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.FASED_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.FASED_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function getRuntimeConfigSnapshot(): FasedAgentConfig | null {
  return runtimeConfigSnapshot;
}

export function loadConfig(): FasedAgentConfig {
