...
export const LEGACY_CONFIG_RULES: LegacyConfigRule[] = [
  ...
  {
    path: ["wallet", "runtime", "policy", LEGACY_NON_SOLANA_POLICY_KEY],
    message:
      "wallet.runtime.policy has a legacy non-Solana policy block; Fased wallet runtime is Solana-only (auto-migrated on load).",
  },
  {
    path: ["wallet", "keystore", "chainSupport"],
    message:
      "wallet.keystore.chainSupport contains legacy non-Solana entries; Fased wallet runtime is Solana-only (auto-migrated on load).",
    match: (value) => Array.isArray(value) && value.some((entry) => entry !== "solana"),
  },
  {
    path: ["wallet", "runtime", "chains"],
    message:
      "wallet.runtime.chains contains legacy non-Solana entries; Fased wallet runtime is Solana-only (auto-migrated on load).",
    match: (value) => Array.isArray(value) && value.some((entry) => entry !== "solana"),
  },
  {
    path: ["agent", "model"],
