...
describe("legacy config detection", () => {
  ...
  it("migrates routing.allowFrom to channels.whatsapp.allowFrom when whatsapp configured", async () => {
    ...
    expect(
      legacyField<{ allowFrom: unknown }>(legacyField<{ routing: unknown }>(res.config).routing)
        .allowFrom,
    ).toBeUndefined();
  });
  ...
  it("migrates routing.groupChat.requireMention to channels whatsapp/telegram/imessage groups when whatsapp configured", async () => {
    ...
    expect(
      legacyField<{ requireMention: unknown }>(
        legacyField<{ groupChat: unknown }>(legacyField<{ routing: unknown }>(res.config).routing)
          .groupChat,
      ).requireMention,
    ).toBeUndefined();
  });
  ...
  it("migrates routing.groupChat.mentionPatterns to messages.groupChat.mentionPatterns", async () => {
    const res = migrateLegacyConfig({
      routing: { groupChat: { mentionPatterns: ["@fased"] } },
    });
    expect(res.changes).toContain(
      "Moved routing.groupChat.mentionPatterns → messages.groupChat.mentionPatterns.",
    );
    expect(res.config?.messages?.groupChat?.mentionPatterns).toEqual(["@fased"]);
    expect(
      legacyField<{ mentionPatterns: unknown }>(
        legacyField<{ groupChat: unknown }>(legacyField<{ routing: unknown }>(res.config).routing)
          .groupChat,
      ).mentionPatterns,
    ).toBeUndefined();
  ...
  it("migrates routing agentToAgent/queue/transcribeAudio to tools/messages/media", async () => {
    ...
    expect(legacyField<{ routing: unknown }>(res.config).routing).toBeUndefined();
  });
  it("migrates audio.transcription with custom script names", async () => {
    ...
    expect(legacyField<{ audio: unknown }>(res.config).audio).toBeUndefined();
  });
