...
export function assertSecretInputResolved(params: { value: unknown; path: string }): void {
  const ref = coerceSecretRef(params.value);
  if (ref) {
    throw new Error(`${params.path}: unresolved SecretRef`);
  }
  if (typeof params.value !== "string") {
    throw new Error(`${params.path}: expected resolved string secret input`);
  }
}
