...
export type ConfigSchema = ReturnType<typeof FasedAgentSchema.toJSONSchema>;
...
function buildBaseConfigSchema(): ConfigSchemaResponse {
  ...
  const hints = applyDerivedTags(mapSensitivePaths(FasedAgentSchema, "", buildBaseHints()));
  const next = {
...
function resolveUiHintMatch(
  ...
  const exact = uiHints[path];
  if (exact) {
    return { path, hint: exact };
  }
  ...
  for (let index = parts.length - 1; index >= 0; index -= 1) {
    ...
    const hint = uiHints[candidatePath];
    if (hint) {
      return { path: candidatePath, hint };
    }
  }
  return null;
}
...
function schemaHasChildren(schema: JsonSchemaObject): boolean {
  ...
  return Boolean(resolveItemsSchema(schema));
}
