...
export type WideAreaDiscoveryConfig = {
  enabled?: boolean;
  /** Optional unicast DNS-SD domain (e.g. "fased.internal"). */
  domain?: string;
};
...
export type TalkProviderConfig = {
  /** Voice ID for this provider (e.g. ElevenLabs voice id). */
  voiceId?: string;
  /** Optional voice name -> ID map. */
  voiceAliases?: Record<string, string>;
  /** Model ID for this provider. */
  modelId?: string;
  /** Output format (e.g. mp3_44100_128). */
  outputFormat?: string;
  /** API key or local SecretRef for this provider. */
  apiKey?: SecretInput | Record<string, unknown>;
  [key: string]: unknown;
};

export type TalkConfig = {
  /** Active talk provider ID (e.g. "elevenlabs"). */
  provider?: string;
  /** Per-provider configuration. */
  providers?: Record<string, TalkProviderConfig>;
  /** Legacy: Default ElevenLabs voice ID for Talk mode. */
  voiceId?: string;
  /** Legacy: Optional voice name -> ElevenLabs voice ID map. */
  voiceAliases?: Record<string, string>;
  /** Legacy: Default ElevenLabs model ID for Talk mode. */
  modelId?: string;
  /** Legacy: Default ElevenLabs output format (e.g. mp3_44100_128). */
  outputFormat?: string;
  /** Legacy: ElevenLabs API key or local SecretRef. */
  apiKey?: SecretInput | Record<string, unknown>;
...
export type GatewayControlUiConfig = {
  ...
  enabled?: boolean;
  /** Optional base path prefix for the Control UI (e.g. "/fased"). */
  basePath?: string;
  ...
  /** DANGEROUS: Allow Host header fallback for browser origin checks (default: false). */
 ...
 * Used when FasedAgent runs behind an identity-aware proxy (Pomerium, Caddy + OAuth, etc.)
