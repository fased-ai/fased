...
function legacyField<T extends object>(value: unknown): Partial<T> {
...
describe("legacy config detection", () => {
  ...
  it("migrates telegram.requireMention to channels.telegram.groups.*.requireMention", async () => {
    ...
    expect(
      legacyField<{ requireMention: unknown }>(res.config?.channels?.telegram).requireMention,
    ).toBeUndefined();
  });
  ...
  it("migrates legacy model config to agent.models + model lists", async () => {
    ...
    const model = legacyField<{ primary: string; fallbacks: string[] }>(
      res.config?.agents?.defaults?.model,
    );
    const imageModel = legacyField<{ primary: string; fallbacks: string[] }>(
      res.config?.agents?.defaults?.imageModel,
    );
    expect(model.primary).toBe("anthropic/claude-opus-4-5");
    expect(model.fallbacks).toEqual(["openai/gpt-4.1-mini"]);
    expect(imageModel.primary).toBe("openai/gpt-4.1-mini");
    expect(imageModel.fallbacks).toEqual(["anthropic/claude-opus-4-5"]);
    ...
    expect(legacyField<{ agent: unknown }>(res.config).agent).toBeUndefined();
  });
