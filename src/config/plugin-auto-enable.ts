...
const STRUCTURED_CHANNEL_CONFIG_SPECS: Record<string, StructuredChannelConfigSpec> = {
  ...
  googlechat: {
    stringKeys: ["serviceAccountFile", "webhookPath", "webhookUrl", "audience"],
    accountStringKeys: ["serviceAccountFile", "webhookPath", "webhookUrl", "audience"],
  },
  slack: {
...
function isGenericChannelConfigured(cfg: FasedAgentConfig, channelId: string): boolean {
  const entry = resolveChannelConfig(cfg, channelId);
  return recordHasKeys(entry);
}
...
function collectModelRefs(cfg: FasedAgentConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: FasedAgentConfig, providerId: string): boolean {
...
function collectCandidateChannelIds(cfg: FasedAgentConfig): string[] {
...
function isPluginExplicitlyDisabled(cfg: FasedAgentConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: FasedAgentConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: FasedAgentConfig, pluginId: string): FasedAgentConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
  ...
  for (const entry of configured) {
    ...
    if (allowMissing || !builtInChannelId) {
      next = ensurePluginAllowlisted(next, entry.pluginId, {
        createIfMissing: !builtInChannelId,
      });
    }
