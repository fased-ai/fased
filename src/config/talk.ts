...
function normalizeTalkApiKey(value: unknown): TalkProviderConfig["apiKey"] | undefined {
...
export function hasTalkApiKey(value: unknown): boolean {
  return normalizeTalkApiKey(value) !== undefined;
}
...
function normalizeTalkProviderConfig(value: unknown): TalkProviderConfig | undefined {
  ...
  for (const [key, raw] of Object.entries(value)) {
    ...
    if (key === "apiKey") {
      const apiKey = normalizeTalkApiKey(raw);
      if (apiKey) {
...
function normalizedLegacyTalkFields(source: Record<string, unknown>): Partial<TalkConfig> {
  ...
  const apiKey = normalizeTalkApiKey(source.apiKey);
  if (apiKey) {
...
function legacyTalkFieldsFromProviderConfig(
  ...
  const apiKey = normalizeTalkApiKey(config.apiKey);
  if (apiKey) {
    legacy.apiKey = apiKey;
  }
  return legacy;
...
export function normalizeTalkConfig(config: FasedAgentConfig): FasedAgentConfig {
  if (!config.talk) {
    return config;
  }
