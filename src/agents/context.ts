...
import { ensureFasedModelsJson } from "./models-config.js";
...
const MODEL_CACHE_KEY = Symbol.for("fased.context.modelCache");
const MODEL_CACHE = (() => {
  const globalState = globalThis as typeof globalThis & {
    [MODEL_CACHE_KEY]?: Map<string, number>;
  };
  if (!globalState[MODEL_CACHE_KEY]) {
    globalState[MODEL_CACHE_KEY] = new Map<string, number>();
  }
  return globalState[MODEL_CACHE_KEY];
})();
let loadPromise: Promise<void> | null = null;
let nextConfigLoadAttemptAtMs = 0;

function shouldWarmContextCacheAtStartup(): boolean {
  const args = process.argv
    .slice(2)
    .map((arg) => arg.trim().toLowerCase())
    .filter((arg) => arg && arg !== "--" && !arg.startsWith("-"));
  const command = args[0];
  return command === "chat";
}

async function loadContextCache(): Promise<void> {
  let cfg: ReturnType<typeof loadConfig> | undefined;
  try {
    cfg = loadConfig();
  } catch {
    // If config can't be loaded, leave cache empty.
    nextConfigLoadAttemptAtMs = Date.now() + 1_000;
    return;
  }
...
function startContextCacheLoad(): void {
  if (loadPromise || Date.now() < nextConfigLoadAttemptAtMs) {
    return;
  }
  loadPromise = loadContextCache()
    .catch(() => {
      nextConfigLoadAttemptAtMs = Date.now() + 1_000;
    })
    .finally(() => {
...
if (shouldWarmContextCacheAtStartup()) {
  startContextCacheLoad();
}

export function lookupContextTokens(
  modelId?: string,
  _options?: { allowAsyncLoad?: boolean },
): number | undefined {
  if (!modelId) {
    return undefined;
  }
...
function resolveContextWindowValue(model: ConfigModelEntry | undefined): number | undefined {
  const value =
    typeof model?.contextTokens === "number"
      ? model.contextTokens
      : typeof model?.contextWindow === "number"
...
function findProviderConfigEntry(
  providers: Record<string, ProviderConfigEntry | undefined> | undefined,
  provider: string,
): ProviderConfigEntry | undefined {
  if (!providers) {
    return undefined;
  }
  const directKey = provider.trim().toLowerCase();
  const direct = Object.entries(providers).find(([key]) => key.trim().toLowerCase() === directKey);
  if (direct) {
    return direct[1];
  }
...
function resolveConfiguredProviderContextWindow(params: {
  ...
  const providerConfig = findProviderConfigEntry(
    params.cfg?.models?.providers as Record<string, ProviderConfigEntry | undefined> | undefined,
    provider,
  );
  const match = providerConfig?.models?.find((model) => model.id?.trim() === modelId);
  return resolveContextWindowValue(match);
}
...
export function resolveContextTokensForModel(params: {
  ...
  if (ref) {
    const configured = params.provider
      ? resolveConfiguredProviderContextWindow({
          cfg: params.cfg,
    ...
    const qualified = lookupContextTokens(`${ref.provider}/${ref.model}`);
    if (qualified) {
      return qualified;
    }
  }
