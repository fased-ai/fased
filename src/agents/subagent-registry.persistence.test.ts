...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["FASED_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "fased-subagent-"));
  ...
  it("uses isolated temp state when FASED_STATE_DIR is unset in tests", async () => {
    delete process.env.FASED_STATE_DIR;
    const registryPath = resolveSubagentRegistryPath();
    expect(registryPath).toContain(path.join(os.tmpdir(), "fased-test-state"));
  });
});
