...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.FASED_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".fased", `workspace-${profile}`);
  }
  return path.join(home, ".fased", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".fased";
...
async function ensureMemoryRoots(dir: string): Promise<void> {
  await fs.mkdir(path.join(dir, "memory"), { recursive: true });
  await writeFileIfMissing(path.join(dir, DEFAULT_MEMORY_FILENAME), DEFAULT_MEMORY_CONTENT);
}
