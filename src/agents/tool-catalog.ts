...
const CORE_TOOL_DEFINITIONS: CoreToolDefinition[] = [
  ...
  },
  {
    id: "diff_view",
    label: "diff_view",
    description: "Create a Fased canvas-style diff viewer",
    sectionId: "ui",
    profiles: ["coding"],
    includeInFasedGroup: true,
  },
...
function buildCoreToolGroupMap() {
  ...
  const fasedTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInFasedGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:fased": fasedTools,
