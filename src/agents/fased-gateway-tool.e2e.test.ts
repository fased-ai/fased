...
describe("gateway tool", () => {
  it("schedules SIGUSR1 restart", async () => {
    ...
    const previousProfile = process.env.FASED_PROFILE;
    ...
    try {
      const tool = createFasedAgentTools({
      ...
      if (previousProfile === undefined) {
        delete process.env.FASED_PROFILE;
      } else {
        process.env.FASED_PROFILE = previousProfile;
      }
    }
  });
