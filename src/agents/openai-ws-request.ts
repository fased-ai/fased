...
function resolveOpenAITextVerbosity(
  input: Record<string, unknown> | undefined,
): "low" | "medium" | "high" | undefined {
  const raw =
    readStringValue(input?.textVerbosity) ?? readStringValue(input?.text_verbosity) ?? undefined;
  if (!raw) {
    return undefined;
  }
  ...
  log.warn(`ignoring invalid OpenAI text verbosity param: ${raw}`);
  return undefined;
}
