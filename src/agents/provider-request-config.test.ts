...
describe("provider request config", () => {
  ...
  it("does not let generic provider request sanitization opt into private network", () => {
    expect(
      sanitizeConfiguredProviderRequest({
        allowPrivateNetwork: true,
      }),
  ...
  it("merges model-provider private-network opt-in with later entries winning", () => {
    expect(
      mergeModelProviderRequestOverrides(
        {
          allowPrivateNetwork: true,
  ...
  it("lets defaults override caller headers when requested", () => {
    ...
    expect(resolved).toMatchObject({
      ...
      "User-Agent": expect.stringMatching(/^fased\//),
      "X-Custom": "1",
    });
  });
  ...
  it("merges header names case-insensitively", () => {
    ...
    expect(resolved?.["User-Agent"]).toMatch(/^fased\//);
  });
  ...
  it("requires explicit model-provider private-network opt-in for custom local base URLs", () => {
    const blocked = resolveProviderRequestPolicyConfig({
      provider: "local-vllm",
      api: "openai-completions",
      baseUrl: "http://127.0.0.1:8000/v1",
    });
    const allowed = resolveProviderRequestPolicyConfig({
      provider: "local-vllm",
      api: "openai-completions",
      baseUrl: "http://127.0.0.1:8000/v1",
      request: {
    ...
    expect(blocked.allowPrivateNetwork).toBe(false);
    expect(allowed.allowPrivateNetwork).toBe(true);
  });
});
