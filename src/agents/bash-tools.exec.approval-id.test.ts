...
function getTestConfigPath() {
  return path.join(process.env.HOME ?? "", ".fased", "fased.json");
}

async function writeFasedAgentConfig(config: Record<string, unknown>, pretty = false) {
...
async function writeExecApprovalsConfig(config: Record<string, unknown>) {
  const approvalsPath = path.join(process.env.HOME ?? "", ".fased", "exec-approvals.json");
...
describe("exec approvals", () => {
  ...
  beforeEach(async () => {
    ...
    previousBundledPluginsDir = process.env.FASED_BUNDLED_PLUGINS_DIR;
    previousDisableBundledPlugins = process.env.FASED_DISABLE_BUNDLED_PLUGINS;
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "fased-test-"));
    process.env.HOME = tempDir;
    // Windows uses USERPROFILE for os.homedir()
    process.env.USERPROFILE = tempDir;
    delete process.env.FASED_BUNDLED_PLUGINS_DIR;
    process.env.FASED_DISABLE_BUNDLED_PLUGINS = "1";
  ...
  afterEach(() => {
    ...
    if (previousBundledPluginsDir === undefined) {
      ...
      process.env.FASED_BUNDLED_PLUGINS_DIR = previousBundledPluginsDir;
    }
    if (previousDisableBundledPlugins === undefined) {
      delete process.env.FASED_DISABLE_BUNDLED_PLUGINS;
    } else {
      process.env.FASED_DISABLE_BUNDLED_PLUGINS = previousDisableBundledPlugins;
    }
  });
  ...
  it("skips approval when node allowlist is satisfied", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "fased-test-bin-"));
  ...
  it("auto-continues the same Discord session after approval resolves without a second user turn", async () => {
    const agentCalls: Array<Record<string, unknown>> = [];
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "fased-exec-followup-discord-"));
  ...
  it("executes approved commands and emits a session-only followup in webchat-only mode", async () => {
    const agentCalls: Array<Record<string, unknown>> = [];
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "fased-exec-followup-sidefx-"));
    const markerPath = path.join(tempDir, "marker.txt");
  ...
  it("runs a skill wrapper chain without prompting when the wrapper is allowlisted", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "fased-skill-wrapper-"));
    try {
      const skillDir = path.join(tempDir, ".fased", "skills", "gog");
