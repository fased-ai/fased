...
describeCacheLive("pi embedded runner prompt caching (live)", () => {
  beforeAll(async () => {
    liveRunnerRootDir = await fs.mkdtemp(path.join(os.tmpdir(), "fased-live-cache-"));
    ...
    previousCacheTraceEnv = {
      enabled: process.env.FASED_CACHE_TRACE,
      file: process.env.FASED_CACHE_TRACE_FILE,
      messages: process.env.FASED_CACHE_TRACE_MESSAGES,
      prompt: process.env.FASED_CACHE_TRACE_PROMPT,
      system: process.env.FASED_CACHE_TRACE_SYSTEM,
    };
    process.env.FASED_CACHE_TRACE = "1";
    process.env.FASED_CACHE_TRACE_FILE = liveCacheTraceFile;
    process.env.FASED_CACHE_TRACE_MESSAGES = "0";
    process.env.FASED_CACHE_TRACE_PROMPT = "0";
    process.env.FASED_CACHE_TRACE_SYSTEM = "0";
  }, 120_000);
  ...
  afterAll(async () => {
    if (previousCacheTraceEnv) {
      const restore = (
        key:
          | "FASED_CACHE_TRACE"
          | "FASED_CACHE_TRACE_FILE"
          | "FASED_CACHE_TRACE_MESSAGES"
          | "FASED_CACHE_TRACE_PROMPT"
          | "FASED_CACHE_TRACE_SYSTEM",
      ...
      restore("FASED_CACHE_TRACE", previousCacheTraceEnv.enabled);
      restore("FASED_CACHE_TRACE_FILE", previousCacheTraceEnv.file);
      restore("FASED_CACHE_TRACE_MESSAGES", previousCacheTraceEnv.messages);
      restore("FASED_CACHE_TRACE_PROMPT", previousCacheTraceEnv.prompt);
      restore("FASED_CACHE_TRACE_SYSTEM", previousCacheTraceEnv.system);
    }
  ...
  describe("openai", () => {
    ...
    beforeAll(async () => {
      fixture = await resolveLiveDirectModel({
        ...
        envVar: "FASED_LIVE_OPENAI_CACHE_MODEL",
  ...
  describe("anthropic", () => {
    ...
    beforeAll(async () => {
      fixture = await resolveLiveDirectModel({
        ...
        envVar: "FASED_LIVE_ANTHROPIC_CACHE_MODEL",
