...
describe("getApiKeyForModel", () => {
  it("migrates legacy oauth.json into auth-profiles.json", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "fased-oauth-"));

    try {
      process.env.FASED_STATE_DIR = tempDir;
      process.env.FASED_AGENT_DIR = path.join(tempDir, "agent");
      ...
      const store = ensureAuthProfileStore(process.env.FASED_AGENT_DIR, {
  ...
  it("suggests the OpenAI sign-in route when only that credential is configured", async () => {
    const previousStateDir = process.env.FASED_STATE_DIR;
    const previousAgentDir = process.env.FASED_AGENT_DIR;
    const previousPiAgentDir = process.env.PI_CODING_AGENT_DIR;
    ...
    try {
      ...
      expect(String(error)).toContain("OpenAI sign-in");
