...
describe("Ollama auto-discovery", () => {
  ...
  let originalLocalDiscovery: string | undefined;
  ...
  afterEach(() => {
    ...
    if (originalLocalDiscovery !== undefined) {
      process.env.FASED_TEST_LOCAL_PROVIDER_DISCOVERY = originalLocalDiscovery;
    } else {
      delete process.env.FASED_TEST_LOCAL_PROVIDER_DISCOVERY;
    }
  ...
  function setupDiscoveryEnv() {
    ...
    originalLocalDiscovery = process.env.FASED_TEST_LOCAL_PROVIDER_DISCOVERY;
    delete process.env.VITEST;
    delete process.env.NODE_ENV;
  ...
  it("auto-registers ollama provider when models are discovered locally", async () => {
    ...
    const providers = await resolveImplicitProviders({ agentDir, allowLocalDiscovery: true });
    ...
    expect(providers?.ollama?.models?.[0]?.reasoning).toBe(false);
