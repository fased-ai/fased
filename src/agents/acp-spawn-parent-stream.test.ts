...
describe("startAcpSpawnParentStreamRelay", () => {
  ...
  it("marks ACP task ledger runs terminal on lifecycle completion", () => {
    createRunningTaskRun({
      runtime: "acp",
      runId: "run-ledger",
      sessionKey: "agent:codex:acp:child-ledger",
      ownerKey: "agent:main:main",
      agentId: "codex",
      task: "Inspect ledger completion",
      deliveryStatus: "pending",
    });
    const relay = startAcpSpawnParentStreamRelay({
      runId: "run-ledger",
      parentSessionKey: "agent:main:main",
      childSessionKey: "agent:codex:acp:child-ledger",
      agentId: "codex",
      streamFlushMs: 10,
      noOutputNoticeMs: 120_000,
    });
    ...
    const task = listTaskRecords({ source: "subagent" }).tasks[0];
    expect(task).toMatchObject({
      runtime: "acp",
      runId: "run-ledger",
      status: "succeeded",
      deliveryStatus: "delivered",
      terminalSummary: "ACP run completed in 2s.",
    });
    relay.dispose();
  });
  ...
  it("resolves ACP spawn stream log path from session metadata", () => {
    readAcpSessionEntryMock.mockReturnValue({
      storePath: "/tmp/fased/agents/codex/sessions/sessions.json",
      entry: {
        sessionId: "sess-123",
        sessionFile: "/tmp/fased/agents/codex/sessions/sess-123.jsonl",
      },
    });
    resolveSessionFilePathMock.mockReturnValue("/tmp/fased/agents/codex/sessions/sess-123.jsonl");

    const resolved = resolveAcpSpawnStreamLogPath({
      childSessionKey: "agent:codex:acp:child-1",
    });

    expect(resolved).toBe("/tmp/fased/agents/codex/sessions/sess-123.acp-stream.jsonl");
