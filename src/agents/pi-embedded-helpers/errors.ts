...
const WRAPPED_PROVIDER_HTTP_STATUS_RE = /\bprovider failed\s*\(\s*HTTP\s*(\d{3})\s*\)/i;
...
function extractClassifiableHttpStatus(raw: string): { code: number; rest: string } | null {
  const trimmed = raw.trim();
  const match = trimmed.match(/^(http\s*)?(\d{3})(?::|\s+)?([\s\S]*)$/i);
  if (!match) {
    return null;
  }
  ...
  const hasHttpPrefix = Boolean(match[1]);
  if (hasHttpPrefix || code !== 402) {
    return { code, rest };
  }
  // Avoid treating ordinary prose/counts like "402 records processed" as billing.
  if (
    /\b(payment|required|billing|credit|credits|quota|limit|exhausted|spend|spent|points?|used up|plan|balance)\b/i.test(
      rest,
    )
  ) {
    return { code, rest };
...
function extractWrappedProviderHttpStatus(raw: string): number | null {
  const match = raw.match(WRAPPED_PROVIDER_HTTP_STATUS_RE);
  if (!match) {
    return null;
  }
  const code = Number(match[1]);
...
function hasSpecificNonTransientProviderHttpSignal(raw: string): boolean {
  return (
    isModelNotFoundErrorMessage(raw) ||
...
export function isTransientHttpError(raw: string): boolean {
  ...
  const wrappedStatus = extractWrappedProviderHttpStatus(trimmed);
  if (wrappedStatus === null || wrappedStatus < 500 || wrappedStatus >= 600) {
    return false;
  }
  return !hasSpecificNonTransientProviderHttpSignal(trimmed);
}
...
export function formatAssistantErrorText(
  ...
  opts?: { cfg?: FasedAgentConfig; sessionKey?: string; provider?: string; model?: string },
): string | undefined {
...
const ZAI_BILLING_CODE_1311_RE = /["']?code["']?\s*[:=]\s*1311\b/i;
const ZAI_AUTH_CODE_1113_RE = /["']?code["']?\s*[:=]\s*1113\b/i;
...
function isSessionExpiredErrorMessage(raw: string): boolean {
  return /\b(?:session|conversation)\b.*\b(?:not found|expired|gone)\b/i.test(raw);
}

function isPeriodicLimitMessage(raw: string): boolean {
  return /\b(?:daily|weekly|monthly|periodic|organization|workspace|org)\b.*\b(?:limit|spend|spending|usage|exhausted|reached|exceeded)\b/i.test(
    raw,
  );
}
...
function classifyProviderScopedGenericMessage(
  raw: string,
  ...
  const lower = raw.toLowerCase();
  if (provider === "anthropic" && lower === "an unknown error occurred") {
    return "timeout";
  }
  if (provider === "openrouter") {
    if (lower === "provider returned error") {
      return "timeout";
    }
    if (lower.includes("key limit exceeded")) {
      return "billing";
    }
  }
  return null;
...
export function extractObservedOverflowTokenCount(raw: string): number | undefined {
  if (!raw || !isLikelyContextOverflowError(raw)) {
    return undefined;
  }
  const patterns = [
    /\b(\d{3,})\s+tokens?\s*>\s*\d{3,}\s+(?:maximum|max|limit)\b/i,
    /\brequested\s+(\d{3,})\s+tokens?\b/i,
    /\bresulted in\s+(\d{3,})\s+tokens?\b/i,
  ];
...
export function isModelNotFoundErrorMessage(raw: string): boolean {
  ...
  // Direct pattern matches from FasedAgent internals and common providers.
  if (
...
export function classifyFailoverReason(
  ...
  const httpStatus = extractClassifiableHttpStatus(raw);
  if (httpStatus) {
    return classifyFailoverReasonFromHttpStatus(httpStatus.code, httpStatus.rest, opts);
  }
  const providerScoped = classifyProviderScopedGenericMessage(raw, opts);
  if (providerScoped) {
  ...
  if (isSpecificOverloadedErrorMessage(raw)) {
    return "overloaded";
  }
...
export function classifyFailoverReasonFromHttpStatus(
  ...
  if (explicit) {
    if (status === 402 && explicit === "billing" && isPeriodicLimitMessage(message)) {
      return hasExplicitBillingMarker(message) ? "billing" : "rate_limit";
    }
    if (status === 400 && explicit === "format" && isLikelyContextOverflowError(message)) {
      return null;
    }
    return explicit;
  }
  ...
  if (status === 402) {
    ...
    return isPeriodicLimitMessage(message) && !hasExplicitBillingMarker(message)
      ? "rate_limit"
      : "billing";
  }
  if (status === 401 || status === 403) {
    const providerScoped = classifyProviderScopedGenericMessage(message, opts);
    return providerScoped ?? "auth";
  }
  if (status === 422) {
    return "format";
  }
  ...
  if (status === 410) {
    return isSessionExpiredErrorMessage(message) ? "session_expired" : "timeout";
  }
  if (status === 499) {
    return "timeout";
  }
  ...
  if (status >= 500 && status < 600) {
    return status === 503 && isSpecificOverloadedErrorMessage(message) ? "overloaded" : "timeout";
  }
  if (status === 400) {
    return null;
  }
...
export function classifyFailoverSignal(
  input: string | { message?: string; status?: number },
): { kind: "context_overflow" } | { kind: FailoverReason } | null {
