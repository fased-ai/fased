...
function makeConfig(): FasedAgentConfig {
...
async function withAgentWorkspace<T>(
  ...
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "fased-model-fallback-"));
...
async function runEmbeddedFallback(params: {
  ...
  return await runWithModelFallback<EmbeddedPiRunResult>({
    cfg,
    provider: "openai",
    model: "mock-1",
