...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "fased-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: FasedAgentConfig) {
    return createFasedAgentCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<FasedAgentConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<FasedAgentConfig["agents"]>["list"]>[number]["tools"];
  }): FasedAgentConfig {
    return {
      tools: params.tools,
      agents: {
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createFasedAgentCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createFasedAgentCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createFasedAgentCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createFasedAgentCodingTools({
      config: cfg,
    ...
    const bobTools = createFasedAgentCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createFasedAgentCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createFasedAgentCodingTools({
      config: cfg,
