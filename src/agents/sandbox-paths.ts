...
export async function resolveSandboxedMediaSource(params: {
  ...
  if (WINDOWS_NETWORK_PATH_RE.test(raw)) {
    throw new Error("Windows network paths are not allowed for sandboxed media.");
  }
  let candidate = raw;
  if (/^file:\/\//i.test(candidate)) {
    assertFileUrlHasNoRemoteHost(candidate);
...
function assertFileUrlHasNoRemoteHost(fileUrl: string): void {
  let parsed: URL;
  try {
  ...
  if (parsed.protocol === "file:" && parsed.hostname && parsed.hostname !== "localhost") {
    throw new Error("Remote hosts are not allowed for sandboxed file:// media URLs.");
  }
}
...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const fasedTmpDir = path.resolve(resolvePreferredFasedAgentTmpDir());
  if (!isPathInside(fasedTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: fasedTmpDir });
  return resolved;
}
