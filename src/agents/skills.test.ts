...
async function writeEnvSkill(workspaceDir: string) {
  ...
  await writeSkill({
    ...
    metadata: '{"fased":{"requires":{"env":["ENV_KEY"]},"primaryEnv":"ENV_KEY"}}',
  });
}

beforeAll(async () => {
  await fixtureSuite.setup();
  tempHome = await createTempHomeEnv("fased-skills-home-");
  await fs.mkdir(path.join(tempHome.home, ".fased", "agents", "main", "sessions"), {
    recursive: true,
  });
});
...
describe("buildWorkspaceSkillCommandSpecs", () => {
  ...
  it("includes enabled Claude bundle markdown commands as native FasedAgent slash commands", async () => {
    const workspaceDir = await makeWorkspace();
    const pluginRoot = path.join(tempHome!.home, ".fased", "extensions", "compound-bundle");
...
describe("applySkillEnvOverrides", () => {
  ...
  it("prefers the active runtime snapshot over raw SecretRef skill config", async () => {
    ...
    const entries = loadWorkspaceSkillEntries(workspaceDir, resolveTestSkillDirs(workspaceDir));
    const sourceConfig: FasedAgentConfig = {
      skills: {
        entries: {
          "env-skill": {
            apiKey: {
    ...
    const runtimeConfig: FasedAgentConfig = {
      skills: {
        entries: {
          "env-skill": {
            apiKey: "resolved-key",
  ...
  it("blocks unsafe env overrides but allows declared secrets", async () => {
    ...
    await writeSkill({
      ...
      description: "Needs env",
      metadata:
        '{"fased":{"requires":{"env":["OPENAI_API_KEY","NODE_OPTIONS"]},"primaryEnv":"OPENAI_API_KEY"}}',
    });
  ...
  it("blocks dangerous host env overrides even when declared", async () => {
    ...
    await writeSkill({
      ...
      description: "Needs env",
      metadata: '{"fased":{"requires":{"env":["BASH_ENV","SHELL"]}}}',
    });
  ...
  it("blocks override-only host env overrides in skill config", async () => {
    ...
    await writeSkill({
      ...
      description: "Needs env",
      metadata:
        '{"fased":{"requires":{"env":["HTTPS_PROXY","NODE_TLS_REJECT_UNAUTHORIZED","DOCKER_HOST"]}}}',
    });
  ...
  it("allows required env overrides from snapshots", async () => {
    ...
    await writeSkill({
      ...
      description: "Needs env",
      metadata: '{"fased":{"requires":{"env":["OPENAI_API_KEY"]}}}',
    });
