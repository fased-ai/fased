...
function escapeSkillXml(str: string): string {
  return str
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;")
      ...
      normalized.length > 0
        ? filtered.filter(
            (entry) =>
              ...
              normalized.includes(resolveSkillKey(entry.skill, entry)),
          )
        : [];
    skillsLogger.debug(
...
function isPathWithinRoot(candidatePath: string, rootPath: string): boolean {
  ...
  try {
    ...
    // Keep lexical path when realpath is unavailable.
  }
  try {
...
function resolveSkillsLimits(config?: FasedAgentConfig): ResolvedSkillsLimits {
...
function loadSkillEntries(
  ...
  const skillEntries: SkillEntry[] = Array.from(merged.values()).map((skill) => {
    ...
    return {
      skill,
      frontmatter,
      metadata: resolveFasedAgentMetadata(frontmatter),
...
function applySkillsPromptLimits(params: { skills: Skill[]; config?: FasedAgentConfig }): {
...
export function buildWorkspaceSkillSnapshot(
  ...
  const skillFilter = resolveEffectiveSkillFilter(opts);
  return {
    prompt,
...
function resolveEffectiveSkillFilter(
  opts?: Pick<WorkspaceSkillBuildOptions, "agentId" | "config" | "skillFilter">,
): string[] | undefined {
...
function resolveWorkspaceSkillPromptState(
  ...
  const eligible = filterSkillEntries(
    skillEntries,
    opts?.config,
    resolveEffectiveSkillFilter(opts),
    opts?.eligibility,
  );
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}. Run \`fased skills check\` to audit.`
    : "";
  const prompt = [
    remoteNote,
    truncationNote,
...
export async function syncSkillsToWorkspace(params: {
  ...
  await serializeByKey(`syncSkills:${targetDir}`, async () => {
    ...
    const entries = filterSkillEntries(
      loadedEntries,
      params.config,
      resolveEffectiveSkillFilter(params),
      params.eligibility,
    ).filter((entry) => isPathWithinRoot(entry.skill.baseDir, sourceDir));
