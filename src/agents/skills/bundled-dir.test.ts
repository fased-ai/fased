...
describe("resolveBundledSkillsDir", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["FASED_BUNDLED_SKILLS_DIR"]);
  });
  ...
  it("returns FASED_BUNDLED_SKILLS_DIR override when set", async () => {
    const overrideDir = await fs.mkdtemp(path.join(os.tmpdir(), "fased-bundled-override-"));
    process.env.FASED_BUNDLED_SKILLS_DIR = ` ${overrideDir} `;
  ...
  it("resolves bundled skills under a flattened dist layout", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fased-bundled-"));
