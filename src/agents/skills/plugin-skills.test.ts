    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "fased.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "fased.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "fased.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("fased-");
  const acpxRoot = await tempDirs.make("fased-acpx-plugin-");
  const helperRoot = await tempDirs.make("fased-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("fased-");
  const pluginRoot = await tempDirs.make("fased-plugin-");
  const outsideDir = await tempDirs.make("fased-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("fased-");
    const pluginRoot = await tempDirs.make("fased-claude-bundle-");
  ...
  it("resolves enabled plugin skills through legacy manifest aliases", async () => {
    const workspaceDir = await tempDirs.make("fased-");
    const pluginRoot = await tempDirs.make("fased-legacy-plugin-");
    await fs.mkdir(path.join(pluginRoot, "skills"), { recursive: true });
