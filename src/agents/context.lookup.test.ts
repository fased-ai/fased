...
function mockContextDeps(params: {
  ...
  const ensureFasedModelsJson = vi.fn(async () => {});
  ...
  vi.doMock("./agent-paths.js", () => ({
    resolveFasedAgentDir: () => "/tmp/fased-agent",
  }));
...
describe("lookupContextTokens", () => {
  ...
  it("only warms eagerly for real fased startup commands that need model metadata", async () => {
    const argvSnapshot = process.argv;
    try {
      for (const scenario of [
        {
          argv: ["node", "fased", "chat"],
          expectedCalls: 1,
        },
        {
          argv: ["node", "fased", "--profile", "--", "config", "validate"],
          expectedCalls: 0,
        },
        {
          argv: ["node", "fased", "logs", "--limit", "5"],
          expectedCalls: 0,
        },
        {
        ...
        const { ensureFasedModelsJson } = mockContextModuleDeps(loadConfigMock);
        process.argv = scenario.argv;
        await importContextModule();
        expect(loadConfigMock).toHaveBeenCalledTimes(scenario.expectedCalls);
        expect(ensureFasedModelsJson).toHaveBeenCalledTimes(scenario.expectedCalls);
      }
    } finally {
