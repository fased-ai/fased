...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses FASED_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      FASED_HOME: "/srv/fased-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/fased-home"), ".fased", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".fased", "workspace-state.json"] as const;
...
async function expectBootstrapSeeded(dir: string) {
  ...
  await expect(fs.access(path.join(dir, DEFAULT_MEMORY_FILENAME))).resolves.toBeUndefined();
  await expect(fs.access(path.join(dir, "memory"))).resolves.toBeUndefined();
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    ...
    expect((await readWorkspaceState(tempDir)).onboardingCompletedAt).toBeUndefined();
  });
  ...
  it("keeps legacy onboardingCompletedAt markers in Fased workspace state", async () => {
    const tempDir = await makeTempWorkspace("fased-workspace-");
    await fs.mkdir(path.join(tempDir, ".fased"), { recursive: true });
    await fs.writeFile(
    ...
    const state = await readWorkspaceState(tempDir);
    expect(state.onboardingCompletedAt).toBe("2026-03-15T02:30:00.000Z");
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "fased-workspace-hardlink-"));
    try {
