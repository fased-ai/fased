...
type SharedAuthProfileFailureReason = Parameters<typeof markAuthProfileFailure>[0]["reason"];

// Avoid Anthropic's refusal test token poisoning session transcripts.
const ANTHROPIC_MAGIC_STRING_TRIGGER_REFUSAL = "ANTHROPIC_MAGIC_STRING_TRIGGER_REFUSAL";
const ANTHROPIC_MAGIC_STRING_REPLACEMENT = "ANTHROPIC MAGIC STRING TRIGGER REFUSAL (redacted)";

function resolveSharedAuthProfileFailureReason(
  reason?: SharedAuthProfileFailureReason | null,
): SharedAuthProfileFailureReason | null {
  if (!reason || reason === "timeout" || reason === "format") {
    return null;
  }
  return reason;
}
...
type PostCompactionOverflowObservation = {
  provider: string;
  modelId: string;
...
function normalizeOverflowErrorFingerprint(message: string): string {
  return message.trim().toLowerCase().replace(/\s+/g, " ");
}

function resolveOverflowObservationPromptTokens(
  usage: ReturnType<typeof normalizeUsage>,
): number | undefined {
...
function createPostCompactionOverflowObservation(params: {
  provider: string;
  modelId: string;
  source: "promptError" | "assistantError";
  errorText: string;
  messageCount: number;
  usage: ReturnType<typeof normalizeUsage>;
}): PostCompactionOverflowObservation {
  return {
    provider: params.provider,
    modelId: params.modelId,
    source: params.source,
    errorFingerprint: normalizeOverflowErrorFingerprint(params.errorText),
    messageCount: params.messageCount,
    promptTokens: resolveOverflowObservationPromptTokens(params.usage),
  };
}

function isRepeatedPostCompactionOverflow(params: {
  previous: PostCompactionOverflowObservation | null;
  current: PostCompactionOverflowObservation;
}): boolean {
  const previous = params.previous;
  if (
    !previous ||
    previous.promptTokens === undefined ||
    params.current.promptTokens === undefined
  ) {
    return false;
  }
  return (
    previous.provider === params.current.provider &&
    previous.modelId === params.current.modelId &&
    previous.source === params.current.source &&
    previous.errorFingerprint === params.current.errorFingerprint &&
    previous.messageCount === params.current.messageCount &&
    previous.promptTokens === params.current.promptTokens
  );
}
...
const toNormalizedUsage = (usage: UsageAccumulator) => {
  ...
  // See: https://github.com/fased-ai/fased/issues/13698
  //
...
export async function runEmbeddedPiAgent(
  ...
  return enqueueSession(() =>
    enqueueGlobal(async () => {
      ...
      try {
        const shouldProbeCooldownedProfile =
          ...
          profileCandidates.every(
            (candidate) => !candidate || isProfileInCooldown(authStore, candidate),
          );
        if (shouldProbeCooldownedProfile) {
      ...
      let pendingPostCompactionOverflowObservation: PostCompactionOverflowObservation | null = null;
      ...
      try {
        while (true) {
          ...
          if (contextOverflowError) {
            ...
            const overflowObservation = createPostCompactionOverflowObservation({
              provider,
              modelId,
              source: contextOverflowError.source,
              errorText,
              messageCount: msgCount,
              usage: lastRunPromptUsage,
            });
            log.warn(
            ...
            if (
              isRepeatedPostCompactionOverflow({
                previous: pendingPostCompactionOverflowObservation,
                current: overflowObservation,
              })
            ) {
              log.warn(
                `[post-compaction-loop-guard] repeated overflow after compaction; ` +
                  `provider=${provider}/${modelId} promptTokens=${overflowObservation.promptTokens} ` +
                  `messages=${msgCount}`,
              );
              return {
                payloads: [
                  {
                    text:
                      "Context overflow: prompt too large for the model after compaction. " +
            ...
            pendingPostCompactionOverflowObservation = null;
            ...
            if (
              ...
              pendingPostCompactionOverflowObservation = overflowObservation;
              continue;
            }
          ...
          if (
            ...
            log.warn(
              `[llm-idle-timeout] ${provider}/${modelId} produced no reply before the idle watchdog; retrying same model`,
            );
            continue;
          }
