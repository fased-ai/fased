...
describe("resolveModel", () => {
  it("includes provider baseUrl in fallback model", () => {
    ...
    expect(getModelProviderRequestTransport(result.model ?? {})).toEqual({
      allowPrivateNetwork: false,
    });
  });

  it("attaches explicit provider private-network request config", () => {
    const cfg = {
      models: {
        providers: {
          custom: {
  ...
  it.each(["gpt-5.2-codex", "gpt-5.5-pro", "gpt-5.4-pro", "gpt-5.3-codex-spark"])(
    ...
    (requestedModel) => {
      ...
      const result = resolveModel("openai-codex", requestedModel, "/tmp/agent");

      expect(result.model).toBeUndefined();
      expect(result.error).toBe(`Unsupported OpenAI-Codex sign-in model: ${requestedModel}`);
    },
  );
  ...
  it("rejects unsupported openai-codex ids before generic fallback", () => {
  ...
  it("includes auth hint for unknown ollama models (#17328)", () => {
    ...
    expect(result.error).toContain("docs.fased.ai/gateway/local-models");
  });
  ...
  it("does not add auth hint for non-local providers", () => {
    const result = resolveModel("retired-provider", "some-model", "/tmp/agent");

    expect(result.model).toBeUndefined();
    expect(result.error).toBe("Unknown model: retired-provider/some-model");
  });
});
