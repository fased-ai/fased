...
export const CACHE_TTL_CUSTOM_TYPE = "fased.cache-ttl";
...
export type CacheTtlAttemptState = {
  timedOutDuringCompaction: boolean;
  compactionOccurredThisAttempt: boolean;
  contextPruningMode?: unknown;
  provider: string;
  modelId: string;
};
...
export function shouldAppendCacheTtlTimestampAfterAttempt(state: CacheTtlAttemptState): boolean {
  if (state.timedOutDuringCompaction || state.compactionOccurredThisAttempt) {
    return false;
  }
  if (state.contextPruningMode !== "cache-ttl") {
    return false;
  }
  return isCacheTtlEligibleProvider(state.provider, state.modelId);
}
