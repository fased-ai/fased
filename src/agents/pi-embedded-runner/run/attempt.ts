...
export async function runEmbeddedAttempt(
  ...
  let bundleMcpRuntime: BundleMcpToolRuntime | undefined;
  ...
  try {
    ...
    if (!toolsDisabled) {
      bundleMcpRuntime = await createBundleMcpToolRuntime({
        workspaceDir: effectiveWorkspace,
        cfg: params.config,
        reservedToolNames: toolsRaw.map((tool) => tool.name),
      });
      if (bundleMcpRuntime.tools.length > 0) {
        toolsRaw = [...toolsRaw, ...bundleMcpRuntime.tools];
      }
    }
    toolsRaw = filterToolsByDisabledToolNames(toolsRaw, params.disabledToolNames);
    ...
    const docsPath = await resolveFasedAgentDocsPath({
      workspaceDir: effectiveWorkspace,
    ...
    try {
      ...
      try {
        ...
        const validated = transcriptPolicy.validateAnthropicTurns
          ...
          : validatedGemini;
        const truncated = params.omitPriorMessages
          ? []
          : limitHistoryTurns(
              validated,
      ...
      try {
        ...
        // double-compaction. See: https://github.com/fased-ai/fased/issues/9282
        // Skip when timed out during compaction — session state may be inconsistent.
        const shouldTrackCacheTtl = shouldAppendCacheTtlTimestampAfterAttempt({
          timedOutDuringCompaction,
          compactionOccurredThisAttempt,
          contextPruningMode: params.config?.agents?.defaults?.contextPruning?.mode,
          provider: params.provider,
          modelId: params.modelId,
        });
        ...
        if (promptError && promptErrorSource === "prompt" && !compactionOccurredThisAttempt) {
          try {
            sessionManager.appendCustomEntry("fased:prompt-error", {
              timestamp: Date.now(),
              runId: params.runId,
              sessionId: params.sessionId,
              provider: params.provider,
      ...
      // See: https://github.com/fased-ai/fased/issues/8643
    ...
    if (bundleMcpRuntime) {
      try {
        ...
        log.warn(`bundle-mcp runtime dispose failed: ${String(error)}`);
      }
    }
    restoreSkillEnv?.();
