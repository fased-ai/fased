...
function normalizeToolParameterSchema(schema: unknown): unknown {
  const normalized = normalizeToolParameters({
    name: "schema",
    label: "Schema",
    description: "Schema normalization placeholder.",
    parameters: schema,
...
export function resolvesToNativeOpenAIStrictTools(
  ...
  return capabilities.provider === "openai" || capabilities.provider === "openai-codex";
}
