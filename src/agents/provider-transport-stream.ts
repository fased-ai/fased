...
const SIMPLE_TRANSPORT_API_ALIAS: Record<string, Api> = {
  "openai-responses": "fased-openai-responses-transport",
  "openai-codex-responses": "fased-openai-responses-transport",
  "openai-completions": "fased-openai-completions-transport",
  "anthropic-messages": "fased-anthropic-messages-transport",
  "google-generative-ai": "fased-google-generative-ai-transport",
};
...
function hasTransportOverrides(model: Model<Api>): boolean {
  ...
  return Boolean(request?.proxy || request?.tls || request?.allowPrivateNetwork !== undefined);
}
...
export function createTransportAwareStreamFnForModel(model: Model<Api>): StreamFn | undefined {
  ...
  if (!isTransportAwareApiSupported(model.api)) {
    throw new Error(`Model-provider request controls are not yet supported for api "${model.api}"`);
  }
