...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "fased-skills-sync-suite-"));
...
describe("buildWorkspaceSkillsPrompt", () => {
  ...
  it("filters skills based on env/config gates", async () => {
    ...
    await writeSkill({
      ...
      metadata: '{"fased":{"requires":{"env":["GEMINI_API_KEY"]},"primaryEnv":"GEMINI_API_KEY"}}',
      body: "# Image Lab\n",
    });
  ...
  it("syncs remote-eligible filtered skills into the target workspace", async () => {
    ...
    await writeSkill({
      ...
      metadata: '{"fased":{"requires":{"anyBins":["missingbin","sandboxbin"]}}}',
    });
