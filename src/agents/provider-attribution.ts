...
const FASED_ATTRIBUTION_PRODUCT = "Fased";
const FASED_ATTRIBUTION_ORIGINATOR = "fased";
...
function formatFasedUserAgent(version: string): string {
  return `${FASED_ATTRIBUTION_ORIGINATOR}/${version}`;
}
...
export function resolveProviderAttributionIdentity(
  ...
  return {
    product: FASED_ATTRIBUTION_PRODUCT,
...
function buildOpenAIAttributionPolicy(
  ...
  return {
    ...
    ...identity,
    headers: {
      originator: FASED_ATTRIBUTION_ORIGINATOR,
      version: identity.version,
      "User-Agent": formatFasedUserAgent(identity.version),
    },
  };
}
