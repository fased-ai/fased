...
describe("provider attribution", () => {
  it("resolves the canonical Fased product and runtime version", () => {
    const identity = resolveProviderAttributionIdentity({
      FASED_VERSION: "2026.3.99",
    });
  ...
  it("returns a documented OpenRouter attribution policy", () => {
    ...
    expect(policy).toEqual({
      ...
      reviewNote: "Documented app attribution headers. Verified in Fased runtime wrapper.",
      product: "Fased",
      version: "2026.3.22",
      headers: {
        "HTTP-Referer": "https://fased.ai",
        "X-OpenRouter-Title": "Fased",
  ...
  it("returns a hidden-spec OpenAI attribution policy", () => {
    expect(resolveProviderAttributionPolicy("openai", { FASED_VERSION: "2026.3.22" })).toEqual({
      provider: "openai",
      enabledByDefault: true,
      ...
      headers: {
        originator: "fased",
        version: "2026.3.22",
        "User-Agent": "fased/2026.3.22",
      },
    });
    expect(resolveProviderAttributionHeaders("openai", { FASED_VERSION: "2026.3.22" })).toEqual({
      originator: "fased",
      version: "2026.3.22",
      "User-Agent": "fased/2026.3.22",
    });
  ...
  it("returns a hidden-spec OpenAI Codex attribution policy", () => {
    expect(
      resolveProviderAttributionPolicy("openai-codex", { FASED_VERSION: "2026.3.22" }),
    ).toEqual({
  ...
  it("lists the current attribution support matrix", () => {
    expect(
      listProviderAttributionPolicies({ FASED_VERSION: "2026.3.22" }).map((policy) => [
        policy.provider,
