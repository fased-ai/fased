...
export function filterToolsByDisabledToolNames(
  tools: AnyAgentTool[],
  disabledToolNames?: string[],
): AnyAgentTool[] {
  const disabled = new Set(
    (disabledToolNames ?? []).map((name) => name.trim().toLowerCase()).filter(Boolean),
  );
  if (disabled.size === 0) {
    return tools;
  }
  return tools.filter((tool) => !disabled.has(tool.name.trim().toLowerCase()));
}
...
function resolveExecConfig(params: { cfg?: FasedAgentConfig; agentId?: string }) {
...
export function createFasedAgentCodingTools(options?: {
  agentId?: string;
  ...
  modelId?: string;
  /** Resolved model capability hints for inventory/debug consumers. */
  modelCompat?: unknown;
  ...
  /** Exact tool names to deny for this run after normal policy resolution. */
  disabledToolNames?: string[];
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createFasedAgentReadTool(freshReadTool, {
        modelContextWindowTokens: options?.modelContextWindowTokens,
        imageSanitization,
      });
  ...
  return filterToolsByDisabledToolNames(withAbort, options?.disabledToolNames);
}
