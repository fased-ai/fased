...
describe("Volcengine and BytePlus providers", () => {
  it("includes volcengine coding aliases when VOLCANO_ENGINE_API_KEY is configured", async () => {
    ...
    try {
      ...
      expect(providers?.["volcengine-coding"]).toBeDefined();
      expect(providers?.["volcengine-plan"]).toBeDefined();
      expect(providers?.volcengine?.apiKey).toBe("VOLCANO_ENGINE_API_KEY");
      expect(providers?.["volcengine-coding"]?.apiKey).toBe("VOLCANO_ENGINE_API_KEY");
  ...
  it("includes byteplus coding aliases when BYTEPLUS_API_KEY is configured", async () => {
    const agentDir = mkdtempSync(join(tmpdir(), "fased-test-"));
    ...
    try {
      ...
      expect(providers?.["byteplus-coding"]).toBeDefined();
      expect(providers?.["byteplus-plan"]).toBeDefined();
      expect(providers?.byteplus?.apiKey).toBe("BYTEPLUS_API_KEY");
      expect(providers?.["byteplus-coding"]?.apiKey).toBe("BYTEPLUS_API_KEY");
