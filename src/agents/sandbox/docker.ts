...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "fased.sandbox=1");
  args.push("--label", `fased.sessionKey=${params.scopeKey}`);
  args.push("--label", `fased.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `fased.configHash=${params.configHash}`);
  }
  ...
  if (!Object.hasOwn(envSanitization.allowed, "FASED_CLI")) {
    args.push("--env", `FASED_CLI=${FASED_CLI_ENV_VALUE}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "fased.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`fased sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`fased sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("fased sandbox recreate --all");
}
