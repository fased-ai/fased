...
export function sanitizeConfiguredModelProviderRequest(
  ...
  const transport = sanitizeConfiguredProviderRequest(request);
  const allowPrivateNetwork =
    request?.allowPrivateNetwork === true
      ...
      : request?.allowPrivateNetwork === false
        ? false
        : undefined;
  if (!transport && allowPrivateNetwork === undefined) {
    return undefined;
  }
  return {
    ...transport,
...
export function mergeModelProviderRequestOverrides(
  ...
  const merged = mergeProviderRequestOverrides(...overrides);
  ...
  if (!merged && allowPrivateNetwork === undefined) {
    return undefined;
  }
  return {
    ...merged,
...
const MODEL_PROVIDER_REQUEST_TRANSPORT_SYMBOL = Symbol.for("fased.modelProviderRequestTransport");
