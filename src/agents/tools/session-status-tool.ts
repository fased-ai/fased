...
> {
  ...
  const allowed = buildCredentialScopedAllowedModelSet({
    ...
    store: ensureAuthProfileStore(resolveAgentDir(params.cfg, params.agentId), {
...
export function createSessionStatusTool(opts?: {
  ...
  return {
    ...
    execute: async (_toolCallId, args) => {
      ...
      const taskSnapshot = buildTaskStatusSnapshotForRelatedSessionKeyForOwner({
        relatedSessionKey: resolved.key,
        callerOwnerKey: opts?.agentSessionKey ?? resolved.key,
      });
      ...
      const statusText = buildStatusMessage({
        ...
        usageLine,
        timeLine,
        subagentsLine: taskSnapshot.line,
        queue: {
