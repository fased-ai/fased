...
export function createCronTool(opts?: CronToolOptions, deps?: CronToolDeps): AnyAgentTool {
  ...
  return {
    ...
    name: "cron",
    ownerOnly: true,
    description: `Manage Gateway scheduled tasks (status/list/add/update/remove/run/runs) and send wake events.

ACTIONS:
- status: Check task scheduler status
- list: List tasks (use includeDisabled:true to include disabled)
- add: Create task (requires task object, see schema below)
- update: Modify task (requires jobId + patch object)
- remove: Delete task (requires jobId)
- run: Trigger task immediately (requires jobId)
- runs: Get task activity log (requires jobId)
...
DELIVERY (top-level):
  ...
  - Default for isolated agentTurn tasks (when delivery omitted): "announce"
...
Default: prefer isolated agentTurn tasks unless the user explicitly wants a main-session system event.

WAKE MODES (for wake action):
- "next-heartbeat" (default): Wake on next heartbeat
- "now": Wake immediately

Use jobId as the canonical identifier; id is accepted for compatibility. Use contextMessages (0-10) to add previous messages as context to the task text.`,
    ...
    execute: async (_toolCallId, args) => {
      ...
      switch (action) {
        ...
        case "add": {
          ...
          // See: https://github.com/fased-ai/fased/issues/11310
          if (
            !params.job ||
