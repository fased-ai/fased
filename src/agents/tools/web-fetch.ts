...
type WebFetchConfig = NonNullable<FasedAgentConfig["tools"]>["web"] extends infer Web
...
function resolveFetchConfig(cfg?: FasedAgentConfig): WebFetchConfig {
...
function resolveFirecrawlApiKey(firecrawl?: FirecrawlFetchConfig): string | undefined {
  const configuredRef = coerceSecretRef(firecrawl?.apiKey);
  if (configuredRef?.source === "env") {
    const fromRefEnv = normalizeSecretInput(process.env[configuredRef.id]);
    if (fromRefEnv) {
      return fromRefEnv;
    }
  }
