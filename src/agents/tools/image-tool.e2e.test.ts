...
describe("image tool implicit imageModel config", () => {
  ...
  afterEach(() => {
    vi.unstubAllEnvs();
    // @ts-ignore global fetch cleanup
    global.fetch = priorFetch;
  });

  it("stays disabled without auth when no pairing is possible", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "fased-image-"));
  ...
  it("allows workspace images outside default local media roots", async () => {
    const workspaceParent = await fs.mkdtemp(path.join(process.cwd(), ".fased-workspace-image-"));
    try {
  ...
  it("allows workspace images via createFasedAgentCodingTools default workspace root", async () => {
    const workspaceParent = await fs.mkdtemp(path.join(process.cwd(), ".fased-workspace-image-"));
    try {
      const workspaceDir = path.join(workspaceParent, "workspace");
      await fs.mkdir(workspaceDir, { recursive: true });
      ...
      const tools = createFasedAgentCodingTools({ config: cfg, agentDir });
  ...
  it("sandboxes image paths like the read tool", async () => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "fased-image-sandbox-"));
  ...
  it("rewrites inbound absolute paths into sandbox media/inbound", async () => {
    ...
    const res = await tool.execute("t1", {
      prompt: "Describe the image.",
      image: "@/Users/steipete/.fased/media/inbound/photo.png",
    });
...
describe("image tool MiniMax VLM routing", () => {
  ...
  it("calls /v1/coding_plan/vlm for minimax image models", async () => {
    ...
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "fased-minimax-vlm-"));
    vi.stubEnv("MINIMAX_API_KEY", "minimax-test");
    const cfg: FasedAgentConfig = {
      agents: { defaults: { model: { primary: "minimax/MiniMax-M2.7" } } },
    };
