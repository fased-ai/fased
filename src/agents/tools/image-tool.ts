...
const ANTHROPIC_IMAGE_PRIMARY = "anthropic/claude-opus-4-8";
...
function resolveDefaultModelRef(cfg?: FasedAgentConfig): {
  provider: string;
  model: string;
} {
  if (cfg) {
...
function pickMaxBytes(cfg?: FasedAgentConfig, maxBytesMb?: number): number | undefined {
...
async function runImagePrompt(params: {
  ...
  const effectiveCfg: FasedAgentConfig | undefined = params.cfg
    ? {
        ...params.cfg,
        agents: {
          ...params.cfg.agents,
  ...
  await ensureFasedAgentModelsJson(effectiveCfg, params.agentDir);
