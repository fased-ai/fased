...
const presentationMessageSchema = Type.Object(
  ...
  {
    description:
      "Shared presentation payload for rich text, buttons, selects, and context. Unsupported blocks degrade through channel adapters.",
  },
);
...
function buildSendSchema(options: {
  ...
  const props: Record<string, unknown> = {
    ...
    presentation: Type.Optional(presentationMessageSchema),
...
function buildMessageToolSchema(params: {
  ...
  const includePresentation = includeButtons || includeCards || includeComponents;
  const includeDeliveryPin = actions.includes("pin");
  ...
  return {
    ...
    execute: async (_toolCallId, args, signal) => {
      ...
      const result = await runMessageAction({
        ...
        requesterAccountId: options?.requesterAccountId ?? undefined,
        requesterSenderId: options?.requesterSenderId,
        requesterSenderName: options?.requesterSenderName ?? undefined,
        requesterSenderUsername: options?.requesterSenderUsername ?? undefined,
        requesterSenderE164: options?.requesterSenderE164 ?? undefined,
