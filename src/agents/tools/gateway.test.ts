...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    fased: process.env.FASED_GATEWAY_TOKEN,
  };
  ...
  afterAll(() => {
    if (envSnapshot.fased === undefined) {
      delete process.env.FASED_GATEWAY_TOKEN;
    } else {
      process.env.FASED_GATEWAY_TOKEN = envSnapshot.fased;
    }
  });

  it("leaves url undefined so callGateway can use configured gateway mode", () => {
    const opts = resolveGatewayOptions();
    expect(opts.url).toBeUndefined();
    expect(opts.config).toBeUndefined();
  });

  it("can force local gateway mode for tools that must match the local UI runtime", async () => {
    configState.value = {
      gateway: {
        mode: "remote",
    ...
    const opts = resolveGatewayOptions({ gatewayTarget: "local" });
    expect(opts.url).toBeUndefined();
    expect(opts.token).toBe("local-token");
  ...
  it("uses FASED_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("prefers config gateway.auth.token over stale env token for local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.FASED_GATEWAY_TOKEN = "local-env-token";
    configState.value = {
      gateway: {
