...
export type GatewayCallOptions = {
  gatewayUrl?: string;
  gatewayToken?: string;
  gatewayTarget?: "configured" | "local";
...
function resolveLocalGatewayCredentials(cfg: ReturnType<typeof loadConfig>): {
  token?: string;
  password?: string;
} {
  return resolveGatewayCredentialsFromConfig({
...
function forceLocalGatewayMode(cfg: ReturnType<typeof loadConfig>): ReturnType<typeof loadConfig> {
  return {
    ...cfg,
    gateway: {
      ...cfg.gateway,
...
export function resolveGatewayOptions(opts?: GatewayCallOptions) {
  ...
  const localCredentials =
    !validatedOverride && opts?.gatewayTarget === "local"
      ? resolveLocalGatewayCredentials(cfg)
      : undefined;
  const token = validatedOverride
    ...
    : (explicitToken ?? localCredentials?.token);
  const config =
    !validatedOverride && opts?.gatewayTarget === "local" ? forceLocalGatewayMode(cfg) : undefined;
  const password = validatedOverride || explicitToken ? undefined : localCredentials?.password;
  const timeoutMs =
    typeof opts?.timeoutMs === "number" && Number.isFinite(opts.timeoutMs)
      ? Math.max(1, Math.floor(opts.timeoutMs))
      : 30_000;
  return { url: validatedOverride?.url, token, password, timeoutMs, config };
}
...
export async function callGatewayTool<T = Record<string, unknown>>(
  ...
  const scopes = opts.operatorScopes ?? resolveLeastPrivilegeOperatorScopesForMethod(method);
  return await callGateway<T>({
    url: gateway.url,
    token: gateway.token,
    password: gateway.password,
    ...(gateway.config ? { config: gateway.config } : {}),
    method,
    params,
