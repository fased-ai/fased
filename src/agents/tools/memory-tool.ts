...
import { resolveQmdScopeDenial } from "../../memory/qmd-scope.js";
...
export function createMemorySearchTool(options: {
  ...
  return {
    ...
    execute: async (_toolCallId, params) => {
      ...
      if (scopeDenial) {
        return jsonResult(buildMemorySearchUnavailableResult(scopeDenial));
      }
...
export function createMemoryGetTool(options: {
  ...
  return {
    ...
    execute: async (_toolCallId, params) => {
      ...
      if (scopeDenial) {
        return jsonResult({ path: relPath, text: "", disabled: true, error: scopeDenial });
      }
      const { manager, error } = await getMemorySearchManager({
        cfg,
        agentId,
...
function resolveMemoryToolScopeDenial(params: {
  ...
  return resolveQmdScopeDenial(resolved.qmd?.scope, params.sessionKey);
}

function resolveMemoryCitationsMode(cfg: FasedAgentConfig): MemoryCitationsMode {
...
function buildMemorySearchUnavailableResult(error: string | undefined) {
  ...
  const isScopeError = /scope denies|scope denied|outside.*scope/.test(reason.toLowerCase());
