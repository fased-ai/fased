...
describe("message tool schema scoping", () => {
  ...
  it("exposes shared presentation and delivery pin params in the default schema", () => {
    const tool = createMessageTool();
    ...
    expect(properties.delivery).toBeDefined();
  });
  ...
  it("hides discord components when scoped to telegram", () => {
    ...
    expect(properties.buttons).toBeDefined();
    expect(properties.presentation).toBeDefined();
    expect(properties.delivery).toBeUndefined();
  ...
  it("does not broaden schema actions from supportsAction alone", () => {
    const narrowPlugin: ChannelPlugin = {
      id: "narrowchat",
      meta: {
        id: "narrowchat",
        label: "Narrow Chat",
        selectionLabel: "Narrow Chat",
        docsPath: "/channels/narrowchat",
        blurb: "Narrow schema test plugin.",
      },
      ...
      actions: {
        ...
        supportsAction: ({ action }) => action === "send" || action === "pin",
      },
    };
    setActivePluginRegistry(
      createTestRegistry([{ pluginId: "narrowchat", source: "test", plugin: narrowPlugin }]),
    );

    const tool = createMessageTool({
      config: {} as never,
      currentChannelProvider: "narrowchat",
    });
    const actionEnum = getActionEnum(getToolProperties(tool));

    expect(actionEnum).toContain("send");
    expect(actionEnum).not.toContain("pin");
  });
});
...
describe("message tool sandbox passthrough", () => {
  ...
  it("forwards trusted requester context to runMessageAction", async () => {
    mockSendResult({ to: "discord:123" });

    const tool = createMessageTool({
      config: {} as never,
      requesterAccountId: "ops-account",
      requesterSenderId: "1234567890",
      requesterSenderName: "Ops User",
      requesterSenderUsername: "ops",
    ...
    await tool.execute("1", {
      ...
      target: "discord:123",
      message: "hi",
      requesterSenderId: "model-controlled",
      senderIsOwner: false,
    });

    const call = mocks.runMessageAction.mock.calls[0]?.[0];
    expect(call?.requesterAccountId).toBe("ops-account");
    expect(call?.requesterSenderId).toBe("1234567890");
    expect(call?.requesterSenderName).toBe("Ops User");
    expect(call?.requesterSenderUsername).toBe("ops");
    expect(call?.requesterSenderE164).toBe("+15551234567");
    expect(call?.senderIsOwner).toBe(true);
    expect(call?.sessionId).toBe("session-1");
    expect(call?.params?.requesterSenderId).toBe("model-controlled");
    expect(call?.params?.senderIsOwner).toBe(false);
  });
});
