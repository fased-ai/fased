...
export function resolveMainSessionAlias(cfg: FasedAgentConfig) {
...
export async function listSpawnedSessionKeys(params: {
  ...
  const limit =
    typeof params.limit === "number" && Number.isFinite(params.limit)
      ...
      : 500;
  try {
    const list = await deps.callGateway<{ sessions: Array<{ key?: unknown }> }>({
...
async function resolveSessionKeyFromSessionId(params: {
  ...
  try {
    ...
    const result = await deps.callGateway<{ key?: string }>({
