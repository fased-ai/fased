...
describe("subagent registry archive behavior", () => {
  ...
  it("sweeps completed session-mode runs even without archiveAtMs", async () => {
    vi.useFakeTimers();
    vi.setSystemTime(new Date("2026-04-29T00:00:00Z"));

    mod.registerSubagentRun({
      runId: "run-session-sweep",
      childSessionKey: "agent:main:subagent:session-sweep",
      requesterSessionKey: "agent:main:main",
      requesterDisplayKey: "main",
      task: "persistent-session",
      cleanup: "keep",
    ...
    expect(
      mod
        .listSubagentRunsForRequester("agent:main:main")
        .some((run) => run.runId === "run-session-sweep"),
    ).toBe(true);

    expect(mod.markSubagentRunTerminated({ runId: "run-session-sweep" })).toBe(1);
