...
const FAST_TEST_MODE = process.env.FASED_TEST_FAST === "1";
...
async function sendAnnounce(item: AnnounceQueueItem) {
  ...
  const gatewayAuth = resolveInternalGatewayCallAuth(cfg);
...
async function sendSubagentAnnounceDirectly(params: {
  ...
  const canonicalAgentCallSessionKey = resolveRequesterStoreKey(
    cfg,
    params.agentCallSessionKey ?? params.targetRequesterSessionKey,
  );
  try {
    ...
    await runAnnounceDeliveryWithRetry({
      ...
      run: async () =>
        await callGateway({
          ...
          method: "agent",
          params: {
            sessionKey: canonicalAgentCallSessionKey,
  ...
  return await runSubagentAnnounceDispatch({
    ...
    direct: async () =>
      await sendSubagentAnnounceDirectly({
        ...
        agentCallSessionKey: params.agentCallSessionKey,
        triggerMessage: params.triggerMessage,
...
export function buildSubagentSystemPrompt(params: {
  ...
  const lines = [
    ...
    "- NO scheduled tasks or persistent state",
  ...
  if (canSpawn) {
    lines.push(
      ...
      `If a child completion event arrives AFTER you already sent your final answer, reply ONLY with ${SILENT_REPLY_TOKEN}.`,
      ...
      ...(acpEnabled
        ? [
            ...
            '`agents_list` and `subagents` apply to FasedAgent sub-agents (`runtime: "subagent"`); ACP harness ids are controlled by `acp.allowedAgents`.',
            "Do not ask users to run slash commands or CLI when `sessions_spawn` can do it directly.",
            "Do not use `exec` (`fased ...`, `acpx ...`) to spawn ACP sessions.",
            'Use `subagents` only for FasedAgent subagents (`runtime: "subagent"`).',
...
function buildAnnounceReplyInstruction(params: {
  ...
  const currentResultOnly =
    "Use only the current result above; do not carry forward older task errors, warnings, balances, access notes, or conclusions unless they are present in this result.";
  if (params.remainingActiveSubagentRuns > 0) {
    const activeRunsLabel = params.remainingActiveSubagentRuns === 1 ? "run" : "runs";
    return `There are still ${params.remainingActiveSubagentRuns} active subagent ${activeRunsLabel} for this session. If they are part of the same workflow, wait for the remaining results before sending a user update. If they are unrelated, respond normally using only the result above. ${currentResultOnly}`;
  }
  if (params.requesterIsSubagent) {
    return `Convert this completion into a concise internal orchestration update for your parent agent in your own words. ${currentResultOnly} Keep this internal context private (don't mention system/log/stats/session details or announce type). If this result is duplicate or no update is needed, reply ONLY: ${SILENT_REPLY_TOKEN}.`;
  }
  if (params.expectsCompletionMessage) {
    return `A completed ${params.announceType} is ready for user delivery. Convert the result above into your normal assistant voice and send that user-facing update now. ${currentResultOnly} Keep this internal context private (don't mention system/log/stats/session details or announce type).`;
  }
  return `A completed ${params.announceType} is ready for user delivery. Convert the result above into your normal assistant voice and send that user-facing update now. ${currentResultOnly} Keep this internal context private (don't mention system/log/stats/session details or announce type), and do not copy the system message verbatim. Reply ONLY: ${SILENT_REPLY_TOKEN} if this exact result was already delivered to the user in this same turn.`;
}
...
export async function runSubagentAnnounceFlow(params: {
  ...
  const gatewayAuth = resolveInternalGatewayCallAuth(loadConfig());
  ...
  try {
    ...
    const agentCallSessionKey =
      announceType === "cron job" && !requesterIsSubagent
...
export const __testing = {
  setDepsForTest(_overrides?: {
    ...
    // Compatibility hook for older harnesses. The current announce flow uses module dependencies directly.
  },
};
