...
describe("subagent announce formatting", () => {
  ...
  it("summarizes cron job announcements in the task run session while delivering to the requester channel", async () => {
    sessionStore = {
      "agent:main:main": {
        sessionId: "session-cron-requester",
        lastChannel: "telegram",
        lastTo: "123",
        lastAccountId: "bot",
      },
    ...
    const didAnnounce = await runSubagentAnnounceFlow({
      childSessionKey: "agent:main:cron:job-1:run:run-1",
      childRunId: "job-1:run-1",
      requesterSessionKey: "main",
      requesterOrigin: { channel: "telegram", accountId: "bot", to: "123" },
      requesterDisplayKey: "main",
      announceType: "cron job",
      roundOneReply: "Wallet balance: SOL 1",
      ...defaultOutcomeAnnounce,
    });

    expect(didAnnounce).toBe(true);
    const params = await getSingleAgentCallParams();
    expect(params.sessionKey).toBe("agent:main:cron:job-1:run:run-1");
    expect(params.deliver).toBe(true);
    expect(params.channel).toBe("telegram");
    expect(params.to).toBe("123");
    expect(params.accountId).toBe("bot");
  });
