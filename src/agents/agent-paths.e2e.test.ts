...
describe("resolveFasedAgentAgentDir", () => {
  ...
  it("defaults to the multi-agent path when no overrides are set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "fased-agent-"));
    process.env.FASED_STATE_DIR = tempStateDir;
    delete process.env.FASED_AGENT_DIR;
    delete process.env.PI_CODING_AGENT_DIR;

    const resolved = resolveFasedAgentAgentDir();
  ...
  it("honors FASED_AGENT_DIR overrides", async () => {
    ...
    process.env.FASED_AGENT_DIR = override;
    delete process.env.PI_CODING_AGENT_DIR;
