...
describe("buildAgentSystemPrompt", () => {
  ...
  it("includes a CLI quick reference section", () => {
    ...
    expect(prompt).toContain("## FasedAgent CLI Quick Reference");
    expect(prompt).toContain("fased gateway restart");
  ...
  it("preserves tool casing in the prompt", () => {
    ...
    expect(prompt).toContain("FasedAgent docs: /tmp/fased/docs");
    expect(prompt).toContain(
  ...
  it("adds ClaudeBot self-update guidance when gateway tool is available", () => {
    ...
    expect(prompt).toContain("## Fased Agent Self-Update");
    expect(prompt).toContain("Use config.schema to");
    expect(prompt).toContain("config.apply");
    expect(prompt).toContain("update.run");
    expect(prompt).toContain("config.schema, config.apply");
  });
  ...
  it("describes sandboxed runtime and elevated when allowed", () => {
    ...
    expect(prompt).toContain(
      "For read/write/edit/apply_patch, file paths resolve against host workspace: /tmp/fased. For bash/exec commands, use sandbox container paths under /workspace (or relative paths from that workdir), not host paths.",
    );
...
describe("buildSubagentSystemPrompt", () => {
  it("renders depth-1 orchestrator guidance, labels, and recovery notes", () => {
    ...
    expect(prompt).toContain("Do not use `exec` (`fased ...`, `acpx ...`)");
    expect(prompt).toContain("Use `subagents` only for FasedAgent subagents");
