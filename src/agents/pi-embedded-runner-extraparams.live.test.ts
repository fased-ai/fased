import type { Model } from "@mariozechner/pi-ai/compat";
import { getModel, streamSimple } from "@mariozechner/pi-ai/compat";
...
describeLive("pi embedded extra params (live)", () => {
  it("applies config maxTokens to openai streamFn", async () => {
    const model = getModel("openai", "gpt-5.4" as never) as unknown as Model<"openai-completions">;
