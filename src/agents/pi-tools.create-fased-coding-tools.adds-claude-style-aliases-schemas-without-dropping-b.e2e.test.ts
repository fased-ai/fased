...
const defaultTools = createFasedAgentCodingTools({ senderIsOwner: true });
...
describe("createFasedAgentCodingTools", () => {
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const enabledConfig: FasedAgentConfig = {
      tools: {
        exec: {
    ...
    const openAiTools = createFasedAgentCodingTools({
    ...
    const anthropicTools = createFasedAgentCodingTools({
      config: enabledConfig,
    ...
    const allowModelsConfig: FasedAgentConfig = {
      tools: {
        exec: {
    ...
    const allowed = createFasedAgentCodingTools({
    ...
    const denied = createFasedAgentCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
    ...
    const oauthTools = createFasedAgentCodingTools({
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
