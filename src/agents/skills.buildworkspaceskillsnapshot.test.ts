...
const fixtureSuite = createFixtureSuite("fased-skills-snapshot-suite-");
...
describe("buildWorkspaceSkillSnapshot", () => {
  ...
  it("matches agent skill allowlists against stable skill keys", async () => {
    const workspaceDir = await fixtureSuite.createCaseDir("workspace");
    await writeSkill({
    ...
    const snapshot = buildSnapshot(workspaceDir, {
      ...
      config: {
        agents: {
          list: [{ id: "writer", skills: ["market-smoke"] }],
        },
      },
    });

    expect(snapshot.skills.map((skill) => skill.name)).toEqual(["Market smoke"]);
    expect(snapshot.skillFilter).toEqual(["market-smoke"]);
  });
