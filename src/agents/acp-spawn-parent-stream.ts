...
export function startAcpSpawnParentStreamRelay(params: {
  ...
  relayLifetimeTimer = setTimeout(() => {
    ...
    failTaskRunByRunId({
      runId,
      status: "timed_out",
      summary: "ACP stream relay timed out.",
      error: `No completion event before ${Math.max(1, Math.round(maxRelayLifetimeMs / 1000))}s relay limit.`,
    });
    emit(
  ...
  const unsubscribe = onAgentEvent((event) => {
    ...
    if (phase === "end") {
      ...
      completeTaskRunByRunId({
        ...
        summary:
          durationMs != null
            ? `ACP run completed in ${Math.max(1, Math.round(durationMs / 1000))}s.`
            : "ACP run completed.",
        deliveryStatus: shouldSurfaceUpdates ? "delivered" : "not_applicable",
      });
      dispose();
      return;
    }
    ...
    if (phase === "error") {
      ...
      failTaskRunByRunId({
        runId,
        status: "failed",
        summary: "ACP run failed.",
        error: errorText ?? "ACP run failed.",
        deliveryStatus: shouldSurfaceUpdates ? "delivered" : "not_applicable",
      });
      dispose();
    }
