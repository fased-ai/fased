...
function isNativeOpenAiEndpoint(model: Model<Api>, baseUrl: string): boolean {
  return model.provider === "openai" && baseUrl.includes("api.openai.com");
}
...
export function normalizeModelCompat(model: Model<Api>): Model<Api> {
  ...
  const shouldConstrainCompat =
    isZai ||
    isMoonshot ||
    isDashScope ||
    (Boolean(baseUrl) && !isNativeOpenAiEndpoint(model, baseUrl));
  if (!shouldConstrainCompat) {
    return model;
  }
  ...
  const nextCompat = {
    ...
    supportsUsageInStreaming: compat.supportsUsageInStreaming ?? (isDashScope || isMoonshot),
    supportsStrictMode: compat.supportsStrictMode ?? false,
  };

  if (
    model.compat &&
    model.compat.supportsDeveloperRole === nextCompat.supportsDeveloperRole &&
    model.compat.supportsUsageInStreaming === nextCompat.supportsUsageInStreaming &&
    model.compat.supportsStrictMode === nextCompat.supportsStrictMode
  ) {
    return model;
  }
