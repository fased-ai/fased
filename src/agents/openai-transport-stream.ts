...
type ProviderRuntimeModel = Model<Api> & Record<string, unknown>;
type FunctionTool = Record<string, unknown>;
type ResponseInput = Array<Record<string, unknown>>;
type ResponseInputMessageContentList = Array<Record<string, unknown>>;
type ResponseFunctionCallOutputItemList = Array<Record<string, unknown>>;
...
export function resolveAzureOpenAIApiVersion(
  env: { AZURE_OPENAI_API_VERSION?: string } = process.env,
): string {
...
function getServiceTierCostMultiplier(serviceTier: OpenAIServiceTier | undefined) {
  switch (serviceTier) {
    case "flex":
      return 0.5;
    case "priority":
...
function resolveOpenAIEndpoint(baseURL: string | undefined, path: string): string {
  const base = (baseURL?.trim() || "https://api.openai.com/v1").replace(/\/+$/, "");
  return `${base}${path}`;
}

async function* parseServerSentJson(response: Response): AsyncIterable<Record<string, unknown>> {
  if (!response.body) {
  ...
  let buffer = "";
  const emitBlock = function* (block: string): Iterable<Record<string, unknown>> {
    const data = block
      .split(/\r?\n/)
...
function createOpenAIStreamClient(options: OpenAIStreamClientOptions) {
  const request = async (
    path: string,
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `OpenAI transport request failed (${response.status} ${response.statusText})${
  ...
  return {
    responses: {
      create: (payload: Record<string, unknown>, init?: { signal?: AbortSignal }) =>
        request("/responses", payload, init),
    },
    chat: {
      completions: {
        create: (payload: Record<string, unknown>, init?: { signal?: AbortSignal }) =>
          request("/chat/completions", payload, init),
      },
    },
  };
}

async function resolvePayloadOverride(
  ...
  for (const msg of transformedMessages) {
    if (msg.role === "user") {
      if (typeof msg.content === "string") {
        ...
        const content = (
          msg.content.map(
            (item: { type: string; text?: string; mimeType?: string; data?: string }) =>
              item.type === "text"
    ...
    if (type === "response.created") {
      ...
      if (options?.applyServiceTierPricing) {
        options.applyServiceTierPricing(
          output.usage,
          (response?.service_tier as OpenAIServiceTier | undefined) ?? options.serviceTier,
        );
      }
...
export function buildOpenAIResponsesParams(
  ...
  const reasoningEffort =
    options?.reasoningEffort ?? options?.reasoning ?? (model.reasoning ? "high" : undefined);
  if (model.reasoning) {
...
function detectCompat(model: OpenAIModeModel) {
  ...
  const isMistral =
    ...
    (isDefaultRoute && provider === "mistral");
  const isVolcengine =
    provider === "volcengine" || provider === "volcengine-coding" || provider === "volcengine-plan";
  const isBytePlus =
    provider === "byteplus" || provider === "byteplus-coding" || provider === "byteplus-plan";
  const reasoningEffortMap: Record<string, string> = isMistral
    ? {
        minimal: "none",
        low: "none",
        medium: "high",
...
export function buildOpenAICompletionsParams(
  ...
  if (compat.thinkingFormat === "modelark-thinking-type" && model.reasoning && reasoningEffort) {
    ...
    params.reasoning = {
      effort: mapReasoningEffort(reasoningEffort, compat.reasoningEffortMap),
    };
