...
describe("openai transport stream", () => {
  ...
  it("prepares a custom simple-completion api alias when transport overrides are attached", () => {
    ...
    expect(resolveTransportAwareSimpleApi(model.api)).toBe("fased-openai-responses-transport");
    expect(prepared).toMatchObject({
      api: "fased-openai-responses-transport",
      provider: "openai",
      id: "gpt-5.4",
    });
  ...
  it("prepares an Anthropic simple-completion api alias when transport overrides are attached", () => {
    ...
    expect(resolveTransportAwareSimpleApi(model.api)).toBe("fased-anthropic-messages-transport");
    expect(prepared).toMatchObject({
      api: "fased-anthropic-messages-transport",
      provider: "anthropic",
      id: "claude-sonnet-4-6",
    });
    expect(buildTransportAwareSimpleStreamFn(model)).toBeTypeOf("function");
  ...
  it("prepares a Google simple-completion api alias when transport overrides are attached", () => {
    ...
    expect(resolveTransportAwareSimpleApi(model.api)).toBe("fased-google-generative-ai-transport");
    expect(prepared).toMatchObject({
      api: "fased-google-generative-ai-transport",
      provider: "google",
      id: "gemini-3.1-pro-preview",
    });
    expect(buildTransportAwareSimpleStreamFn(model)).toBeTypeOf("function");
    ...
    void streamFn!(
      {
        id: "anthropic/claude-sonnet-4",
        name: "Claude Sonnet 4",
        api: "fased-openai-completions-transport",
        provider: "openrouter",
        baseUrl: "https://proxy.example.com/v1",
        reasoning: true,
        input: ["text"],
        ...
        contextWindow: 200000,
        maxTokens: 8192,
      } as Model<"fased-openai-completions-transport">,
      {
        systemPrompt: "system",
        messages: [],
        tools: [],
  ...
  it("adds native OpenAI turn metadata on direct Responses routes", () => {
    const params = buildOpenAIResponsesParams(
      ...
      {
        fased_session_id: "session-123",
        fased_turn_id: "turn-123",
        fased_turn_attempt: "1",
        fased_transport: "stream",
      },
  ...
  it("uses Mistral compat defaults and reasoning mapping for direct Mistral completions providers", () => {
    const params = buildOpenAICompletionsParams(
      {
  ...
  it("uses Mistral compat defaults and reasoning mapping for custom providers on native Mistral hosts", () => {
    const params = buildOpenAICompletionsParams(
      {
  ...
  it("maps low Mistral reasoning to none because Mistral only accepts high or none", () => {
    const params = buildOpenAICompletionsParams(
      {
  ...
  it("maps Volcano Engine thinking levels to Ark reasoning_effort values", () => {
    const params = buildOpenAICompletionsParams(
      {
  ...
  it("maps BytePlus thinking levels to ModelArk thinking.type", () => {
    const enabledParams = buildOpenAICompletionsParams(
      {
    ...
    expect(enabledParams).toMatchObject({
      thinking: { type: "enabled" },
    });
    expect(enabledParams).not.toHaveProperty("reasoning_effort");

    const disabledParams = buildOpenAICompletionsParams(
      {
        id: "seed-2-0-lite-260228",
        name: "ByteDance Seed 2.0 Lite",
        api: "openai-completions",
    ...
    expect(disabledParams).toMatchObject({
      thinking: { type: "disabled" },
    });
    expect(disabledParams).not.toHaveProperty("reasoning_effort");
  });
});
