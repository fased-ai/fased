import { completeSimple, getModel } from "@mariozechner/pi-ai/compat";
...
describeLive("gemini live switch", () => {
  ...
  for (const modelId of googleModels) {
    it(`handles unsigned tool calls from Gemini CLI history when switching to ${modelId}`, async () => {
      const now = Date.now();
              ...
              content: [
                {
                  ...
                  // No thoughtSignature: simulates persisted third-party CLI history.
                },
              ],
