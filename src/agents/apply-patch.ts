...
async function resolvePatchPath(
  ...
  if (options.sandbox) {
    ...
    const ioPath = resolved.hostPath ?? resolved.containerPath;
    if (!ioPath) {
      throw new Error(`Sandbox bridge did not resolve path: ${filePath}`);
    }
    ...
    return {
      resolved: ioPath,
      display: resolved.relativePath || resolved.hostPath || ioPath,
    };
  }
