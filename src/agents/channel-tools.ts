...
const channelAgentToolMeta = new WeakMap<ChannelAgentTool, { channelId: string }>();

export function getChannelAgentToolMeta(tool: unknown): { channelId: string } | undefined {
  if (!tool || typeof tool !== "object") {
    return undefined;
  }
  return channelAgentToolMeta.get(tool as ChannelAgentTool);
}
...
export function listChannelSupportedActions(params: {
  ...
  const cfg = params.cfg ?? ({} as FasedAgentConfig);
...
export function listChannelAgentTools(params: { cfg?: FasedAgentConfig }): ChannelAgentTool[] {
