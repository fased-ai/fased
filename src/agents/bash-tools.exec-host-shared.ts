...
export function buildHeadlessExecApprovalDeniedMessage(params: {
  ...
  return [
    ...
    "Stricter values from tools.exec and ~/.fased/exec-approvals.json both apply.",
    "Fix one of these:",
    ...
    'Tip: run "fased doctor" and "fased approvals get --gateway" to inspect the effective policy.',
  ].join("\n");
}
