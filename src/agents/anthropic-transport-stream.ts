...
function resolveAnthropicMessagesEndpoint(baseURL: string | undefined): string {
  const base = (baseURL?.trim() || "https://api.anthropic.com").replace(/\/+$/, "");
  return base.endsWith("/v1") ? `${base}/messages` : `${base}/v1/messages`;
}

async function* parseAnthropicServerSentJson(
  response: Response,
...
function createAnthropicMessagesClient(params: {
  ...
  fetch: typeof fetch;
  authKind: "api_key" | "bearer";
}): AnthropicMessagesClient {
  return {
    messages: {
      stream: async (payload, options) => {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            `Anthropic transport request failed (${response.status} ${response.statusText})${
    ...
    if (msg.role === "assistant") {
      ...
      for (const block of msg.content) {
        ...
        if (block.type === "thinking") {
          const thinkingBlock = block as typeof block & { redacted?: boolean };
  ...
  return (rawModel, context, rawOptions) => {
    ...
    void (async () => {
      ...
      try {
        ...
        const onPayload = transportOptions.onPayload as
          | ((payload: unknown, payloadModel: AnthropicTransportModel) => unknown)
          | undefined;
