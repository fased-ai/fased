...
describe("pickSandboxToolPolicy", () => {
  ...
  it("keeps global alsoAllow additive in effective tool policy resolution", () => {
    const cfg: FasedAgentConfig = {
      tools: {
        profile: "coding",
        alsoAllow: ["sample_plugin_tool"],
      },
    };

    const resolved = resolveEffectiveToolPolicy({ config: cfg, agentId: "main" });
    expect(resolved.globalPolicy).toEqual({ allow: ["*", "sample_plugin_tool"], deny: undefined });
    expect(resolved.profileAlsoAllow).toEqual(["sample_plugin_tool"]);
  });
