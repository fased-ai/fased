...
function createDefaultSpawnConfig(): FasedAgentConfig {
  return {
    acp: {
      enabled: true,
      backend: "acpx",
...
function replaceSpawnConfig(next: FasedAgentConfig): void {
...
async function createCrossAgentWorkspaceFixture(options?: {
  ...
  const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "fased-acp-spawn-"));
...
describe("spawnAcpDirect", () => {
  ...
  it("records accepted ACP spawns in the Agent task ledger", async () => {
    const result = await spawnAcpDirect(
      {
        task: "Investigate flaky tests",
        agentId: "codex",
    ...
    expect(hoisted.sessionBindingBindMock).toHaveBeenCalledWith(
      expect.objectContaining({
        metadata: expect.objectContaining({
          introText: expect.stringContaining("cwd: /home/bob/fased-agent"),
        }),
      }),
    );
  });
