...
describe("Ollama provider", () => {
  ...
  it("should use native ollama api type", async () => {
    ...
    try {
      ...
      expect(providers?.ollama?.request?.allowPrivateNetwork).toBe(true);
    } finally {
  ...
  it("does not implicitly allow existing private provider base URLs during normalization", () => {
    const agentDir = mkdtempSync(join(tmpdir(), "fased-test-"));
    ...
    expect(providers?.local?.request?.allowPrivateNetwork).toBeUndefined();
  });
