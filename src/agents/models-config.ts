...
export async function ensureFasedModelsJson(
  config?: FasedAgentConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const cfg = config ?? loadConfig();
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveFasedAgentDir();
...
export async function ensureFasedAgentModelsJson(
  config?: FasedAgentConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  return await ensureFasedModelsJson(config, agentDirOverride);
}
