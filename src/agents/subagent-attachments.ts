...
function resolveAttachmentLimits(config: FasedAgentConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".fased", "attachments");
  const relDir = path.posix.join(".fased", "attachments", attachmentId);
