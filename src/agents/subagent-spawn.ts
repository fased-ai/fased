...
type GatewayCallOptions = Parameters<typeof callGateway>[0];
...
function resolveGatewaySelfAuth(cfg: ReturnType<typeof loadConfig>) {
  const token =
    readStringParam(cfg.gateway?.auth ?? {}, "token") ??
    readStringParam(process.env, "FASED_GATEWAY_TOKEN");
  const password =
    readStringParam(cfg.gateway?.auth ?? {}, "password") ??
    readStringParam(process.env, "FASED_GATEWAY_PASSWORD");
  return {
    ...(token ? { token } : {}),
    ...(password ? { password } : {}),
  };
...
async function callGatewayFromLoadedConfig<T = Record<string, unknown>>(
...
export async function spawnSubagentDirect(
  ...
  const callSelfGateway = <T = Record<string, unknown>>(opts: GatewayCallOptions) =>
    callGatewayFromLoadedConfig<T>(cfg, opts);
  ...
  if (targetAgentId !== requesterAgentId) {
    ...
    const approvedSet = new Set(
      (ctx.approvedTargetAgentIds ?? [])
        .filter((value) => value.trim())
        .map((value) => normalizeAgentId(value).toLowerCase()),
    );
    const approvedForThisRun = approvedSet.has("*") || approvedSet.has(normalizedTargetId);
    if (!allowAny && !allowSet.has(normalizedTargetId) && !approvedForThisRun) {
  ...
  try {
    const response = await callSelfGateway<{ runId: string }>({
      method: "agent",
      params: {
