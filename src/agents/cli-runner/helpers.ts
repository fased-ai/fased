...
function buildResumeSignature(backend: CliBackendConfig, sessionId: string): string | null {
  const resumeArgs = backend.resumeArgs;
  if (!resumeArgs || resumeArgs.length === 0) {
    return null;
  }
  return [backend.command, ...resumeArgs.map((entry) => entry.replaceAll("{sessionId}", sessionId))]
    .join(" ")
    .trim();
}

function isOwnChild(row: ProcessRow): boolean {
  return Number(row.ppid) === process.pid;
}

async function killProcessIds(signal: "-TERM" | "-9", pids: string[]) {
  if (pids.length === 0) {
    return;
  }
  await runExec("kill", [signal, ...pids]);
}

export async function cleanupSuspendedCliProcesses(
  backend: CliBackendConfig,
  minimumResumeArgMatches = 0,
): Promise<void> {
  if (process.platform === "win32") {
    return;
  }
  const hasSessionToken = Boolean(
    backend.sessionArg || backend.sessionArgs?.length || backend.resumeArgs?.length,
  );
  if (!hasSessionToken) {
    return;
  }
  const { stdout } = await runExec("ps", ["-axo", "pid=,ppid=,state=,command="]);
  const resumeSignature = buildResumeSignature(backend, "");
  const pids = parseProcessRows(stdout)
    .filter(isOwnChild)
    .filter((row) => row.state?.includes("T"))
    .filter((row) => {
      if (backend.sessionArg && row.command.includes(`${backend.command} ${backend.sessionArg}`)) {
        return true;
      }
      if (!resumeSignature || !backend.resumeArgs?.length) {
        return false;
      }
      const fixedParts = backend.resumeArgs.filter((entry) => entry !== "{sessionId}");
      const matchedParts = fixedParts.filter((entry) => row.command.includes(entry)).length;
      return row.command.includes(backend.command) && matchedParts >= minimumResumeArgMatches;
    })
    .map((row) => row.pid);
  await killProcessIds("-9", pids);
}
...
export async function cleanupResumeProcesses(
  ...
  const signature = buildResumeSignature(backend, sessionId);
  if (!signature) {
    return;
  }
  ...
  const pids = parseProcessRows(stdout)
    .filter(isOwnChild)
    .filter((row) => row.command.includes(signature))
    .map((row) => row.pid);
  if (pids.length === 0) {
    return;
  }
  await killProcessIds("-TERM", pids);
  await killProcessIds("-9", pids);
}
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "fased-cli-images-"));
  const paths: string[] = [];
