...
export async function resolveSessionAuthProfileOverride(params: {
  ...
  const lockedProfileId = params.lockedProfileId?.trim();
  if (
    lockedProfileId &&
    store.profiles[lockedProfileId] &&
    isProfileForProvider({ provider, profileId: lockedProfileId, store }) &&
    order.includes(lockedProfileId)
  ) {
    if (
      sessionEntry.authProfileOverride !== lockedProfileId ||
      sessionEntry.authProfileOverrideSource !== "user"
    ) {
      sessionEntry.authProfileOverride = lockedProfileId;
      sessionEntry.authProfileOverrideSource = "user";
      sessionEntry.authProfileOverrideCompactionCount = sessionEntry.compactionCount ?? 0;
      sessionEntry.updatedAt = Date.now();
      sessionStore[sessionKey] = sessionEntry;
      if (storePath) {
        await updateSessionStore(storePath, (store) => {
