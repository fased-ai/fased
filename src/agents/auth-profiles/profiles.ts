...
function hasStoredCredentialMaterial(cred: AuthProfileCredential): boolean {
  if (cred.type === "api_key") {
    return Boolean(cred.key?.trim() || cred.keyRef);
  }
  if (cred.type === "token") {
    return Boolean(cred.token?.trim() || cred.tokenRef);
  }
  return Boolean(cred.access?.trim() || cred.refresh?.trim());
}

export function listProvidersWithStoredCredentials(store: AuthProfileStore): string[] {
  const providers = new Set<string>();
  for (const cred of Object.values(store.profiles)) {
    if (!cred.provider?.trim() || !hasStoredCredentialMaterial(cred)) {
      continue;
    }
    providers.add(resolveProviderIdForAuth(cred.provider));
  }
