...
describe("resolveApiKeyForProfile fallback to main agent", () => {
  const envSnapshot = captureEnv(["FASED_STATE_DIR", "FASED_AGENT_DIR", "PI_CODING_AGENT_DIR"]);
  let tmpDir: string;
  ...
  beforeEach(async () => {
    ...
    // Set environment variables so resolveFasedAgentAgentDir() returns mainAgentDir
    process.env.FASED_STATE_DIR = tmpDir;
    process.env.FASED_AGENT_DIR = mainAgentDir;
