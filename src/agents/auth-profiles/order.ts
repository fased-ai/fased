...
export function resolveAuthProfileOrder(params: {
  ...
  const isValidProfile = (profileId: string): boolean => {
    ...
    if (resolveProviderIdForAuth(cred.provider, { config: cfg }) !== providerAuthKey) {
      return false;
    }
    const profileConfig = cfg?.auth?.profiles?.[profileId];
    if (profileConfig) {
      if (resolveProviderIdForAuth(profileConfig.provider, { config: cfg }) !== providerAuthKey) {
        return false;
      }
      if (!isProfileModeCompatible(profileConfig.mode, cred.type)) {
        return false;
      }
    }
...
function isProfileModeCompatible(
  configMode: string,
  credentialType: AuthProfileCredential["type"],
): boolean {
  if (configMode === credentialType) {
    return true;
  }
  if (configMode === "oauth" && credentialType === "token") {
    return true;
  }
  // Older provider setup flows could leave fased.json with mode=oauth while
  // auth-profiles.json holds the actual provider API key. Trust the stored
  // credential when the profile id and provider already match so chat does
  // not lose working auth after provider UI/onboarding migrations.
  if (configMode === "oauth" && credentialType === "api_key") {
    return true;
  }
  return false;
}
