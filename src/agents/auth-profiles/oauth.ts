...
const isProfileConfigModeCompatible = (params: {
  mode: string | undefined;
  type: string | undefined;
  allowStoredApiKeyForStaleOAuthConfig?: boolean;
}): boolean => {
  if (isCompatibleModeType(params.mode, params.type)) {
    return true;
  }
  return (
    params.allowStoredApiKeyForStaleOAuthConfig === true &&
    params.mode === "oauth" &&
    params.type === "api_key"
  );
};
...
function isProfileConfigCompatible(params: {
  ...
  if (
    profileConfig &&
    !isProfileConfigModeCompatible({
      mode: profileConfig.mode,
      type: params.mode,
      allowStoredApiKeyForStaleOAuthConfig: params.allowStoredApiKeyForStaleOAuthConfig,
    })
  ) {
    return false;
  }
...
type SecretDefaults = NonNullable<FasedAgentConfig["secrets"]>["defaults"];
  ...
  return await withFileLock(authPath, AUTH_STORE_LOCK_OPTIONS, async () => {
    ...
    const result =
      String(cred.provider) === "chutes"
        ...
        : String(cred.provider) === "xai"
          ? await (async () => {
              const newCredentials = await refreshXaiOAuthCredential(cred);
              return { apiKey: newCredentials.access, newCredentials };
            })()
          : await (async () => {
...
async function resolveProfileSecretString(params: {
  ...
  configForRefResolution: FasedAgentConfig;
...
export async function resolveApiKeyForProfile(
  ...
  if (
    !isProfileConfigCompatible({
      ...
      // Older provider UI/onboarding states could leave fased.json with mode=oauth while
      // auth-profiles.json contains the working API key.
      allowStoredApiKeyForStaleOAuthConfig: true,
    })
  ) {
    return null;
  }
