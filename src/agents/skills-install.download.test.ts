...
function sri(buffer: Buffer, algorithm = "sha256"): string {
...
beforeAll(async () => {
  workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "fased-skills-install-"));
...
describe("installDownloadSpec extraction safety", () => {
  ...
  it("verifies declared download sha256 and integrity metadata before extraction", async () => {
    const entry = buildEntry("zip-integrity");
    const targetDir = path.join(resolveSkillToolsRootDir(entry), "target");
    mockArchiveResponse(new Uint8Array(STRIP_COMPONENTS_ZIP_BUFFER));
    ...
    const result = await installDownloadSkill({
      ...
      stripComponents: 1,
      targetDir,
      sha256: createHash("sha256").update(STRIP_COMPONENTS_ZIP_BUFFER).digest("hex"),
      integrity: sri(STRIP_COMPONENTS_ZIP_BUFFER),
    });
  ...
  it("rejects downloads when declared sha256 does not match", async () => {
    const entry = buildEntry("zip-integrity-mismatch");
    const targetDir = path.join(resolveSkillToolsRootDir(entry), "target");
    mockArchiveResponse(new Uint8Array(STRIP_COMPONENTS_ZIP_BUFFER));

    const result = await installDownloadSkill({
      name: "zip-integrity-mismatch",
      url: "https://example.invalid/good.zip",
      archive: "zip",
      stripComponents: 1,
      targetDir,
    ...
    expect(result.message).toContain("download sha256 mismatch");
