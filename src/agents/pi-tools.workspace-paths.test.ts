...
describe("workspace path resolution", () => {
  it("resolves relative read/write/edit paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("fased-ws-", async (workspaceDir) => {
      await withTempDir("fased-cwd-", async (otherDir) => {
        const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(otherDir);
        try {
          const tools = createFasedAgentCodingTools({ workspaceDir });
          ...
          expect(await fs.readFile(path.join(workspaceDir, editFile), "utf8")).toBe("hello fased");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("fased-ws-", async (workspaceDir) => {
      await withTempDir("fased-override-", async (overrideDir) => {
        const tools = createFasedAgentCodingTools({
          workspaceDir,
          exec: { host: "gateway", ask: "off", security: "full" },
        });
  ...
  it("rejects @-prefixed absolute paths outside workspace when workspaceOnly is enabled", async () => {
    await withTempDir("fased-ws-", async (workspaceDir) => {
      const cfg: FasedAgentConfig = { tools: { fs: { workspaceOnly: true } } };
      const tools = createFasedAgentCodingTools({ workspaceDir, config: cfg });
      const { readTool } = expectReadWriteEditTools(tools);

      const outsideAbsolute = path.resolve(path.parse(workspaceDir).root, "outside-fased.txt");
      await expect(
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("fased-sandbox-", async (sandboxDir) => {
      await withTempDir("fased-workspace-", async (workspaceDir) => {
        ...
        const tools = createFasedAgentCodingTools({ workspaceDir, sandbox });
        const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
