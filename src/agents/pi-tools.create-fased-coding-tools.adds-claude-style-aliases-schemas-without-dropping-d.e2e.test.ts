...
describe("createFasedAgentCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "fased-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    ...
    try {
      ...
      const contents = "Hello from fased read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandboxDir = path.join(os.tmpdir(), "fased-sandbox");
    const sandbox = {
      enabled: true,
      sessionKey: "sandbox:test",
      workspaceDir: sandboxDir,
      agentWorkspaceDir: path.join(os.tmpdir(), "fased-workspace"),
      workspaceAccess: "none" as const,
      containerName: "fased-sbx-test",
    ...
    const tools = createFasedAgentCodingTools({ sandbox });
