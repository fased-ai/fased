...
} from "./fased-tools.subagents.sessions-spawn.test-harness.js";
...
const fastModeEnv = vi.hoisted(() => {
  const previous = process.env.FASED_TEST_FAST;
...
describe("fased-tools: subagents (sessions_spawn lifecycle)", () => {
  beforeEach(() => {
    resetSessionsSpawnAnnounceFlowOverride();
    resetSessionsSpawnHookRunnerOverride();
    resetSessionsSpawnConfigOverride();
  ...
  afterAll(() => {
    ...
    process.env.FASED_TEST_FAST = fastModeEnv.previous;
  });
