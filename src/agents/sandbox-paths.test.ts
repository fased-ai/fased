...
async function withOutsideHardlinkInFasedAgentTmp<T>(
  params: {
    fasedTmpDir: string;
  ...
  const hardlinkPath = path.join(params.fasedTmpDir, makeTmpProbePath(params.hardlinkPrefix));
  const symlinkPath = params.symlinkPrefix
    ? path.join(params.fasedTmpDir, makeTmpProbePath(params.symlinkPrefix))
    : undefined;
  try {
    if (isPathInside(params.fasedTmpDir, outsideFile)) {
      return;
    }
    await fs.writeFile(outsideFile, "secret", "utf8");
    await fs.mkdir(params.fasedTmpDir, { recursive: true });
    try {
...
describe("resolveSandboxedMediaSource", () => {
  const fasedTmpDir = resolvePreferredFasedAgentTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred FasedAgent tmp root",
      media: path.join(fasedTmpDir, "image.png"),
      expected: path.join(fasedTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred FasedAgent tmp root",
      media: pathToFileURL(path.join(fasedTmpDir, "photo.png")).href,
      expected: path.join(fasedTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred FasedAgent tmp root",
      media: path.join(fasedTmpDir, "subdir", "deep", "file.png"),
      expected: path.join(fasedTmpDir, "subdir", "deep", "file.png"),
    },
  ...
  it.each([
    ...
    },
    {
      ...
      media: path.join(fasedTmpDir, "..", "etc", "passwd"),
      expected: /sandbox/i,
    },
    {
      name: "absolute paths under host tmp outside fased tmp root",
      media: path.join(os.tmpdir(), "outside-fased", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked FasedAgent tmp paths escaping tmp root", async () => {
    if (process.platform === "win32") {
      return;
    }
    const outsideTmpTarget = path.resolve(process.cwd(), "package.json");
    if (isPathInside(fasedTmpDir, outsideTmpTarget)) {
      return;
    }

    await withSandboxRoot(async (sandboxDir) => {
      await fs.access(outsideTmpTarget);
      await fs.mkdir(fasedTmpDir, { recursive: true });
      const symlinkPath = path.join(fasedTmpDir, `tmp-link-escape-${process.pid}`);
  ...
  it("rejects hardlinked FasedAgent tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInFasedAgentTmp(
      {
        fasedTmpDir,
  ...
  it("rejects symlinked FasedAgent tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInFasedAgentTmp(
