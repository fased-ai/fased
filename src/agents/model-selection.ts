...
export type ThinkLevel = "off" | "minimal" | "low" | "medium" | "high" | "xhigh" | "max" | "ultra";
...
const OPENAI_SIGN_IN_MODEL_PREFIXES = ["gpt-5.4-codex", "gpt-5.3-codex"] as const;
...
export function isCliProvider(provider: string, cfg?: FasedAgentConfig): boolean {
...
function shouldUseOpenAICodexProvider(provider: string, model: string): boolean {
  ...
  return OPENAI_SIGN_IN_MODEL_PREFIXES.some(
...
function buildDefaultModelKeys(params: {
  defaultProvider: string;
  defaultModel?: string;
}): Set<string> {
  const keys = new Set<string>();
  const defaultModel = params.defaultModel?.trim();
  if (!defaultModel || !params.defaultProvider) {
    return keys;
  }
  const direct = normalizeModelRef(params.defaultProvider, defaultModel);
  keys.add(modelKey(direct.provider, direct.model));

  // Back-compat: some older call sites passed a full provider/model string as
  // defaultModel. Keep accepting it, while the direct key above correctly
  // handles provider model IDs that contain slashes, such as OpenRouter
  // "z-ai/glm-5.1" under provider "openrouter".
  const parsed = parseModelRef(defaultModel, params.defaultProvider);
  if (parsed) {
...
export function resolvePersistedModelRef(params: {
  ...
  if (runtimeModel) {
    ...
    if (runtimeProvider) {
      return normalizeModelRef(runtimeProvider, runtimeModel);
    }
    return (
      parseModelRef(runtimeModel, params.defaultProvider) ?? {
  ...
  if (overrideProvider) {
    return normalizeModelRef(overrideProvider, overrideModel);
  }
  return (
    parseModelRef(overrideModel, params.defaultProvider) ?? {
      provider: normalizeProviderId(params.defaultProvider),
      model: overrideModel,
    }
  );
...
export function buildAllowedModelSet(params: {
  ...
  const rawAllowlist = (() => {
    ...
    const addModelFallbacks = (model: Parameters<typeof resolveAgentModelPrimaryValue>[0]) => {
      for (const fallback of resolveAgentModelFallbackValues(model)) {
        addRef(fallback);
      }
    };
    const addTaskModels = (taskModels: object | undefined) => {
      for (const model of Object.values(taskModels ?? {})) {
        addRef(model);
      }
    };
    const modelMap = params.cfg.agents?.defaults?.models ?? {};
    refs.push(...Object.keys(modelMap));
    addModelFallbacks(params.cfg.agents?.defaults?.model);
    addTaskModels(params.cfg.agents?.defaults?.taskModels);
    for (const agent of params.cfg.agents?.list ?? []) {
      addModelFallbacks(agent.model);
      addTaskModels(agent.taskModels);
      for (const providerConfig of Object.values(agent.modelProviders ?? {})) {
        addRef(providerConfig.primary);
        for (const fallback of providerConfig.fallbacks ?? []) {
          addRef(fallback);
        }
        addTaskModels(providerConfig.taskModels);
      }
    }
    return refs;
  })();
  const allowAny = rawAllowlist.length === 0;
  const defaultKeys = buildDefaultModelKeys({
    defaultProvider: params.defaultProvider,
