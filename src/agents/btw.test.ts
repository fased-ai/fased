...
const ensureFasedAgentModelsJsonMock = vi.fn();
...
vi.mock("./models-config.js", () => ({
  ensureFasedModelsJson: (...args: unknown[]) => ensureFasedAgentModelsJsonMock(...args),
}));
...
describe("runBtwSideQuestion", () => {
  beforeEach(() => {
    ...
    ensureFasedAgentModelsJsonMock.mockReset();
  ...
  it("streams blocks without persisting BTW data to disk", async () => {
    ...
    expect(onBlockReply).toHaveBeenCalledWith({
      text: "Side answer.",
      channelData: { btw: { question: DEFAULT_QUESTION } },
    });
  });
  ...
  it("forces provider reasoning off even when the session think level is set", async () => {
    ...
    const result = await runSideQuestion({ resolvedThinkLevel: "high" });
