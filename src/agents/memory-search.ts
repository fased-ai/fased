...
import type { FasedAgentConfig, MemorySearchConfig } from "../config/config.js";
...
const DEFAULT_MULTIMODAL_MAX_FILE_BYTES = 10 * 1024 * 1024;
const DEFAULT_SOURCES: Array<"memory" | "sessions"> = ["memory"];

function normalizeProviderId(
  provider: MemorySearchConfig["provider"] | undefined,
): ResolvedMemorySearchConfig["provider"] {
  return provider ?? "auto";
}

function resolveDefaultModel(provider: ResolvedMemorySearchConfig["provider"]): string {
  const registeredDefault = getMemoryEmbeddingProvider(provider)?.defaultModel;
...
function normalizeMultimodal(params: {
  defaults: MemorySearchConfig | undefined;
  overrides: MemorySearchConfig | undefined;
  provider: ResolvedMemorySearchConfig["provider"];
  model: string;
  fallback: ResolvedMemorySearchConfig["fallback"];
}): ResolvedMemorySearchConfig["multimodal"] {
  const config = params.overrides?.multimodal ?? params.defaults?.multimodal;
  const rawModalities = config?.modalities ?? [];
  const modalities = new Set<"image" | "audio">();
  ...
  const maxFileBytes = clampInt(
    config?.maxFileBytes ?? DEFAULT_MULTIMODAL_MAX_FILE_BYTES,
    1,
  ...
  if (!multimodal.enabled || multimodal.modalities.length === 0) {
    return multimodal;
  }
  if (params.fallback !== "none") {
    throw new Error("memorySearch.multimodal does not support memorySearch.fallback");
  }
  const provider = getMemoryEmbeddingProvider(params.provider);
  const supported =
    provider?.supportsMultimodalEmbeddings?.({ model: params.model }) ??
...
function mergeConfig(
  ...
  const provider = normalizeProviderId(overrides?.provider ?? defaults?.provider);
  ...
  const model = overrides?.model ?? defaults?.model ?? resolveDefaultModel(provider);
  const multimodal = normalizeMultimodal({ defaults, overrides, provider, model, fallback });
  const local = {
  ...
  return {
    ...
    sync: {
      ...
      sessions: {
        deltaBytes,
        deltaMessages,
        postCompactionForce: Boolean(sync.sessions.postCompactionForce),
      },
    },
    query: {
      ...query,
