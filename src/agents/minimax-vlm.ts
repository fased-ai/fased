...
export function isMinimaxVlmModel(provider: string, model: string): boolean {
  const normalizedProvider = provider.trim().toLowerCase();
  const normalizedModel = model.trim().toLowerCase();
  return (
    (normalizedProvider === "minimax" || normalizedProvider === "minimax-portal") &&
    normalizedModel === "minimax-vl-01"
  );
}
...
export async function minimaxUnderstandImage(params: {
  ...
  const apiKey = normalizeSecretInput(params.apiKey).replace(/[^\x20-\x7E]/g, "");
  if (!apiKey) {
