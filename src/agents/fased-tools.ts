...
import { createDiffViewTool } from "./tools/diff-view-tool.js";
...
export function createFasedAgentTools(options?: {
  ...
  /** Skill identity when a skill command dispatch invokes tools directly. */
  requesterSkillId?: string | null;
  /** Trusted source hint used by wallet tool allowSources policy. */
  ...
  const walletTool = createWalletTool({
    ...
    agentSessionKey: options?.agentSessionKey,
    requesterSkillId: options?.requesterSkillId,
    requestSource: options?.requestSource,
  });
  const walletActionTool = createWalletActionTool({
    config: options?.config,
    requesterAgentIdOverride: options?.requesterAgentIdOverride,
    agentSessionKey: options?.agentSessionKey,
    requesterSkillId: options?.requesterSkillId,
