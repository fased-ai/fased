...
interface WsSession {
  ...
  /** Stable signature for request/URL/header options used to create the manager. */
...
type ProviderRuntimeModel = Parameters<StreamFn>[0] & Record<string, unknown>;
...
function resolveNativeOpenAITransportTurnState(params: {
  ...
  return {
    headers: {
      ...sessionHeaders,
      "x-fased-turn-id": turnId,
      "x-fased-turn-attempt": attempt,
    },
    metadata: {
      fased_session_id: sessionHeaders["x-fased-session-id"] ?? "",
      fased_turn_id: turnId,
      fased_turn_attempt: attempt,
      fased_transport: params.transport,
    },
  };
}
    ...
    const run = async () => {
      ...
      while (true) {
        ...
        const onPayload = options?.onPayload as
          | ((payload: unknown, payloadModel: Parameters<StreamFn>[0]) => unknown)
          | undefined;
        const nextPayload = await Promise.resolve(onPayload?.(payload, model));
        const resolvedPayload =
          nextPayload && typeof nextPayload === "object" && !Array.isArray(nextPayload)
            ? (nextPayload as Record<string, unknown>)
            : payload;
        payload = mergeTransportMetadata(resolvedPayload, turnState?.metadata);
...
async function fallbackToHttp(
  ...
  const baseOnPayload = streamOptions?.onPayload as
    | ((payload: unknown, payloadModel: Parameters<StreamFn>[0]) => unknown)
    | undefined;
  const mergedOptions = {
    ...streamOptions,
    ...
    ...(fallbackOptions?.turnState?.metadata
      ? {
          onPayload: async (payload: unknown, payloadModel: Parameters<StreamFn>[0]) => {
            const nextPayload = await Promise.resolve(baseOnPayload?.(payload, payloadModel));
  ...
  const httpStream = await httpStreamFn(model, context, mergedOptions as Parameters<StreamFn>[2]);
