...
const CLEANUP_STATE_KEY = Symbol.for("fased.sessionWriteLockCleanupState");
const HELD_LOCKS_KEY = Symbol.for("fased.sessionWriteLockHeldLocks");
const WATCHDOG_STATE_KEY = Symbol.for("fased.sessionWriteLockWatchdogState");
...
export function resetSessionWriteLockStateForTest(): void {
  releaseAllLocksSync();
  const cleanupState = resolveCleanupState();
  for (const [signal, handler] of cleanupState.cleanupHandlers.entries()) {
    process.off(signal, handler);
  }
  ...
  watchdogState.intervalMs = DEFAULT_WATCHDOG_INTERVAL_MS;
}
