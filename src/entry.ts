...
const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "fased.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "fased.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  function ensureExperimentalWarningSuppressed(): boolean {
    ...
    if (isTruthyEnvValue(process.env.FASED_NO_RESPAWN)) {
      return false;
    }
    if (isTruthyEnvValue(process.env.FASED_NODE_OPTIONS_READY)) {
      return false;
    }
    ...
    process.env.FASED_NODE_OPTIONS_READY = "1";
    ...
    child.once("error", (error) => {
      console.error("[fased] Failed to respawn CLI:", formatUncaughtError(error));
      process.exit(1);
    });
  ...
  if (!ensureExperimentalWarningSuppressed()) {
    ...
    if (!parsed.ok) {
      ...
      console.error(`[fased] ${parsed.error}`);
      process.exit(2);
    }
