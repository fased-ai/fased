...
export function createToolFactoryHarness(cfg: FasedAgentPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<FasedAgentPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
  ...
  return {
    api: api as FasedAgentPluginApi,
    resolveTool,
  };
}
