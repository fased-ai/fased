...
import { createDedupeCache, createPersistentDedupe } from "fased/plugin-sdk";
...
function resolveStateDirFromEnv(env: NodeJS.ProcessEnv = process.env): string {
  ...
  if (env.VITEST || env.NODE_ENV === "test") {
    return path.join(os.tmpdir(), ["fased-vitest", String(process.pid)].join("-"));
  }
  return path.join(os.homedir(), ".fased");
}
