...
import type { FasedAgentPluginApi, FasedAgentPluginService } from "fased/plugin-sdk";
...
function normalizeDenyList(cfg: FasedAgentPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: FasedAgentPluginApi["config"]): string[] {
...
export default function register(api: FasedAgentPluginApi) {
  ...
  const timerService: FasedAgentPluginService = {
