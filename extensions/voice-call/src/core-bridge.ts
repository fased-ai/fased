...
function resolveFasedAgentRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.FASED_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set FASED_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveFasedAgentRoot(), "dist", "extensionAPI.js");
