...
export function verifySatVNextRuntimeActivationRecords(
  ...
  for (const [roleIndex, role] of roles.entries()) {
    ...
    if (
      ...
      throw new Error(`SAT vNext ${role} program binding does not match SAT-DEP-0012`);
    }
    ...
    if (
      ...
      throw new Error(`SAT vNext ${role} ProgramData does not match SAT-DEP-0012`);
    }
...
export async function inspectSatVNextRuntimeActivation(
  ...
  if (programId.toBase58() !== SAT_VNEXT_ACTIVATION.programs.mining.programId) {
    throw new Error("SAT vNext activation mining program does not match SAT-DEP-0012");
  }
  const roles = ["mining", "mint", "bond"] as const;
  const addresses = [
