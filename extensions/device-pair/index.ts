...
function resolveGatewayPort(cfg: FasedAgentPluginApi["config"]): number {
  const envPort = parsePositiveInteger(process.env.FASED_GATEWAY_PORT?.trim());
  if (envPort) {
    return envPort;
  }
...
function resolveScheme(
  cfg: FasedAgentPluginApi["config"],
...
function resolveAuth(cfg: FasedAgentPluginApi["config"]): ResolveAuthResult {
  const mode = cfg.gateway?.auth?.mode;
  const token =
    pickFirstDefined([process.env.FASED_GATEWAY_TOKEN, cfg.gateway?.auth?.token]) ?? undefined;
  const password =
    pickFirstDefined([process.env.FASED_GATEWAY_PASSWORD, cfg.gateway?.auth?.password]) ??
    undefined;
...
async function resolveGatewayUrl(api: FasedAgentPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the FasedAgent iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the FasedAgent iOS app:",
            "",
            "```",
            qrAscii,
            "```",
