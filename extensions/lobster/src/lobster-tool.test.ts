...
function fakeApi(overrides: Partial<FasedAgentPluginApi> = {}): FasedAgentPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(
  overrides: Partial<FasedAgentPluginToolContext> = {},
): FasedAgentPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "fased-lobster-plugin-"));
  });
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: FasedAgentPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
