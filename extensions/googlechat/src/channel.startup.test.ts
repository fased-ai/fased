...
describe("googlechatPlugin gateway.startAccount", () => {
  ...
  it("requires credentials, webhook, and audience before marking an account configured", async () => {
    const baseAccount: ResolvedGoogleChatAccount = {
      accountId: "default",
      enabled: true,
      credentialSource: "file",
      credentialsFile: "/run/secrets/google-chat-service-account.json",
      config: {},
    };

    const withWebhook: ResolvedGoogleChatAccount = {
      ...baseAccount,
      config: {
    ...
    const complete: ResolvedGoogleChatAccount = {
      ...baseAccount,
      config: {
        webhookPath: "/googlechat",
    ...
    const config = googlechatPlugin.config!;
    const cfg = {} as FasedAgentConfig;
    expect(await config.isConfigured!(baseAccount, cfg)).toBe(false);
    expect(await config.isConfigured!(withWebhook, cfg)).toBe(false);
    expect(await config.isConfigured!(complete, cfg)).toBe(true);
    expect(config.describeAccount!(complete, cfg).configured).toBe(true);
    const snapshot = await googlechatPlugin.status!.buildAccountSnapshot!({
      cfg: {} as FasedAgentConfig,
      account: complete,
    });
