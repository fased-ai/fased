...
import { GoogleChatConfigSchema } from "fased/plugin-sdk";
import {
...
const isGoogleChatAccountConfigured = (account: ResolvedGoogleChatAccount): boolean =>
  account.credentialSource !== "none" &&
  Boolean(account.config.webhookPath?.trim() || account.config.webhookUrl?.trim()) &&
  Boolean(
...
export const googlechatPlugin: ChannelPlugin<ResolvedGoogleChatAccount> = {
  ...
  config: {
    ...
    isConfigured: (account) => isGoogleChatAccountConfigured(account),
