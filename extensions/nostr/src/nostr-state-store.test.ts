...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.FASED_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "fased-nostr-"));
  process.env.FASED_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const stateEnv = env ?? process.env;
        const override = stateEnv.FASED_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        const resolveHome = homedir ?? os.homedir;
        return path.join(resolveHome(), ".fased");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.FASED_STATE_DIR = previous;
    }
