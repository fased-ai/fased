...
describe("nostrPlugin", () => {
  ...
  describe("onboarding", () => {
    it("is attached to the channel plugin", () => {
      expect(nostrPlugin.onboarding?.channel).toBe("nostr");
      expect(nostrPlugin.onboarding?.uiSetup?.fields).toEqual(
        expect.arrayContaining([
          expect.objectContaining({ label: "Private key" }),
          expect.objectContaining({ label: "Relays" }),
        ]),
      );
    });
  });
