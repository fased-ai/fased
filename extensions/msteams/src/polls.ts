...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["fasedPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["fased", "pollId"]) ??
    readNestedString(value, ["fased", "poll", "id"]) ??
    readNestedString(value, ["data", "fasedPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "fased", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        msteams: {
          type: "messageBack",
          text: "fased poll vote",
          displayText: "Vote recorded",
          value: { fasedPollId: pollId, pollId },
        },
      },
    ],
  };
