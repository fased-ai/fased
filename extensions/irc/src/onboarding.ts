...
export const ircOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  uiSetup: {
    ...
    notes: [
      ...
      "Optional NickServ settings can be set after the base account works.",
    ],
    fields: [
      {
        label: "Server host",
      ...
      {
        label: "Username",
        path: ["channels", "irc", "username"],
        placeholder: "fased",
      },
      {
        label: "Real name",
        path: ["channels", "irc", "realname"],
        placeholder: "FasedAgent",
      },
      {
  ...
  configure: async ({
    ...
    if (useEnv) {
      ...
      const username = String(
        await prompter.text({
          ...
          initialValue: resolved.config.username || nick || "fased",
          validate: (value) => (String(value ?? "").trim() ? undefined : "Required"),
        }),
      ).trim();
      ...
      const realname = String(
        await prompter.text({
          ...
          initialValue: resolved.config.realname || "FasedAgent",
          validate: (value) => (String(value ?? "").trim() ? undefined : "Required"),
        }),
      ).trim();
    ...
    await prompter.note(
      [
        ...
        "Command: fased channels status --probe",
        `Docs: ${formatDocsLink("/channels/irc", "channels/irc")}`,
      ].join("\n"),
      "IRC next steps",
    );
