...
describe("irc onboarding", () => {
  it("exposes UI setup fields and channel access controls", () => {
    expect(ircOnboardingAdapter.uiSetup).toMatchObject({
      title: "IRC",
      detail: "Server and nick.",
      fields: expect.arrayContaining([
        expect.objectContaining({ label: "Server host", path: ["channels", "irc", "host"] }),
        expect.objectContaining({ label: "Nick", path: ["channels", "irc", "nick"] }),
        expect.objectContaining({ label: "Channels", path: ["channels", "irc", "channels"] }),
      ]),
      access: expect.objectContaining({ kind: "irc-channels", label: "IRC channels" }),
    });
  });
  ...
  it("configures host and nick via onboarding prompts", async () => {
    ...
    expect(result.cfg.channels?.irc?.nick).toBe("fased-bot");
    expect(result.cfg.channels?.irc?.tls).toBe(true);
    expect(result.cfg.channels?.irc?.channels).toEqual(["#fased", "#ops"]);
    expect(result.cfg.channels?.irc?.groupPolicy).toBe("allowlist");
    expect(Object.keys(result.cfg.channels?.irc?.groups ?? {})).toEqual(["#fased", "#ops"]);
  });
