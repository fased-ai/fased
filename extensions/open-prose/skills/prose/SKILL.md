...
metadata: { "fased": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
## FasedAgent Runtime Mapping

- **Task tool** in the upstream spec == FasedAgent `sessions_spawn`
- **File I/O** == FasedAgent `read`/`write`
- **Remote fetch** == FasedAgent `web_fetch` (or `exec` with curl when POST is required)
