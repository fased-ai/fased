...
FasedAgent extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
fased message send --channel zalouser --target <threadId> --message "Hello from FasedAgent!"
```
...
```bash
fased channels login --channel zalouser              # Login via QR
fased channels login --channel zalouser --account work
...
```bash
# Text
fased message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
fased message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `fased channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
fased channels login --channel zalouser --account work
fased message send --channel zalouser --account work --target <id> --message "Hello"
