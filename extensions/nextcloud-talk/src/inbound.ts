...
export async function handleNextcloudTalkInbound(params: {
  ...
  const defaultGroupPolicy = resolveDefaultGroupPolicy(config as FasedAgentConfig);
  ...
  const allowTextCommands = core.channel.commands.shouldHandleTextCommands({
    cfg: config as FasedAgentConfig,
    surface: CHANNEL_ID,
  });
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as FasedAgentConfig);
