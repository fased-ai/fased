...
vi.mock("fased/plugin-sdk", async () => {
  const actual = await vi.importActual<typeof import("fased/plugin-sdk")>("fased/plugin-sdk");
  return {
    ...actual,
...
import { emitDiagnosticEvent } from "fased/plugin-sdk";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/fased-diagnostics-otel-test";
...
function createTraceOnlyContext(endpoint: string): FasedAgentPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("fased.webhook.received")?.add).toHaveBeenCalled();
    expect(telemetryState.histograms.get("fased.webhook.duration_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("fased.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("fased.message.processed")?.add).toHaveBeenCalled();
    expect(telemetryState.histograms.get("fased.message.duration_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.histograms.get("fased.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("fased.session.stuck")?.add).toHaveBeenCalled();
    expect(telemetryState.histograms.get("fased.session.stuck_age_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("fased.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("fased.webhook.processed");
    expect(spanNames).toContain("fased.message.processed");
    expect(spanNames).toContain("fased.session.stuck");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["fased.token"];
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("fased.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "fased.reason": expect.stringContaining("…"),
      }),
    );
    const attrs = sessionCounter?.add.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(typeof attrs?.["fased.reason"]).toBe("string");
    expect(String(attrs?.["fased.reason"])).not.toContain("ghp_abcdefghijklmnopqrstuvwxyz123456");
    await service.stop?.(ctx);
  });
});
