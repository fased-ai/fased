...
import { createAcpxMcpStatusServer } from "./mcp-status-server.js";
...
const bridgeEnv = () => ({
  enabled: process.env.FASED_ACPX_MCP_BRIDGE_ENABLED,
  mode: process.env.FASED_ACPX_MCP_BRIDGE_MODE,
  url: process.env.FASED_ACPX_MCP_BRIDGE_URL,
  token: process.env.FASED_ACPX_MCP_BRIDGE_TOKEN,
  tools: process.env.FASED_ACPX_MCP_BRIDGE_TOOLS,
});
const writeLog = (entry) => {
  if (!logPath) return;
  fs.appendFileSync(logPath, JSON.stringify({ ...entry, bridgeEnv: bridgeEnv() }) + "\n");
};
...
async function createMockRuntime(params?: {
  ...
  mcpStatusEndpoint?: {
    ...
    token: string;
    toolNames: string[];
    mode?: ResolvedAcpxPluginConfig["mcpBridge"]["mode"];
  };
}): Promise<{
  runtime: AcpxRuntime;
  logPath: string;
  ...
  const dir = await mkdtemp(path.join(os.tmpdir(), "fased-acpx-runtime-test-"));
  tempDirs.push(dir);
  ...
  const config: ResolvedAcpxPluginConfig = {
    ...
    mcpBridge: {
      enabled: Boolean(params?.mcpStatusEndpoint),
      mode: params?.mcpStatusEndpoint?.mode ?? "status-only",
      allowTools: [],
      denyTools: [],
    },
  };
...
describe("AcpxRuntime", () => {
  ...
  it("passes only Fased-owned MCP status endpoint metadata through environment", async () => {
    const { runtime, logPath } = await createMockRuntime({
      mcpStatusEndpoint: {
    ...
    const handle = await runtime.ensureSession({
      sessionKey: "agent:codex:acp:mcp-env",
      agent: "codex",
      mode: "persistent",
    });
    ...
    const ensure = logs.find((entry) => entry.kind === "ensure");
    const prompt = logs.find((entry) => entry.kind === "prompt");
    expect(ensure?.bridgeEnv).toEqual({
      enabled: "1",
      mode: "status-only",
      url: "http://127.0.0.1:45678/mcp",
      token: "secret-test-token",
      tools: "fased_tools_effective",
    });
    expect(prompt?.bridgeEnv).toEqual(ensure?.bridgeEnv);
    expect(((prompt?.args as string[]) ?? []).join(" ")).not.toContain("secret-test-token");
  });

  it("passes read-only-tools bridge mode through environment when explicitly configured", async () => {
    const { runtime, logPath } = await createMockRuntime({
      mcpStatusEndpoint: {
    ...
    const handle = await runtime.ensureSession({
      sessionKey: "agent:codex:acp:mcp-readonly-env",
      agent: "codex",
      mode: "persistent",
    });
    ...
    for await (const _event of runtime.runTurn({
      handle,
      text: "mcp-readonly-env",
      mode: "prompt",
      requestId: "req-mcp-readonly-env",
    })) {
      // drain
    }
    ...
    const logs = await readLogEntries(logPath);
    const prompt = logs.find((entry) => entry.kind === "prompt");
    expect(prompt?.bridgeEnv).toEqual({
      enabled: "1",
      mode: "read-only-tools",
      url: "http://127.0.0.1:45680/mcp",
      token: "read-only-token",
    ...
    expect(((prompt?.args as string[]) ?? []).join(" ")).not.toContain("read-only-token");
  });

  it("consumes the Fased-owned MCP status endpoint as a read-only ACPX prompt preview", async () => {
    const endpoint = {
    ...
    const { runtime, logPath } = await createMockRuntime({
      mcpStatusEndpoint: endpoint,
    });

    try {
      const handle = await runtime.ensureSession({
        sessionKey: "agent:codex:acp:mcp-adapter",
        agent: "codex",
        mode: "persistent",
      });
      ...
      for await (const _event of runtime.runTurn({
        handle,
        text: "adapter-hook-user-prompt",
        mode: "prompt",
        requestId: "req-mcp-adapter",
      })) {
        // drain
      }
      ...
      const logs = await readLogEntries(logPath);
      const prompt = logs.find((entry) => entry.kind === "prompt");
      const stdinText = String(prompt?.stdinText ?? "");
      expect(stdinText).toContain("Fased MCP status preview:");
      expect(stdinText).toContain("mode: status-only");
      expect(stdinText).toContain("ACPX cannot call these tools through this bridge");
      expect(stdinText).toContain("session_status");
      expect(stdinText).toContain("adapter-hook-user-prompt");
      expect(stdinText).not.toContain(endpoint.token);
      expect(stdinText).not.toContain("rawDescription");
      expect(((prompt?.args as string[]) ?? []).join(" ")).not.toContain(endpoint.token);
      ...
      const request = JSON.parse(String((fetchSpy.mock.calls[0]?.[1] as { body?: unknown })?.body));
      expect(request).toMatchObject({
        method: "tools/call",
        params: {
          name: "fased_tools_effective",
          arguments: {
            agentId: "codex",
          },
        },
  ...
  it("validates the real loopback MCP status endpoint during an ACPX runtime turn", async () => {
    const context: FasedAgentPluginServiceContext = {
      config: {},
      workspaceDir: "/tmp/workspace",
      stateDir: "/tmp/state",
      logger: NOOP_LOGGER,
    };
    const statusServer = createAcpxMcpStatusServer({
      bridgeConfig: {
        enabled: true,
        mode: "status-only",
        allowTools: [],
      ...
      context,
      effectiveToolsPreviewResolver: ({ agentId }) => ({
    ...
    const endpoint = await statusServer.startEndpoint();
    const { runtime, logPath } = await createMockRuntime({
      mcpStatusEndpoint: endpoint,
    });
    ...
    try {
      const handle = await runtime.ensureSession({
        sessionKey: "agent:codex:acp:mcp-real-endpoint",
        agent: "codex",
        mode: "persistent",
      });
      ...
      for await (const _event of runtime.runTurn({
        handle,
        text: "real-endpoint-user-prompt",
        mode: "prompt",
        requestId: "req-mcp-real-endpoint",
      })) {
        // drain
      }
      ...
      expect(stdinText).toContain("Fased MCP status preview:");
      expect(stdinText).toContain("mode: status-only");
      expect(stdinText).toContain("scope: read-only effective-tool inventory");
      expect(stdinText).toContain("ACPX cannot call these tools through this bridge");
      expect(stdinText).toContain("session_status");
      expect(stdinText).toContain("real-endpoint-user-prompt");
      expect(stdinText).not.toContain(endpoint.token);
      expect(stdinText).not.toContain("rawDescription");
      expect(prompt?.bridgeEnv).toEqual({
        enabled: "1",
  ...
  it("does not mark backend unhealthy when a per-session cwd is missing", async () => {
    const { runtime } = await createMockRuntime();
    const missingCwd = path.join(os.tmpdir(), "fased-acpx-runtime-test-missing-cwd");
