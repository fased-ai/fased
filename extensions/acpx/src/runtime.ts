...
import { AcpRuntimeError } from "fased/plugin-sdk";
import {
...
import { prependAcpxMcpStatusPreview } from "./mcp-status-adapter.js";
import type { AcpxMcpStatusEndpoint } from "./mcp-status-server.js";
import {
  parseJsonLines,
...
export class AcpxRuntime implements AcpRuntime {
  ...
  private readonly mcpStatusEndpoint?: AcpxMcpStatusEndpoint;
  ...
  constructor(
    ...
    this.mcpStatusEndpoint = opts?.mcpStatusEndpoint;
  ...
  async *runTurn(input: AcpRuntimeTurnInput): AsyncIterable<AcpRuntimeEvent> {
    ...
    const promptText = await prependAcpxMcpStatusPreview({
      prompt: input.text,
      endpoint: this.mcpStatusEndpoint,
      agentId: state.agent,
      onError: (error) => {
        this.logger?.warn?.(`acpx MCP status preview unavailable: ${String(error)}`);
      },
    });
  ...
  private buildSpawnEnv(): NodeJS.ProcessEnv | undefined {
    const endpoint = this.mcpStatusEndpoint;
    if (!endpoint) {
      return undefined;
    }
    return {
      ...process.env,
      FASED_ACPX_MCP_BRIDGE_ENABLED: "1",
      FASED_ACPX_MCP_BRIDGE_MODE: endpoint.mode ?? "status-only",
      FASED_ACPX_MCP_BRIDGE_URL: endpoint.url,
      FASED_ACPX_MCP_BRIDGE_TOKEN: endpoint.token,
      FASED_ACPX_MCP_BRIDGE_TOOLS: endpoint.toolNames.join(","),
    };
  }
}
