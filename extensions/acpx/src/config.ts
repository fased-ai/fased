...
export const ACPX_MCP_BRIDGE_MODES = [
  "status-only",
  "read-only-tools",
  "operator-approved-mutating-tools",
] as const;
export type AcpxMcpBridgeMode = (typeof ACPX_MCP_BRIDGE_MODES)[number];
...
export type AcpxMcpBridgeConfig = {
  enabled?: boolean;
  mode?: AcpxMcpBridgeMode;
  allowTools?: string[];
  denyTools?: string[];
};

export type ResolvedAcpxMcpBridgeConfig = {
  enabled: boolean;
  mode: AcpxMcpBridgeMode;
  allowTools: string[];
  denyTools: string[];
...
export type ResolvedAcpxPluginConfig = {
  ...
  timeoutSeconds?: number;
  queueOwnerTtlSeconds: number;
  mcpBridge: ResolvedAcpxMcpBridgeConfig;
};

const DEFAULT_PERMISSION_MODE: AcpxPermissionMode = "approve-reads";
const DEFAULT_NON_INTERACTIVE_POLICY: AcpxNonInteractivePermissionPolicy = "fail";
const DEFAULT_QUEUE_OWNER_TTL_SECONDS = 0.1;
const DEFAULT_MCP_BRIDGE_MODE: AcpxMcpBridgeMode = "status-only";

type ParseResult =
  | { ok: true; value: AcpxPluginConfig | undefined }
  | { ok: false; message: string };
type ToolListParseResult = { ok: true; value?: string[] } | { ok: false; message: string };
type McpBridgeParseResult =
  | { ok: true; value?: AcpxMcpBridgeConfig }
  | { ok: false; message: string };
...
function isMcpBridgeMode(value: string): value is AcpxMcpBridgeMode {
  return ACPX_MCP_BRIDGE_MODES.includes(value as AcpxMcpBridgeMode);
}

function parseToolList(value: unknown, label: string): ToolListParseResult {
  if (value === undefined) {
...
function parseMcpBridgeConfig(value: unknown): McpBridgeParseResult {
  if (value === undefined) {
    return { ok: true, value: undefined };
  }
  if (!isRecord(value)) {
    return { ok: false, message: "mcpBridge must be an object" };
  }
  const allowedKeys = new Set(["enabled", "mode", "allowTools", "denyTools"]);
  for (const key of Object.keys(value)) {
    if (!allowedKeys.has(key)) {
      return { ok: false, message: `unknown mcpBridge config key: ${key}` };
    }
  }

  const enabled = value.enabled;
  if (enabled !== undefined && typeof enabled !== "boolean") {
    return { ok: false, message: "mcpBridge.enabled must be a boolean" };
  }

  const mode = value.mode;
  if (mode !== undefined && (typeof mode !== "string" || !isMcpBridgeMode(mode))) {
    return {
      ok: false,
      message: `mcpBridge.mode must be one of: ${ACPX_MCP_BRIDGE_MODES.join(", ")}`,
    };
  }

  const allowTools = parseToolList(value.allowTools, "mcpBridge.allowTools");
  if (!allowTools.ok) {
    return allowTools;
  }
  const denyTools = parseToolList(value.denyTools, "mcpBridge.denyTools");
  if (!denyTools.ok) {
    return denyTools;
  }
  ...
  return {
    ...
    value: {
      ...
      mode: typeof mode === "string" && isMcpBridgeMode(mode) ? mode : undefined,
      allowTools: allowTools.value,
      denyTools: denyTools.value,
    },
  };
}
...
function parseAcpxPluginConfig(value: unknown): ParseResult {
  ...
  const mcpBridge = parseMcpBridgeConfig(value.mcpBridge);
  if (!mcpBridge.ok) {
    return mcpBridge;
  }
  ...
  return {
    ...
    value: {
      ...
      mcpBridge: mcpBridge.value,
    },
  };
}

export function createAcpxPluginConfigSchema(): FasedAgentPluginConfigSchema {
  return {
...
function resolveMcpBridgeConfig(config?: AcpxMcpBridgeConfig): ResolvedAcpxMcpBridgeConfig {
  return {
    enabled: config?.enabled === true,
    mode: config?.mode ?? DEFAULT_MCP_BRIDGE_MODE,
...
export function resolveAcpxPluginConfig(params: {
  ...
  return {
    ...
    mcpBridge: resolveMcpBridgeConfig(normalized.mcpBridge),
  };
}
