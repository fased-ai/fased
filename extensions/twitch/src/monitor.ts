...
import type { ReplyPayload, FasedAgentConfig } from "fased/plugin-sdk";
import { createReplyPrefixOptions } from "fased/plugin-sdk";
...
export type TwitchMonitorOptions = {
  ...
  accountId: string;
  config: unknown; // FasedAgentConfig
...
async function processTwitchMessage(params: {
  ...
  const cfg = config as FasedAgentConfig;
