/**
 * Twitch channel plugin for FasedAgent.
 *
...
import { buildChannelConfigSchema } from "fased/plugin-sdk";
 ...
 * for FasedAgent. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: FasedAgentConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: FasedAgentConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: FasedAgentConfig): boolean => {
