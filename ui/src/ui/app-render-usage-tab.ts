...
export function renderUsageTab(state: AppViewState) {
  ...
  const dailyFromSessionUsage =
    state.usageResult?.aggregates?.daily.map((day) => ({
      date: day.date,
      input: 0,
      output: 0,
      cacheRead: 0,
      ...
      cacheReadCost: 0,
      cacheWriteCost: 0,
      missingCostEntries: day.tokens > 0 && day.cost === 0 ? 1 : 0,
    })) ?? [];
  ...
  return renderUsage({
    ...
    costDaily: state.usageCostSummary?.daily ?? dailyFromSessionUsage,
