...
const catalog = createModelCatalog(OPENAI_GPT54_MINI_MODEL, {
...
describe("chat-model-ref helpers", () => {
  ...
  it("normalizes raw overrides when the catalog match is unique", () => {
    expect(normalizeChatModelOverrideValue(createChatModelOverride("gpt-5.4-mini"), catalog)).toBe(
      "openai/gpt-5.4-mini",
    );
  });
  ...
  it("keeps ambiguous raw overrides unchanged", () => {
    expect(
      normalizeChatModelOverrideValue(
        createChatModelOverride("gpt-5.4-mini"),
        createAmbiguousModelCatalog("gpt-5.4-mini", "openai", "openrouter"),
      ),
    ).toBe("gpt-5.4-mini");
  });

  it("formats qualified model refs consistently for default labels", () => {
    expect(formatChatModelDisplay("openai/gpt-5.4-mini")).toBe("gpt-5.4-mini · OpenAI");
    expect(formatChatModelDisplay("alias-only")).toBe("alias-only");
  });

  it("resolves server session data to qualified option values", () => {
    expect(resolveServerChatModelValue("gpt-5.4-mini", "openai")).toBe("openai/gpt-5.4-mini");
  ...
  it("reports the override resolution source for unique catalog matches", () => {
    expect(resolveChatModelOverride(createChatModelOverride("gpt-5.4-mini"), catalog)).toEqual({
      value: "openai/gpt-5.4-mini",
      source: "catalog",
    });
  });
  ...
  it("falls back to the server provider when the catalog misses or is ambiguous", () => {
    expect(resolvePreferredServerChatModel("gpt-5.4-mini", "openai", [])).toEqual({
      value: "openai/gpt-5.4-mini",
      source: "server",
      reason: "missing",
    });
  ...
  it("preserves provider-qualified openrouter slash ids in option values", () => {
    expect(
      buildChatModelOption({
        id: "openrouter/openai/gpt-5.1-mini",
        name: "GPT-5.1 Mini",
        provider: "openrouter",
      }),
    ).toEqual({
      value: "openrouter/openai/gpt-5.1-mini",
      label: "openrouter/openai/gpt-5.1-mini · OpenRouter",
    });
  });

  it("prefers the catalog's already-qualified option when the server provider matches", () => {
