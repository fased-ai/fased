...
function renderProviderModelUsage(aggregates: UsageAggregates, chartMode: "tokens" | "cost") {
  const rows = aggregates.byModel.slice(0, 12);
  const isTokenMode = chartMode === "tokens";

  return html`
    <div class="usage-provider-model-card">
      <div class="usage-provider-model-header">
        <div>
          <div class="usage-insight-title">Provider / Model Usage</div>
          <div class="usage-insight-subtitle">
            Real chat and task transcript usage grouped by provider and model.
          </div>
        </div>
        <div class="usage-provider-model-total">
          ${rows.length ? `${rows.length} model${rows.length === 1 ? "" : "s"}` : "No data"}
        </div>
      </div>
      ${
        rows.length === 0
          ? html`
              <div class="muted">
                No provider/model usage in this date range. Chat or task replies must write transcript usage with
                provider and model metadata before this table can fill.
              </div>
            `
          : html`
              <div class="usage-provider-model-table">
                <div class="usage-provider-model-row usage-provider-model-heading">
                  <span>Provider</span>
                  <span>Model</span>
                  <span>Calls</span>
                  <span>${isTokenMode ? "Tokens" : "Cost"}</span>
                </div>
                ${rows.map(
                  (entry) => html`
	                    <div
	                      class="usage-provider-model-row"
	                      title=${`${entry.provider ?? "unknown"} · ${entry.model ?? "unknown"} · ${formatTokens(entry.totals.totalTokens)} · ${formatCostOrUnpriced(entry.totals)}`}
	                    >
                      <span class="usage-provider-pill">${entry.provider ?? "unknown"}</span>
                      <span class="usage-provider-model-name">${entry.model ?? "unknown"}</span>
                      <span>${entry.count}</span>
                      <span>
                        ${
                          isTokenMode
                            ? formatTokens(entry.totals.totalTokens)
                            : formatCostOrUnpriced(entry.totals)
                        }
                      </span>
                    </div>
                  `,
...
function renderUsageInsights(
  ...
  const topModels = aggregates.byModel.slice(0, 5).map((entry) => ({
    ...
    value: formatCostOrUnpriced(entry.totals),
  ...
  const topAgents = aggregates.byAgent.slice(0, 5).map((entry) => ({
    label: formatAgentDisplayName({ id: entry.agentId }),
    value: formatCostOrUnpriced(entry.totals),
  ...
  return html`
    <section class="card" style="margin-top: 16px;">
      ...
      ${renderProviderModelUsage(aggregates, chartMode)}
...
function renderSessionsCard(
  ...
  const buildSessionMeta = (s: UsageSessionEntry): string[] => {
    ...
    if (showColumn("agent") && s.agentId) {
      parts.push(`agent:${formatAgentDisplayName({ id: s.agentId })}`);
    }
