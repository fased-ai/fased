      ...
      agents: {
        defaults: {
          ...
          models: {
            ...
            "openai/gpt-5.5": { alias: "main" },
          },
        },
        list: {
          writer: {
            model: { primary: "xai/grok-4", fallbacks: ["openai/gpt-5.4-mini"] },
          },
          planner: {
...
describe("resolveAgentConfig", () => {
  it("reads keyed Agent model and task role config", () => {
    const result = resolveAgentConfig(
      {
        agents: {
          ...
          list: {
            main: {
              model: { primary: "openai/gpt-5.5", fallbacks: ["openrouter/auto"] },
              taskModels: {
    ...
    expect(result.entry?.model).toEqual({
      primary: "openai/gpt-5.5",
      fallbacks: ["openrouter/auto"],
    });
    expect(result.entry?.taskModels).toEqual({
      cheapCheck: "openrouter/cheap",
      escalation: "openai/strong",
    });
  });
...
describe("agentLogoUrl", () => {
  it("keeps base-mounted control UI logo paths absolute to the mount", () => {
    ...
    expect(agentLogoUrl("/apps/fased/")).toBe("/apps/fased/favicon.svg");
  });
...
describe("normalizeAgentLabel", () => {
  it("shows the internal main agent as Assistant", () => {
    expect(normalizeAgentLabel({ id: "main" })).toBe("Assistant");
    expect(normalizeAgentLabel({ id: "main", name: "main" })).toBe("Assistant");
  });
});
...
describe("buildAgentContext", () => {
  it("falls back to agent payload workspace/model when config form is unavailable", () => {
    ...
    expect(context.identityName).toBe("Assistant");
