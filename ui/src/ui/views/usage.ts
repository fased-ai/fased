...
export function renderUsage(props: UsageProps) {
  ...
  const selectedAgentValues = selectedValuesFor("agent");
  const selectedAgentValue =
    selectedAgentValues.length === 1
      ? selectedAgentValues[0]
      : selectedAgentValues.length > 1
        ? "__multiple__"
        : "";
  const visibleAgentOptions = unique([...selectedAgentValues, ...agentOptions]);
  const applyAgentScope = (agentId: string) => {
    if (agentId === "__multiple__") {
      return;
    }
    const nextQuery = setQueryTokensForKey(
      props.queryDraft || props.query,
      "agent",
      agentId ? [agentId] : [],
    );
    props.onQueryDraftChange(nextQuery);
    props.onApplyQuery();
  };
  ...
  return html`
    ...
    <style>
      ...
      .usage-agent-scope span {
        color: var(--muted);
        font-size: 12px;
        font-weight: 700;
      }
      .usage-agent-scope select {
        min-height: 36px;
      }
    </style>
    ...
    <section class="card usage-header ${props.headerPinned ? "pinned" : ""}">
      <div class="usage-header-row">
        <div class="usage-header-title">
          ...
          ${
            props.loading
              ? html`
                  <span class="usage-refresh-indicator">Loading usage data</span>
                `
              : nothing
          }
          ${
            isEmpty
              ? html`
                  <span class="usage-query-hint">Choose filters, then load usage for that range.</span>
                `
              : nothing
          }
        </div>
            ...
            <div class="usage-export-popover">
              <div class="usage-export-list">
                <button
                  ...
                  @click=${() =>
                    downloadTextFile(
                      `fased-usage-sessions-${exportStamp}.csv`,
                ...
                <button
                  ...
                  @click=${() =>
                    downloadTextFile(
                      `fased-usage-daily-${exportStamp}.csv`,
      ...
      <div class="usage-header-row">
        <div class="usage-controls">
          ...
          >
            ${props.loading ? "Loading..." : props.sessions.length > 0 || props.totals ? "Refresh" : "Load usage"}
          </button>
        </div>
      ...
      <div style="margin-top: 12px;">
        <div class="usage-filter-row" style="margin-bottom: 10px;">
          <label class="usage-agent-scope">
            <span>Agent usage</span>
            <select
              .value=${selectedAgentValue}
              @change=${(event: Event) =>
                applyAgentScope((event.target as HTMLSelectElement).value)}
            >
              ...
              ${visibleAgentOptions.map(
                (agentId) => html`<option value=${agentId}>${agentId}</option>`,
              )}
            </select>
          </label>
          <span class="usage-query-hint">
            ${selectedAgentValue ? "Filtered to one Agent." : "Showing usage across all Agents."}
          </span>
        </div>
          ...
          <input
            ...
            placeholder="Filter loaded rows (e.g. source:task provider:openrouter model:gpt has:errors minTokens:2000)"
        ...
        <div class="usage-filter-row">
          ${renderFilterSelect("source", "Source", sourceOptions)}
