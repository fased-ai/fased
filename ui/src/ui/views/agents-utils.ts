...
export function isVisibleAgentToolId(toolId: string) {
  return normalizeToolName(toolId) !== "common.reloadconfig";
}
...
export function resolveToolSections(
  ...
  if (toolsCatalogResult?.groups?.length) {
    return toolsCatalogResult.groups
      .map((group) => ({
        id: group.id,
        label: group.label,
        source: group.source,
...
type AgentConfigEntry = {
  ...
  modelProviders?: Record<string, AgentModelProviderSettings>;
  model?: unknown;
  taskModels?: AgentTaskModelSlots;
  identity?: {
    name?: string;
    emoji?: string;
    avatar?: string;
...
export type AgentTaskModelSlots = {
  cheapCheck?: string;
  strong?: string;
  escalation?: string;
  coding?: string;
...
export type AgentModelProviderSettings = {
  profileId?: string;
  primary?: string;
  fallbacks?: string[];
  taskModels?: AgentTaskModelSlots;
...
export function resolveAgentConfig(config: Record<string, unknown> | null, agentId: string) {
  ...
  const rawList = (cfg?.agents as { list?: unknown } | undefined)?.list;
  const entry = Array.isArray(rawList)
    ? rawList.find((agent): agent is AgentConfigEntry => {
        return Boolean(
          agent &&
          typeof agent === "object" &&
          "id" in agent &&
          (agent as { id?: string }).id === agentId,
        );
      })
    : rawList && typeof rawList === "object"
      ? ((rawList as Record<string, unknown>)[agentId] as AgentConfigEntry | undefined)
      : undefined;
  return {
    entry,
...
export function buildAgentContext(
  ...
  const identityName = formatAgentDisplayName({
    ...agent,
    name: config.entry?.name || agent.name,
    identity: { name: agentIdentity?.name || agent.identity?.name },
  });
...
export function resolveModelLabel(model?: unknown): string {
  ...
  if (typeof model === "object" && model) {
    ...
    if (primary) {
      const fallbackCount = normalizeAgentModelFallbackValues(record.fallbacks)?.length ?? 0;
  ...
  if (typeof model === "object" && model) {
    ...
    const fallbacks = Array.isArray(record.fallbacks)
      ...
      : Array.isArray(record.fallback)
        ? record.fallback
        : null;
    return normalizeAgentModelFallbackValues(fallbacks) ?? null;
  }
  return null;
}

export function resolveTaskModelSlots(value: unknown): AgentTaskModelSlots | null {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return null;
  }
  const record = value as Record<string, unknown>;
  const slots: AgentTaskModelSlots = {};
  for (const key of ["cheapCheck", "strong", "escalation", "coding", "summarizer"] as const) {
    const current = typeof record[key] === "string" ? record[key].trim() : "";
    if (current) {
      slots[key] = current;
    }
  }
...
export function normalizeModelProviderId(value: unknown): string | null {
...
export function resolveAgentModelProviders(
  ...
  const out: Record<string, AgentModelProviderSettings> = {};
  for (const [providerId, raw] of Object.entries(value as Record<string, unknown>)) {
    ...
    const fallbacks = normalizeAgentModelFallbackValues(record.fallbacks);
    const taskModels = resolveTaskModelSlots(record.taskModels) ?? undefined;
    const next: AgentModelProviderSettings = {
      ...(profileId ? { profileId } : {}),
      ...(primary ? { primary } : {}),
...
export function resolveConfiguredCronModelSuggestions(
  ...
  if (defaults && typeof defaults === "object") {
    ...
    const defaultTaskModels = resolveTaskModelSlots(defaultsRecord.taskModels);
    if (defaultTaskModels) {
      for (const modelId of Object.values(defaultTaskModels)) {
  ...
  if (list && typeof list === "object") {
    for (const entry of Object.values(list as Record<string, unknown>)) {
      ...
      const taskModels = resolveTaskModelSlots((entry as Record<string, unknown>).taskModels);
      if (taskModels) {
        for (const modelId of Object.values(taskModels)) {
          addModelId(out, modelId);
        }
      }
      for (const providerConfig of Object.values(
        resolveAgentModelProviders((entry as Record<string, unknown>).modelProviders),
      )) {
        addModelId(out, providerConfig.primary);
        for (const fallback of providerConfig.fallbacks ?? []) {
          addModelId(out, fallback);
        }
        for (const modelId of Object.values(providerConfig.taskModels ?? {})) {
          addModelId(out, modelId);
        }
      }
    }
