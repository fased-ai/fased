...
function isRenderableChannelConfigSchema(schema: JsonSchema): boolean {
  if (schema.enum || schema.anyOf || schema.oneOf) {
    return true;
  }
  ...
  if (type === "object") {
    return (
      ...
      (Boolean(schema.additionalProperties) && typeof schema.additionalProperties === "object")
    );
  }
  return false;
}
...
export function renderChannelConfigForm(props: ChannelConfigFormProps) {
  ...
  const extraFields = renderExtraChannelFields(value);
  if (!isRenderableChannelConfigSchema(node)) {
    return html`
      ${
        extraFields ??
        html`
          <div class="channel-config-empty">No setup fields for this channel.</div>
        `
      }
    `;
  }
...
export function renderChannelConfigSection(params: {
  ...
  const setup = props.snapshot?.channelSetup?.[channelId];
  return html`
    <details class="channel-config-details" ?open=${!configured}>
      <summary>${configured ? "Setup fields" : "Connect setup"}</summary>
      <div class="channel-config-details-body">
      ${setup?.access ? renderChannelDmAccessCard({ channelId, props, access: setup.access }) : nothing}
      ${
