...
export function renderGoogleChatCard(params: {
  ...
  googleChatAccounts: ChannelAccountSnapshot[];
  accountCountLabel: unknown;
}) {
  const { props, googleChat, googleChatAccounts, accountCountLabel } = params;
  ...
  return html`
    <div class="card">
      ...
      ${renderChannelConfigSection({
        ...
        props,
        configured:
          googleChat?.configured === true ||
          googleChatAccounts.some((account) => account.configured),
      })}
      ${renderChannelRuntimeControls({
        props,
        channelId: "googlechat",
        running: googleChat?.running,
        probe: true,
        probeStatus: googleChat?.probe,
      })}
    </div>
  `;
}
