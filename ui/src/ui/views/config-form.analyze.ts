...
function normalizeUnion(
  ...
  if (literals.length > 0 && normalizedRemaining.length === 0) {
  ...
  if (normalizedRemaining.length === 1 && literals.length === 0) {
    const [only] = normalizedRemaining;
    only.nullable = nullable || only.nullable;
    return {
      schema: only,
      unsupportedPaths: Array.from(unsupported),
    };
  ...
  const primitive = normalizedRemaining.find((entry) => {
    const type = schemaType(entry);
    return type && primitiveTypes.has(String(type));
  });
  if (primitive) {
    return {
      schema: {
        ...schema,
        anyOf: schema.anyOf ? [...normalizedRemaining, ...literalSchemas(literals)] : undefined,
        oneOf: schema.oneOf ? [...normalizedRemaining, ...literalSchemas(literals)] : undefined,
        nullable,
      },
      unsupportedPaths: Array.from(unsupported),
    };
  ...
  const renderableTypes = new Set(["string", "number", "integer", "boolean", "object", "array"]);
  if (
    normalizedRemaining.length > 0 &&
    literals.length === 0 &&
    normalizedRemaining.every((entry) => {
      const type = schemaType(entry);
      return type && renderableTypes.has(String(type));
    })
  ) {
    return {
      schema: {
        ...schema,
        anyOf: schema.anyOf ? normalizedRemaining : undefined,
        oneOf: schema.oneOf ? normalizedRemaining : undefined,
        nullable,
      },
      unsupportedPaths: Array.from(unsupported),
    };
...
function literalSchemas(literals: unknown[]): JsonSchema[] {
  const unique: unknown[] = [];
  for (const literal of literals) {
    if (!unique.some((existing) => Object.is(existing, literal))) {
      unique.push(literal);
    }
  }
