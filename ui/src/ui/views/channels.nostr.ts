...
export function renderNostrCard(params: {
  ...
  const renderProfileSection = () => {
    ...
    return html`
      <div
        style="margin-top: 16px; padding: 12px; background: var(--secondary); border: 1px solid var(--border); border-radius: 16px;"
      >
                    ...
                    ? html`
                      <div style="margin-bottom: 8px;">
                        <img
                          src=${picture}
                          alt="Profile picture"
                          style="width: 48px; height: 48px; border-radius: 50%; object-fit: cover; border: 2px solid var(--border); box-shadow: var(--shadow-sm);"
                          @error=${(e: Event) => {
  ...
  return html`
    <div class="card">
      ...
      ${renderChannelRoutePanel({ props, channelId: "nostr", accounts: nostrAccounts })}
      ...
      ${renderChannelConfigSection({
        channelId: "nostr",
        props,
        configured: summaryConfigured || nostrAccounts.some((account) => account.configured),
      })}
      ${
        hasMultipleAccounts
          ? nothing
