...
function matchesPathPattern(pattern: string, segments: string[]): boolean {
  const patternSegments = pattern.split(".");
  if (patternSegments.length !== segments.length) {
    return false;
  }
  for (let idx = 0; idx < patternSegments.length; idx += 1) {
    if (patternSegments[idx] !== "*" && patternSegments[idx] !== segments[idx]) {
      return false;
    }
  }
  return true;
...
function hasUnsupportedPath(unsupported: Set<string>, path: Array<string | number>): boolean {
  const key = pathKey(path);
  if (unsupported.has(key)) {
    return true;
  }
  const stringSegments = path.filter((segment): segment is string => typeof segment === "string");
  const allSegments = path.map((segment) => String(segment));
  for (const pattern of unsupported) {
    if (
      pattern.includes("*") &&
      (matchesPathPattern(pattern, stringSegments) || matchesPathPattern(pattern, allSegments))
    ) {
      return true;
    }
  }
...
function isHiddenConfigPath(path: Array<string | number>): boolean {
  return CONFIG_HIDDEN_PATHS.has(pathKey(path));
}

function visibleOptionsForConfigPath(params: {
  path: Array<string | number>;
  options: unknown[];
}): { options: unknown[]; note?: string } {
  const key = pathKey(params.path);
  if (key === "wallet.provider.id") {
    return {
      options: params.options.filter(
        (option) => typeof option === "string" && SELF_HOSTED_WALLET_PROVIDER_OPTIONS.has(option),
      ),
      note: "Normal Config only exposes self-hosted wallet providers. Hosted provider ids remain CLI/admin-only compatibility plumbing.",
    };
  }
  if (key === "wallet.keystore.chainSupport" || key === "wallet.runtime.chains") {
    return {
      options: params.options.filter(
        (option) => typeof option === "string" && SOLANA_ONLY_WALLET_CHAIN_OPTIONS.has(option),
      ),
      note: "Normal Config only exposes Solana wallet chains for the current Fased wallet runtime.",
    };
  }
...
function pickUnionVariant(variants: JsonSchema[], value: unknown): JsonSchema | null {
  const valueType = valueSchemaType(value);
  if (valueType) {
    const exact = variants.find((variant) => schemaType(variant) === valueType);
    if (exact) {
      return exact;
    }
    if (valueType === "number") {
      const integer = variants.find((variant) => schemaType(variant) === "integer");
      if (integer) {
        return integer;
      }
    }
  ...
  return (
    variants.find((variant) => {
      ...
      return type && ["object", "array", "string", "number", "integer", "boolean"].includes(type);
    }) ?? null
  );
}
...
export function renderNode(params: {
  ...
  if (hasUnsupportedPath(unsupported, path)) {
  ...
  if (schema.anyOf || schema.oneOf) {
    ...
    const picked = pickUnionVariant(nonNull, value ?? schema.default);
    if (picked) {
      return renderNode({ ...params, schema: picked });
    }
  }

  // Enum - use segmented for small, dropdown for large
  if (schema.enum) {
    const policy = visibleOptionsForConfigPath({ path, options: schema.enum });
    ...
    if (options.length <= 5) {
      ...
      return html`
        <div class="cfg-field">
          ${showLabel ? html`<label class="cfg-field__label">${label}</label>` : nothing}
          ${help ? html`<div class="cfg-field__help">${help}</div>` : nothing}
          ${policy.note ? html`<div class="cfg-field__help">${policy.note}</div>` : nothing}
          ${renderTags(tags)}
          <div class="cfg-segmented">
            ${options.map(
              (opt) => html`
...
function renderSelect(params: {
  ...
  return html`
    <div class="cfg-field">
      ${showLabel ? html`<label class="cfg-field__label">${label}</label>` : nothing}
      ${help ? html`<div class="cfg-field__help">${help}</div>` : nothing}
      ${params.optionNote ? html`<div class="cfg-field__help">${params.optionNote}</div>` : nothing}
      ${renderTags(tags)}
      <select
        class="cfg-select"
        ?disabled=${disabled}
