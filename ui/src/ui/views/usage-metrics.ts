        ...
        <div class="usage-mosaic-section">
          ...
          <div class="usage-daypart-grid">
            ${stats.weekdayTotals.map((part) => {
              ...
              const bg =
                part.tokens > 0
                  ? `color-mix(in srgb, var(--accent) ${Math.round(16 + intensity * 52)}%, transparent)`
                  : "transparent";
              return html`
        ...
        <div class="usage-mosaic-section">
          ...
          <div class="usage-hour-grid">
            ${stats.hourTotals.map((value, hour) => {
              ...
              const bg =
                value > 0
                  ? `color-mix(in srgb, var(--accent) ${Math.round(12 + intensity * 58)}%, transparent)`
                  : "transparent";
...
function formatCostOrUnpriced(
  totals: { totalTokens?: number; totalCost?: number; missingCostEntries?: number },
  decimals = 2,
): string {
  const tokens = totals.totalTokens ?? 0;
  const cost = totals.totalCost ?? 0;
  const missing = totals.missingCostEntries ?? 0;
  if (tokens > 0 && missing > 0 && cost <= 0) {
    return "unpriced";
  }
  return formatCost(cost, decimals);
}
