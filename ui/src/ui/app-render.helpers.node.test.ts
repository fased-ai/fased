...
describe("parseSessionKey", () => {
  it("identifies main session (bare 'main')", () => {
    expect(parseSessionKey("main")).toEqual({ prefix: "", fallbackName: "Local chat" });
  });

  it("identifies main session (agent:main:main)", () => {
    expect(parseSessionKey("agent:main:main")).toEqual({
      prefix: "",
      fallbackName: "Local chat",
    });
  });
...
describe("resolveSessionDisplayName", () => {
  ...
  it("returns 'Local chat' for agent:main:main key", () => {
    expect(resolveSessionDisplayName("agent:main:main")).toBe("Local chat");
  });

  it("returns 'Local chat' for bare 'main' key", () => {
    expect(resolveSessionDisplayName("main")).toBe("Local chat");
  });
  ...
  it("returns 'Task:' for task key without row", () => {
    expect(resolveSessionDisplayName("agent:main:cron:abc-123")).toBe("Task:");
  });
  ...
  it("prefixes task label with Task:", () => {
    expect(
      resolveSessionDisplayName(
        "agent:main:cron:abc-123",
        row({ key: "agent:main:cron:abc-123", label: "daily-briefing" }),
      ),
    ).toBe("Task: daily-briefing");
  });

  it("prefixes task displayName with Task:", () => {
    expect(
      resolveSessionDisplayName(
        "agent:main:cron:abc-123",
        row({ key: "agent:main:cron:abc-123", displayName: "Nightly Sync" }),
      ),
    ).toBe("Task: Nightly Sync");
  });

  it("does not double-prefix task labels that already include Task:", () => {
    expect(
      resolveSessionDisplayName(
        "agent:main:cron:abc-123",
        row({ key: "agent:main:cron:abc-123", label: "Task: Nightly Sync" }),
      ),
    ).toBe("Task: Nightly Sync");
  });

  it("normalizes legacy Cron labels to Task labels", () => {
    expect(
      resolveSessionDisplayName(
        "agent:main:cron:abc-123",
