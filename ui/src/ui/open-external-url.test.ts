...
describe("resolveSafeExternalUrl", () => {
  const baseHref = "https://fased.ai/chat";
  ...
  it("allows relative URLs resolved against the current origin", () => {
    expect(resolveSafeExternalUrl("/assets/pic.png", baseHref)).toBe(
      "https://fased.ai/assets/pic.png",
    );
  });

  it("allows blob URLs", () => {
    expect(resolveSafeExternalUrl("blob:https://fased.ai/abc-123", baseHref)).toBe(
      "blob:https://fased.ai/abc-123",
    );
  });
