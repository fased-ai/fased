...
import type { AgentsListResult, ConfigSnapshot } from "../types.ts";
...
function resolveDreamingAgentId(state: DreamingState): string | undefined {
...
export async function loadDreamingStatus(state: DreamingState): Promise<void> {
  ...
  const agentId = resolveDreamingAgentId(state);
  try {
