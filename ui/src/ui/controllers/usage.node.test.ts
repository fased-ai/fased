...
describe("usage controller date interpretation params", () => {
  ...
  it("does not keep the page loading when the secondary cost summary hangs", async () => {
    vi.useFakeTimers();

    const request = vi.fn((method: string) => {
      if (method === "sessions.usage") {
        return Promise.resolve({ sessions: [], totals: null, aggregates: null });
      }
      return new Promise<unknown>(() => {
        // Keep usage.cost pending; the controller should still finish the visible load.
      });
    });
    const state = createState(request);
    ...
    expect(state.usageResult).toEqual({ sessions: [], totals: null, aggregates: null });
