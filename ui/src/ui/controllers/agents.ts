...
): Promise<AgentCreateResult | null> {
  const name = draft.name.trim();
  const workspace = draft.workspace.trim();
  const model = draft.model?.trim() || null;
  const avatar = draft.avatar?.trim() || null;
  if (!name || !workspace) {
    state.agentsError = "Agent name and workspace are required.";
    state.agentsCreateMessage = null;
    state.agentsCreateBusy = false;
    return null;
  }
  if (!state.client || !state.connected) {
    state.agentsError = "Gateway is not connected.";
    state.agentsCreateMessage = null;
    state.agentsCreateBusy = false;
    return null;
  }
  if (state.agentsCreateBusy) {
    return null;
  }

  state.agentsCreateBusy = true;
  state.agentsCreateMessage = null;
  state.agentsError = null;
  try {
    const result = await state.client.request<AgentCreateResult>("agents.create", {
      name,
      workspace,
      ...(model ? { model } : {}),
      ...(avatar ? { avatar } : {}),
    ...
    if (result?.agentId) {
      ...
      state.agentsCreateMessage = `Created ${result.agentId}.`;
    } else {
      state.agentsCreateMessage = "Agent created.";
    }
    await loadAgents(state);
    if (result?.agentId && state.agentsList?.agents.some((entry) => entry.id === result.agentId)) {
      state.agentsSelectedId = result.agentId;
    }
    return result ?? null;
  } catch (err) {
