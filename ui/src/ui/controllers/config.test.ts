...
describe("updateConfigFormValue", () => {
  ...
  it("writes primary and fallback models under the selected keyed Agent", () => {
    const state = createState();
    state.configSnapshot = {
      config: {
        agents: {
          list: {
            main: { name: "Assistant", model: "openrouter/auto" },
            research: { name: "Research", model: "openai/gpt-5.4-mini" },
          },
        },
      },
      valid: true,
    ...
    updateConfigFormValue(state, ["agents", "list", "research", "model"], {
    ...
    expect(state.configRaw).toContain('"research"');
    expect(state.configRaw).toContain('"fallbacks"');
  });

  it("writes task role models under the selected keyed Agent", () => {
    const state = createState();
    state.configSnapshot = {
      config: {
        agents: {
    ...
    updateConfigFormValue(state, ["agents", "list", "main", "taskModels"], {
      cheapCheck: "openrouter/cheap",
      escalation: "openai/strong",
    });
...
describe("saveConfig", () => {
  ...
  it("refreshes the base hash and retries when config changed before save", async () => {
    const request = vi.fn().mockImplementation(async (method: string, params?: unknown) => {
      if (method === "config.set") {
        const baseHash = (params as { baseHash?: string }).baseHash;
        if (baseHash === "hash-stale") {
          throw new Error(
    ...
    state.configForm = {
      agents: {
        list: [
          {
            id: "main",
            taskModels: { cheapCheck: "openrouter/cheap" },
          },
        ],
      },
    };
    state.configSnapshot = { hash: "hash-stale" };
    ...
    expect(request).toHaveBeenNthCalledWith(
      1,
      "config.set",
      expect.objectContaining({ baseHash: "hash-stale" }),
    );
    expect(request).toHaveBeenNthCalledWith(2, "config.get", {});
    expect(request).toHaveBeenNthCalledWith(
      3,
      "config.set",
      expect.objectContaining({ baseHash: "hash-fresh" }),
    );
    const retryParams = request.mock.calls[2]?.[1] as { raw: string };
    expect(JSON.parse(retryParams.raw)).toEqual({
      agents: { list: [{ id: "main", taskModels: { cheapCheck: "openrouter/cheap" } }] },
    });
