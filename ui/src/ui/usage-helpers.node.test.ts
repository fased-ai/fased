...
describe("usage-helpers", () => {
  ...
  it("supports source filters for chat/channel/task rows", () => {
    const task = { key: "task:a", source: "task", usage: { totalTokens: 10, totalCost: 0 } };
    const chat = { key: "chat:a", source: "chat", usage: { totalTokens: 10, totalCost: 0 } };
    expect(filterSessionsByQuery([task, chat], "source:task").sessions).toEqual([task]);
  });
