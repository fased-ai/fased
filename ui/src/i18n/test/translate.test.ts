...
describe("i18n", () => {
  ...
  it("loads saved non-English locale on startup", async () => {
    localStorage.setItem("fased.i18n.locale", "zh-CN");
    const { I18nManager } = await import("../lib/translate.ts");
    const freshManager = new I18nManager();
    const fresh = {
      i18n: freshManager,
      t: (key: string, params?: Record<string, string>) => freshManager.t(key, params),
    };

    for (let index = 0; index < 20 && fresh.i18n.getLocale() !== "zh-CN"; index += 1) {
