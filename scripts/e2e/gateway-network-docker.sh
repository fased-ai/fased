...
IMAGE_NAME="fased-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="fased-net-e2e-$$"
GW_NAME="fased-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "FASED_GATEWAY_TOKEN=$TOKEN" \
  -e "FASED_SKIP_CHANNELS=1" \
  -e "FASED_SKIP_GMAIL_WATCHER=1" \
  -e "FASED_SKIP_CRON=1" \
  -e "FASED_SKIP_CANVAS_HOST=1" \
  "$IMAGE_NAME" \
