...
BASE_IMAGE="${BASE_IMAGE:-fased-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-fased-sandbox-common:bookworm-slim}"
...
docker build \
  -t "${TARGET_IMAGE}" \
  -f deploy/containers/Dockerfile.sandbox-common \
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=fased.sandbox=1)
NOTE
