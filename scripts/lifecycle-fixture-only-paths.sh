...
readonly FASED_LIFECYCLE_FIXTURE_ONLY_PATH_RE='^(\.github/workflows/hosted-runtime-release\.yml|docs/maintainers/codex-skills/fased-release-manager/(SKILL\.md|references/(lifecycle|release)\.md)|scripts/lifecycle-fixture-only-paths\.sh|scripts/test-lifecycle-hosting-acceptance\.sh|scripts/docker/hosting-systemd/(lifecycle-acceptance\.sh|Containerfile\.(ubuntu|rocky))|scripts/(hosted-installer-artifact-layout|ci-workflow-contract|lifecycle-d8-contract|lifecycle-version-neutral|npm-free-managed-lifecycle-contract)\.test\.ts|scripts/lifecycle-configuration-preservation\.(mjs|test\.ts)|scripts/build-(public|canonical-managed)-predecessor-capsule\.(mjs|test\.ts)|scripts/prepare-branch-predecessor-capsule\.sh|scripts/(predecessor-capsule|lifecycle-installed-state-capsule|lifecycle-acceptance-contract|lifecycle-receipt-verifier)\.(mjs|test\.ts))$'
