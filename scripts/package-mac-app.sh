...
# Build and bundle FasedAgent into a minimal .app we can open.
# Outputs to dist/FasedAgent.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/FasedAgent.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="FasedAgent"
BUNDLE_ID="${BUNDLE_ID:-ai.fased.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/fased-ai/fased/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/FasedAgent/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :FasedAgentBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :FasedAgentGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/FasedAgent"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/FasedAgent"
fi
chmod +x "$APP_ROOT/Contents/MacOS/FasedAgent"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/FasedAgent" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/FasedAgent/Resources/FasedAgent.icns" "$APP_ROOT/Contents/Resources/FasedAgent.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/FasedAgent/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying FasedAgentKit resources"
FASEDKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/FasedAgentKit_FasedAgentKit.bundle"
if [ -d "$FASEDKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/FasedAgentKit_FasedAgentKit.bundle"
  cp -R "$FASEDKIT_BUNDLE" "$APP_ROOT/Contents/Resources/FasedAgentKit_FasedAgentKit.bundle"
else
  echo "WARN: FasedAgentKit resource bundle not found at $FASEDKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running FasedAgent"
killall -q FasedAgent 2>/dev/null || true
