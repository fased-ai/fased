  ...
  it("derives every privileged path and service from one random instance identity", () => {
    ...
    expect(protectedLocalControllerConfiguration(instanceId)).toEqual({
      ...
      paths: {
        ...
        controllerUnitPath: `/etc/systemd/system/fased-local-controller-${instanceId}.service`,
        applicationReleasesDir: `/opt/fased/local/${instanceId}/application/releases`,
        applicationCurrentLink: `/opt/fased/local/${instanceId}/application/current`,
        gatewayUnitPath: `/etc/systemd/system/fased-gateway-${instanceId}.service`,
