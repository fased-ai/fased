...
function resolveStateDir(): string {
  const override = process.env.FASED_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.FASED_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".fased");
}
...
function usage(): string {
  return (
    ...
    "1) FasedAgent spawned an ACP thread binding\n" +
    ...
    "  FASED_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  FASED_DISCORD_SMOKE_DRIVER\n" +
    "  FASED_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  FASED_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  FASED_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  FASED_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  FASED_DISCORD_SMOKE_AGENT\n" +
    "  FASED_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  FASED_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  FASED_DISCORD_SMOKE_POLL_MS\n" +
    "  FASED_DISCORD_SMOKE_THREAD_BINDINGS_PATH"
  );
}

function parseArgs(): Args {
  const channelId = resolveArg("--channel") || process.env.FASED_DISCORD_SMOKE_CHANNEL_ID || "";
  const driverModeRaw = resolveArg("--driver") || process.env.FASED_DISCORD_SMOKE_DRIVER || "token";
  ...
  const driverMode: DriverMode =
    normalizedDriverMode === "webhook"
      ...
      : normalizedDriverMode === "token"
        ? "token"
        : "token";
  const driverToken = resolveArg("--token") || process.env.FASED_DISCORD_SMOKE_DRIVER_TOKEN || "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.FASED_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.FASED_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") || process.env.FASED_DISCORD_SMOKE_BOT_TOKEN_PREFIX || "Bot";
  const targetAgent = resolveArg("--agent") || process.env.FASED_DISCORD_SMOKE_AGENT || "codex";
  const mentionUserId =
    resolveArg("--mention") || process.env.FASED_DISCORD_SMOKE_MENTION_USER_ID || undefined;
  const instruction =
    resolveArg("--instruction") || process.env.FASED_DISCORD_SMOKE_INSTRUCTION || undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.FASED_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.FASED_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.FASED_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `fased-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
  ...
  try {
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from FasedAgent.`,
        diagnostics: {
          bindingCandidates: [
            {
