...
# Fased Agent auth widget for Termux
...
# Check auth status
termux-toast "Checking Fased Agent auth..."

STATUS=$(ssh "$SERVER" "$REMOTE_AUTH_STATUS simple" 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" "$REMOTE_AUTH_STATUS json" 2>&1)
    ...
    CLAUDE_EXPIRING|FASED_EXPIRING|FASED_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$REMOTE_REAUTH'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|FASED_EXPIRED|FASED_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: Fased Agent won't work until re-auth"
                ;;
        esac
        ;;
