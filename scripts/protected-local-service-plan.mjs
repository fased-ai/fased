...
ReadOnlyPaths=${unitPath(path.join(layout.installDir, "controller"))} ${escaped.supervisorInstallDir} ${unitPath(path.join(layout.installDir, "signer-owner"))} ${unitPath(operatorSocketFinalize)} ${unitPath(path.join(layout.controllerStateDir, "controller-version.json"))} ${escaped.supervisorStateDir} /etc/systemd/system/${layout.controllerUnit} /etc/systemd/system/${layout.controllerUnit}.d /etc/systemd/system/${layout.supervisorUnit} /etc/systemd/system/${layout.supervisorUnit}.d
ProtectKernelTunables=true
ProtectKernelModules=true
ProtectKernelLogs=true
ProtectControlGroups=true
...
ProtectHome=true
ProtectSystem=strict
ReadWritePaths=${unitPath(layout.supervisorStateDir)} ${unitPath(path.join(layout.installDir, "controller"))} ${escaped.supervisorInstallDir} /run/fased-local-controller/${layout.instanceId}
ProtectKernelTunables=true
ProtectKernelModules=true
ProtectKernelLogs=true
ProtectControlGroups=true
