...
SMOKE_PREVIOUS_VERSION="${FASED_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${FASED_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="fased"
PACKAGE_NAME="${FASED_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (FASED_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${FASED_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${FASED_INSTALL_LATEST_OUT:-}"
fi
if [[ -n "$CMD_PATH" ]]; then
