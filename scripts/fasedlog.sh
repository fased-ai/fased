...
# Fased Agent logging utility
# Simplifies access to Fased Agent logs using the macOS unified logging system.
...
handle_sudo_error() {
    ...
    echo -e "${RED}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}\n"
    echo -e "fasedlog needs to use sudo to show complete log data (Apple hides sensitive info by default)."
...
show_usage() {
    cat << EOF
fasedlog - Fased Agent logging utility

USAGE:
    fasedlog [OPTIONS]

DESCRIPTION:
    View Fased Agent logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    Fased Agent logs flow through the macOS unified log (subsystem: ai.fased).
...
QUICK START:
    fasedlog -n 100             Show last 100 lines from all components
    fasedlog -f                 Follow logs in real-time
    fasedlog -e                 Show only errors
    fasedlog -c ServerManager   Show logs from ServerManager only
...
EXAMPLES:
    fasedlog                   Show last 50 lines from past 5 minutes (default)
    fasedlog -f                Stream logs continuously
    fasedlog -n 100            Show last 100 lines
    fasedlog -e                Show only recent errors
    fasedlog -l 30m -n 200     Show last 200 lines from past 30 minutes
    fasedlog -c ServerManager  Show recent ServerManager logs
    fasedlog -s "fail"         Search for "fail" in recent logs
    fasedlog --server -e       Show recent server errors
    fasedlog -f -d             Stream debug logs continuously
...
list_categories() {
    echo -e "${BLUE}Fetching Fased Agent log categories from the last hour...${NC}\n"
...
if [[ "$STREAM_MODE" == true ]]; then
    ...
    echo -e "${GREEN}Streaming Fased Agent logs continuously...${NC}"
