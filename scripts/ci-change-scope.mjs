...
const HOSTING_PATH_RE =
  /^(?:install\.sh$|scripts\/(?:docker\/streamed-hosting-bootstrap\/|build-hosted-runtime-artifact|fased-host-|fased-managed-updater|fased-signer-(?:enroll|policy)-hosting|hosted-|hosting-|install-(?:hosted-runtime|managed-runtime|platform-preflight|release-pin|runtime-profile)|managed-runtime-layout|migrate-hosted-signer|signer-(?:enrollment-launchers|owner-policy-installers)|test-(?:hosted-runtime-install|install-runtime-profile|streamed-hosting-bootstrap-container)|start-managed)[^/]*|src\/(?:cli\/daemon-cli\/(?:install|restart-health)|cli\/program\/register\.onboard|commands\/(?:daemon-install-helpers|doctor-(?:gateway-health|state-integrity)|hosted-dashboard-probe|onboard-non-interactive\/local)|config\/io|daemon\/systemd|infra\/(?:host|managed-runtime|update-runner|local-source-paired-update)|security\/(?:audit|fix)|wizard\/(?:host-security-capability|onboarding))[^/]*|\.github\/workflows\/hosted-runtime-release\.yml$)/;
