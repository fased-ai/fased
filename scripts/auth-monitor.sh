...
#
# Suggested cron: */30 * * * * /path/to/fased/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send FasedAgent notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., fased-alerts)
...
STATE_FILE="$HOME/.fased/auth-monitor-state"
...
send_notification() {
    ...
    # Send via FasedAgent if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use fased
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via FasedAgent to $NOTIFY_PHONE..."
            fased send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! FasedAgent may need re-auth. Run $SCRIPT_DIR/mobile-reauth.sh on the host, or SSH to the host and run it there." "urgent"
    exit 1
