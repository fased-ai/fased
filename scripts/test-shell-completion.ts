 ...
 * `fased update`. Use it to verify the completion installation flow
...
\`fased update\`, \`fased doctor\`, and \`fased onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# FasedAgent Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`fased completion --install\` later to enable.`));
    return;
  }
