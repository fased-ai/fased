...
const unitIsolatedFilesRaw = [
  ...
  "src/agents/fased-tools.subagents.sessions-spawn.lifecycle.test.ts",
...
// FASED_TEST_VM_FORKS=0, and let users force-enable with =1.
...
const shardIndexOverride = (() => {
  const parsed = Number.parseInt(process.env.FASED_TEST_SHARD_INDEX ?? "", 10);
  return Number.isFinite(parsed) && parsed > 0 ? parsed : null;
})();

if (shardIndexOverride !== null && shardCount <= 1) {
  console.error(
    `[test-parallel] FASED_TEST_SHARD_INDEX=${String(
      shardIndexOverride,
    )} requires FASED_TEST_SHARDS>1.`,
  );
  process.exit(2);
}
...
if (shardIndexOverride !== null && shardIndexOverride > shardCount) {
  console.error(
    `[test-parallel] FASED_TEST_SHARD_INDEX=${String(
      shardIndexOverride,
    )} exceeds FASED_TEST_SHARDS=${String(shardCount)}.`,
  );
  process.exit(2);
}
const windowsCiArgs = isWindowsCi ? ["--dangerouslyIgnoreUnhandledErrors"] : [];
const silentArgs = process.env.FASED_TEST_SHOW_PASSED_LOGS === "1" ? [] : ["--silent=passed-only"];
const rawPassthroughArgs = process.argv.slice(2);
const passthroughArgs =
  rawPassthroughArgs[0] === "--" ? rawPassthroughArgs.slice(1) : rawPassthroughArgs;
const rawTestProfile = process.env.FASED_TEST_PROFILE?.trim().toLowerCase();
const testProfile =
  rawTestProfile === "low" ||
  rawTestProfile === "max" ||
...
const maxOldSpaceSizeMb = (() => {
  ...
  const raw = process.env.FASED_TEST_MAX_OLD_SPACE_SIZE_MB ?? "";
...
function resolveReportDir() {
  const raw = process.env.FASED_VITEST_REPORT_DIR?.trim();
  if (!raw) {
    return null;
  }
  try {
