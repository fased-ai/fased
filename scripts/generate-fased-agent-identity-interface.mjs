...
const rows = contract.instructions
  .map((instruction) => {
    const accounts = instruction.accounts
      ...
      .join(", ");
    return `${JSON.stringify(instruction.action)}: {Discriminator: [8]byte{${instruction.discriminator.join(", ")}}, DataSize: ${instruction.dataSize ?? -1}, Accounts: []agentIdentityAccountContractV1{${accounts}}}`;
  })
  .join(",\n\t");
