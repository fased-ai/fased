#!/usr/bin/env node
// Full managed update implementation. The stable entrypoint is the
// version-neutral fased-managed-updater.mjs bootstrap.
...
const LEGACY_MANAGED_UPDATE_ADOPTION_FILE = "legacy-managed-update-adoption.v1.json";
const LEGACY_MANAGED_UPDATE_JOURNAL_FILE = "hosted-update-transaction.json";
const LEGACY_MANAGED_UPDATE_HINT_FILE = "protected-local-controller-transaction.json";
const LEGACY_MANAGED_UPDATE_JOURNAL_KEYS = Object.freeze([
  "createdAt",
  "nextManifest",
  "phase",
  "previousManifest",
...
const LEGACY_MANAGED_UPDATE_ADOPTION_KEYS = Object.freeze([
  "adoptedAt",
...
function canonicalDigest(value) {
  return taggedSha256(canonicalReleaseJSON(value));
}

function adoptionPath(stateDir, suppliedPath, basename, label) {
  const expected = path.join(stateDir, basename);
  if (suppliedPath !== undefined && path.resolve(suppliedPath) !== expected) {
    throw new Error(`${label} must use its fixed managed-state path`);
  }
  return expected;
}

async function readBoundedOwnerJson(filePath, label, expectedOwnerUid, options = {}) {
  let named;
  try {
  ...
  const namedMode = named.mode & 0o777;
  if (
    !named.isFile() ||
    ...
    (options.allowedModes
      ? !options.allowedModes.includes(namedMode)
      : (named.mode & 0o177) !== 0) ||
    named.size < 2 ||
    named.size > (options.maxBytes ?? 2 * 1024 * 1024)
  ) {
    throw new Error(`${label} is not a safe owner-only bounded file`);
  }
  ...
  try {
    ...
    return Object.freeze({ bytes, value, stat: opened });
  } finally {
    await handle.close();
  }
}

function validateLegacyManagedManifest(manifest, paths, version) {
  const required = [
    "configPath",
    "package",
    "profile",
  ...
  if (
    ...
    throw new Error("legacy managed updater manifest identity is invalid");
  }
  const serviceMatch = /^fased-gateway-([a-f0-9]{16})\.service$/u.exec(
  ...
  if (
    !serviceMatch ||
    manifest.service.launcher !== `/opt/fased/local/${serviceMatch[1]}/gateway-launch`
  ) {
    throw new Error("legacy managed updater service identity is invalid");
  }
  return Object.freeze({ instanceId: serviceMatch[1] });
}

function validateLegacyManagedUpdateJournal(value, paths) {
  if (
    !exactObjectKeys(value, LEGACY_MANAGED_UPDATE_JOURNAL_KEYS) ||
    value.schemaVersion !== 1 ||
    value.phase !== "rolling-back" ||
    !TRANSACTION_ID_PATTERN.test(String(value.transactionId ?? "")) ||
    !EXACT_RELEASE_VERSION_PATTERN.test(String(value.previousVersion ?? "")) ||
    !EXACT_RELEASE_VERSION_PATTERN.test(String(value.targetVersion ?? "")) ||
    Number.isNaN(Date.parse(String(value.createdAt ?? ""))) ||
    Number.isNaN(Date.parse(String(value.updatedAt ?? "")))
  ) {
    throw new Error(
      "legacy managed updater transaction is not a supported rolling-back schema1 journal",
    );
  }
  const expectedPreviousRoot = path.join(paths.releasesDir, value.previousVersion);
  const expectedTargetRoot = path.join(paths.releasesDir, value.targetVersion);
  if (value.previousRoot !== expectedPreviousRoot || value.targetRoot !== expectedTargetRoot) {
    throw new Error("legacy managed updater transaction release roots are invalid");
  }
  const previous = validateLegacyManagedManifest(
    value.previousManifest,
    paths,
    value.previousVersion,
  );
  const target = validateLegacyManagedManifest(value.nextManifest, paths, value.targetVersion);
  if (previous.instanceId !== target.instanceId) {
    throw new Error("legacy managed updater transaction changes the protected Local instance");
  }
  return Object.freeze({
    ...value,
    transactionId: value.transactionId.toLowerCase(),
    serviceIdentity: previous,
  });
}

function validateLegacyControllerHint(value) {
  if (
    !exactObjectKeys(value, ["schemaVersion", "transactionId", "version"]) ||
    value.schemaVersion !== 1 ||
    !TRANSACTION_ID_PATTERN.test(String(value.transactionId ?? "")) ||
    !EXACT_RELEASE_VERSION_PATTERN.test(String(value.version ?? ""))
  ) {
    throw new Error("legacy protected Local controller hint is malformed");
  }
  return value;
}

async function verifiedManagedReleaseIdentity(root, version) {
  await assertManagedRuntime(root, version);
  const metadata = await readHostedRuntimeMetadata(root);
  if (!metadata) {
    throw new Error(`managed runtime ${version} has no verified hosted metadata`);
  }
  const release = await readHostedReleaseBinding(root, metadata, version);
  if (!release) {
    throw new Error(`managed runtime ${version} has no verified hosted release binding`);
  }
...
async function defaultReadRootApplicationIdentity({ instanceId, previousVersion }) {
  const expectedRoot = `/opt/fased/local/${instanceId}/application/releases/v${previousVersion}`;
  const currentLink = `/opt/fased/local/${instanceId}/application/current`;
  const currentRoot = await resolveLinkTarget(currentLink);
  if (currentRoot !== expectedRoot) {
    throw new Error(
      "protected Local root application/current does not select the previous release",
    );
  }
  return await verifiedManagedReleaseIdentity(currentRoot, previousVersion);
}

function expectedGatewayGeneration(identity, runtimeRoot) {
  ...
  return Object.freeze({
    ...
    updaterBundleDigest: null,
    runtimeRootDigest: taggedSha256(runtimeRoot),
  });
}

async function readExpectedGatewayGeneration(rootIdentity) {
  const updaterPath = path.join(rootIdentity.root, ".fased-managed-updater-bundle.json");
  let parsed;
  try {
    parsed = JSON.parse(await fsp.readFile(updaterPath, "utf8"));
  } catch (error) {
    if (error?.code === "ENOENT") {
      return null;
    }
  ...
  if (
    ...
    parsed.release?.version !== rootIdentity.version ||
    parsed.release?.commit !== rootIdentity.identity.release.commit ||
    !TAGGED_SHA256_PATTERN.test(String(parsed.bundleDigest ?? ""))
  ) {
    throw new Error("protected Local root application updater identity is invalid");
  }
  return Object.freeze({
    ...expectedGatewayGeneration(rootIdentity.identity, rootIdentity.root),
    updaterBundleDigest: parsed.bundleDigest,
  });
}
...
async function defaultProbeLegacyGateway({
  ...
  return await new Promise((resolve, reject) => {
      ...
      (response) => {
        ...
        response.on("data", (chunk) => {
          ...
          if (Buffer.byteLength(body) > 256 * 1024) {
            request.destroy(new Error("Gateway readiness response exceeded its bound"));
          }
        });
        response.on("end", () => {
          try {
    ...
    request.on("timeout", () => request.destroy(new Error("Gateway readiness probe timed out")));
    request.on("error", reject);
  });
}

export function validateLegacyManagedUpdateAdoptionReceipt(value) {
  if (
    !exactObjectKeys(value, LEGACY_MANAGED_UPDATE_ADOPTION_KEYS) ||
    value.schemaVersion !== 1 ||
    ...
    !EXACT_RELEASE_VERSION_PATTERN.test(String(value.previousVersion ?? "")) ||
    !EXACT_RELEASE_VERSION_PATTERN.test(String(value.targetVersion ?? "")) ||
    Number.isNaN(Date.parse(String(value.adoptedAt ?? ""))) ||
    ![
      value.controllerHintSha256,
      value.currentRuntimeSha256,
      value.legacyJournalSha256,
      value.previousManifestSha256,
      value.previousRootSha256,
      value.receiptDigest,
      value.stateEvidenceDigest,
    ].every((digest) => TAGGED_SHA256_PATTERN.test(String(digest ?? ""))) ||
    !exactObjectKeys(value.gateway, [
      "generationDigest",
      "pid",
      "runtimeSource",
      "startedAt",
    ...
    !TAGGED_SHA256_PATTERN.test(String(value.gateway.generationDigest ?? "")) ||
    !Number.isSafeInteger(value.gateway.pid) ||
    value.gateway.pid < 1 ||
    !new Set(["managed-package", "packaged-runtime"]).has(value.gateway.runtimeSource) ||
    Number.isNaN(Date.parse(String(value.gateway.startedAt ?? ""))) ||
    value.gateway.version !== value.previousVersion ||
    !exactObjectKeys(value.service, ["instanceId", "launcher", "name", "scope"]) ||
    !/^[a-f0-9]{16}$/u.test(String(value.service.instanceId ?? "")) ||
    value.service.name !== `fased-gateway-${value.service.instanceId}.service` ||
    value.service.scope !== "system" ||
    value.service.launcher !== `/opt/fased/local/${value.service.instanceId}/gateway-launch`
  ) {
    throw new Error("legacy managed update adoption receipt is invalid");
  }
  const unsigned = { ...value };
  delete unsigned.receiptDigest;
  if (canonicalDigest(unsigned) !== value.receiptDigest) {
    throw new Error("legacy managed update adoption receipt digest is invalid");
  }
  return Object.freeze(value);
}

async function writeDurableOwnerReceipt(receiptPath, receipt) {
  const temporary = path.join(
    path.dirname(receiptPath),
    `.${path.basename(receiptPath)}.${process.pid}.${randomUUID()}.tmp`,
  );
  ...
  try {
    await fsp.link(temporary, receiptPath);
    await fsp.rm(temporary);
    await fsyncManagedPath(path.dirname(receiptPath));
  } catch (error) {
...
export async function adoptLegacyManagedUpdate(options = {}) {
  const stateDir = path.resolve(options.stateDir || resolveManagedRuntimePaths().stateDir);
  const paths = resolveManagedRuntimePaths({ stateDir });
  const journalPath = adoptionPath(
    stateDir,
    options.journalPath,
    LEGACY_MANAGED_UPDATE_JOURNAL_FILE,
    "legacy journal",
  );
  const controllerHintPath = adoptionPath(
    stateDir,
    options.controllerHintPath,
    LEGACY_MANAGED_UPDATE_HINT_FILE,
    "controller hint",
  );
  const receiptPath = adoptionPath(
    stateDir,
    options.adoptionReceiptPath,
    LEGACY_MANAGED_UPDATE_ADOPTION_FILE,
    "adoption receipt",
  );
  const expectedOwnerUid = options.expectedOwnerUid ?? process.getuid?.() ?? process.geteuid?.();
  const stateInfo = await fsp.lstat(stateDir);
  if (
    !stateInfo.isDirectory() ||
    stateInfo.isSymbolicLink() ||
    stateInfo.uid !== expectedOwnerUid ||
    (stateInfo.mode & 0o002) !== 0
  ) {
    throw new Error("managed state directory is unsafe for legacy update adoption");
  }
  const existingReceiptFile = await readBoundedOwnerJson(
    receiptPath,
    "legacy managed update adoption receipt",
    expectedOwnerUid,
    { optional: true, maxBytes: 256 * 1024 },
  );
  const existingReceipt = existingReceiptFile
    ? validateLegacyManagedUpdateAdoptionReceipt(existingReceiptFile.value)
    : null;
  if (!existingReceipt && options.requireExistingReceipt === true) {
    throw new Error("legacy managed update adoption receipt is missing during root import");
  }
  const journalFile = await readBoundedOwnerJson(
    journalPath,
    "legacy managed updater transaction journal",
    expectedOwnerUid,
    { optional: true },
  );
  if (!journalFile) {
    return Object.freeze({
      status: existingReceipt ? "pending-root-verification" : "not-applicable",
      receipt: existingReceipt,
      receiptPath,
      replayed: Boolean(existingReceipt),
    });
  }
  const journal = validateLegacyManagedUpdateJournal(journalFile.value, paths);
  const journalDigest = taggedSha256(journalFile.bytes);
  if (existingReceipt && existingReceipt.legacyJournalSha256 !== journalDigest) {
    throw new Error("legacy managed update adoption receipt does not match the surviving journal");
  }
  const hintFile = await readBoundedOwnerJson(
    controllerHintPath,
    "legacy protected Local controller hint",
    expectedOwnerUid,
    { maxBytes: 256 * 1024 },
  );
  validateLegacyControllerHint(hintFile.value);
  const installedFile = await readBoundedOwnerJson(
    paths.manifestPath,
    "managed install manifest",
    expectedOwnerUid,
    { allowedModes: [0o600, 0o660] },
  );
  validateLegacyManagedManifest(installedFile.value, paths, journal.previousVersion);
  if (
    canonicalReleaseJSON(installedFile.value) !== canonicalReleaseJSON(journal.previousManifest)
  ) {
    throw new Error(
      "managed install manifest does not match the durable previous transaction outcome",
    );
  }
  const currentRoot = await resolveLinkTarget(paths.currentLink);
  if (currentRoot !== journal.previousRoot) {
    throw new Error(
      "managed runtime/current does not select the durable previous transaction outcome",
    );
  }
  const previousRoot = await verifiedManagedReleaseIdentity(
    journal.previousRoot,
    journal.previousVersion,
  );
  const currentRuntime = await verifiedManagedReleaseIdentity(currentRoot, journal.previousVersion);
  const readRootApplicationIdentity =
    options.readRootApplicationIdentity ?? defaultReadRootApplicationIdentity;
  const rootApplication = await readRootApplicationIdentity({
  ...
  if (
    rootApplication?.version !== journal.previousVersion ||
    !TAGGED_SHA256_PATTERN.test(String(rootApplication?.identityDigest ?? "")) ||
    rootApplication.identityDigest !== previousRoot.identityDigest
  ) {
    throw new Error(
      "protected Local root application does not match the durable previous release identity",
    );
  }
  ...
  const previousManifestSha256 = canonicalDigest(journal.previousManifest);
  const stateEvidenceDigest = canonicalDigest({
    controllerHintSha256: taggedSha256(hintFile.bytes),
    currentRuntimeSha256: currentRuntime.identityDigest,
    gateway,
    legacyJournalSha256: journalDigest,
    previousManifestSha256,
    previousRootSha256: previousRoot.identityDigest,
    rootApplicationSha256: rootApplication.identityDigest,
    service,
  });
  const unsignedReceipt = Object.freeze({
    adoptedAt: existingReceipt?.adoptedAt ?? new Date(options.now?.() ?? Date.now()).toISOString(),
    controllerHintSha256: taggedSha256(hintFile.bytes),
    currentRuntimeSha256: currentRuntime.identityDigest,
  ...
  const receipt = validateLegacyManagedUpdateAdoptionReceipt({
    ...unsignedReceipt,
  ...
  if (existingReceipt) {
    ...
    await writeDurableOwnerReceipt(receiptPath, receipt);
  }
  await options.crashPoint?.("after-receipt-durable");
  const afterReceipt = await fsp.lstat(journalPath);
  if (
    !afterReceipt.isFile() ||
    afterReceipt.isSymbolicLink() ||
    afterReceipt.dev !== journalFile.stat.dev ||
    afterReceipt.ino !== journalFile.stat.ino ||
    taggedSha256(await fsp.readFile(journalPath)) !== journalDigest
  ) {
    throw new Error("legacy managed updater transaction changed during adoption");
  }
  return Object.freeze({
...
async function recoverHostedSupervisorTransaction(
  transactionId,
  version,
  timeoutMs,
  socketPath,
  ...
  try {
    return await recoverPendingSupervisorTransaction(
      {
        socketPath,
        transactionId,
...
async function handoffTargetOwnedRelease({
  ...
  recoverSupervisor = recoverHostedSupervisorTransaction,
  applyRelease = async () =>
    await requestHostedSignerTransactionWithRetry(
      "applyRelease",
      transactionId,
  ...
  await recoverSupervisor(transactionId, targetVersion, timeoutMs, socketPath);
...
export async function run(argv = process.argv.slice(2)) {
  if (argv[0] === "adopt-legacy-managed-update") {
    if (argv.length !== 1) {
      throw new Error("legacy managed update adoption accepts no path or release arguments");
    }
    const result = await adoptLegacyManagedUpdate();
...
export const __testing = {
  ...
  recoverHostedSupervisorTransaction,
