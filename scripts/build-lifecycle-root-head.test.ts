...
async function fixture() {
  ...
  const index = path.join(directory, "fased-release-index-v1.json");
  await fs.writeFile(
    root,
  ...
  await fs.writeFile(
    index,
    `${JSON.stringify({ schemaVersion: 1, type: "fased-release-index", channel: "beta", version: "0.1.76-rc.90", releaseSequence: 90, securityEpoch: 7, commit })}\n`,
  );
