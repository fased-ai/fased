...
function stripWorkspaceFasedDevDependency(pkg: PackageJson): boolean {
  const devDeps = pkg.devDependencies;
  if (!devDeps || devDeps.fased !== "workspace:*") {
    return false;
  }
  delete devDeps.fased;
...
export function syncPluginVersions(rootDir = resolve(".")) {
  ...
  for (const dir of dirs) {
    ...
    const removedWorkspaceDevDependency = stripWorkspaceFasedDevDependency(pkg);
