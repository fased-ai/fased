...
void test("Fased activates only when the finalized deployment matches the candidate interface", () => {
  ...
  const activationContract = JSON.parse(
    fs.readFileSync(
      path.join(root, "extensions/sat-mining/protocol-generation/activation.sat-dep-0012.json"),
      "utf8",
    ),
  );
  ...
  assert.match(activation, /deploymentId: "SAT-DEP-0012"/u);
  ...
  assert.match(activation, /activationGeneration: 11/u);
